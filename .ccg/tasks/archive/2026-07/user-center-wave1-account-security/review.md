...
- 本地候选：`bbdd4176`，已整合 `origin/main@f69bf1b7`。
...
- SQLite：全新数据库重放 58 条正式 migration；账户状态、step-up、member-auth、QR 登录均通过。
- PostgreSQL 16.14：全新数据库重放 30 条正式 migration；账户状态、step-up、member-auth、QR 登录、会员打印订单、governed job-fit 均通过；测试数据库和角色已删除。
- 静态与构建：shared/API/Admin typecheck，shared/API lint，API build，PostgreSQL schema sync 均通过。
- 主线回归：release provenance、Admin 手机转移 API/UI 验证均通过。
