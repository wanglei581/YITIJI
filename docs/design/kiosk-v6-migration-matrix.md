...
> ⚠️ **2026-09-03 前提已变更 —— 本文的目标设定已不成立，但分析仍有参考价值。**
>
> 本文写作时的前提是「产品负责人的目标是全站 V6」。产品负责人 2026-09-03 已改口：
> **当前真值是 `kiosk-redesign-2026-08/`（青序流光），上线前端页面全部出自该套；
> V6（`kiosk-ai-os-v3-2026-08`）不再是目标。**
>
> 因此**不要照本文的迁移矩阵排施工**。仍然有用的部分是它对「哪些页面用哪套壳、
> 覆盖面取证到什么程度」的实测记录 —— 迁移目标换成青序流光后，那套方法可以复用，
> 但逐页的目标列必须重算。
