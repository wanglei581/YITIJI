...
(function (window, document) {
  ...
  function auditFakeAffordance() {
    ...
    for (i = 0; i < all.length; i++) {
      ...
      if (!txt) continue;
      // 文字本身在说「不可用」的，不是假入口 —— 它没有声称自己是入口，
      // 恰恰相反，它在告诉用户这条路现在走不通。
      // 2026-08-11 实测误报 10 处：06 的「设备离线 · 暂不可用」、
      // 07 的「识别中断 · 暂不可选」（类名都是 src-tag，不在上面那串类名豁免里）。
      // 这一条按**文字**判而不是按类名判，正是上面那句
      // 「判定假入口要看它是否声称是入口」的直接落实 ——
      // 类名豁免表会随新类名不断漏，语义判据不会。
      if (/暂不可用|暂不可选|暂未开放|尚未开放|未开放|不可用|不可选|已停用|维护中/.test(txt)) continue;
      out['疑似假入口'] += 1;
      if (out['明细'].length < 8) {
