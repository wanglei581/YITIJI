...
| `41` claim「确认认领」自助取件核销 | 远程订单到机释放后出纸 | **未立项** | ❌ | 历史 `PrintPickupClaimPage` 曾调用不存在端点，已下线。不能把用户动作写成 `pending → claimed`：`claimed` 是 Agent 持终端凭证获得的领取租约，提前写入会令 Agent 永远找不到任务。二期应采用 Order-only 待释放模型，由受认证终端原子释放一次后创建 `PrintTask(pending)`，再复用现有 Agent claim |
...
| A8 | **远程到机释放能力** | `41` claim「确认认领」 | 当前不实现匿名 `claim-pickup`。二期新建独立远程订单/释放契约：未到机前不创建 `PrintTask(pending)`；由带终端凭证的 Agent 触发释放，服务端原子校验码、支付/退款、目标终端、有效期与一次性，再创建任务供现有 Agent 领取。必须限流、防枚举、审计不记原始码、并发只生成一个任务 |
