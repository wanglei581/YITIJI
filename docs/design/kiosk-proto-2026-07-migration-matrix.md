...
| 03 | 03-print-settings.html | 打印参数 | 打印扫描 | `/print/params` →（2026-09-08 起重定向）`/print/desk` | PrintDeskPage | src/pages/print/PrintDeskPage.tsx | sessionStorage 流转 | 份数/色彩/双面/缩放/页范围（原型 03 的参数区已由打印台预览阶段承载） | loading / error(文件不可用) | verify-print-parameter-capability |
...
| 31 | 31-print-material-check.html | 材料检查 | 打印扫描 | `/print/material-check` →（2026-09-08 起重定向）`/print/desk` | PrintDeskPage | src/pages/print/PrintDeskPage.tsx | ApiHttpError / sessionStorage 流转 | 文件体检、A4 规范化、隐私片段检查 | loading / error | verify-print-entry-source-split |
...
| 38 | 38-interview-setup.html | 面试设置 | 面试助手政策 | `/interview/setup` →（2026-09-08 起重定向）`/interview?stage=setup` | InterviewWorkbenchPage | src/pages/interview/InterviewWorkbenchPage.tsx | interview.createInterview+startInterview / files.kioskUploadFile | 岗位/难度/题量/方式、简历上传 | loading / error | verify-lightflow-k2c-interview |
| 39 | 39-interview-session.html | 答题现场 | 面试助手政策 | `/interview/session` →（2026-09-08 起重定向）`/interview?stage=session` | InterviewWorkbenchPage | src/pages/interview/InterviewWorkbenchPage.tsx | interview.answerInterview+endInterview+fetchQuestionAudio+getVoiceCapability+transcribeAnswer | 问题卡、作答、计时、语音 | loading / error / 语音能力态 | verify-lightflow-k2c-interview |
| 40 | 40-interview-report.html | 面试报告 | 面试助手政策 | `/interview/report` →（2026-09-08 起重定向）`/interview?stage=report` | InterviewWorkbenchPage | src/pages/interview/InterviewWorkbenchPage.tsx | interview.getInterviewReport+printInterviewReport | 总分/四维度/逐题点评、打印 | loading / error | verify-lightflow-k2c-interview / verify-ai-artifact-print-url-contract |
| 41 | 41-interview-tips.html | 面试技巧 | 面试助手政策 | `/interview/tips` →（2026-09-08 起重定向）`/interview?stage=tips` | InterviewWorkbenchPage | src/pages/interview/InterviewWorkbenchPage.tsx | 静态(仅样式 import) | STAR 法则等、打印手册 | 静态 | verify-lightflow-k2c-interview(间接) |
| 42 | 42-interview-reports.html | 历史报告 | 面试助手政策 | `/interview/reports` →（2026-09-08 起重定向）`/interview?stage=reports` | InterviewWorkbenchPage | src/pages/interview/InterviewWorkbenchPage.tsx | interview.getMyInterviews+deleteMyInterview | 记录列表、删除、空态 | loading / empty / error / 未登录 | verify-lightflow-k2c-interview |
...
| 64 | 64-print-preview.html | 打印预览 | 打印扫描 | `/print/preview` →（2026-09-08 起重定向）`/print/desk` | PrintDeskPage | src/pages/print/PrintDeskPage.tsx | priceConfigApi / sessionStorage 流转 | 单页放大、缩略图、页面范围 | loading / error / 文件不可用兜底 | verify-price-single-source / verify-legal-retention-copy / verify-print-entry-source-split |
...
| 档位 | 数量 | 屏号 |
| --- | --- | --- |
| IMPL_OK 实现/结构/真实 service 已核实 | 71 | 01–20,24,25,27–75 中除下列差异屏外全部(明细见第一节矩阵) |
| VISUAL_DIFF 实现完整、视觉与原型有差 | 3 | 21,23,26 |
| STRUCTURE_DIFF 结构差(有意) | 1 | 22(通知+反馈:原型合屏,生产拆 `/me/notifications`+`/me/feedback` 两页,更清晰,保留) |
| NOT_IMPL 未实现/空壳 | 0 | — |

> "IMPL_OK 71 屏"仅指**实现、结构、真实 service 接入已核实**,**不表示**其视觉已逐像素对齐原型——视觉结论以 6.2 为准。
...
> 合计 35 + 40 = 75。
> VISUAL_VERIFIED 的 35 屏中,视觉比对结论细分为:与原型一致 31 屏、`VISUAL_DIFF` 3 屏(21,23,26)、`STRUCTURE_DIFF` 1 屏(22)——即视觉已看过、差异已定性。**03/05/06 已于 2026-08-07 复核为与原型双栏布局一致(见 6.3),从 `VISUAL_DIFF` 移除。**
...
### 6.3 2026-08-07 复核:03/05/06 双栏布局已落地(原 `VISUAL_DIFF` 已关闭)

按当前 `main@f4981ce0` 源码与 `verify-fusion-w3.mjs` 契约复核,三屏原「生产单列」结论已过时:

| 屏 | 当前实现证据 | 复核结论 |
| --- | --- | --- |
| 03 打印参数 | `PrintParamsPage.tsx` 根布局 `pp-split`(desktop `display:flex; gap:22px`),左 `pp-params-left`(flex-1,独立滚动)+ 右 `<aside class="pp-side-col">`(设备/用量预估/费用明细/须知);`print-preview-params.css` 窄屏媒体查询折叠为单列 | 与原型「左参数 + 右侧栏」一致(1080×1920) |
| 05 简历来源 | `ResumeSourcePage.tsx` `resume-source-split grid`,fusion CSS `grid-template-columns: minmax(0,1fr) minmax(440px,0.9fr)`,方向栏 `min-width:440px`(W3 契约断言);上传通道命名差异(原型「纸质简历扫描」↔ 生产「云端上传」)仍为**有意分流**:纸质扫描走独立 `/scan`,不回退 | 与原型「左上传 + 右诊断方向」一致(1080×1920) |
| 06 诊断报告 | `ResumeReportPage.tsx` 分数卡 + 能力雷达 + 提示为一行(notice `lg:w-[300px]`),`resume-report-grid-a`/`resume-report-grid-b` 在 `lg` 双栏;W3 契约含对应 selector 归属断言 | 与原型「分数+雷达一行 + 两组 2 栏 grid」一致(1080×1920) |

> 判档影响:03/05/06 由 `VISUAL_DIFF` 改为「与原型一致」(布局级),6.1/6.2 计数已同步;剩余 21/23/26 为决策型差异(暂缓),40 屏 `VISUAL_UNVERIFIED` 不变。
...
> 说明:B 类不涉及前端代码缺口,均为"外部前置 / 真机 / 第三方 / 授权"到位问题。A 类 03/05/06 双栏视觉对齐已于 2026-08-07 落地(见 6.3);剩余 21/23/26 为决策型差异(暂缓),当前可纯前端推进的仅剩逐屏证据清单与 40 屏 `VISUAL_UNVERIFIED` 复核,方案见 `next-tasks.md`。
...
| 76A 百宝箱待配置 | `/toolbox` | 历史空态视觉参考；当前无安全可启动项时由父级 Capability Boundary 输出 unavailable，且不挂载 `ToolboxZonePage`，不是新路由。 |
...
- AI 简历与扫描流程：`/resume`、`/resume/upload`、`/resume/source`、`/resume/generate`、`/resume/generate/preview`、`/resume/parse`、`/resume/report`、`/resume/optimize`、`/resume/optimize/compare`、`/resume/export`、`/resume/templates`、`/resume/materials`、`/scan/start`、`/scan/settings`、`/scan/progress`、`/scan/result`。
