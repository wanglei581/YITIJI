...
| 34 | 34-scan-start.html | 扫描类型选择 | 打印扫描 | `/scan/start` →（2026-09-08 起重定向）`/scan` | ScanWorkbenchPage | src/pages/scan/ScanWorkbenchPage.tsx | API_BASE_URL | 简历/证件/普通文档三选一 | 静态选择 | verify:scan-session-truth |
| 35 | 35-scan-settings.html | 扫描指引 | 打印扫描 | `/scan/settings` →（2026-09-08 起重定向）`/scan` | ScanWorkbenchPage | src/pages/scan/ScanWorkbenchPage.tsx | scanTasks.createScanSession+cancelScanSession / screensaver.getTerminalId | 面板操作四步、任务会话 | loading / error | verify:scan-session-truth |
| 36 | 36-scan-progress.html | 扫描等待 | 打印扫描 | `/scan/progress` →（2026-09-08 起重定向）`/scan` | ScanWorkbenchPage | src/pages/scan/ScanWorkbenchPage.tsx | scanTasks.getScanSessionStatus+cancelScanSession / ApiHttpError | 诚实等待态、处理阶段时间线 | loading / error / 轮询态 | verify:scan-session-truth |
| 37 | 37-scan-result.html | 扫描结果 | 打印扫描 | `/scan/result` →（2026-09-08 起重定向）`/scan` | ScanWorkbenchPage | src/pages/scan/ScanWorkbenchPage.tsx | 待核实(经 state 流转,无直接 service import) | AI简历识别/打印/保存文档 | 待核实(依赖上游态) | verify:scan-session-truth |
...
- AI 简历与扫描流程：`/resume`、`/resume/upload`、`/resume/source`、`/resume/generate`、`/resume/generate/preview`、`/resume/parse`、`/resume/report`、`/resume/optimize`、`/resume/optimize/compare`、`/resume/export`、`/resume/templates`、`/resume/materials`、`/scan`、`/scan/start`、`/scan/settings`、`/scan/progress`、`/scan/result`。
