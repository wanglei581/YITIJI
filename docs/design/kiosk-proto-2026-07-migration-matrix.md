...
**VISUAL_DIFF 六屏差异说明（2026-07-20 审计原文；方案 B 后状态）**:

| 屏 | 2026-07-20 差异 | 2026-07-25 方案 B 后 |
| --- | --- | --- |
| 03 打印参数 | 生产单列 | W10：有内容双栏+步骤条候选；无文件诚实空态 |
| 05 简历来源 | 生产单列 | W10：有内容双栏+步骤条；「云端上传」+ `/scan` 分流保留 |
| 06 诊断报告 | 生产单列 | W10：有内容双栏候选；无报告诚实空态 |
| 21 我的权益 | 缺「去权益活动领取」 | W11：已补 → `/activities` |
| 23 设置 | PageHeader/neutral | W11：`KioskPageFrame`+触控行；换绑/撤回保留 |
| 26 简历预览 | 逐段 textarea | W11：A4 纸面双栏+诚实空态候选 |

**W16–W22 细对齐候选（2026-07-25，PR #328 + 本轮壳收口）**：岗位/企业/线下、招聘会套件、校园/智慧校园/百宝箱、活动/帮助/人事/法务、简历余页、打印收银收尾与 print-scan 附属、系统/手机全屏——统一 `KioskPageFrame`/`KioskPageHeader`（或 fusion-w5 全屏态）+ 诚实空/错态。本轮另将 `/activities/:id`、`/legal/*`、`/toolbox` 从 `PageHeader`/自绘 pagehead 迁入统一页壳。  
**仍不得写**：86 屏 ±4px 像素级全部封板；50–55 等若需像素级结论须另开截图抽检。`STRUCTURE_DIFF` 22 仍为有意拆页。
