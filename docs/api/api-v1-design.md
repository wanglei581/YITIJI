...
> **方案快照（2026-09-26 标注）：** 本文写于 （2026-05），功能实现以代码为准；与 2026-09-26「设备与软件供应方、托管 a」口径冲突的地方，以 docs/product/feature-scope.md 为准。
>
> 端点以代码与 docs/graph/ 为准。
...
Kiosk 创建打印任务，后端生成 `actionToken`，由 Terminal Agent 通过 `POST /api/v1/terminals/:terminalId/tasks/claim` 领取执行。
...
| paperSize | string | `'A4'` | 纸张规格（CM2800ADN/CM2820ADN 系列仅支持 A4） |
