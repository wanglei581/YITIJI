...
- `apps/partner/`：机构后台，给运营机构（高校就业中心、公共就业服务机构、持证人力资源服务机构）使用。托管 a 下在我们云上只维护本机构官方渠道二维码、政策与本机构终端数据；岗位、招聘会、企业资料、数据源页面的代码保留给私有化部署（b），按 next-tasks 3.13 在我们云上隐藏（切换前代码仍可用）。
