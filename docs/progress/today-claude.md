...
- [x] **packages/ui ComplianceBanner 组件**(warning / info / success)@ `683ddf6`
- [x] **packages/shared COMPLIANCE_COPY 合规文案 SSOT**(8 个标准横幅文案 + 禁词 / 推荐词清单)@ `683ddf6`
- [x] **装依赖**:recharts(3 端)+ react-diff-viewer-continued(kiosk)@ `3df9e3f`
- [x] **Prisma 模型**:FileObject + AuditLog + migration 已 apply 到 dev.db @ `2df963e`
- [x] **shared types/file.ts**:FileMetadata / FileUploadResponse / SignedUrlResponse / FileCleanupResponse + 默认 TTL 矩阵 @ `2df963e`
- [x] **BE-1 文件通道**:storage.ts + signing.ts(HMAC-SHA256)+ service + controller(6 路由)+ cron(每小时清理过期)@ `8ea2946`
- [x] **app.module 注册** FilesModule + ScheduleModule.forRoot() @ `8ea2946`
- [x] **.env.example 模板** + .gitignore 加 /services/api/storage/ @ `8ea2946`
- [x] **typecheck 全员通过**(8 workspace projects)
- [x] **API 启动验证通过**:6 条 /api/v1/files 路由全部 mapped

## 总产出统计

- 本日 commit 数(本地):4(`683ddf6` / `3df9e3f` / `2df963e` / `8ea2946`)
