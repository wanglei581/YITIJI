...
- 2026-06-22 预生产服务器当前仍是阶段性验收状态：代码包已升级到 AI 导出产物复验候选 `76c06ca8`，health 指向 PostgreSQL，COS 已切换到隔离预生产 bucket 并通过 G3-06 live 验证，Gate 4 账号/API 级验收已通过并回滚短信 Provider；真实 AI 导出产物 `optimized/sourceFileId` 链路已在预生产通过自动 Gate 与 COS/DB 脱敏补证。2026-06-26 已补真实短信登录、合成 PDF 上传和 `/me/documents` 会员文件路径的浏览器截图证据，并清理本轮预生产测试记录；Admin 生命周期、签名 URL 等剩余浏览器证据仍待补齐。2026-06-29 已把求职材料库刷新到预生产 `64596c18` 并完成公网浏览器受控会员生成 PDF 到打印确认链路，测试数据和短信临时配置均已回滚清理。百度 OCR Key 与 live 验证、AI/TRTC/ASR/TTS 按启用范围 live 验证、正式域名 HTTPS，以及腾讯短信审核后的真实手机号 E2E 仍未完成。
...
2. 求职材料库闭环：`/resume/templates`、会员生成 PDF、`/me/documents` 预览 / 打印、Admin 只读统计已形成代码侧闭环，并已在预生产完成公网浏览器受控会员生成 PDF 到打印确认链路验收；下一步只做 Windows 真机出纸、正式域名 HTTPS / 真实短信上线 E2E，以及后续动态模板治理设计，不把预生产 verify 误写成正式生产或真机完成。
3. 招聘会 / 校园招聘闭环：Branch 1 列表页 `terminalId` 本校优先接线、Branch 2 `fair_company` 外部跳转记录、Branch 3 大页面零行为拆分均已完成；下一步进入上线前真实验收或另起分支处理 T4/T5 派生任务。
4. T5 派生任务：已跟踪旧 PDF 已完成仓库外归档并移出 Git；商业计划书转正、对外简介转正、路线图转正、OPC 策略输入归档仍需另起独立分支。
5. T4 派生任务：业务闭环审计文档化、代码瘦身候选确认、预生产部署事实脱敏收口，均需另起独立分支。
