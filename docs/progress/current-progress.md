...
## dev 库 CLI 测试残留清理（2026-06-16，Mavis）

按 Claude 校园招聘会验收报告（见下方「校园招聘会验收 Warning 修复」段）发现 1 条公开测试脏数据（标题「验证招聘会」/ 场馆「验证展馆」），追查发现今天 13:45 + 14:05 的两次 verify 跑过之后 `cleanup()` 因异常中断，在 dev.db 留下整套 16 行测试残留（公开 JobFair 只暴露了 1 条，其余隐藏在 pending/未公开资源里）。**全部为 hash 后缀账号 / 测试 org / 假真实名字（青岛滨海.../机构 A/机构 B），永远不会复用于任何 verify**。上线前生产库必须不含此类记录。

**清理范围（事务硬删，dev.db 备份 `prisma/dev.db.bak-20260616-cleanup`）：**

| 表 | 数量 | 备注 |
|----|------|------|
| `JobFair` | 2 | `cmqgp2k1q0000rrzgdwc9nk09`（公开「验证招聘会」，13:45）/ `cmqgps7d3000308zgh7qjipvl`（「验证招聘会(改)」pending，14:05） |
| `Job` | 1 | `cmqgps7cz000108zgtinqkeh7`（「验证岗位(改)」pending，14:05） |
| `User` | 7 | 5 admin `vff_admin_1e96.../vff_admin_6abc.../vvg_admin_0981.../vff_admin_ea0e.../vcp_admin_f36f...` + 2 partner `vpe_partner_9808...(orgId=null)/vpe_partner_fbc1...` |
| `Organization` | 5 | `org_vff_aca06e1d54ed`（验证大学就业...）/ `org_vpe_a_fbc167cd17e1`（机构A）/ `org_vpe_b_fbc167cd17e1`（机构B 孤儿）/ `org_vvg_0981e8125a3c`（验证机构）/ `org_real_school_aca06e1d54ed`（青岛滨海...假真实） |
| `AuditLog` | 1 | `cmqgps7h9000508zgzu7krplr`（fair.partner_update，actorId 引用被删 partner） |
| **合计** | **16** | |

**未清理（保留）：**

- `org-demo-companies` + 6 条标题带「演示」的 Job（06-12 seed 演示数据，dev 库保留供开发预览，生产/严格模式自动过滤）
- seed 的 `admin` / `partner1` / `partner2` 账号（无 hash 后缀、06-03 创建）
- 用户的真实测试 partner `wang`（山东大学 / org_3b61...）/ `11111111`（org_a459...）
- `cmq82yhmk0008zbzg4h8ft0vr`「链路验证前端工程师 1781096648630」（org-uni-001 下 06-10 创建，名字含"链路验证"是真实数据，不是测试残留）

**附带动作：**

- 停掉 5273 的 kiosk 预览实例（Claude 起的，已留 dev.db 卫生验证截图，无需长跑）
- 停 API（67778）→ 事务清理 → 重启 API（7143，nohup detached，`/tmp/api.log`）

**验证：**

- `prisma/dev.db` 公开 `JobFair WHERE reviewStatus='approved' AND publishStatus='published'` 由 4 → 3（剩 3 条 seed：fair-uni-campus-2026q2 / fair-uni-corp-ai-2026 / fair-hr-1k-2026q2）
- `User` / `Organization` / `JobFair` LIKE '%验证%' / '%演示%' 全部 0 命中
- curl `http://localhost:3010/api/v1/job-fairs?pageSize=20` 0 命中「验证/演示」字段
- kiosk `:5173/campus` HTTP 200 正常返回
- 备份 `prisma/dev.db.bak-20260616-cleanup`（md5 `b85a282a905971b60e5e4a934bafd0db`，1150976 bytes）

**没碰的事：**

- 不重新加回「验证/演示」泛文本过滤（W 轮决策不变，regression 风险 > 收益）
- 不动 verify 脚本本身的 cleanup 逻辑（清理失败属偶发，下次跑同样脚本仍会跑自己的 cleanup；本次清理是数据卫生一次性处置）
- 涉及行数 16 全部用事务包起来，rollback 已就绪（如需回滚：`cp prisma/dev.db.bak-20260616-cleanup prisma/dev.db && pnpm dev`）
...
## 校园招聘会验收 Warning 修复（2026-06-16，Claude，Codex 调度复核）

按校园招聘会上线前验收复查报告修复剩余 Warning/Info（无 Critical）。合规边界不变：非招聘平台，仅第三方/官方来源入口，无任何平台内投递闭环。

- **W-1 详情页诚实性**：`CampusFairDetailPage` OverviewTab 删除写死「活动亮点」整段（后端无对应字段，原样渲染同一组文案=占位冒充真实）；「面向人群」「现场服务」改为仅在后端 `audienceLabel`/`onsiteServices` 有真实值时渲染，否则中性空态/隐藏；「活动介绍」空时显示「暂无活动介绍，详情以来源平台为准」，不再用 schoolOf 套话兜底；统计「面向学生范围」兜底改「以来源平台为准」。
- **W-2 同源 /job-fairs 姊妹页 20 条截断**：`JobFairsPage`/`JobFairDetailPage`/`FairCompaniesPage` 的裸 `getJobFairs()`/`getFairCompanies()` 改为分页循环拉全（pageSize=100，同 `/campus` 口径），消除 http 真实模式默认 20 条静默截断与计数错误。
- **W-3 后端子资源分页契约对齐**：`jobs.service.getFairCompanies` 与 `admin-fairs.service.getPublishedFairMaterials` 返回补 `pagination:{page,pageSize,total,totalPages}`（与 `getPublishedFairs` 一致），保留原扁平字段不破坏现有调用。
- **W-4 子资源错误不再静默吞**：`CampusFairDetailPage` 与普通 `/job-fairs/:id` 详情页子资源以 `settle()` 区分「接口失败」与「真实为空」，企业/资料/统计失败渲染可重试 `ErrorState`，场馆导览失败显重试入口；新增 `resourceKey` 重试。
- **W-5 http 丢失 zoneName**：后端 `getFairCompanies`/`getFairCompanyById`/`getPublishedFairDetail`/Admin `getFairDetail` 经 `zoneId` join `FairZone` 回填 `zoneName`（`mapFairCompany(row, zoneName)`）；`httpAdapter` `WireFairCompany`/`mapWireCompany` 增 `zoneName` 映射；`fair.types.ts` `FairCompany` 增 `zoneName`；`seed-fairs.ts` 按企业真实总部城市/技术方向/行业关联展区并补 `boothNumber`（非杜撰），使真实 http 模式展厅筛选可工作，mock 不回退。
- **W-6 内联资料打印语义**：详情页内联「活动资料」改为仅 `allowPrint && previewUrl` 时直接 `navigate('/print/confirm')` 打印该资料（与 `FairMaterialsPage` 同口径），否则标注「暂不开放打印」；去掉硬编码「可打印」；新增「查看全部资料」入口。
- **演示数据隔离二次收紧**：生产/严格模式过滤保留结构化 demo 来源、`example.com/example.org`、`某大学/某市` 等明确演示标记；移除 `验证/演示` 泛文本匹配，避免真实学校或活动名称中包含这些词时被误过滤。
- **Info**：`FairMaterialsPage` 头部「所有资料均可免费打印」→「可打印资料免费取用」（I-4）；「岗位匹配参考」跳转 `/jobs`→`/resume/job-fit`（I-5）；I-6 并入 W-1。
- **未改（需决策）**：I-1「查看来源平台」补埋点——`JUMP_ACTION_BY_TARGET[job_fair]` 仅允许 `external_appointment`，补 `external_open` 会被后端 400；记为 `external_appointment` 又会污染「预约」语义，故保持不记录，待产品定义合法动作。I-2 扫码导航同理未补。
- **验证**：kiosk/api typecheck、kiosk verify:jobfair-ui、api verify:{activity-logs,jobfair-venue-guide,public-fair-demo-guard,fair-company-positions,admin-fairs,fair-info-fields,partner-edit}、kiosk/api lint、`VITE_API_MODE=http` 生产 build 全绿。

---

## 青岛 OPC 创业大赛商业计划书初稿（2026-06-16，Codex）

按用户参加 2026“创·在青岛”创业大赛 OPC 挑战赛的需求，新增参赛商业计划书初稿：[职易达AI求职服务终端-青岛OPC创业大赛商业计划书.md](../business/职易达AI求职服务终端-青岛OPC创业大赛商业计划书.md)。

- 项目暂定名称：`职易达 AI 求职服务终端`；副标题：`AI 智能体驱动的线下自助就业服务终端`。
- 文档按正式商业计划书结构撰写：真实性与合规声明、项目名称、项目简介、执行摘要、大赛适配、市场机会、产品功能、技术与 AI 智能体、商业模式、财务预测假设、试点路径、竞争分析、合规风控、团队待补充项、发展规划和资料来源。
- 公开资料引用青岛 OPC 挑战赛公告、青创通 OPC 专区、青岛青年人才聚青就业创业新闻发布会、央视网教育部 2026 届高校毕业生规模信息等；项目事实引用正式进度、功能范围、合规边界、设备与 Agent 文档。
- 所有装机、收入、转化率、服务费均标注为预测假设；未写已签约、已营收、已规模化上线等无依据事实。
- 文档继续坚持项目合规定位：AI 求职材料服务工具 + 线下就业服务触点 + 第三方/官方信息入口，不写招聘平台闭环。

正式提交前仍需用户补充：参赛人姓名、年龄与 OPC 资格、学历/经历、知识产权状态、公司注册状态、是否有青岛落地计划、是否已有真实试点意向或合作证明、实机照片与界面截图。

---

## 校园招聘会重构：学校优先公开聚合 + 详情 5 Tab（2026-06-16，Codex）

用户确认采用“全公开聚合 + 学校分类 + 本校/推荐优先”的校园招聘会方案，不做本校私域隔离，不做平台内投递/收简历闭环。本轮完成数据契约、Partner/Admin 维护入口、Kiosk `/campus` 与 `/campus/:id` 重构，并清理旧 `/campus` Top-1 单场选择行为。

**数据与端口分工：**

- `JobFair` 新增展示字段：`hostSchoolName`、`audienceLabel`、`onsiteServicesJson`、`admissionMethod`，SQLite 与 PostgreSQL schema/migration 同步。
- Partner 学校账号可维护招聘会基础信息、主办/承办学校、面向对象、现场服务、入场方式；保存后仍强制回 `pending + draft`，重新进入管理员审核。
- Admin 招聘会基本信息抽屉新增“校园招聘展示字段”，用于审核、补全展示字段和合规文案；来源机构、外部编号、来源链接仍不可改成无来源数据。
- Kiosk `/campus` 改为学校优先公开聚合页，支持学校/城市/状态/关键词筛选，按学校分组展示全部已审核发布校园招聘会。
- 新增 Kiosk `/campus/:id` 校园招聘会详情页，5 Tab：活动概览、参展企业与岗位、场馆导览、活动资料、AI求职准备。

**合规边界：**

- 预约只提供“扫码预约”外跳来源平台，记录 `external_appointment` 外跳，不记录预约结果。
- 企业岗位只提供“扫码投递”外跳来源页面，记录 `external_apply` 外跳，不接收、不保存、不转发简历，不记录投递结果。
- 旧 `/campus` 中 Top-1 单场选择、AI 百分比匹配展示和潜在违规文案已从校园招聘会新页面移除；共享 `/job-fairs/*` 路由保留。

**验证结果：**

- API：`typecheck` ✅；`verify:admin-fairs` ✅；`verify:fair-info-fields` ✅；`verify:partner-edit` ✅；`verify:fair-company-positions` ✅。
- Kiosk：`typecheck` ✅；`lint` 0 error（保留既有 `KioskBusyContext` Fast Refresh warning）✅；`verify:jobfair-ui` ✅；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 build` ✅。
- Admin / Partner：`typecheck` ✅；`lint` ✅；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 build` ✅。
- 浏览器烟测：`VITE_API_MODE=mock` 下 `/campus` 展示推荐场次与按学校浏览，点击进入 `/campus/:id` 后 5 Tab 可见，无控制台 error，无招聘闭环禁词。HTTP 模式本地未启 API 时会诚实显示加载失败，不造数据。

### 校园招聘会正式收口：生产演示数据隔离 + 前台视觉精修（2026-06-16，Codex）

在上轮功能重构基础上，完成上线前收口：

- Kiosk `/campus` 从功能型列表精修为“学校专场与双选会大厅”：顶部统计、合规说明、推荐场次、学校分组、可预约数量、正式卡片层次已补齐。
- Kiosk `/campus/:id` 接入统一合规文案库 `COMPLIANCE_COPY.KIOSK_CAMPUS_TOP`；详情页继续保持 5 Tab，不出现平台内投递/收简历闭环。
- `KioskRoot` 将校园专区沉浸式判断从仅 `/campus` 扩展为 `/campus/*`，详情页和资料页不再漏出全局头部/底部导航。
- 新增 `/campus/:id/materials` 路由别名；`FairMaterialsPage` 根据当前路径返回 `/campus/:id` 或 `/job-fairs/:id`，避免从校园专区跳回普通招聘会专区。
- `JobsService` 新增公开招聘会生产/严格模式演示数据隔离：`NODE_ENV=production` 或 `EXCLUDE_DEMO_PUBLIC_DATA=true` 时，公开列表、详情、企业、导览、资料、统计统一过滤 `org-uni-001`、`org-hr-002`、`org_vff_*`、`example.com/example.org`、`某大学/某市` 等明确演示/验证来源；不再用 `验证/演示` 泛文本规则，避免误伤真实招聘会名称。
- `seed.ts`、`seed-fairs.ts`、`seed-companies.ts` 增加生产护栏：生产环境或疑似生产库禁止写入演示数据，除非显式 `ALLOW_DEMO_SEED=true`。
- 新增 `verify:public-fair-demo-guard`，构造演示数据 + 正式学校数据，验证严格模式只展示正式数据、演示数据详情与子资源不可公开读取。

**验证结果补充：**

- API：`typecheck` ✅；`lint` ✅；`verify:public-fair-demo-guard` ✅；`verify:partner-edit` ✅；`verify:fair-info-fields` ✅；`verify:admin-fairs` ✅；`verify:fair-company-positions` ✅；`verify:jobfair-venue-guide` ✅；`verify:companies` ✅。
- Seed 护栏：`NODE_ENV=production db:seed` / `db:seed:fairs` / `db:seed:companies` 均按预期拒绝写入演示数据 ✅。
- Kiosk：`typecheck` ✅；`verify:jobfair-ui` ✅；`lint` 0 error（保留既有 Fast Refresh warning）✅；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 build` ✅。
- Admin / Partner：`typecheck` ✅；`lint` ✅；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 build` ✅。
- 浏览器验收：`/campus` 正式大厅标题、统计、合规文案、推荐场次、按学校浏览均可见；`/campus/:id` 5 Tab 可见且无全局导航泄露；`/campus/:id/materials` 可打开并返回校园详情；控制台 0 error；招聘闭环禁词 0。

说明：本地开发库仍可保留“某大学”等 seed 数据用于开发预览；生产环境和严格模式会自动过滤，真实上线数据必须由学校账号/管理员审核链路进入。

### 校园招聘会前台按四张参考图重构（2026-06-16，Codex + Claude/Antigravity）

用户明确要求 `/campus` 首页采用参考图 1 的竖屏信息卡效果，点击进入学校招聘会后分别采用参考图 2/3/4 的详情效果。上一版“横屏大厅/垂直 Tab”方向已被替换，本轮限定在 Kiosk 校园招聘会前台，不改后端、数据库、Admin、Partner 和普通招聘会页面。

- 已调用 Antigravity 做 UI/UX 分析，结论为当前页必须改为参考图式竖屏触控页面：蓝色主推卡、四筛选、分组场次、底部服务入口；详情页使用横向 Tab 与分状态内容区。
- 已调用 Claude 做架构/合规审查，确认必须保留真实 API、`recordBrowse` / `recordExternalJump` 埋点、`COMPLIANCE_COPY` 合规文案和扫码/来源平台边界。
- `/campus` 首页改为：顶部返回 + 居中标题；蓝色“本校优先”主推卡；高校/城市/状态/时间四个触控筛选弹层且真实过滤数据；本校场次/同城高校/公共就业机构分组；AI简历诊断/活动资料打印/场馆导览三个服务入口。
- `/campus/:id` 活动概览改为参考图 2：蓝色活动 Hero、横向 5 Tab、活动介绍/亮点/面向人群/现场服务/准备清单，以及右侧来源机构、官方二维码、来源链接和数据说明。
- `/campus/:id` 参展企业页改为参考图 3：统计条、左侧筛选、企业列表、企业详情与岗位列表、右侧“投递方式说明”二维码。按钮只做“扫码投递/去来源平台投递”，不接收简历。
- `/campus/:id` 场馆导览/活动资料/AI求职准备改为参考图 4：顶部返回/收藏、横向 Tab、场馆平面导览优先、活动资料真实空态或资料行、AI 求职准备入口。
- `verify:jobfair-ui` 已改为新参考图守卫，要求 `FeaturedCampusCard`、`FairSection`、`ServiceShortcut`、`OverviewHero`、`CompanyPageShell`、`ResourcePageShell`、`VenueMaterialsAiPanel`、`SourceUrlQr` 和 `external_apply` 等结构标记，避免回退旧页面。

**已验证：**

- `pnpm --filter @ai-job-print/kiosk typecheck` ✅
- `pnpm --filter @ai-job-print/kiosk verify:jobfair-ui` ✅
- `pnpm --filter @ai-job-print/kiosk lint` ✅（仅保留既有 `KioskBusyContext` Fast Refresh warning）
- `VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 pnpm --filter @ai-job-print/kiosk build` ✅（仅保留既有 chunk size warning）
- 浏览器按参考图尺寸截图验证：`/campus` 943×1665、概览 941×1666、企业页 940×1663、场馆页 941×1666 均可渲染；请求 0 个 400/500；招聘闭环禁词 0 命中。

### 校园招聘会可用性复核与阻塞修复（2026-06-16，Codex + Claude/Antigravity）

用户要求确认“是否已经到可以让用户正常操作的节点”，本轮按上线前审查而非视觉验收处理。调用 Claude、Antigravity，并行本地前端/后端/合规审查后，确认上一版仍存在真实可用性问题：公开招聘会/参展企业/资料只取默认第一页、`更多场次` 等按钮无行为、职业规划路由错误、无真实场馆图时前端绘制虚构展馆、参展企业投递外跳记录被后端 target/action 校验拒绝。

**已修复：**

- Kiosk 招聘会 API 适配层支持 `page/pageSize`；`/campus`、`/campus/:id`、`/campus/:id/materials` 循环读取全部分页，避免生产数据超过 20 条时静默截断。
- `更多场次` 改为真实展开/收起；移除无后端能力的 `收藏本场`；`默认排序` 改为静态说明；职业规划入口修正为 `/resume/career-plan`。
- 无 `mapImageUrl` 时不再绘制虚构 A/B/C 展馆图，改为诚实空态；展厅指标从真实企业展区/展位字段派生，无数据时显示“现场公布”。
- 新增 `fair_company + external_apply` 活动记录类型：仅允许已审核发布招聘会下的参展企业记录“打开来源投递入口”，不记录投递结果、不接收简历；校园企业投递入口已改用该 target。
- “查看来源平台”与“扫码预约”弹窗分离；首页状态文案改为“未开始/进行中/已结束”，避免暗示站内报名。

**验证结果：**

- Kiosk：`typecheck` ✅；`verify:jobfair-ui` ✅；`lint` 0 error（保留既有 Fast Refresh warning）✅；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 build` ✅。
- API：`typecheck` ✅；`lint` ✅；`verify:activity-logs` ✅（新增 `fair_company/external_apply` 回归）；`verify:jobfair-venue-guide` ✅；`verify:public-fair-demo-guard` ✅；`verify:fair-company-positions` ✅；`verify:admin-fairs` ✅；`verify:fair-info-fields` ✅；`verify:partner-edit` ✅。
- 浏览器烟测：mock 模式下从 `/campus` 点击首个“查看招聘会”进入详情，活动概览 / 参展企业与岗位 / 场馆导览可渲染；AI 求职准备里的职业规划能进入 `/resume/career-plan`；请求 0 个 400/500；招聘闭环禁词 0。

当前判断：校园招聘会前台“浏览真实已发布招聘会、查看参展企业岗位、查看/打印真实活动资料、查看场馆导览、扫码预约/投递到来源平台、记录本人外部入口打开行为”已达到可让用户正常操作的代码节点。生产上线仍依赖真实学校数据、生产 API/数据库/存储、域名 HTTPS 与真机验收。

---

## 上线前 P0 使用阻塞修复：扫描生产禁模拟 + Agent printerName 必填（2026-06-15，Codex）

基于对 Claude/Mavis 上线评估复核，本轮只修真实使用时会误导或打错设备的阻塞项，不新增业务功能：

- Kiosk 扫描入口在 `VITE_API_MODE=http` 时直接提示“真机扫描待接入”并禁用“下一步”，避免用户进入演示扫描流程。
- Kiosk 扫描进度页在 `VITE_API_MODE=http` 时不再走 `mockFile()` / `Math.random()` 生成假扫描文件；若被旧路由或异常 state 进入，改为诚实失败并跳转失败结果页。
- `ScanResultPage` 在 http 模式下禁用“直接打印 / 保存文档 / AI 简历识别”三个出口，防止演示扫描文件进入真实打印、我的文档或 AI 解析链路。
- Terminal Agent 删除 `DEFAULT_PRINTER = 'Pantum CM2800ADN Series'` 和 `printerName || DEFAULT_PRINTER` 回退；`loadConfig()` 启动期校验 `printerName` 必填，缺失直接 fail-fast。
- CLI `print` 命令改为必须显式传 `--printer`；`agent-config.example.json` 的 `printerName` 留空，部署者必须用 Windows 实际识别名填写。
- 新增守卫：`pnpm --filter @ai-job-print/kiosk verify:scan-production-guard`、`pnpm --filter terminal-agent verify:printer-config`。

验证结果：两项新增 verify ✅；Kiosk `typecheck` ✅ / `lint` 0 error（保留既有 `KioskBusyContext` Fast Refresh 警告）/ `build`（`VITE_API_MODE=http`）✅；Terminal Agent `typecheck` ✅ / `build` ✅。

---

## Kiosk 21.5 寸竖屏首页设计预览 → 已删除（2026-06-15，用户不采纳）

前一轮（Codex）曾新增独立预览路由 `/design-preview/portrait-home`，用于评估 21.5 寸竖屏首页方向（四主服务入口 + 同页服务详情视图）。2026-06-15 用户评审后**明确不采纳该设计方向，决定保留现有正式首页**（`/` → `HomePage`，6 个服务分组），并确认首页与所有业务页 UI/UX 设计风格冻结，后续只做真实化 / 部署 / 验收 / bugfix / 合规与状态补齐。

据此删除被否决的预览脚手架（仅死代码清理，**未改动任何正式首页 / 业务页 / 入口结构 / 视觉 / 布局**）：

- 删除 `/design-preview/portrait-home` 路由注册（`apps/kiosk/src/routes/index.tsx` 的 import + route）。
- 删除 `apps/kiosk/src/pages/design-preview/PortraitHomePreviewPage.tsx` 及 `design-preview/` 目录。
- 删除 `apps/kiosk/portrait-home-preview-1080x1920.png`、`apps/kiosk/portrait-service-preview-1080x1920.png`。
- 删除守卫脚本 `apps/kiosk/scripts/verify-portrait-preview.mjs` 及 `package.json` 的 `verify:portrait-preview` 入口。

验证结果：Kiosk `typecheck` ✅；`lint` 0 error（仅保留既有 `KioskBusyContext` 两条 Fast Refresh 警告）；`VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 pnpm --filter @ai-job-print/kiosk build` ✅。
...
- 待续（P1-B 后续）：本组 SQLite service 级守门已补齐；后续统一整理是否把 P1-B 这组扩进 `postgres-readiness`（PG 版）。

---

## P1-B 验证守门 ⑤：岗位审核 / 发布状态机 verify（2026-06-14，Claude，分支 `chore/verify-job-review`）

岗位审核/发布状态机（`JobsService.reviewJobSource` / `publishJobSource`）含多条合规红线（未审核不得发布、reject 强制下架、终态不可回退），此前无专属状态机 verify。本轮补 service 级守门（temp SQLite，不改 JobsService 生产逻辑）。

- 新增 `services/api/scripts/verify-job-review.ts`（按验收顺序，service 直调）：① 初始 pending+draft；② 未 approved publish → 400 `PUBLISH_REQUIRES_APPROVAL`（合规红线）；③ approve → approved+draft（不自动发布）；④ approved 后 publish → published；⑤ Kiosk 公开查询（`getPublishedJobs`）只返回 approved+published；⑥ unpublish → unpublished 且不再进 Kiosk；⑦ 终态（approved/rejected）再 review → 400 `INVALID_STATE_TRANSITION`；⑧ reject 必填 reason（service 守卫 `REJECT_REASON_REQUIRED`），带 reason → rejected+draft+rejectReason；⑨ reject 强制 publishStatus=draft（人造 reviewing+published 脏态验证防御）。
- `package.json` 加 `verify:job-review`；CI 接进 build-and-verify 的 Verify 步骤（P1-B 组，紧随 verify:ai-config）。隔离同口径：temp SQLite（DATABASE_URL 由 runner/CI 提供），脚本只建+清自身夹具（Organization + admin User + Job + 审计行）。
- **验证**：本地 `verify:job-review` ALL PASS（11/11，未暴露 JobsService 真实 bug；过程修了脚本自身的 `.items`→`.data` 与缺 User 夹具导致审计 best-effort FK 报错——均测试侧，非生产逻辑）。
- ✅ **P1-B 这组 SQLite service 级守门已补齐**：打印链路 / 待机屏 Content / 审计 / AI 配置 / 岗位审核状态机。

---

## P1-B 验证守门 ⑥：3 个核心 DB verify 接入 postgres-readiness（2026-06-14，Claude，分支 `chore/verify-pg-extend`）

按「PG job 优先证明生产 PG 下核心状态机/事务/审计写读」原则，把 3 个高价值 DB 守门接进 `postgres-readiness` 的「Core verify suites on PG」段：`verify:print-jobs`（`$transaction` 原子 claim + 状态机幂等）、`verify:job-review`（审核/发布状态机 + FK + 分页）、`verify:audit-logs`（写读 + 过滤/倒序/分页）。

- **不接** `verify:ai-config`（纯 JSON 文件 + 加密，不碰 DB，接了零收益）；**暂缓** `verify:screensaver-content`（价值偏 AdAsset/Playlist CRUD、混本地存储/签名，PG 价值低于前三）。原则：PG job 优先证生产 PG 核心行为，不把所有 SQLite verify 原样塞进去拖慢 CI。
- 只改 `ci.yml`（postgres-readiness 段加 3 行；该 job env 已含 FILE_SIGNING / TERMINAL_ADMIN / TERMINAL_ACTION secret，无需加 env）。**不改任何 verify 脚本**（复用现有；SQLite 主 CI 仍同时跑这 3 个）。
- **本地真 PG 等价验证（PostgreSQL 16）**：throwaway 库 `db:pg:sync:check`（同步校验通过）→ `prisma generate` + `db:pg:generate` → `db:pg:deploy`（迁移全应用）→ `verify:print-jobs` 13/13、`verify:job-review` 11/11、`verify:audit-logs` 13/13 **全 ALL PASS**，跑完 dropdb。事务 / FK / 分页 / 状态机 / 审计在真 PG 与 SQLite 行为一致，无 PG 专属问题。
...
- **数据模型**：新增 `BrowseLog` 与 `ExternalJumpLog`（SQLite + PostgreSQL schema / migration 同步）。`targetType=job|job_fair|policy`（**枚举已扩展**：后续新增 `company_profile`、`fair_company`，详见 activity.types.ts；`fair_company`/`job` 对应 `external_apply`、`job_fair` 对应 `external_appointment`、`policy`/`company_profile` 对应 `external_open`）；`ExternalJumpLog.action=external_apply|external_appointment|external_open`；短 TTL 留存；删除为本人硬删 + 审计。匿名用户不落库，也不写本机存储，避免共享一体机泄露给下一位用户。
...
- **「我的」接真**：Profile 既有建设中入口「招聘会浏览记录 / 招聘会预约跳转记录」不再新增入口，提示用户查看下方账号资产；账号资产区新增真实「浏览与跳转记录」分组，按岗位 / 招聘会 / 政策分段展示最近记录、加载更多、删除、查看目标、再次打开来源平台 / 官方入口，含空态 / 加载 / 错误重试。（**注：2026-06-14 IA 整改已删除「账号资产聚合分组」，浏览/跳转明细移至 `/me/activity`；本段描述为当时实现的历史记录。**）
...
- **验证结果（Claude 复核 2026-06-12）**：`verify:activity-logs` **12/12 PASS，SQLite 与本地 PostgreSQL 双跑通过**；PG 空库 `migrate deploy`（0_init + `20260612200000_add_browse_jump_logs`）演练通过。浏览器登录态验收：岗位/招聘会/政策三链路浏览+跳转全部落库且来源快照齐全；「我的·浏览与跳转记录」分组渲染、Tab 切换、两步删除与删除审计通过；中途整页刷新（token 内存态清空）后匿名点击官方入口实测零落库——匿名隐私设计生效。

## 2E 职业规划真实化（2026-06-12，Claude，`feature/career-plan-2e`）

闭环矩阵 §五 落点全兑现：**真实化首页既有「职业规划」磁贴（不新增卡片）**。

- **链路**：磁贴 → `/resume/career-plan`：无简历 → 引导上传诊断（矩阵状态分支）；有简历 → 生成（**自动聚合本人同任务岗位匹配参考 + 会员最近模拟面试表现摘要**，依据材料在结果页/建议单如实展示；匿名面试凭证独立绝不跨链）→ 结果页（总览 / 现状画像含简历原文依据 / 1-3 发展方向+第一步 / 分阶段技能计划 / 行动清单）→ 打印建议单（专属版式 PDF → FileObject 进我的文档 + 打印订单）→ CTA 串联 优化简历 / 岗位匹配 / 模拟面试（结果页链路，不新增首页入口）。
- **后端**：`POST/GET /resume/career-plan/:taskId` + `POST .../print`；kind=career_plan upsert（同任务单行，继承 C-2A 归属与 TTL）；会员 AI 服务记录标签「职业规划」；审计仅元数据。
- **合规双层防线**（继承 2D 全套并扩展）：禁词 + 任何百分比 + **薪资数字承诺**（月薪/年薪可达…）→ 重试 → 连续命中 `AI_CAREER_PLAN_FAILED`；诱导编造/无依据示例数字 → 建议级过滤 + 安全兜底；现状画像 evidence 必须归一化子串命中简历原文；不判断人格/心理。
- **验证**：`verify:career-plan` **11 PASS**（双 CI job 均纳入）：闭环/上下文聚合（含匿名不聚合面试的隔离断言）/防编造/薪资承诺/建议级过滤/越权/upsert/文件清理诚实失败/PDF/日志脱敏。
- **浏览器真实验收（DeepSeek）**：磁贴点亮进入 → 无简历引导 → 上传诊断 → 生成 → 五分节完整渲染（现状画像三条全带真实原文引用，大专学历如实表述）→ 打印建议单 145KB 真实 PDF 进打印确认页 → CTA 三连在位；页面零禁词零百分比；运行日志零简历内容泄露。

---

## 生产部署与 Windows 本地主机换机验收清单（2026-06-12，Mavis）

针对用户提出的「当前页面功能打通后，后续上线服务器部署和更换 Windows 本地主机是否就没有问题」做上线前验收口径沉淀。
...
## 模拟面试记录接入「我的」AI服务记录口径（2026-06-12，Claude，`feature/interview-profile-ai-records`）

闭环矩阵 §六 P0-闭环项，零新模型零新端点（复用已验收的 GET/DELETE /me/mock-interviews）：

- 「我的」AI服务记录组顶部并列「模拟面试报告」条目（独立加载失败不阻塞 AI 记录列表）：岗位/面试官/完成时间/状态（报告过期如实标注），仅元数据**不含回答正文/转写/报告正文**；操作=查看报告（→ /interview/report 走原归属校验）+ 两步删除（既有硬删级联+审计）；首屏最多 3 条 +「查看全部」互链 /interview/reports。
- `/interview/reports` 头部新增「去我的 AI服务记录」（仅登录态）→ /profile，双向互链闭合。
- 匿名边界不变：账号资产区仅登录态渲染，匿名面试仍走 token/session 口径，不伪装跨会话资产。
- `verify:mock-interview` 16→**17 PASS**（新增 4b：列表仅元数据无正文泄露 / configured 未完成会话不出现）；member-assets-c2d 9 PASS 回归；双端三检+build 全绿。
- 浏览器登录态验收：真实短信登录 → 完成一场面试 → 「我的」见「模拟面试报告 · 行政专员 · HR 初筛」条目（无回答正文泄露）→ 查看报告跳详情 ✓ → /interview/reports「去我的 AI服务记录」回 /profile ✓。

---

## 首页入口与「我的」数据闭环原则确认（2026-06-12，Mavis）

用户确认：当前首页与各业务板块里的功能入口已经定版，后续不要再往板块里增加重复功能。下一阶段重点从「继续堆入口」切换为「已有入口真实化 + 操作逻辑闭环 + 用户数据归属打通」。

- **入口稳定**：不新增重复入口 / 同义卡片 / 额外菜单层；已有「职业规划」「岗位大师」「求职材料」「扫码签到」等占位后续只能真实化现有入口，不另起新入口。
- **闭环标准**：首页/板块入口 → 业务操作 → 产生用户数据 → 进入「我的」对应资产/记录 → 可查看、继续使用、打印、删除或复用。
- **正式矩阵**：新增 `docs/product/user-data-flow-matrix.md`，梳理 AI简历服务、岗位信息、招聘会、打印扫描、AI面试训练、政策服务、AI助手、设备状态与「我的」资产分组的对应关系。
- **2E 口径修正**：2E 不是新增「职业规划建议」卡片，而是把首页 AI简历服务组中已有的「职业规划」入口真实化；结果进入 AI服务记录，PDF 建议单进入我的文档，打印任务进入打印订单。
- **同步文档**：`feature-scope.md`、`CLAUDE.md`、`AGENTS.md` 已写入入口稳定与数据闭环规则，避免后续模型继续按「阶段任务 = 新增入口」执行。
...
- **后端**：`POST /resume/job-fit` + `GET /resume/job-fit/:taskId`（独立 JobFitService/LlmJobFitService/JobFitController，复用 resume_optimize 功能位与 C-2A parse 行归属门禁）；结果 upsert 落 `AiResumeResult kind='job_fit'`（同任务保留最近一次，继承归属与 TTL）；简历原文按 2B 模式凭 fileId 重提（清理后诚实失败不调 LLM）；member-assets AI 记录兼容 job_fit 标签「岗位匹配参考」。
- **合规双层防线**：输出只允许参考等级——prompt 禁止 + 服务端扫描**任何数字百分比**（如「85%」）与禁词（录用概率/通过率/匹配率/保过/精准命中/一键投递/立即投递/平台投递…）→ 命中重试一次 → 仍命中 `AI_JOB_FIT_FAILED` 诚实失败；**防编造**：matchPoints.evidence 必须归一化子串命中简历原文，全部编造视为无效输出；投递只引导「去来源平台投递」。审计仅元数据。
- **验证**：`verify:job-fit` 10 PASS（进 CI）：闭环/防编造/百分比拦截/禁词失败/未发布岗位 404/越权统一 NOT_FOUND/手填模式/upsert 单行/文件清理诚实失败/日志脱敏。浏览器真实验收（DeepSeek）：上传简历→真实诊断→选系统内真实岗位→「匹配参考：偏低」（行政简历对技术岗，结果诚实）+ 原文依据 + 来源平台引导，页面零禁词零百分比。三端三检全绿。

---

## 2C+ 模拟面试 Kiosk 交互修复（2026-06-11，Codex + Mavis 复测）

针对 2C+ 上线后用户反馈的「设置页像长表单、数字人面试页观感混乱、点击开始回答无明显反馈」做 Kiosk 前端交互收口，保持后端 2C/2C+ 能力与合规边界不变。

- **设置页重构**：`/interview/setup` 从纵向长表单调整为「左侧分组配置 + 右侧本次练习摘要 + 底部固定主按钮」；面试官/难度、岗位行业、经验时长、简历可选分区更清晰；目标岗位未填写时主按钮与摘要区明确提示，不再让用户进入空配置。
- **数字人面试间重构**：`/interview/session` 调整为顶部状态栏、左侧「小青」数字人面试官卡片、右侧对话记录、底部回答操作区；状态标签显示剩余时间、题号、播报方式、麦克风可用性；无会话直接访问改为明确失效页并提供重新开始。
- **语音按钮即时反馈**：点击「开始回答（语音）」后立即进入 `正在请求麦克风权限…` loading/disabled 状态；录音、转写、报告生成等阶段均禁用冲突操作，避免用户误以为按钮无反应。
- **麦克风失败强兜底**：浏览器拒绝或无麦克风时展示红色错误卡，提供「重新尝试语音」与「改用文字输入」；文字兜底仍可提交回答、触发追问并生成报告。
- **报告链路复测**：Mavis 浏览器复测通过：设置页创建会话 → 语音权限失败强提示 → 文字回答提交 → AI 追问 → 结束面试 → 报告页生成十模块报告；直接访问 session 无 state 显示会话失效。
- **验证**：`pnpm typecheck` 通过；`pnpm lint` 通过（仅既有 `KioskBusyContext.tsx` Fast Refresh warning 2 条）；`pnpm build` 通过（仅既有前端大 chunk warning）；`pnpm --filter ./services/api verify:mock-interview` **16 PASS**。
...
- **ASR provider**（对齐 OCR 范式）：`ASR_PROVIDER=disabled|tencent|baidu`。**生效配置为 tencent（用户确认数字人走腾讯云生态）**：腾讯「一句话识别」SentenceRecognition（≤60s，16k_zh），TC3-HMAC-SHA256 签名全在服务端，**复用 TRTC 对话式 AI 的 TENCENT_SECRET_ID/KEY（实测自带语音权限，零新增配置）**；百度通道保留为备选（需另行开通）。未配置时全链路自动文字兜底。
- **腾讯 ASR live 验证**：macOS 合成中文语音 → 真实 SentenceRecognition 693ms 精确转写（含标点）；浏览器全真链路（真实语音注入 MediaStream → 录音 → 真实腾讯转写 → 编辑确认）通过。
...
- **验证**：`verify:mock-interview` 12→**15 PASS**（进 CI）；五端三检全绿；浏览器验收六点全过（语音 UI/麦克风拒绝回退/文字兜底/录音→转写→编辑→确认（受控 stub ASR + 注入 MediaStream，transcriptEdited 落库实测）/报告基于转写/零违规文案含语速语调）。语音主路径已全真验收（腾讯）。
- **面试官官方语音播报（用户确认走腾讯语音包）**：新增 `TtsService`（腾讯 TextToVoice，**与数字人小青同音色 VoiceType=TRTC_TTS_VOICE(1008)**，密钥复用 TENCENT_SECRET_ID/KEY 实测含 TTS 权限，零新增配置）；`POST /mock-interviews/:id/turns/:idx/audio` 只为本会话已落库的面试官问题合成（防任意文本滥用；问题已过禁词扫描）；按句分段（≤90 字）合成拼接 mp3；TC3 签名抽共享 `common/tencent/tc3.ts`（ASR/TTS 复用）。前端播报优先官方语音（Audio data:mp3），失败自动降级浏览器本地 TTS（兜底保留）。浏览器验收：播报为真实腾讯 mp3、本地 TTS 零触发。不缓存不落盘，文本/音频不写日志。
...
- 待办（不阻塞提交）：身份横条为固定宽横向布局，需在真机/真视口核对竖屏 9:16 是否挤压溢出。
- 图片层叠修复（commit 531946f）：Hero 为 `position:relative`，身份横条原为 static，CSS 绘制顺序下被 Hero 图遮挡顶部；给横条加 `relative z-10` 叠在 Hero 之上。
- 首页瓦片接线复核（产品确认后执行，2026-06-09）：按「功能名只链接到真实可用页面、无真实执行页面则不链接」的原则统一：
  - 「格式转换」「证件照打印」原指向 `/print-scan/feature/{convert,id-photo}`（即"即将上线说明页"，非真功能），改为禁用「即将上线」、去掉链接。
  - 「AI模拟面试」「模拟面试」「面试技巧」原均跳通用 AI 助手 `/assistant`（无专门面试页），改为禁用「即将上线」。
  - 「求职材料」「证件复印」原 disabled 但残留死链接（feature/materials、feature/copy），清掉链接。
  - 结论：当前所有「可点」瓦片均指向真实可用页面；其余统一「即将上线」不可点。
  - 澄清：用户提供的招聘会/校园「富设计」截图（渐变卡 + 选地区三步条 + 场馆导览 + AI匹配率 + 标签详情）经三重核实（当前代码 / git 全历史 / legacy-miaoda 目录）均不存在于本项目，属外部（疑似秒哒）设计；其中「AI匹配率」正是合规边界明令移除项。产品确认：保持本项目现有简洁合规页，不重建该富设计。
- 首页瓦片新增与实质性审计（2026-06-09，产品确认后执行）：
  - 新增「职业规划」（AI简历服务组）、「岗位大师」（岗位信息组）——暂无真实执行页面，按规则标即将上线、不可点。
  - 去重：移除 AI简历服务组内重复的「AI模拟面试」（与 AI面试训练组重复）。
  - 实质性审计后再降级为即将上线：「简历素材库」（/resume/templates 真页面但素材为本地占位样例 MVP，非真实可下载模板）、「云打印」（与「文档打印」同跳 /print/upload，无独立云打印能力）。
  - 当前「可点」瓦片清单（均为真实实现页）：AI简历诊断/AI简历优化(/resume/source)、简历打印/文档打印(/print/upload)、纸质扫描(/scan/start)、全职/实习/兼职/全部岗位(/jobs)、社会招聘会(/job-fairs)、校园招聘会(/campus)、就业政策/补贴申请/档案登记(/renshi)。其余全部「即将上线」不可点。
