...
| 2026-06-30 | `codex/windows-acceptance-candidate` / 本分支 | 从干净 `origin/main@eba2dd0f` 完成终端设备档案与 Kiosk 统一配置代码候选迁入：Terminal 新增设备名、MAC、摆放位置、启停字段和 SQLite/PostgreSQL additive migration；Admin 终端页可编辑设备档案并写 `terminal.profile.update` 审计；终端 register 可写初始档案，heartbeat 仅允许安全补录空 MAC，且不覆盖 Admin 手工设备名 / MAC / 摆放位置；Kiosk 智慧校园优先读取统一配置并保留旧接口回退。公开 Kiosk config 收窄为智慧校园模块开关、配置版本、刷新间隔和服务端时间，不返回设备名、摆放位置、MAC、机构 ID 或机构名称，且不再用 MAC 作为公开查询键；停用终端允许心跳但拒绝 claim / 状态回写，避免继续打印。本分支保留求职材料库与手机扫码上传主线能力；真实 Windows Agent 上报、生产 PostgreSQL 迁移和奔图真机出纸仍需后续验收。 |
