...
| 2026-06-29 | `codex/terminal-device-profile-closure` / 本分支 | 补齐终端级动态百宝箱配置候选：新增 `TerminalToolboxConfig` SQLite/PostgreSQL additive migration、Admin `/toolbox` 终端配置页、Admin 百宝箱配置接口与 `toolbox_config.update` 审计；Kiosk 首页通过统一终端配置读取百宝箱，终端启用且百宝箱未显式关闭时固定显示在智慧校园上方，无配置/空配置显示“待配置”占位。后端默认启用百宝箱占位并限制功能项为已允许的 Kiosk 站内路径，拒绝外部 URL 和未知后台路径；智慧校园与百宝箱共用统一配置缓存，减少首页重复请求。`verify:terminal-device-config` 已补百宝箱 schema、公开白名单、默认显示占位、路径限制和空配置占位门禁；生产 PostgreSQL 迁移、预生产后台保存、Kiosk 真实终端展示和 Windows 真机仍需上线前验收。 |
