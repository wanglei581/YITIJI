...
1. 用户文件与简历资产商用闭环：文件资产留存模型、保存期限服务、Kiosk 用户自助设置 UI、Admin 文件生命周期运营视图、COS 生命周期与隐私文案验收材料、生产/试运营验收证据包、预生产候选集成、Gate 2 执行与 Gate 3 安全子集已完成；下一步必须先补齐 COS bucket 预生产/非生产用途证明，或切换到明确隔离的预生产 bucket，再执行 G3-06 `verify:cos:live` 与 Gate 4 浏览器账号验收，不能直接宣布生产/试运营完成。
