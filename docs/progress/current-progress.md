...
| 2026-06-21 | `codex/normalization-external-materials-index` / 本次提交 | 完成 T5 D 类外部材料索引：只登记外部材料清单和处置规则，不提交 PDF、OPC 原始输出或交付物。 |
...
1. 首批业务闭环重构：从“我的页商用闭环”开始，按旧入口、新目录、验证命令、双模型审查、删除旧实现条件推进。
