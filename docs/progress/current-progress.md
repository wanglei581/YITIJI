...
| 2026-06-22 | `codex/file-assets-preprod-cos-switch-plan` / 本分支 | 补齐用户文件资产预生产 COS bucket 切换审批包：确认本地和服务器均无腾讯云控制面 CLI，现有 `verify:cos:live` 只能做对象级 put/head/get/delete，不能创建或配置 bucket；后续必须先由用户在腾讯云创建或确认明确隔离的预生产 bucket、region、私有读写、CAM 最小权限、生命周期和 CORS 边界，再单独执行服务器 `.env` 切换、PM2 重启、health 和 G3-06 COS live。本分支只做审批包和执行边界，不修改腾讯云、不改服务器 `.env`、不重启 PM2、不执行 COS live 或 Gate 4。 |
...
1. 用户文件与简历资产商用闭环：文件资产留存模型、保存期限服务、Kiosk 用户自助设置 UI、Admin 文件生命周期运营视图、COS 生命周期与隐私文案验收材料、生产/试运营验收证据包、预生产候选集成、Gate 2 执行、Gate 3 安全子集和预生产 COS bucket 切换审批包已完成；下一步必须先由用户在腾讯云创建或确认明确隔离的预生产 COS bucket、region、私有读写、CAM 最小权限和生命周期/CORS 边界，再执行服务器 `.env` 切换、G3-06 `verify:cos:live` 与 Gate 4 浏览器账号验收，不能直接宣布生产/试运营完成。
