...
1. T3 E 类本地工具落地：确认 `.product-pm` PRD、`.superpowers` HTML 预览、`.ccg/commander` 桥和 `.workbuddy` 记忆的去向，再改 `.gitignore`。
2. T4 C 类任务证据筛选：只写清单，优先保留 plan / review / verify / deploy / audit。
3. T5 D 类外部材料索引：PDF/PNG/PPT/DOCX/ZIP 先确认仓库外归档位置。
4. 首批业务闭环重构：从“我的页商用闭环”开始，按旧入口、新目录、验证命令、双模型审查、删除旧实现条件推进。
