...
1. T4 C 类任务证据筛选：只写清单，优先保留 plan / review / verify / deploy / audit。
2. T5 D 类外部材料索引：PDF/PNG/PPT/DOCX/ZIP 先确认仓库外归档位置。
3. 首批业务闭环重构：从“我的页商用闭环”开始，按旧入口、新目录、验证命令、双模型审查、删除旧实现条件推进。
