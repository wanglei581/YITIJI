...
| 2026-06-22 | `codex/file-assets-gate4-runbook-preflight` / 本分支 | PREPRODUCTION GATE 4 API-LEVEL ACCEPTANCE PASSED WITH NOTES：用户确认 B 方案后，临时将预生产 `SMS_PROVIDER=log`，使用受控 MEMBER_A / MEMBER_B / 临时 Admin 通过真实 HTTP API + PostgreSQL + Redis + COS 完成会员登录、原始文件上传、默认 90 天、设置 180 天、原始文件长期保存拒绝、签名 URL 内部访问、跨账号 403、删除三态、过期清理和 Admin 生命周期汇总；执行后已回滚 `SMS_PROVIDER=tencent`，公网 health 复核 `db=postgres`，SSH 只读复核确认 `SMS_PROVIDER=tencent`、`FILE_STORAGE_DRIVER=cos`、`DATABASE_URL=postgres`、`REDIS_URL=set`。注意：完整浏览器截图、签名 URL 过期等待窗口和 COS 控制台/HEAD 证据仍待补；当时优化成果长期保存使用受控 DB 夹具标记 `assetCategory=optimized`，真实 AI 导出产物自动分类链路已由后续 `codex/file-assets-gate4-browser-ai-output` 做代码侧补齐，仍待部署后预生产复验。 |
| 2026-06-22 | `codex/file-assets-gate4-browser-ai-output` / 本分支 | 补齐真实 AI 导出产物文件资产链路：`FilesService.upload` 支持内部写入 `assetCategory/sourceFileId`，`/resume/generate/export` 产物统一按 `assetCategory=optimized` 入库；控制器只从已授权会员 AI parse 记录推导源文件，不信任前端 `sourceFileId`，并对源文件不存在或归属不匹配回退 `null`；`verify:resume-generate` 已覆盖普通上传默认 original、成果物长期保存、原始文件长期保存拒绝、匿名 system 文件不可长期保存和 sourceFileId 安全回退。该结果是代码侧验证，尚未重新部署预生产，也未补 Gate 4 浏览器/COS 人工证据。 |
| 2026-06-22 | `codex/member-login-data-closure-p0` / 本分支 | 完成会员登录个人数据隔离 P0：腾讯短信真号登录已完成线上冒烟；后端补齐简历提取文件读取归属门禁，会员只能提取本人文件，匿名只能提取匿名文件；新增 `verify:member-login-data-closure` 组合验证，覆盖提取层、文件服务、JWT fail-closed、会员登录和会员资产隔离。 |
| 2026-06-22 | `codex/member-login-session-closure-p1` / 本分支 | 完成会员登录个人数据隔离 P1：Kiosk 主要会员 API 统一 401 / 账号禁用失效通知并清空内存会话；手机号验证码发送和登录传递会话级 `deviceId`；收藏目标必须已审核已发布且标题由服务端派生；账号禁用后既有 Redis session fail-closed；新增个人数据留存矩阵和验证门禁。 |
...
- 2026-06-22 预生产服务器当前仍是阶段性验收状态：代码包已升级到用户文件资产 Gate 2 冻结候选 `2187f6a7`，health 指向 PostgreSQL，COS 已切换到隔离预生产 bucket 并通过 G3-06 live 验证，Gate 4 账号/API 级验收已通过并回滚短信 Provider；代码侧已补真实 AI 导出产物 `optimized/sourceFileId` 链路，但尚未重新部署到预生产；仍需补 Gate 4 浏览器截图和人工证据、百度 OCR Key 与 live 验证、AI/TRTC/ASR/TTS 按启用范围 live 验证、正式域名 HTTPS，以及腾讯短信审核后的真实手机号 E2E。

## 近期优先级

1. 用户文件与简历资产商用闭环：文件资产留存模型、保存期限服务、Kiosk 用户自助设置 UI、Admin 文件生命周期运营视图、COS 生命周期与隐私文案验收材料、生产/试运营验收证据包、预生产候选集成、Gate 2 执行、Gate 3 自动命令门禁、预生产 COS bucket 切换、G3-06 `verify:cos:live` 和 Gate 4 账号/API 级验收已完成；真实 AI 导出产物自动标记 `assetCategory=optimized` 与 `sourceFileId` 安全绑定已完成代码侧验证。下一步补部署后预生产复验、Gate 4 浏览器截图、签名 URL 过期等待窗口和 COS 控制台/HEAD 脱敏证据；不能直接宣布生产/试运营完成。
...
3. 会员登录个人数据隔离：P0 后端门禁与 P1 前端会话态 / 数据质量 / 账号状态 / 留存治理均已完成；后续进入线上配置复验、生产 Redis/PostgreSQL/COS 与真机闭环验收。
4. T5 派生任务：已跟踪旧 PDF 清理、商业计划书转正、路线图转正、OPC 策略输入归档，均需另起独立分支。
