...
1. 首批业务闭环重构：我的页商用闭环三段已完成（`ProfilePage` 拆分、`/me/ai-records` 页面、打印订单关联反馈）；下一组业务闭环从 AI 简历上传 / 资产中心开始，先执行 `docs/superpowers/plans/2026-06-21-ai-resume-assets-closure.md` 的 `/me/resumes` 页面分支。
