...
1. 招聘会 / 校园招聘闭环：Branch 1 列表页 `terminalId` 本校优先接线、Branch 2 `fair_company` 外部跳转记录、Branch 3 大页面零行为拆分均已完成；下一步进入上线前真实验收或另起分支处理 T4/T5 派生任务。
2. 会员登录个人数据隔离：P0 后端门禁与 P1 前端会话态 / 数据质量 / 账号状态 / 留存治理均已完成；后续进入线上配置复验、生产 Redis/PostgreSQL/COS 与真机闭环验收。
