...
| 2026-06-29 | `codex/terminal-device-profile-closure` / 本分支 | 补齐终端级动态百宝箱配置候选：新增 `TerminalToolboxConfig` SQLite/PostgreSQL additive migration、Admin `/toolbox` 终端配置页、Admin 百宝箱配置接口与 `toolbox_config.update` 审计；Kiosk 首页通过统一终端配置读取百宝箱，仅在终端启用、配置启用且存在可展示功能项时显示，无配置/空配置不保留占位。后端默认关闭百宝箱并限制功能项为已允许的 Kiosk 站内路径；智慧校园与百宝箱共用统一配置缓存，减少首页重复请求。`verify:terminal-device-config` 已补百宝箱 schema、公开白名单、默认关闭、路径限制和空配置不渲染门禁；生产 PostgreSQL 迁移、预生产后台保存、Kiosk 真实终端展示和 Windows 真机仍需上线前验收。 |
