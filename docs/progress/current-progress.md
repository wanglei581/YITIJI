...
| 2026-06-21 | `codex/normalization-evidence-triage` / 本次提交 | 完成 T4 C 类任务证据筛选：只登记高价值摘要、脱敏规则和后续任务，不整包提交 `.ccg/tasks/`。 |
...
1. T5 D 类外部材料索引：PDF/PNG/PPT/DOCX/ZIP 先确认仓库外归档位置。
2. 首批业务闭环重构：从“我的页商用闭环”开始，按旧入口、新目录、验证命令、双模型审查、删除旧实现条件推进。
