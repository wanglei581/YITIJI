...
1. 用户文件与简历资产商用闭环：文件资产留存模型、保存期限服务、Kiosk 用户自助设置 UI、Admin 文件生命周期运营视图、COS 生命周期与隐私文案验收材料、生产/试运营验收证据包、预生产候选集成、Gate 2 刷新方案、Gate 2 执行审批包、Gate 2 本地候选包预检、Gate 2 裁剪包本地构建预检、Gate 3/Gate 4 证据模板、Gate 3 AuditLog 命令证据链和完成度审计矩阵已完成；下一步必须先经用户确认执行预生产 Gate 2，再按证据包在 PostgreSQL + COS + 会员账号环境执行 Gate 3/Gate 4，不能直接宣布生产/试运营完成。
