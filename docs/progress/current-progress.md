...
## Terminal Agent 丢弃旧代理连接池（2026-06-05，`6784390`，已推送）
...
- `apps/kiosk/src/auth/IdleLogoutGuard.tsx`：监听全局触摸/鼠标/键盘事件；登录态闲置 **90s** 弹预警（30s 倒计时弹层）；**120s** 自动调 `logout()`；`busy=true` 时暂停计时；游客态不激活。

**修改文件（2 个）：**

- `apps/kiosk/src/layouts/KioskRoot.tsx`：`headerRight` 接 `useAuth`——未登录显示「登录」按钮（`navigate('/login')`）；已登录显示脱敏手机号 `displayName`（`phoneMasked`）+ 退出按钮（`logout()`）；挂载 `<IdleLogoutGuard />`。
- `apps/kiosk/src/pages/profile/ProfilePage.tsx`：顶部身份块区分两态——游客态显示登录引导条（含「登录」按钮）；登录态显示 `phoneMasked` + 退出按钮。现有简历/扫描/订单/AI 记录列表逻辑不变。

**合规验证：**

- token 仍只存 React state，不写任何浏览器存储（`localStorage / sessionStorage / IndexedDB / cookie`）。
- 无全局强制登录守卫：未登录访问 `/` 不跳 `/login`，游客可正常使用终端。
- tsc --noEmit 零错误，lint 零错误，build 通过。
- 未触碰 LoginPage / KioskNumPad / routes / services/api / jobs / jobsMeta。

---

## L2-4B：Kiosk /login 顶级全屏登录页（2026-06-04，`dcc0b27`，已合入当前分支）
...
- 无全局路由守卫：未登录访问 / 不跳 /login；L2-4C 只新增闲置登出守卫，不做强制登录。
- tsc --noEmit 零错误，lint 零错误。

**→ L2-4C 已完成**（见上方段落）：KioskRoot IdentityBlock 接 useAuth + ProfilePage 登录/游客态 + IdleLogoutGuard 90s/120s。

---

## L2-4A：Kiosk 前端纯内存会话层 + memberAuthApi（2026-06-04，`14bfb5e`，已合入当前分支）
...
**后续状态：** L2-4B 登录页与 L2-4C Auth Shell 均已完成并进入 `claude/l2-4c-kiosk-auth-shell` 提交链。
...
### L2-4C Kiosk Auth Shell 已完成（2026-06-04）✅

- **KioskRoot header 身份入口**：未登录显示"登录"按钮（navigate('/login')）；已登录显示 `displayName`（phoneMasked）+ 退出按钮（logout()）。
- **ProfilePage 身份块**：游客态显示登录引导条（含"登录"按钮）；登录态显示 phoneMasked + 退出按钮；现有简历/扫描/订单/AI 记录列表逻辑不变。
- **IdleLogoutGuard**：新建 `apps/kiosk/src/auth/IdleLogoutGuard.tsx`；登录态闲置 90s 弹预警（30s 倒计时显示）；120s 自动 logout；busy=true 时暂停计时；游客态不激活。
- tsc --noEmit 0 错误，eslint 0 warnings，未触碰禁止文件。

### P0-1/P0-2 Kiosk UX 第一批优化（2026-06-04，`1bea60f`，已推送）

**参考资料：** `legacy-miaoda/screenshots/kiosk/` 04、05（我的页）、01、02（首页）

**P0-1 ProfilePage 重构（`apps/kiosk/src/pages/profile/ProfilePage.tsx`）：**

- 移除扁平 PageHeader + 灰色身份条，改为 **蓝色 Hero 区**（`#0B2A5B`，同首页色调）：头像圆圈 + 游客/登录态文案 + 大登录/退出 CTA 按钮。
- 新增 **快捷服务图标网格**（4 个）：简历服务 → `/resume`、文档打印 → `/print/upload`、材料扫描 → `/scan/start`、AI 助手 → `/assistant`；每格 ≥88px 高，满足 21.5 寸触控。
- 新增 **账户与服务** 列表区（帮助中心 → `/assistant`；账号设置 → "即将上线"占位）。
- 保留并复用全部已有记录列表（我的简历/扫描文件/打印订单/AI 服务记录）逻辑不变。

**P0-2 HomePage Hero 登录感知（`apps/kiosk/src/pages/home/HomePage.tsx`）：**

- 导入 `useAuth`，在 HeroSection 中加登录感知条（device strip 下方）：
  - 游客态：蓝色横条 + "登录后可识别身份，后续将接入服务记录" + "立即登录"白色按钮（`navigate('/login')`）。
  - 登录态：横条显示 `UserIcon + "欢迎回来，{displayName}"`。

**纠偏（2026-06-04）：** 删除了 ProfilePage 的"后台管理"section（`window.open('/admin')`/`window.open('/partner')`）——公共前台不应暴露后台入口；修正了"跨设备查看"/"本次记录已保存"等过度承诺文案为诚实表述；移除了 `Building2Icon`/`MonitorCheckIcon` 未使用导入。

**验证：** tsc --noEmit 0 错误 · eslint 0 warnings · build 成功 · 本批未新增招聘闭环禁词；扫描命中均为既有合规说明/白名单/允许 CTA（如"去来源平台投递"），无新增违规文案。

### P1 第二批 Kiosk 服务中心"可用能力优先"优化（2026-06-04，`1bea60f`，已推送）

**范围：** `PrintScanHomePage.tsx` + `ResumeHomePage.tsx`，不改路由/API/后台/岗位/招聘会。

**PrintScanHomePage 改动：**
- 拆分为两区：**主区**（3 个有真实或演示链路的能力：文档打印/材料扫描/照片打印）大卡片，清晰"进入"CTA，min-height 176px 满足触控。
- **次级区**（证件照/格式转换/签名盖章）改为虚线边框+灰底小卡片，带"即将上线"标签，视觉上明确区分，不再和已上线能力混排。
- 移除了 6 个能力混在同一网格的视觉歧义；未新增路由或 API。

**ResumeHomePage 改动：**
- 主区：3 个核心入口（AI简历诊断/AI简历优化/简历素材库）用 `grid-cols-1 md:grid-cols-3` 三列展示，首屏同行完整可见，不被底部导航遮挡；每卡含彩色图标、说明文字和明确 CTA 按钮（"开始诊断"/"开始优化"/"浏览素材"）。（纠偏：原 grid-cols-2 导致素材库掉入第二行，2026-06-04 修正为 3 列）
- 次级区：面试准备从主网格移出，降级为虚线边框横行，带"即将上线"标签。
- 四步流程条改为紧凑单行；最近记录/合规提示保留位置不变。
- 未新增 fake 成功状态；面试准备 key 保留但不可导航。

**验证：** typecheck / lint / build 全绿（2393 模块）· 禁词扫描命中均为既有合规说明/白名单/允许 CTA，无新增违规文案。

**仍未做（P1 后续）：**
- HomePage 主服务卡片内子入口图标（参考旧版 2-3 个快捷子项）
- 印扫描/简历流程页细节打磨（如扫描演示提示更突出）
- AssistantPage 快捷词扩展

### 第三批 AssistantPage 快捷咨询面板（2026-06-05，`b22d95c`，已推送）

**范围：** `apps/kiosk/src/pages/assistant/AssistantPage.tsx` 只，不改路由/API/其他页面。

**改动内容：**

- 新增 `QUICK_GROUPS` 常量（6 组 × 2-3 题）：简历服务/打印扫描/岗位信息/招聘会/政策·人社/使用帮助，问题均为中性查询，不含招聘闭环词。
- 新增 `sendMessage(text)` 共用核心发送函数（提取自原 `handleSend`），同时供输入框和快捷问题调用，不新增 API。
- 新增 `handleQuickQuestion(text)` — 一点即问：点击快捷问题直接调 `sendMessage`，不需手动点发送按钮。
- 快捷咨询面板渲染条件：`messages.length === 1 && !loading`（仅在初始欢迎消息时展示，首次发问后自动隐藏）。
- 面板布局：移动端 1 列 / 平板 2 列 / 大屏 3 列，每题按钮 `min-h-[52px]`（满足 21.5 寸触控），无 hover 依赖，`active:bg-gray-100` 触控反馈。
- 初始欢迎态不自动滚到底部，避免移动端进入文字咨询后把快捷咨询面板顶部顶出首屏；用户发送消息后仍自动滚到最新回复。
- 保留 TRTC 懒加载、路由白名单、底部快捷入口、合规声明，不改 `AiAdvisorCall`。
- 岗位/招聘会快捷问题仅问"怎么查看信息"，不含平台内招聘流程表述。

**验证：** typecheck PASS · lint PASS · build 2393 模块 · 禁词扫描：本批新增代码和注释无招聘闭环词，无新增违规 UI 文案。

**仍未做：**
- 岗位/招聘会快捷问题的 AI 回复可进一步调校（需后端 prompt，非前端范围）
- 政策问题答案依赖后端 AI，当前为透传

---

### 工程收口：AiAdvisorCall TRTC API base 统一（2026-06-05，未提交）

**文件：** `apps/kiosk/src/components/AiAdvisorCall.tsx`

**改动：**
- 新增 `import { API_BASE_URL } from '../services/api/client'`
- `startCall` 中 TRTC 会话启动接口：`'/api/v1/trtc/session'` → `` `${API_BASE_URL}/trtc/session` ``
- `cleanup` 中 TRTC 会话停止接口：`'/api/v1/trtc/session/stop'` → `` `${API_BASE_URL}/trtc/session/stop` ``

**原因：** 其余 Kiosk API 调用均已通过 `API_BASE_URL`（由 `VITE_API_BASE_URL` 配置）路由，TRTC 两处仍硬编码 `/api/v1` 前缀；多实例/反向代理部署时若 API 前缀或 origin 不同将导致 TRTC 会话接口失效。本次仅补齐复用，不改业务逻辑、不改超时、不改 UI 文案、不新增环境变量。

**验证：** typecheck PASS · lint PASS · build PASS（模块数不变）
