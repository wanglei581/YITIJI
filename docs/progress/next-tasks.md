...
## 小程序 PR #756 合入退出标准与真机走查（2026-09-03 立，四家评审收敛结论）

> 立此节的原因：三家外部评审对「#756 现在转不转正式」给出三种答案（立刻合 / 证据齐了合 /
> 保持 draft），分歧本质是**退出标准没写下来**。写下来之后不必再问第四次；
> 勾满即合，不勾满不合。分支 `claude/miniapp-integrate-main`，本节只管这一个 PR。

**合入退出标准（全部勾满才 `gh pr ready`，缺一不合）**

- [ ] **E1 微信公众平台「用户隐私保护指引」把「麦克风」列入采集清单**。只有账号持有人能做。
      `app.json` 的 `permission['scope.record'].desc` 只是授权弹窗文案，不能代替后台声明。
      未声明则真机上录音链路直接失败，而开发者工具与 `tools/voice-degrade-falsify.mjs`
      ...
      **剪切板也勾**（`wx.setClipboardData` 10 处；见下方分歧说明）。
      摄像头的完整论据不止 scanCode：`resume-upload.js:91` 是 `sourceType:['camera']`
      **纯拍摄、没有相册兜底**，另两处 `chooseMedia` 也含 camera。

      **三家复核后的分歧与处置（2026-09-03，第二次更正）**：本清单初版把摄像头列为「留给产品负责人判断」、
      把剪切板列为该勾；第一次更正改成摄像头必勾、剪切板不勾；三家复核后**两项都定为勾**。
      代码事实三家一致，分歧在**微信映射口径**：一家认为 `wx.scanCode` 不走 `scope.camera`、
      `setClipboardData` 只是写入不算采集，故两项都不勾；另一家举出 2023/2025 两次真机实测报告，
      称只声明「选中的照片或视频」时拍照会 errno 112、`setClipboardData` 未声明也会 errno 112。
      **判据取代价不对称**：漏勾 → 真机功能直接断（扫码登录、纯拍摄上传、10 个复制按钮）；
      多勾 → 审核多问一句，而这两项都有真实用途答得上来。E2 走查第 8 步（复制动作）
      专为实测掉剪切板这条分歧而加。

      **`身份证号码` 是未决项，需产品负责人拍板**：小程序没有「身份证号」表单字段，
      但后端 `services/api/src/materials/pii-scan.util.ts:239` 会把扫描结果标成 `type:'id_card'` 并掩码，
      `id_scan` 是正式 `FilePurpose`，文档页也对用户写着「身份证等敏感文件已设置有效期」。
      一家认为无字段即非采集，一家认为既然识别并标记了证件号就该声明。倾向勾，用途写
      「检测你上传材料中可能含有的证件号，用于隐私遮挡与短期保存；不用于实名认证，不提供给企业」。

      **第 5 节存储期限是 E1 的显式子项**，不是顺带提醒：现填 0 天，须与
      `resume-upload.wxml` / `documents.wxml` 对用户的「敏感文件已设有效期、到期自动清理」承诺一致。

      **位置信息不勾的理由要写准**：`wx.getLocation` 只在注释里；但 `store-select.js:149`
      有活的 `wx.openLocation`，只是 :145 有守卫且后端从不下发坐标，永远到不了调用，
      且 `openLocation` 是显示调用方给的坐标、不读用户位置。（初版写「只有注释没真调」不精确。）

      **相册（仅写入）/ 通讯录 / 日历不勾的理由是「代码里没有这个接口」**：
...
- [ ] **E2 真机走查一次（约 15 分钟，体验版二维码）**，按下面的动线，留一份笔记。
- [ ] **E3 走查中未出现阻断级问题**；出现则修完重跑本节。
- [ ] **E4 合入前最后一次 `git merge origin/main`，且合并树 CI 三 job 全绿。**
      `main` 每天多笔合入，「0 落后」是易腐事实，不能拿过期绿灯说话；
      GitHub 的 `mergeable_state: clean` 只表示能干净合成，不表示 CI 跑过合成树。
- [ ] **E5 合入同时更新 [feature-scope.md](../product/feature-scope.md) §1.2**：
      该行现写「53 个注册页、98 PASS」，那是 `origin/main` 的事实（已核对，当前正确）；
      本分支是 62 页 / 112 PASS，**合入的那一刻 §1.2 就会变成陈述旧数字**。

**E2 真机走查动线（按顺序，不要临场设计）**

E1 未完成时：语音简历**只走「用手打 / 拒绝授权」，不要点录音**——那条链路已被
`voice-degrade-falsify.mjs` 在开发者工具里钉死（11 PASS / 0 FAIL），真机重复没有增量。

1. 登录（微信一键登录；失败则短信降级）
2. 我的文档：列表能出、预览一张 —— **这一步在开发者工具里测不出**，
   工具勾了「不校验合法域名」，真机走的是 `downloadFile` 合法域名
3. 发起打印：当前生产终端 0 台，**正确表现是诚实空态**；不要为了走通而造数据
4. 打印订单 → 点「订单详情」—— 曾必 404（`7a1a4133c` 已修：一体机历史任务
   不再渲染那个按钮）。这步是**验证修复**，不是探雷
5. 语音简历：不勾同意 → 应进整场手打；E1 勾完之后再回来试一次真录音。
   **关键判定**：仓里**零隐私弹窗代码**（`requirePrivacyAuthorize` / `onNeedPrivacyAuthorization` /
   `getPrivacySetting` / `__usePrivacyCheck__` 全仓命中 0），走的是微信默认弹窗 ——
   弹窗出现 → 点同意 → 能录，才算通过；**若没弹窗、直接 5.6 秒后降级成手打，
   那是隐私弹窗没接住，不是「授权被拒」**，两者要分清
6. 简历优化：看完对照 —— 结果带不走（导出要后端端点），**记一笔，不要现场开工**
7. 材料包：入口应明写未开放，点进去不应出到机码
8. **随便点一个「复制」按钮**（招聘会详情 / 政策详情 / 隐私页都有）——
   专为实测剪切板那条分歧：复制成功 = 勾对了；报 `errno 112` 或没反应 = 指引里剪切板没勾上。
   全仓 10 个复制按钮都吃这一条

**不要在这次走查里验**：职业生活圈、今日早报、招聘会内容（生产库为空，看到的空态是
真的空不是 bug）、材料包下单、方言切换（服务端还没有这个能力）。

**明确不进 #756 的（backlog，不是 blocker）**

- 方言切换、转写按人按天额度：都动 `services/api`。额度不是「约 15 行」——
  `POST /resume/voice/transcribe` **没有任何 Guard**，`endUserId` 拿不到，
  按人限额得先把身份绑上，那是鉴权面。现有闸是 6 次/分/客户端 + 每 IP 每小时
  `AI_IP_HOURLY_CEILING`（默认 300）。硬时点是 `ASR_PROVIDER` 翻 enabled 之前。
- 简历优化结果导出：要后端端点，同上桶。
- 版式第 6 类全量扫描：合入后只扫新 diff，不在合入前做。

**已澄清的两条误判（免得后来人照旧结论办事）**

- 招聘会提醒**不是**「等微信订阅消息模板」的断头路：全仓 `requestSubscribeMessage`
  命中 0 处，`utils/reminders.js` 文件头写明「不依赖后端，不需要微信订阅消息模板」，
  是 `wx.Storage` 本地提醒、App 启动时检查 24h 内临近项。它的真实局限是
  **用户不打开小程序就不会被提醒**，这是设计选择不是待办。
- #756 **已经包含 `services/api` 改动**：3 个文件 +55/−0 纯增量
  （self-assessment controller +25、policies controller/service 各 +15）。
  「小程序 lane 不动后端」是操作纪律不是分支事实，不要拿它当挡箭牌，
  也不要反过来当成可以再开一刀的许可。
