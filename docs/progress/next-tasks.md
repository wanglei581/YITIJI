...
- [ ] **终端设备档案生产验收**：`codex/windows-acceptance-candidate` 代码侧已补终端设备名 / MAC / 摆放位置 / 启停、Admin 编辑、公开 Kiosk config 白名单和停用终端拒绝打印任务；heartbeat 只允许安全补录空 MAC，不覆盖 Admin 手工设备名 / MAC / 摆放位置；Mac 侧预验收与仓库外交付包已完成（`/Users/wanglei/windows-acceptance-63931fe2`）。当前标记为**待预生产窗口执行**：其他页面任务仍在共享预生产或相关页面分支验收时，不覆盖 `/srv/ai-job-print`、不执行共享 PostgreSQL `db:pg:deploy`、不重启共享 API/Kiosk/Admin、不合入 `main`。上线前仍需在独立窗口完成 PostgreSQL 预生产 additive migration、确认无重复 MAC、用 Windows Terminal Agent 真机 register / heartbeat 上报 MAC，并实测 `enabled=false` 后 Kiosk 智慧校园关闭且 Agent 不再 claim 打印任务。
