...
- **前端：** Claude 单写者以用户指定的原项目 `docs/design/kiosk-redesign-2026-08/` 中 51 张编号稿为一体机新版设计基准，逐页核对真实 Kiosk 路由、状态、接口与操作；**1080×1920 竖屏是实际一体机主验收尺寸**，390 宽仅作兼容辅助。新版 `01-home.html` 为首页基准，线上旧首页不保留为目标版，被否决的替代布局试稿不并入。隔离分支同名稿有 48/51 份内容差异，已验证且不冲突的可读性修复可保留，须逐页实屏说明。小程序按相同产品语言适配原生交互；最后做 Windows Edge/27 寸与微信真机触控及动画验收，Codex 复核与集成。
...
| 13-print-desk | `/print/material-check` → `/print/desk?step=check` | 本地无文件态修复 1080/390 布局与真实去选文件出口；W2 5/5。有效文件等待/结果/隐私片段、参数报价及真机逐态待验，UI NO-GO |
