...
- **05/10/18/32/37 与 51 稿 UI：** Claude 的原稿 05 顾问舱 `0fe0dd941`、10 打印 Hub `140bdedf5`、18 扫描工作台 `8a163930b`、32/37 收银支付 `9f90c9d56`，以及 51 手机 QR 登录 `5c9ed5d4f`/换票据串态修复 `ba32fdc5a` 已合流；本地 W3 助手/解析 15/15、打印 Hub 9/9、扫描 26/26、收银 1080 6/6+390 1/1+金额 3/3+W2 打印 55/55、手机 390 浏览器 4/4 与相关 typecheck/静态门禁通过。这只证明本地夹具与浏览器，不等于 23 个支付状态或 51 号手机上传分屏全部验收。下一步核真实模型/语音/支付/设备与 Windows Edge/27 寸触控，按原始编号 HTML 继续 51 号手机上传分屏及其余稿逐态修复；#1031 旧动效层须先对照 Claude 动效。原稿、能力闸门和真实路由不反向改旧版；UI/商用仍 NO-GO。
...
- **容量治理：** 2026-09-25 本机约 45GB 项目根目录（其中 `.claude/worktrees` 30GB、`.worktrees` 7.2GB），Codex worktrees 另 49GB，Git 登记 205 个 worktree；根与候选的 `node_modules` 各约 1GB，单个大树不能按体积直接删。首棵干净旧树的可重装依赖缓存逻辑体积 1,140,124 KiB，实际 `df` 仅增加 16,140 KiB；暂停按 `du` 估算的批量清理，改按物理可用空间实测优先级。Agy 独立只读复核超时，尚无新的安全回收清单。先逐路径盘点未提交改动、独有提交、分支/任务所有权和真实占用，再给可审清理清单；不直接删除候选资产。此前百度云只读快照为 40GB 盘/约 21GB 可用、`/srv` 4.5GB；两份 runtime 备份各约 1.2GB，PG dump 可列目录但未恢复。需重新核对当前生产容量、保留期、恢复演练与发布峰值空间，未批准前不删线上备份/缓存。
...
| 51-phone-relay | `/member/qr-login` | QR 登录分屏已按原稿结构合流；390 宽有效票据/错误/确认等 4 个夹具浏览器用例通过，真实跨设备 claim 未验；同稿手机上传分屏待推进 |
