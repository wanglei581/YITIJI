...
## 2026-09-26：小程序首发提审准备（小程序专管窗口；步骤 0.11、2.6、2.7、3.14、5.4）

已完成部分见 `current-progress.md` 同日条目（分支 `claude/miniapp-nonrecruit-review-v2`）。剩下的按谁来做分三块。

**产品负责人在微信公众平台操作（内容已备好）：**

1. **用户隐私保护指引**（设置 → 服务内容声明）：只声明下面五类。清单与代码由 `verify:package-layout` 锁成一一对应，代码里新增或删掉隐私接口，门禁会红，提醒同步改后台。

   | 声明的信息 | 用途（填「为了____」这一空） | 代码里用在哪 |
   | --- | --- | --- |
   | 手机号 | 登录职易达账号，并在一体机和小程序之间同步你本人的简历、文档和打印订单 | 登录页「手机号快捷登录」 |
   | 选中的照片或视频信息 | 上传你要打印的图片，或拍摄纸质简历交给 AI 识别整理 | 我的文档、上传简历（拍照） |
   | 选中的文件 | 从微信聊天中选择你要打印、整理或让 AI 分析的简历和文档 | 我的文档、上传简历、材料包、手机传文件到一体机 |
   | 麦克风 | 在语音说简历、模拟面试语音作答和向小青语音提问时，把你说的话转成文字 | 语音说简历、模拟面试、小青 |
   | 剪切板 | 复制到机码、订单号和你导出的个人数据，方便你粘贴到别处 | 到机码页、隐私页的数据导出 |

   不要勾「摄像头」：拍照走的是「选择照片」接口，归在照片类；扫一扫不是隐私接口。位置、昵称头像、相册写入也都没有用到。其余栏目：存储期限按现有保存规则写；对外提供按「不主动共享或转让给第三方」填写，受托处理方写进补充文档（隐私政策）：DeepSeek / 通义千问（AI 生成）、百度智能云（文字识别）、腾讯云或百度智能云（语音转文字，以生产配置为准）——受托处理算不算「对外提供」，请律师一并确认。隐私政策正式发布见步骤 3.5a。微信会自动弹出官方隐私弹窗，小程序不需要自己做。
2. **服务类目**：工具 > 办公（主营）、工具 > 信息查询；深度合成 > AI 问答 / AI 创作 等模型厂商的算法备案证明和盖章协议到位后再加，赶不上就先提不含 AI 的版本（9/25 已定）。
3. **小程序备案**（步骤 0.11）。

**交给主执行窗口派后端（挂 3.13–3.14 这一批；小程序这边已按新口径改好页面）：**

1. **简历对照报告与提示词（3.14）：** `job-fit-pdf.service.ts` 去掉「参考等级」一行，两栏标题改为「简历里已经写到的要求 / 简历里还没体现的要求」；`llm-job-fit.service.ts` 的系统提示不再称「岗位匹配度参考」「准备投递」，删掉第 4 条「投递请引导用户前往岗位来源平台」和第 5 条「优先选择更匹配的岗位」，`summary` 改成不带评价的对照概述或删掉，`fitLevel` 不再生成；输出违规词加「建议投递、适合投递、胜任、匹配度」；`career-plan.service.ts` 不再把 `fitLevel` 喂进职业规划。**这一条不改完，小程序不能提审**（打印出来的报告上会有等级）。
...
2. 年满 14 周岁声明：第一次登录、上传、录音或发起 AI 分析前问一次，只问是否年满 14 周岁、不收出生日期；选未满就拦住这几类操作并说明。已登录会员写服务端（依赖上面第 2 条后端改动）。
3. 录音单独同意：模拟面试、语音说简历、小青三处共用一次同意，可在隐私页撤回；已登录会员写服务端（同上）。
...
**产品负责人决定（2026-09-25 晚）：** ① 人力资源服务许可证由产品负责人自行办理；一体机招聘会保留；小程序首发不上岗位、招聘会、找企业页面，按非招聘类目提审，拿证后再加回；一体机岗位列表保留，另加全局与逐台开关，律师或主管部门意见不利时立即关闭（9/25 三方定）。② 小程序与一体机打通成一个生态：小程序围绕线上功能，一体机保留线下定位与优势，不以小程序抢占、弱化一体机（见步骤 2.0 分工表）。③ 小程序可以付款，一体机交付实物；付款后 7 天未取自动整单退。④ 退款可在小程序、一体机「我的」、AI 客服自助申请；条件明确的即时自动退，其余人工审核；AI 客服不动钱。⑤ 取件码可分享给他人代取，也可本人取；一次性、7 天有效、可作废重发。⑥ 大模型 DeepSeek 或千问均可：默认 DeepSeek 主用、千问备用，公示备案号，做属地登记。⑦ 两个后台问题全部修复补全，三个空页做成真功能。⑧ 生产只读检查与安全处置由产品负责人自行负责，不在执行范围。⑨ 其余拍板事项按三方最佳推荐执行。⑩ 质量标准：像市面上正常的软件，每个功能、按钮、数据都正常；前端按 51 稿完成更新，不合理布局随做随改。 ⑪ 9/25 夜产品负责人授权按三方推荐执行：删除 259 个提交已进 main 或 PR 已合并且之后未再提交的远程分支（已执行，清单见 docs/reviews/remote-branch-cleanup-2026-09-25.md）；修过的 7 张原稿与 52 号回写产品负责人目录（仅覆盖与修正前原稿逐字节一致的文件，已执行并由他确认保留）；双端分工表采用（彩色、双面仍逐台开通）；福利活动按 31 号原稿迁移、有活动才显示入口；备用管理员平时停用。 ⑫ 9/26 产品负责人决定：岗位、招聘会、企业资料、政策这类招聘相关内容，改由机构端的运营机构自己审核、发布、署名；我们的管理员后台对这些内容只保留紧急下架；管理员端现有的岗位类页面按 3.13–3.15 调整，避开许可风险。公司定位、收费、托管方式与试点点位由 3.11 拍板（9/26 晚已拍板，见 ⑬）（三方建议做「设备与软件供应方」：卖终端、随机交付软件、按年收维保，不做招聘信息运营方）。 ⑬ 9/26 晚产品负责人拍板 3.11「按推荐执行，托管选 a」：公司做设备与软件供应方，收费只有设备款与维保费；一体机只放客户官网或官方渠道的二维码，我们云上不存岗位、招聘会、企业资料，客户要完整功能时再按 b 私有化部署到客户那边；客户机器上的打印由客户买断额度或走客户自己的商户号；试点若在自营点位，招聘模块整体关闭；生产现状核查（3.11 ⑥）由你处理。Claude、Grok、Agy 复核后一致确认选 a；已告知的代价：招聘会现场的展位图、到场码、拜访单与 AI 岗位推荐在我们云上暂停（b 版本可恢复），岗位匹配改为用户自填岗位要求。

**唯一候选：** 本分支 `claude/codex-task-history-progress-0de1dc`（由 `codex/commercial-closeout-active-20260924@ee5a6085c` 快进而来；Codex 分支保留不动，Codex 恢复后接在本分支之后）。远程备份 `backup/commercial-closeout-candidate-20260925`。任何时候只有一个开放的整合 PR（当前 #1042）。

**硬规则：** 每一处改动都要对应清单里的一个编号；不接受「顺手重构」和「为了看起来完整」的堆砌；改之前先查项目图谱和现有实现，能复用就复用；同一件事不许出现第二套实现；新文件必须说明为什么不能放进现有文件；不新增交接文档，进度只写 current-progress.md 和 next-tasks.md；不提交临时脚本、截图、日志；单文件行数守 CLAUDE.md 的线：超过 800 行不再加功能，超过 1000 行的只减不增；每个 bug 先复现、再修；回归测试只写在钱、权限、状态机这类会出大事的地方，不照着现状写断言；Grok 写的每个改动由 Claude 逐行审（三点 diff），看重复、命名、死代码和注释是否属实；Agy 审合同与反例，超时不算通过；按模块分路：每个模块只有一个写入方（分路表见下），共享文件只由所属分路改；任何时候只有一个开放的整合 PR；一个工作目录只许一个写入方；所有修复先进唯一候选分支，最后只通过 #1042 一个 PR 合入 main；删除旧代码要有证据（无引用、无门禁依赖、无文档声明）；删除任何历史资产前先保全、再列清单，由你确认；生产写操作、发布、真实资金、硬件操作，逐次找你授权。

**分路（每个模块一个写入方）：** Grok · 资金履约（订单、支付、退款、打印任务与取件，步骤 1.2、1.3、1.11、2.9 后端、3.6b、4.1）；Grok · 文件与会员（上传会话、会员绑定、文件与导出，步骤 1.1、1.5）；Grok · 登录与身份（鉴权、终端身份、取件会话，步骤 1.4）；Grok · 发布运维（CI、发布脚本、回滚、证书、生产检查，步骤 0.5 协调、1.6、3.6d、5.x）；Grok · 内容与同步（岗位与招聘会、内容关口、同步、按机器隔离，步骤 3.1 后端、3.3、3.4、3.13–3.15 后端）；Grok · AI 与合规后端（AI 配置与降级、模型白名单、注销执行器、真实 AI 验收，步骤 3.5b/d、3.6a、4.7）；Grok · Agent（Windows Agent、安装包、更新器，步骤 1.7、4.2）；Claude · 共享层（主会话）（一体机外壳、动效、字阶；只有这一路能改共享样式，步骤 2.2、2.3 统筹）；Claude · 一体机三路子代理（打印链 / 简历链 / 岗位招聘会与我的及其余；只动自己的页面文件，步骤 2.3、2.4、2.5、2.8、3.14 岗位招聘会页面）；Claude · 小程序（专用工作目录，契约门禁，步骤 2.6、2.7、2.9 页面、3.14「我的」岗位类条目）；Claude · 两个后台页面（管理员与合作机构后台的页面，步骤 3.1 页面、3.2、3.8、3.9 页面、3.13 与 3.15 页面）；Agy（合同与反例、每批独立复核；超时不算通过，步骤 贯穿）；你（产品负责人）（安全处置、生产检查、证书、云资源续费、许可证、商户与资源，步骤 0.2、0.8、0.9、1.10、3.11、3.15 存量授权、5.2 生产部分）。

### 阶段 0 · 止血与基线（9/25 – 10/9（含国庆））

先保住已有成果、堵住安全漏洞，再让 274 个提交第一次一起过 CI；外部依赖在国庆前全部发出去。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 0.1 保全未提交资产（已完成） | A-01、A-02 | 用临时索引给 5 处脏目录各拍一张快照（含未跟踪文件，设计渲染截图除外），存进本机备份引用；候选推远程备份分支并扫过密钥。 | 5 个本机备份引用 + 1 个远程备份分支；原目录零改动。（git for-each-ref refs/backup 列出 5 个引用；原目录 git status 与之前一致。） | 0.2 安全止血 | 9/25 |  |
| 0.2 安全止血（你负责） | L-07、L-10 | 由你自行完成（你已明确：这一项不需要我们处理）。我们只在需要时提供检查清单。 | 需要轮换的凭据由你处理完毕。（由你确认完成。） | — | 9/26 – 9/28 |  |
| 0.3 确定唯一候选与写入方，写入推进方案（进行中） | A-01、A-03 | Claude 会话分支快进到 ee5a6085c 作为唯一候选（Codex 分支保留不动，Codex 恢复后接在本分支之后）；本方案写进 next-tasks.md 与 current-progress.md；你把微信开发者工具改指向候选目录。 | 只有一个候选、一个写入方；进度文档和本方案一致。（9/25 已完成候选确立与方案入库；微信开发者工具改指向候选目录由你操作。）（git rev-parse 候选 = 本方案提交；图谱校验通过。） | 0.4 首次全量 CI | 9/26 | 你改开发者工具打开的目录 |
| 0.4 候选首次全量 CI（已完成） | R-01、Q-02 | 把候选快进推到 #1042 的分支，跑三个必需作业；再手动触发一次 Windows 安装包流程（这次仍是测试签名，不进现场）。 | 一张红灯清单——第一批隐藏 bug，每个红灯标了归属模块。（9/25 首轮 CI 36122234749、安装包 36122234761：时间炸弹夹具、原稿几何 52 处、服务台文案漂移、Burn 引擎签名；Grok 本地预跑另查出 6 处。）（gh run view 看 build-and-verify、postgres-readiness、kiosk-browser-smoke 三个作业。） | 0.5 修到全绿 | 9/27 – 9/28 |  |
| 0.5 修到三个必需作业全绿（进行中） | R-01、Q-02、R-09 | 按红灯所属模块交给对应分路修（见协作规则的分路表），Claude 逐个审；每个红灯记原因和修复提交。10/9 是目标，不是死线：红灯多就顺延，但不跳过。 | #1042 在候选上三作业全绿。（三个作业在同一个提交号上都是 success。） | 1.9 发布前提之一 | 9/28 – 10/9（按红灯数量浮动） |  |
| 0.6 开放 PR 收拢成 1 个（已完成） | A-03 | 核实 #1021、#937 的内容已被候选中 941d10813、8feaadfac 取代后关闭；#1024、#742、#1031、#606 先关闭并注明「在 3.10 / 1.6 / 2.2 / 4.2 抽取」，分支保留到抽完。（原稿把 #1024 列为内容已在候选，核实后其走查脚本并未进候选，改归待抽取组。） | 开放 PR 只剩 #1042；每个关闭的 PR 都写明内容去向。（9/25 完成。）（gh pr list --state open 只剩 #1042。） | 0.7 旧分支清理 | 9/29 – 10/2 |  |
| 0.7 远程旧分支清理（已完成） | A-07 | 逐个用只读接口确认没有开放 PR 在用、内容已进 main 或已明确放弃，再分批删；保留 main、整合分支、备份分支和待抽取的四支。9/25 清单：368 个中可直接删 259（86 个提交已在 main，173 个 PR 已合并且之后未再提交），逐个判断 102（合并后又有提交 13、PR 关闭未合并 70、从未开 PR 19），保留 7。 | 远程分支降到约 35 个。（9/25 两批共删 334 个，368 → 34：第一批 259 个提交已在 main 或 PR 已合并无新提交；第二批 75 个经 Grok 逐个比对、Claude 抽查、Agy 复核。保留 27 个有未吸收内容的分支待对应步骤后再清。清单、完整提交号与恢复方法见 docs/reviews/remote-branch-cleanup-2026-09-25.md。）（git ls-remote --heads 计数；删除清单入档（已合并 PR 的分支可在 GitHub 恢复）。） | 收尾项，不阻塞阶段 1 | 10/2 – 10/9 | 需要你同意 |
| 0.8 国庆前发出全部外部依赖 | L-01、L-02、L-04、L-05、L-06、L-08、L-09、M-10、R-11、D-01、D-07 | 我在 9/29 前备好每一项的材料清单；你在 9/30 前发出：法务（小程序类目与许可、AI 登记或备案、隐私政策、账号注销、肖像授权、来源授权与免责）、微信支付商户核对、手机号登录的企业认证与额度、云资源到期与自动续费、正式代码签名证书采购、两个微信测试账号；奔图彩色参数发厂家询问（不阻塞）。 | 每一项都有负责人和预计办完日期，写进进度文档。这一步是「启动」，不是「办完」。（进度文档里的外部依赖表，每行有负责人、发出日期、预计日期。） | 0.11 小程序备案；3.5 法务落地；4.1 商户实单；4.2 正式签名 | 9/26 – 9/30 | 需要你和法务 |
| 0.9 生产只读体检（你负责） | R-03、R-08、R-12、R-13、R-15 | 由你自行完成（你已明确：这一项不需要我们处理）。我们只在需要时提供检查清单。 | 体检报告入档；要修的项排进 1.6、1.10、5.2。（Deploy Precheck 运行记录 + 预检脚本输出 PREFLIGHT OK。） | — | 授权后 1 – 2 天 |  |
| 0.10 本机资产判定与空间回收 | A-01、A-02、A-04、A-05、A-06 | Grok 只读做并集盘点（每项是否已在候选、有没有独有价值）；Claude 判定迁移或废弃；只回收「干净 + 已并入 + 无进程引用」的目录，以 df 实测为准。 | 每项资产有结论；有价值的迁入候选；确认无用的回收空间。（盘点表入档；df 前后对比。） | 收尾项，与阶段 1 并行 | 10/2 – 10/23 | 删除清单需要你确认 |
| 0.11 小程序备案与类目申报 | L-01、M-04 | 按法务对类目的结论，你在公众平台提交小程序备案和类目；我准备备案与类目需要的页面说明和截图。 | 备案通过、类目确定。（公众平台备案与类目截图入档。） | 5.4 首次提审 | 10/8 – 11/6 | 法务类目结论；公众平台管理员 |

### 阶段 1 · 资金与数据 P0（9/28 – 11/2）

关掉所有「会丢钱、丢文件、发布回不去」的问题，然后做第一次受控发布，让线上不再落后。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 1.1 会员手机上传绑定 P0 | F-01 | 分路「文件与会员」：把 f76a88dde 接到候选上，补修第三类（归属切换后仍发布 confirmed、过期清扫用旧记录写回、删对象失败被吞）；Claude 逐行审，Agy 出反例；隔离的 PostgreSQL、Redis、存储上做故障注入。 | 四类故障（杀进程、锁过期、并发确认与取消、删对象失败）注入全部通过；手机到一体机实测放在 4.4。（pnpm --filter @ai-job-print/api verify:upload-sessions（扩充后，PG 作业已含）。） | 1.5 收费导出（同一分路） | 9/28 – 10/12 |  |
| 1.2 已付款异常：堵住「收钱不出纸」（进行中） | F-03、F-04、F-06 | 分路「资金履约」：领任务跳过失效文件时写入「已付未履约、需退款」标记（不自动打款、不自动重派）；缺纸、断电、未确认分别写清去向，不和文件失效混成一条；只做整单退，删掉后台订单页和收银台里「部分退款」的误导文案。 | 注入「付款后文件失效」时订单进入待退款、不出纸；缺纸、断电、未确认三条各有断言；收银台和后台不再出现部分退款。（扩充 verify:payment-flow、verify:refund-idempotent、verify:admin-ops。） | 1.3 取件凭证（同一分路） | 10/8 – 10/20 | 你定退款规则（拍板第 3 条） |
| 1.3 付款后取件凭证：先定用途再做 | F-02 | 等你定用途（用于失败补打或离开后再取，还是不再显示这个码）。要核销就复用现有取件页和已付失败重试，加一次性、终端一致和限流；不要就去掉显示。不做第二套接口。 | 凭证要么能一次性核销出纸，要么不再出现；重复使用、跨终端、猜码都被拒绝。（扩充 verify:print-jobs、verify:payment-flow。） | 4.4 现场闭环 | 10/20 – 10/30 | 你定凭证用途、补打与有效期（拍板第 4 条） |
| 1.4 取件与权限的真实库补测 | F-08、F-12 | 先列出这四条没覆盖的场景（真实换票、跨终端、内部账号降权后旧会话）再补测；读压测做成一条不进默认 CI 的脚本。 | 补测场景清单与结果入档；读压测数据入档。（verify:print-jobs、verify:member-account-status 等 PG 作业记录 + 压测输出。） | 1.8 排雷 | 10/12 – 10/17 |  |
| 1.5 收费导出一致性 | F-09 | 沿用现有暂存机制，只补并发和删对象失败两种场景，不另做导出意图。 | 并发、崩溃、删对象失败三种场景下收费与文件一致。（扩充 pnpm --filter @ai-job-print/api verify:resume-export-formats（已有崩溃注入）。） | 1.8 排雷 | 10/13 – 10/20 |  |
| 1.6 可执行回滚与恢复演练 | R-02、R-12、F-13 | 分路「发布运维」：移植 #742 的恢复脚本，接进发布失败分支（自动恢复库和运行目录）；用临时 PostgreSQL 和样例数据演练：恢复备份、执行全部新迁移、故意让一次发布失败并回到旧版本。不取生产数据。 | 健康检查失败会自动回到旧版本；临时库恢复并迁移成功；有演练记录。（临时库 pg_restore 后 pnpm --filter @ai-job-print/api db:pg:deploy；verify:release-provenance、verify:deploy-data-safety-gate。） | 1.9 发布前提之一 | 10/9 – 10/23 |  |
| 1.7 现场测试日一 | D-03 | 集中在一天做完：测试页出纸、无个人信息的扫描样张、扫码枪、U 盘；再做物理故障注入——拔纸盒、开前盖，看 Agent 是否上报、一体机是否在付款前拦住。 | 测试页与样张照片、扫码枪识别记录、故障注入录像；发现的问题进排雷清单。（现场录像与照片入档；静态预检 pnpm --filter terminal-agent verify:print-scan-agent。） | 4.2 现场测试日二 | 10/12 – 10/16 选一天 | 现场人员一天 |
| 1.8 模块排雷第一批 | Q-01 | 每个模块由对应分路按「钱、状态机、并发、权限、异常路径、隐私」六类逐条查，Agy 出反例，Claude 审；每个 bug 先复现、再修、再复核。 | 四个模块各一份排雷结果（写进进度文档）；发现的 P0 在 1.9 前修完。（每个 bug 用所属模块已有的 verify 复现；没有的补一条最小断言。） | 1.9 第一次受控发布 | 10/12 – 10/30 |  |
| 1.9 第一次受控发布 | R-05、R-06 | 前提：0.5 全绿、1.6 回滚接好并演练过、1.1 与 1.2 已进候选、0.9 体检完成。合入 main → 手动全量发布 → 健康检查 → 保留回退；真实支付保持关闭到 4.1；发完把自动发布开关关回去；随后开一个新的整合 PR 接替。线上此时仍是试运营、不对外宣传。 | 线上换成新候选，新版界面开始上线，失败能自动退回。（健康检查 ok，首页版本号与发布记录一致；DEPLOY_API_ENABLED 回到 false。） | 阶段 2、3 持续推进 | 10/27 – 11/2 | 你授权发布 |
| 1.10 HTTPS 证书续期确认（你负责） | R-10 | 由你自行完成（你已明确：这一项不需要我们处理）。我们只在需要时提供检查清单。 | 续期演练成功，续期后 nginx 自动重载，有到期提醒。（certbot renew --dry-run 成功；到期日检查。） | — | 10/19 – 10/30 |  |
| 1.11 自助申请退款 | F-14 | 分路「资金履约」：新增「申请退款」接口，资格与自动任务复用现有退款服务；条件明确的即时自动整单退，其余进后台「待审核」；一单一次、每日限次；小程序订单页、一体机「我的打印订单」加按钮（Claude）；AI 客服加「退款」意图，只解释进度并带用户到订单页。 | 用户三处都能自助申请；符合条件的秒退，其余可见审核进度。（扩充 verify:refund-idempotent、verify:payment-flow；新增申请资格与限次断言。） | 4.1 实单验证 | 10/20 – 11/3 |  |

### 阶段 2 · 前端与小程序（10/12 – 11/27）

一体机按 51 张原稿逐页逐状态落地，小程序扫清提审障碍；分四路并行，各管各的文件。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 2.0 双端生态分工表（已完成） | E-01 | Claude、Grok、Agy 盘点小程序 64 页和一体机全部路由，按「一体机线下优势 / 小程序线上功能 / 两端打通点 / 重复需调整」出一张分工表，交你确认后作为 2.x 各步的依据。 | 一张经你确认的双端分工表。（9/25 按三方推荐采用；打印一行按 Grok 意见改为「彩色、双面仍逐台验收开通、先改价目描述，两端同时开放」。）（分工表写进 next-tasks.md；后续页面改动都对照它。） | 2.1、2.6、2.7、2.9 | 9/26 – 10/9 | 你确认分工表 |
| 2.1 51 稿逐状态差距清单（进行中） | K-03 | 在现有 51 稿登记表上补列「空、等待、错误、禁用、结果」各状态的差距；1080×1920 下原稿与运行页逐状态截图对照。 | 每页每状态一行：差距、工作量、归哪一路。（现有截图脚本逐页出图；pnpm --filter @ai-job-print/kiosk test:browser:route-sweep。） | 2.4 逐页修正 | 10/12 – 10/17 |  |
| 2.2 共享动效曲线并入 | K-05 | 只移植 #1031 的共享曲线和按钮、页框原语（1–2 天）；逐页的入场、按压、等待呼吸跟着 2.4 各页一起做，不单独施工。 | 共享动效就位；开启「减少动态效果」后所有动画停止、布局不跳。（pnpm --filter @ai-job-print/kiosk verify:qingxu-proto-geometry（含动效断言）+ 减少动态效果录屏。） | 2.4 逐页修正 | 10/16 – 10/20 |  |
| 2.3 6 个旧外壳页迁移 | K-01 | 按对应原稿迁到青序流光外壳；合同审查保持能力闸门关闭，只换外壳。51 稿之外的智慧校园 4 页、福利活动 2 页，按你的决定迁移或下线入口。 9/25 三方更正：福利活动两页（/activities、/activities/:id）其实在 31 号原稿里，改为按稿迁移，入口在服务端有进行中的已发布活动时才出现，没有就整块隐藏；智慧校园维持「本机没开通就不出现」。 | 一体机不再出现旧界面。（pnpm --filter @ai-job-print/kiosk verify:fusion-w5、verify:fusion-w6。） | 2.4 逐页修正 | 10/20 – 11/3 |  |
| 2.4 逐页逐状态还原原稿 | K-03、K-04、K-07 | 按 2.1 清单分三路并行：打印链、简历链、岗位招聘会与「我的」及其余；每路一个工作目录、只动自己的页面文件，共享层只由主会话改。能力口径不对的地方改运行时文案，不改编号原稿。9/26 晚托管选 a：26、27、28 号稿与找企业相关页面在我们云上不显示，逐态还原留到 b 版本；首页岗位、招聘会入口按 3.14 调整。 | 51 稿逐页逐状态对照截图通过；打印链与简历链在 11/13 前完成。（test:browser:w2、test:browser:w3、verify:print-parameter-capability、verify:print-confirm-honest；截图对照表。） | 4.3 实屏走查 | 10/20 – 11/20 |  |
| 2.5 断网诚实态与 AI 降级 | K-06、K-09 | 在现有设备状态之上加失联引导，自动切 09 离线页；出付款码前读接口可达和支付通道状态；把 AI 降级门禁扩到优化、材料、顾问。 | 断网时付款码出不来并有说明；关掉 AI 后每个 AI 入口都能走手动路径。（verify:device-status-honest、verify:ai-down-fallbacks（扩充后）；新增一条「失联后不调用下单」断言。） | 4.3 实屏走查 | 10/27 – 11/10 |  |
| 2.6 小程序提审障碍：分包、隐私弹窗、拿掉招聘类页面 | M-08、M-01、L-01 | 小程序专用工作目录里：四个 Tab 留在主包，其余按业务域分包；接入官方隐私弹窗（弹窗文案依赖 3.5a 发布的隐私政策）。按你的决定，小程序首发提审包里拿掉岗位、招聘会、找企业三类页面，按非招聘类目提审；拿证后再加回。 | 开发者工具上传时主包明显低于 2MB；未同意隐私时敏感接口被拦、同意后登录成功。（pnpm --filter @ai-job-print/miniapp verify:static（加一条断言：含分包与隐私检查）；开发者工具上传体积截图。） | 2.7 小程序逐页走查 | 10/12 – 10/26 |  |
| 2.7 小程序逐页走查与求职进度 | M-09、M-07、M-02 | 逐页走查出问题单并修；写一个同一测试会员两端读订单、简历、AI 记录的对照脚本；小程序新增求职进度页、一体机补手填，调用现有 /me/job-applications（后端已注明不需要许可证，与类目问题脱钩）。 | 问题单逐条关闭；两端同一会员数据一致；求职进度两端可增删改、第三方不可写。（verify:job-application-track；小程序 verify:static；两端对照脚本输出。） | 4.3 真机走查 | 10/27 – 11/20 |  |
| 2.8 52 号两种 AI 产物入口 | K-02 | 补一体机对现有顾问接口（换型、填槽、运行）的调用，做「草稿槽」「对比报告」两种发起入口；02 按你的决定恢复或不做。 | 三种产物都能从前台发起并打印。（pnpm --filter @ai-job-print/api verify:advisor-work；浏览器用例。） | 4.3 实屏走查 | 11/3 – 11/13 | 拍板：02 是否恢复 |
| 2.9 小程序付款与取件码分享 | M-11、F-02 | 分路「资金履约」补小程序内付款（JSAPI），支付回调同时认一体机与小程序两个 appid，付款后先查单再确认；7 天未取自动整单退。Claude 做小程序付款确认页和取件页的「复制/分享取件码」「作废重发」。一体机侧沿用现有到机码核销。 | 小程序下单付款后，本人或被分享的人都能凭码在一体机取件。（扩充 verify:payment-flow（小程序支付回调、双 appid）；小程序 verify:static；测试商户实单在 4.1。） | 4.1 实单验证 | 10/27 – 11/13 | 商户号；把小程序 appid 绑到同一商户号 |

### 阶段 3 · 后台、内容与合规（10/12 – 11/20）

先把招聘相关内容的运营权交给机构（3.11–3.15，排在 3.1 之前），再把内容关口和合规要求落进代码，AI 与对账补齐，隐藏 bug 第二批排雷。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 3.11 角色定位、收费与托管拍板 | L-11、L-12 | **9/26 晚已拍板：按推荐执行，托管选 a。** ① 公司做「设备与软件供应方」：卖终端并随机交付软件，按年收维保与技术支持；不做招聘信息运营方，不承接招聘会承办这类要持证的就业服务外包；以后办下许可证再评估做服务方。② 收费只有设备款与维保费，不收任何与岗位、招聘会、浏览、投递、入职挂钩的费用。③ 托管 a：一体机只放客户官网或官方渠道的二维码，我们云上不存岗位、招聘会、企业资料。客户要完整的岗位与招聘会功能时，按 b 私有化部署到客户的服务器或政务云；升级到 b 的条件：客户愿意单独付部署费，并由客户提供服务器、以客户名义备案。c（我们云上按机构隔离）律师书面同意前不做。④ 客户机器上的打印：客户买断额度、用户免费，或走客户自己的商户号；律师确认前不走我们的商户号。⑤ 试点若在自营点位，招聘模块整体关闭。⑥ 生产现状（你负责，现在就做）：确认生产库里没有已发布的岗位与招聘会，有就用现有后台下架；1.9 发布时岗位板块全局关闭。另把「岗位匹配」的名称与说明文案、合同文本一并交 0.8 的律师。 | ①–⑤ 已拍板，写进本方案头部与 compliance-boundary.md §1.1；⑥ 待你回执。（决定条目入档。） | 3.12 | 9/26（⑥ 待你回执） | ⑥ 生产核查回执；律师对 ④ 与岗位匹配文案的意见 |
| 3.12 角色口径落文 | C-08、C-11 | Claude（主会话）改写 docs/product/role-boundary.md：机构是招聘类内容的运营与发布主体（托管 a：我们云上不存岗位、招聘会、企业资料，机构在我们这里只维护官方渠道二维码与政策；完整招聘功能只在客户私有化部署的 b 版本里，由客户运营）；我方是设备与软件供应方，管理员对招聘类内容只保留紧急下架，另有机构入驻核验（是否公共机构、许可证号与许可范围）、终端绑定、全局开关、审计；删去「路径 A / B」和数据接入费、上架费、流量结算等收益写法。同步 CLAUDE.md §18 第 3 条与末条（审核主体改为运营机构）、§9A 一体机「岗位信息」「招聘会信息」的口径（我们云上改为官方渠道入口），以及 feature-scope.md 里把岗位与招聘会写成在营能力的地方。合同与告知要点交 0.8 的律师：设备买卖与维保合同、个人信息委托处理协议、机构发布责任承诺、招聘页投诉渠道。只改文档，Agy 复核。 | 文档口径与 3.11 结论一致，其他会话不会再按「管理员审核发布」加功能。（Agy 复核；pnpm graph 与 pnpm verify:repository-integrity 通过。） | 3.13、3.14、3.15 | 10/12 – 10/16 | 3.11 结论 |
| 3.13 关闭我们云上的招聘内容托管，管理员只留紧急下架 | C-08、C-10 | 分路「内容与同步」（后端，Grok 估 3–3.5 人日）：把现有岗位板块开关扩成「招聘内容托管」总开关，**我们云上默认关**（现在没配置时默认开，而且只挡岗位读取）。关闭时，岗位、招聘会（导览、展位、资料与打印链接、统计、拜访单、到场码）、企业资料、校招场次与统计、线下机构、AI 岗位推荐、岗位解读与系统内岗位匹配、今日早报里的岗位数与招聘会倒计时、收藏与足迹里的岗位和招聘会，一律不返回；写入也一律停：机构导入、Excel、Webhook、每 30 分钟的定时同步，以及管理员和机构的审核、发布、代建接口。一体机要读这个开关、把入口藏起来，不能只关接口留下报错页。政策不是招聘信息，继续放在我们云上，但改由机构自己审核发布，管理员只留紧急下架。紧急下架按单向设计：只能下架、不能恢复；必选事由并留痕；自动通知机构；不给日常批量工具，只保留按机构或按来源熔断。代码与数据模型保留，给 b 版本用。备注（2026-09-26 后端）：环境变量 `RECRUITMENT_CONTENT_HOSTING_ENABLED`，未设置或非 true/1 为关，显式 true/1 为开（b 版本）。验证脚本没有例外（复核后去掉了按脚本入口自动打开的后门）：本地跑既有 verify 要显式设 true，CI 的 verify job 已设置。关闭时列表与聚合 HTTP 200 且对应数组为空、计数为 0；详情、写入、审核、发布、导入、同步触发 HTTP 403，`error.code=RECRUITMENT_HOSTING_DISABLED`。一体机读 `GET /api/v1/terminals/:id/config` 的 `recruitmentHosting`（`enabled=false` 时隐藏招聘入口）；同时把响应里的 `jobBoard.enabled` 置为 false，避免旧客户端只关接口留下报错页。开关打开时逐台岗位板块语义不变。政策机构发布走 `PATCH /partner/policies/:id/review` 与 `PATCH /partner/policies/:id/release`（`responsibilityAcknowledged=true`，审计记下确认人、时间、`contentVersion`）。紧急下架 `POST /admin/recruitment-emergency/takedown`，熔断 `POST /admin/recruitment-emergency/circuit-break`（scope=org|source）；事由码 illegal_content / false_information / rights_complaint / authority_order / other。下架写入 `RecruitmentEmergencyHold` 后不能再发布（`EMERGENCY_TAKEDOWN_IRREVERSIBLE`），并写 `PartnerOrgNotice`，机构读 `GET /partner/org-notices`。页面（Claude · 两个后台页面）：管理员端岗位信息源、招聘会信息源、招聘会管理、企业展示、Excel 导入记录、数据接入通道、线下机构只留查看与紧急下架；机构端岗位、企业、招聘会、数据源、同步日志在我们云上隐藏，统计页去掉在架岗位与招聘会数；政策页加「审核通过 / 发布」和发布责任确认。不新增页面。进度（2026-09-26）：后端与两个后台页面已并入候选（`38d69ea9a`，Agy 复核后返工 `c6c335cb9`：机构端读不到开关时按关闭处理），未部署；一体机读开关、藏入口在做，未并入。 | 我们云上没有任何招聘内容能被写入、发布或读出；政策由机构自发；管理员只能下架。（扩充 verify:job-review、verify:jobfair-review、verify:bulk-publish、verify:companies、verify:policies、verify:partner-edit、verify:admin-fairs、verify:job-sync；新增「托管关闭时招聘类读写与定时同步一律停止」「管理员发布被拒」「下架后管理员不能恢复」三条断言。） | 3.4 | 10/12 – 10/23 |  |
| 3.14 官方渠道二维码与 AI 岗位匹配改为用户自填 | C-09、L-12 | 分路「内容与同步」（后端，Grok 估 2 人日）：复用已有的平台目录表（已有机构字段、官方域名、证明文件、链接检测），改成「本机构官方渠道」：机构在机构端录入名称、链接、排序；链接域名只能落在入驻时核验过的该机构官方域名内（入驻交盖章确认函，这是机构身份核验，不是内容审核）；公开接口按终端所属机构只返回该机构已启用的渠道，没绑机构的终端返回空。简历匹配岗位：只在引用系统内岗位（jobId）时才校验开关，用户自己填的岗位要求（manualJob）照常可用；打印、查看两处先读存档，是系统内岗位才拒绝；现有门禁把「开关关闭时手填也必须拒绝」写死了，要随口径一起改。职业规划读取旧结果时，只去掉里面的系统岗位标题，不整份拒绝。 页面（Claude，Grok 估 4–5 人日）：一体机「线上招聘平台」页改成「本机构官方渠道」，删掉 4 个商业网站，二维码下写「本渠道由 XX 提供，信息以其官网为准」；首页岗位、招聘会入口在机构终端指向这一页，自营点位隐藏；岗位匹配入口改为「填一份岗位要求，AI 对照你的简历」；收藏、足迹里藏掉岗位与招聘会。小程序：复用分支 `claude/miniapp-nonrecruit-review-v1`（fba3136db，已停放招聘类页面、四个 Tab 改排）；再补「我的」收藏、足迹与今日早报里的岗位和招聘会。 进度（2026-09-26）：后端半成品在 `claude/official-channels-20260926`（`52260ff6a`，Grok 余额耗尽中途停下，未验证，未并入）。Agy 预审列出待收尾项：域名校验缺重定向参数、非 443 端口、`github.io` 这类公共托管后缀、中文域名（先转 punycode）与结尾点规范化（入库要存规范化后的链接）；只改了两份 schema，缺 PostgreSQL 迁移，平台目录缺按机构取渠道的复合索引；复用平台目录表时要核对原有必填字段；渠道写入与审计不在同一事务；只改排序时会拿旧链接重验域名而误报；没有删除（归档）接口与对应审计动作；DTO 缺最小长度与去空格。岗位匹配「只在 jobId 时校验开关、查看与打印先读存档」已在 3.13 做完，不必重做。一体机与机构端页面等后端并入后再接。 | 机构终端只显示本机构官方渠道二维码；自营点位与小程序看不到任何岗位；用户自填岗位要求时 AI 匹配可用。（扩充 verify:job-fit、verify:job-ai-backend、verify:compliance-copy；新增「官方渠道只在本机构终端出现」「域名不在核验范围内拒绝保存」「manualJob 在开关关闭时可用、jobId 被拒」三条断言；小程序 verify:static。） | 3.4、4.3 | 10/19 – 10/30 |  |
| 3.15 停放越界类型与代建入口，处置存量 | C-10、C-11 | 「停放」指源码保留，但不注册、不打包、不再写入；拿证、律师同意或客户要 b 版本时可恢复。停放只是不上线，源码和旧数据还在，所以存量要按清单处理。停放范围：企业来源方与无证商业招聘会主办方（不能导入、不能发布、不能绑终端）；线下机构目录及其岗位；管理员新建企业资料、代改招聘会、批量发布入口；线上招聘平台原来那 4 个商业网站（3.2）。存量：托管选 a 后，生产上所有已发布的岗位、招聘会、企业资料、线下机构都要出清单，你授权后下架；是否删除，等你看过清单再定（线上内容多半本来就是空的）。 | 系统里不再有「我们替别人发布招聘信息」的入口；存量有清单和授权记录。（verify:backend-p0-contracts、verify:partner-edit、verify:offline-agencies-page、verify:admin-offline-agencies-ui；清单与授权记录入档。） | 3.4 | 10/19 – 10/30 | 存量下架授权（生产写操作） |
| 3.1 内容发布关口 | C-03 | **9/26 晚按 a 暂缓：** 我们云上不发布岗位，针对岗位的发布规则（必填字段、虚假宣传、歧视词、有效期）留到 b 版本启动时再做，届时加在机构发布上。岗位板块开关（9/25 三方定）后端已进候选，3.13 把它扩成招聘内容托管总开关。 | 暂缓，不占本阶段工期。（b 版本启动时恢复原验收：verify:publish-expiry-completeness、verify:content-trust-publish-gate、verify:job-content-screening、verify:job-review。） | b 版本 | 暂缓 | 你定发布规则（拍板第 9 条，b 版本时） |
| 3.2 线上平台目录下线 | C-01 | 按你的同意，下线一体机「线上招聘平台」里的 4 条商业招聘网站链接。9/26 起这一页并入 3.14，改成「本机构官方渠道」。 | 一体机不再展示未授权的商业招聘网站。（pnpm verify:compliance-copy；一体机路由回归。） | 3.14 | 10/19 – 10/23 |  |
| 3.3 岗位同步修复 | C-05 | 每次运行用独立任务号，或完成即删并加数据库锁；同步日志写失败要可见。不另建任务表。 | 每小时同步不再被挡；重启后能对上账。（扩充 pnpm --filter @ai-job-print/api verify:job-sync（PG 作业已跑）。） | 3.4 真实来源 | 10/19 – 10/30 |  |
| 3.4 真实来源全链路 | C-04 | 3.13、3.14 生效后，用一个合作机构（学校或人社）走完两条链：官方渠道——机构在机构端录入官网链接 → 只在该机构的终端出现二维码 → 手机扫码落到机构官方域名 → 我方紧急下架演练 → 审计日志；政策——机构发布 → 只在该机构的终端展示 → 紧急下架演练 → 机构修改后重新发布。请求号、发布人、审计编号记入进度文档。到时还没有合作机构，就用测试机构账号走完，并如实标注「非真实来源」。 | 两条链证据入档。（请求号与审计编号入档；verify:policies 作为夹具回归。） | 5.1 冻结 | 11/9 – 11/20 | 一个合作机构账号 |
| 3.5 法务结论落地（拆成四件） | L-02、L-03、L-04、L-09 | a. 用后台「法务文档版本」发布定稿的隐私政策和协议，备案号放进一体机与小程序；b. 生产模型白名单（只放境内已备案模型）；c. 两端补齐 AI 生成标识和语音提示（**9/26 进度：** 后端——提示词固定追加「不得编造」与公平就业两句、合同审查去掉「法律意见」、AI 报告 PDF 每页页眉「AI 生成，仅供参考」、GB 45438 隐式标识与可回溯 ProduceID；一体机屏显——统一用语集中在 `complianceCopy.ts`，通话头部写明「小青是 AI 数字人，形象与声音由 AI 生成」，门禁 `verify:kiosk-ai-label-copy`；均已并入候选。剩两项待你拍板：简历 PDF / DOCX 正文是否印可见标识〔建议默认一行，用户申请可给不带标识版本并留存提供记录 6 个月，《人工智能生成合成内容标识办法》第九条〕、`AIGC_CONTENT_PRODUCER` 正式值〔建议公司统一社会信用代码或全称，不用产品名〕；演示态 `aiMockAdapter.ts` 与合同审查演示态免责句已改用 3.5c 用语（`b1e07b299`）；已归档与规划中的 `toolboxMicroApp.ts` 两个条目只在管理员治理面板出现，保留否定式免责句「不构成正式法律意见」，由 `verify:toolbox-micro-app-platform` 钉住）；d. 做账号注销的删除执行器，做好之前注销继续如实拒绝。 模型：DeepSeek 主用（与现有配置一致）、千问自动备用，两家都公示备案号。许可证由你办理。 | 法务要求在代码与配置里逐条落地；注销可用。（verify:legal-doc-version、verify:kiosk-ai-primitives、verify:legal-retention-copy；注销加一条「执行器就绪前仍拒绝」的断言。） | 5.4 提审 | a/b 10/26 – 11/6；c 11/2 – 11/13；d 11/2 – 11/20 | 0.8 的法务结论；定稿文本；简历导出可见标识与 `AIGC_CONTENT_PRODUCER` 两项拍板 |
| 3.6 AI 降级、渠道对账、依赖漏洞、对象存储（拆成四件） | F-11、F-07、R-07、R-04 | a. 缺 AI 配置时只降级 AI 路由，不退回 mock、不放宽其它密钥；b. 渠道日账单下载与差异作业（4.1 和试点对账的前提）；c. 分运行时和开发工具两批升级，重新评估被接受的高危项；d. 对象存储保留与备份策略在控制台配置，不用 API 写。 | 删掉一个 AI 密钥后打印与支付照常；渠道账单可对账；高危漏洞清零；对象存储策略截图。（verify:production-runtime-gates、verify:boot-resilience、verify:reconciliation、pnpm verify:dependency-security、verify:cos-lifecycle-policy。） | 4.1 商户实单 | 10/19 – 11/13 |  |
| 3.7 模块排雷第二批 | Q-01、Q-03 | 同 1.8 的六类清单；超千行文件只在修 bug 时按边界拆，拆后单文件变短；不单独立项合并两份登录样式，碰到登录问题时再抽共享部分。 | 两个模块的排雷结果入档；触及的超千行文件只减不增。（verify:job-content-screening、verify:admin-billing、verify:ai-down-fallbacks。） | 4.6 现场问题队列 | 10/26 – 11/20 |  |
| 3.8 两个后台商用走查 | C-06 | 逐页走查筛选、详情、操作日志、状态表达；问题回到 3.1、3.2、3.5 对应步骤修，空列表保持如实空态、不填假数据。 | 问题清单入档，P0/P1 在 5.1 前关闭。（现有 Admin/Partner Playwright 用例；走查表。） | 5.1 冻结 | 11/9 – 11/20 |  |
| 3.9 两个后台补全 | C-02、C-07 | 三个空页做成真功能：内部账号名册（含备用管理员）、外设真实状态、机构终端运营数据；16 处未完成点逐条做成真功能或如实下线入口。Grok 做接口，Claude 做页面。 备用管理员（9/25 三方定）：在名册里审计创建，绑定独立手机号，平时停用，只在主账号被锁、手机号丢失或人员变动时由你启用；后台显示最后登录时间。 9/26 起：管理员端不得新增任何代建、审核通过或发布招聘类内容的功能（见 3.13）；「机构终端运营数据」只给机构看它自己终端的数据。 | 两个后台没有空页、假数据和死按钮。（现有 Admin/Partner Playwright + route-sweep；机构数据字段白名单断言。） | 3.8 后台走查 | 10/19 – 11/13 | 你确认是否保留备用管理员 |
| 3.10 全功能验收体系 | Q-04 | 复用现有 route-sweep、走查脚本（抽自 #1024）、Admin/Partner Playwright、小程序开发者工具探针，搭三层验收：CI 全页面、4 条主流程硬断言、按钮与出口人工表；「暂未开放」必须在允许名单里。 | 上线前能一键跑完三层验收并出结果。（三层验收脚本在最终版本上全绿。） | 5.1 冻结 | 11/2 – 11/13 |  |

### 阶段 4 · 真机与真实资金（11/9 – 12/4）

用真机、真实资金、真实微信账号把主链路走通；现场任务集中在两个测试日。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 4.1 测试商户实单与对账（含小程序付款、自助退款） | F-05、L-06、M-11、F-14 | 3.6b 完成后，用测试商户跑：下单报价与回调金额一致 → 支付回调验签幂等 → 整单退款与迟到回调 → Agent 只领已付任务 → 出纸 → 订单沉淀；当天渠道账单与本地差异为 0。同时实测：小程序付款 → 一体机取件出纸；7 天未取自动退款；三个入口的自助退款（自动退与人工审核各一笔）。 | 真实资金链路证据；当天差异为 0。（商户平台记录 + 本地差异清单；verify:payment-flow 作为回归。） | 5.1 冻结 | 11/13 – 11/24 | 商户号与小额资金 |
| 4.2 现场测试日二（Windows 全量） | D-01、D-02、D-04、D-05、D-06、D-08 | 集中在一到两天：装正式签名的包；更新器做成独立模块（同时调整对应门禁）做升级演练；按镜像清单执行锁定与自启；核对彩色价目描述后按台开通并核双面翻页方向；拔纸盒看耗材告警；验证超时退出与本地文件清理。 | 现场录像与记录；长边/短边翻页如实记录，不写「问题清零」。（pnpm --filter terminal-agent verify:signing-workflow-contract、verify:installer-inputs；API 侧 verify:print-color-duplex-capability、verify:admin-billing。） | 5.1 冻结 | 11/18 – 11/24 选一到两天 | 正式证书；现场人员一到两天 |
| 4.3 实屏与真机走查 | K-08、M-03 | 一体机逐页触控走查录像；两个微信账号真机走登录、上传、下单、查看订单。 | 问题单入档，P0 进 4.6 队列。（录像与问题单；预检 test:browser:route-sweep。） | 4.6 现场问题队列 | 11/18 – 11/27 | 两个微信账号；与 4.2 同一天 |
| 4.4 跨端现场闭环 | F-01、F-02 | 拆成两条：上传领取在 1.1 合入后就能做；凭证出纸要等 1.3 和 4.1。都在现场测试日二执行。另测取件码分享：本人分享给他人代取、作废重发。 | 两条主链路现场通过，各有录像。（现场录像；小程序 verify:static 中的上传用例作为预检。） | 5.1 冻结 | 11/18 – 11/27 | 现场人员与测试账号 |
| 4.5 运营规程 | O-01、O-02、O-03 | 只改现有三份运营文档（退款异常处理、内容接入、打印部署矩阵），不新开手册；规程里每一步都要能在现有后台点到，点不到的回到 1.2 补。 | 运营人员照规程能处理异常；演练一次。（演练记录写进进度文档；verify:refund-idempotent 作为回归。） | 5.5 试点 | 11/16 – 12/4 | 你确认规程 |
| 4.6 现场问题队列 | Q-01 | 只收现场 P0 与阻塞项，当批修；12/4 仍开着的转为 5.2 的放行条件，不写「清零」。 | 现场 P0 清零；其余有结论和负责人。（每个问题用所属模块的 verify 复现。） | 5.1 冻结 | 11/9 – 12/4 |  |
| 4.7 真实 AI 与 OCR 验收 | F-10 | 0.2 轮换 OCR 密钥后，用合成样本跑真实 OCR 与大模型；材料包和转换没有真网脚本的各补一条；费用与服务商账单对照。 | 每类 AI 产物有真实样本证据与费用核账。（verify:ocr-baidu-live、verify:llm-connectivity -- --all、verify:ai-cost-coverage。） | 5.1 冻结 | 11/9 – 11/20 | 服务端真实密钥 |

### 阶段 5 · 发布与试点（11/30 – 12/28）

冻结最终版本，发布、提审、两周受控试点，最后商业签收。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 5.1 冻结最终版本 | R-01 | 冻结整合 PR → 三个必需作业全绿 → 合入 main。 | 最终版本号。（三个作业在最终提交号上 success。） | 5.2 发布前检查 | 11/30 – 12/4 |  |
| 5.2 发布前检查 | R-03、R-11、R-12、R-13、R-14 | 重跑 Deploy Precheck 与预检；核对云资源续费；按你的规则处理试运营数据（先备份）；逐条确认 4.6 遗留项的放行条件。 | 发布前检查单全部通过或有签字的放行条件。（Deploy Precheck + PREFLIGHT OK + 检查单。） | 5.3 受控全量发布 | 12/1 – 12/8 | 拍板：试运营数据、云资源到期负责人 |
| 5.3 受控全量发布 | R-05、R-06 | 手动全量发布最终版本 → 健康检查 → 首页版本号与发布记录一致；回退已在 1.6 接好。 | 线上 = 最终版本；失败能自动退回。（健康检查 + 发布记录里的提交号。） | 5.5 试点 | 12/8 – 12/11 | 你授权发布 |
| 5.4 小程序首次提审与发布 | M-04 | 冻结版本打体验版 → 你内测 → 提审；预留 3–5 天整改重审；发布后一体机二维码切成小程序码。类目没通过就按预案：试点先以一体机为主，小程序单独顺延。 | 小程序上线，一体机码直达小程序；或按预案顺延。（线上出码接口能出首页码；审核结果截图。） | 5.5 试点 | 11/30 – 12/11（审核时长不可控） | 0.11 备案与类目通过 |
| 5.5 受控试点两周 | O-04 | 单网点、真实用户与资金；每天用 3.6b 的渠道对账核一次；事故当天记录。试点若在自营点位，招聘模块整体关闭（3.14）；若在学校或人社，由对方机构账号运营（3.11）。 | 资金错账 0、收钱未出纸未退款 0、崩溃未恢复 0；14 天渠道对账差额 0。（每日对账差异清单 + 事故记录。） | 5.6 商业签收 | 12/14 – 12/27 | 试点网点与运营人员 |
| 5.6 商业签收 | O-04 | 按事先定好的标准逐项核对：P0（资金错账或卡钱、数据泄露或越权、主链路不可用、合规红线）全部清零，P1 有结论，证据齐全，试点达标。 | GO，或带条件的 GO；缓冲期到 2027/1/15。（证据索引与试点报告。） | — | 12/28 | 你签字 |

**每周检查点：** W1（10/2）0.1、0.3、0.4、0.6 完成：基线确立，首次 CI 红灯清单拉出，开放 PR 只剩 1 个；外部依赖（许可证、商户、证书、测试账号）由你发出。；W2（10/9）0.5 三作业全绿（或红灯剩余数与归属明确）；0.7 分支清理完成；2.0 双端分工表经你确认；3.11 六件事拍板（第 ⑥ 条已做）。；W3（10/16）1.1 故障注入通过；1.6 临时库回滚演练通过；1.7 现场测试日一完成；2.1 差距清单出炉；3.12 口径落文。；W4（10/23）1.2、1.4、1.5 通过；2.2 动效就位；2.6 小程序提审障碍清除；0.10 资产判定完成；3.13 通过。；W5（10/30）1.3、1.8 完成；3.3、3.14、3.15 通过；1.9 发布条件全部满足（发布时岗位板块全局关）。；W6（11/6）1.9 第一次受控发布完成；1.11 自助退款可用；2.3 旧外壳清零；3.2、3.5a/b 完成（3.1 按 a 暂缓）。；W7（11/13）2.4 打印链与简历链还原完成；2.8、2.9、3.5c、3.6、3.9、3.10 完成。；W8（11/20）2.4 全部 51 稿、2.5、2.7、3.4、3.5d、3.7、3.8、4.7 完成。；W9（11/27）4.1 实单差异为 0（含小程序付款与自助退款）；4.2–4.4 现场测试日二完成并留档。；W10（12/4）4.5 规程演练完成；4.6 现场 P0 清零；5.1 最终版本冻结。；W11（12/11）5.2 检查单通过；5.3 全量发布完成；5.4 小程序上线或按预案顺延。；W12–13（12/14 – 12/27）5.5 试点两周：每日对账差额 0，零资金事故。；W14（12/28）5.6 商业签收。

**问题编号索引（94 项，P0 27；9/26 增补 C-08–C-11、L-11、L-12）：** F-01[P0] 会员手机上传：确认、取消、过期清扫三处数据不一致；F-02[P0] 取件码：可以分享给别人代取，也可以自己取；F-03[P0] 已付款但文件失效：任务被静默跳过，钱收了纸没出；F-04[P0] 已付款异常的状态机合同不完整；F-05[P0] 支付全链路从未真实跑通；F-06[P1] 部分退款实际不支持；F-07[P1] 与微信商户平台的日账单对账没做；F-08[P0] 取件会话安全只在桩上验过；F-09[P1] 收费简历导出：进程崩溃后文件与收费可能不一致；F-10[P1] AI 产物没用真实模型和真实存储验过；F-11[P1] AI 配置缺失会让整个 API 拒启，打印和支付一起停；F-12[P1] 内部账号禁用/降权后的旧缓存越权；F-13[P1] 候选迁上生产 PostgreSQL 的前向演练；F-14[P1] 自助申请退款（小程序、一体机「我的」、AI 客服）；K-01[P1] 6 个页面还是旧外壳；另有 6 个稿外旧页待定；K-02[P1] 02「全部服务」没有页面；52 号 AI 产物只有一种入口；K-03[P1] 约 30 张稿已挂新外壳，但没逐个状态对照原稿；K-04[P1] 已对齐的稿还有剩余状态；K-05[P1] 动效层缺失：「结构对、手感不对」；K-06[P1] 断网、弱网时的诚实状态；K-07[P2] 原稿里打印能力口径与真实能力不一致；K-08[P1] 没做过真机实屏验收；K-09[P1] AI 入口的降级完整性；E-01[P1] 双端生态分工：小程序做线上，一体机保留线下优势；M-01[P0] 缺微信隐私授权弹窗；M-02[P1] 求职进度：后端已做好，两端都不能手填；M-03[P1] 扫码登录、上传回执等只在本地夹具通过；M-04[P0] 小程序从没正式发布过：这次是首次提审；M-07[P1] 跨端同一会员验收；M-08[P0] 主包体积已顶到微信上限；M-09[P1] 小程序逐页界面与交互商用走查；M-10[P1] 手机号登录的资质与计费；M-11[P0] 小程序付款，一体机交付实物；C-01[P1] 线上平台目录：一体机有入口，后台没管理页；C-02[P1] 后台三个空页做成真功能；C-03[P0] 内容发布规则有合规缺口（9/26 按 a 暂缓到 b 版本）；C-04[P1] 内容全链路没用真实来源跑过；C-05[P1] 岗位同步：任务号用法会挡住每小时的再同步；C-06[P2] 两个后台的商用化走查；C-07[P1] 两个后台还有 16 处没做完；C-08[P0] 招聘类内容由我方管理员审核发布，机构只能提交和下架；C-09[P0] 公开读取不按终端所属机构隔离（含自营点位、小程序与网页直接调用），AI 推荐岗位跨机构取；C-10[P0] 管理员代建内容：企业资料与挂岗位、线下机构及其岗位、招聘会正文与展位资料、批量发布；C-11[P1] 机构类型与收益口径越界：企业来源方、无证商业招聘会主办方可导入，架构文档写有数据接入费、上架费、流量结算；L-01[P0] 岗位与招聘会信息需要人力资源服务许可证；L-02[P1] 生成式 AI 的登记/备案，以及境外模型；L-03[P1] AI 生成内容标识不完整；L-04[P1] 隐私政策与实际数据处理不一致；L-05[P1] 数字人「小青」的肖像授权；L-06[P0] 微信支付商户与资金准备；L-07[P0] 凭据轮换（服务器登录与 OCR 密钥）；L-08[P2] 来源授权与岗位免责声明；L-09[P1] 账号注销没有开通；L-10[P0] 代码仓库可见性与 Git 历史密钥扫描；L-11[P0] 公司角色、收费与托管方式（9/26 晚已拍板：设备与软件供应方，托管 a；待落文与合同）；L-12[P1] AI 岗位推荐可能构成「为劳动者介绍用人单位」（44 号令第十一条），只能在运营机构的终端上以其名义提供；R-01[P0] 候选 274 个提交从没跑过 CI；R-02[P0] 发布没有可执行的回滚；R-03[P0] 百度云容量、备份恢复没重新核过；R-04[P1] 对象存储（COS）没有备份恢复方案；R-05[P0] 生产落后：一体机和后台还是旧版；R-06[P1] 自动发布开关一打开就是全量覆盖；R-07[P1] 15 个依赖安全漏洞；R-08[P1] 日志轮转与告警推送；R-09[P2] CI 小缺口；R-10[P1] HTTPS 证书 12 月 3 日到期；R-11[P1] 云资源到期与快照；R-12[P1] 数据库每日自动备份没确认在跑；R-13[P1] 生产配置没核对过；R-14[P1] 试运营数据怎么处理；R-15[P2] 服务器安全基线与资源；D-01[P0] 没有正式代码签名证书；D-02[P1] Agent 在线更新没进候选；D-03[P0] 现场硬件证据不足；D-04[P1] 安装后自启与系统锁定；D-05[P1] 彩色/双面开通前的价目与方向核对；D-06[P2] 耗材告警与出纸遗留提醒；D-07[P2] 奔图开放 API 彩色参数；D-08[P1] 会话超时与本地临时文件清理没在现场验过；Q-01[P0] 隐藏 bug 还没被系统地找过；Q-02[P1] 候选首次全量 CI 会暴露一批问题；Q-03[P1] 屎山代码只在修 bug 时顺手治理；Q-04[P1] 「和市面上正常软件一样」的全功能验收；A-01[P1] 项目主目录是「脏树」，还是微信开发者工具入口；A-02[P1] 4 棵最大的 Grok 工作目录也是同一个脏分支；A-03[P1] 开放 PR 从 7 个收拢成 1 个；A-04[P1] 13 个本地分支有独有补丁；A-05[P2] 本机空间；A-06[P2] 小程序历史工作目录；A-07[P1] 368 个远程分支，334 个已无用；O-01[P1] 人工核纸与退款处置规程；O-02[P1] 三方对账；O-03[P1] 客服、报障、巡检与耗材；O-04[P0] 受控试点与商业签收。

## 2026-09-25：商业闭环逐步推进板（已由上方「商业收口推进方案」取代，保留作历史）
