...
## 2026-09-23：以 `dd44e3cf9` 为当前唯一集成候选继续收口

1. **Claude 前端：** `/resume/source`、`/resume/parse`、`/resume/materials` 已本地合流，不再重做。
   先核实 390px `/scan?stage=result` 的「AI 简历识别」按钮覆盖，再按稿 21 逐项验收来源异常态、
   预览工具条与真实后端交接；稿 25 还缺真实文件整屏预览、旧文件版本展示和单双面打印口径决策。
   前端只能由 Claude 修改；孤儿 CSS 要有零引用与门禁证据后才能删除。
2. **Grok 后端：** `dd44e3cf9` 已使首次收费导出暂存不可见并与权益核销同事务激活，
   `b3085bb7b` 的 catch-only 候选不再合入。下一步补打印建单/页数识别对非 active 文件的显式拒绝，
   保持有效 HMAC 也不能读取暂存字节；再用 PostgreSQL 并发、真实进程退出与 COS 删除失败验证
