...
5. 🔄 **Phase B-2 真实材料处理**：基础页数识别 + 体检摘要最小增量已推进：`inspection` 对图片返回 `pageCount=1` / `canPrint=true`，PDF 先做轻量 `/Type /Page` 字节扫描，Kiosk 会把识别页数写回当前打印会话，并在材料检查页展示页数、可继续状态和安全提示，供 preview/confirm 继续显示。后续仍需接入真实 OCR / 清晰度检查 / A4 归一化 / PII 遮挡产物 / 材料包合并。AI 只输出结构化 JSON 与建议，最终 DOCX/PDF 由模板渲染链生成。
