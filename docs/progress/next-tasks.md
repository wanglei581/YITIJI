...
| 2.4 逐页逐状态还原原稿 | K-03、K-04、K-07 | 按 2.1 清单分三路并行：打印链、简历链、岗位招聘会与「我的」及其余；每路一个工作目录、只动自己的页面文件，共享层只由主会话改。能力口径不对的地方改运行时文案，不改编号原稿。 | 51 稿逐页逐状态对照截图通过；打印链与简历链在 11/13 前完成。（test:browser:w2、test:browser:w3、verify:print-parameter-capability、verify:print-confirm-honest；截图对照表。） | 4.3 实屏走查 | 10/20 – 11/20 |  |
...
| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 3.11 角色定位、收费与托管拍板 | L-11、L-12 | 三方建议，**待你拍板，可以改**：① 公司做「设备与软件供应方」：卖终端并随机交付软件，按年收维保与技术支持；不做招聘信息运营方，不承接要持证的就业服务外包（招聘会承办等）；以后办下许可证再评估做服务方。② 收费只有设备款与维保费，不收任何与岗位、招聘会、浏览、投递、入职挂钩的费用。③ 第一个要展示招聘内容的客户，选一种接入方式：a 只放客户官网或官方渠道的二维码，我们不存岗位；b 私有化部署到客户服务器或政务云；c 放在我们云上、按机构隔离——三方都不建议在律师书面同意前选 c。④ 客户机器上的打印收费：客户买断额度、用户免费，或走客户自己的商户号；律师确认前不走我们的商户号。⑤ 试点若在自营点位，招聘模块整体关闭。⑥ 生产现状（你负责，现在就做）：确认生产库里没有已发布的岗位与招聘会，有就用现有后台下架；1.9 发布时岗位板块全局关闭，3.13、3.14 生效前不打开。另把「岗位匹配」的名称与说明文案、合同文本一并交 0.8 的律师。 | 六件事有书面结论，写进本方案头部与 compliance-boundary.md §1.1。（决定条目入档。） | 3.12 | 9/26 – 10/9（第 ⑥ 条现在就做） | 六件事拍板；律师对 ③④ 与岗位匹配文案的意见 |
| 3.12 角色口径落文 | C-08、C-11 | Claude（主会话）改写 docs/product/role-boundary.md：机构是招聘类内容的运营与发布主体；我方是设备与软件供应方，管理员对招聘类内容只保留紧急下架，另有机构入驻核验（是否公共机构、许可证号与许可范围）、终端绑定、全局开关、审计；删去「路径 A / B」和数据接入费、上架费、流量结算等收益写法。同步 CLAUDE.md §18 第 3 条与末条（审核主体改为运营机构）。合同与告知要点交 0.8 的律师：设备买卖与维保合同、个人信息委托处理协议、机构发布责任承诺、招聘页投诉渠道。只改文档，Agy 复核。 | 文档口径与 3.11 结论一致，其他会话不会再按「管理员审核发布」加功能。（Agy 复核；pnpm graph 与 pnpm verify:repository-integrity 通过。） | 3.13、3.14、3.15 | 10/12 – 10/16 | 3.11 结论 |
| 3.13 机构自审自发，管理员只留紧急下架 | C-08、C-10 | 分路「内容与同步」（后端，Grok 估 6 人日）：岗位、招聘会、政策、企业资料、招聘会资料五个现有发布函数只接受本机构账号（sourceOrgId 锁定为登录机构），管理员调用一律 403，下架函数不动；现有的内容可信核验、有效期校验与关键词筛查跟着发布函数移到机构发布上，3.1 的新规则之后加在同一处；批量发布只对机构开放。管理员侧的代建与代改一并关闭：企业资料的新建、编辑、挂岗位，招聘会正文、展位企业、分区、资料、场馆导览。数据来源（API、Webhook）的日常启停与触发交给机构，新接入地址的安全核验仍由管理员做（这是技术安全，不是内容审核）。紧急下架是单向的：只能下架、不能恢复；必选事由并留痕；自动通知机构；只有机构修改后重新发布才能恢复，重新发布再过一遍自动筛查；不给管理员日常批量工具，只保留按机构或按来源熔断和全局岗位开关；自动关键词筛查保留，我方不做人工初审，也不核验用人单位资质（那是发布机构的法定审查义务，44 号令第十七条）。页面（Claude · 两个后台页面）：机构端现有列表加「审核通过 / 发布 / 批量发布」和发布责任确认；管理员端信息源、企业、招聘会页去掉审核通过、发布、新建和代编辑，只留查看与紧急下架。不新增页面。 | 机构账号能自审自发；管理员账号发布和代建一律被拒；下架可追溯并通知到机构。（扩充 verify:job-review、verify:jobfair-review、verify:bulk-publish、verify:content-trust-publish-gate、verify:companies、verify:policies、verify:partner-edit、verify:admin-fairs；新增「管理员发布被拒」「下架后管理员不能恢复」两条断言。） | 3.1、3.4 | 10/12 – 10/23 |  |
| 3.14 按机器隔离与 AI 岗位池收口 | C-09、L-12 | 分路「内容与同步」（后端，Grok 估 3.5 人日）：统一一处「终端运营机构」解析（终端身份取请求头，与岗位板块开关同源，不认 query 里的 terminalId）。公开的岗位、招聘会、企业、政策、校招统计、岗位要求统计、AI 推荐候选池、岗位解读与匹配、简历匹配岗位、招聘会拜访单，都只返回该机构已发布的内容；请求里的 sourceOrgId 不能放宽范围；不带终端身份的读取（包括小程序和网页直接调用）一律为空。岗位板块改为默认关：终端没绑机构或机构类型不合格即关闭，由服务端判定、不靠前端开关，不做跨机构兜底。终端绑定只接受公共就业服务机构、高校就业中心、持证人力资源服务机构。一体机页面（Claude · 岗位招聘会路）：列表与详情写「由 XX 发布」，显示发布机构的咨询与投诉电话（设备和打印问题才找我们）；空态写「本机构暂未发布」；详情里的二维码只指向来源平台，不指向我们的小程序。小程序（Claude）：「我的」里的岗位类收藏、足迹、匹配记录不展示岗位内容，提示到发布机构的终端或来源平台查看。 | 每台机器只看到自己机构的内容；自营点位、小程序、网页直接调用都看不到岗位。（扩充 verify:jobfair-campus-priority、verify:fair-list-integrity、verify:job-requirement-stats、verify:job-ai-backend、verify:job-fit、verify:companies、verify:policies；新增「A 机构终端读不到 B 机构岗位」「不带终端读不到任何岗位」两条断言；小程序 verify:static。） | 3.4、4.3 | 10/19 – 10/30 |  |
| 3.15 停放越界类型，处置存量 | C-10、C-11 | 「停放」指源码保留，但不注册、不打包、不再写入；拿证或律师同意后可恢复。停放只是不上线，源码和旧数据还在，律师仍可能关注，所以存量要按清单处理。停放范围：企业来源方（enterprise_source）与无证的商业招聘会主办方，不能导入、不能发布、不能绑终端；线下机构目录及其岗位（管理员代建）不在一体机出现，管理员页只留查看与下架；线上招聘平台目录按 3.2 下线。存量：先出清单（哪些已发布内容不属于合格运营机构），你授权后下架，不删除。 | 系统里不再有「我们替别人发布招聘信息」的入口；存量有清单和授权记录。（verify:backend-p0-contracts、verify:partner-edit、verify:offline-agencies-page、verify:admin-offline-agencies-ui；清单与授权记录入档。） | 3.4 | 10/19 – 10/30 | 存量下架授权（生产写操作） |
| 3.1 内容发布关口 | C-03 | （9/26 起：规则加在机构发布上，接在 3.13 之后。）按你定的规则，在现有发布逻辑里读取已有标记和质量快照，拒绝或转入现有审核队列；加虚假宣传关口；年龄等词表按你的规则补；机构端待发布列表与管理员端查看列表都加标记列。下架始终放行。 另加岗位板块开关（9/25 三方定）：终端配置里全局默认开、逐台可关、全局关优先；关闭后首页、服务台、帮助、深链一律不出现，且公开的 GET /jobs、/jobs/:id 一并拒绝。（开关后端 9/25 已进候选；3.14 起没有合格运营机构的终端一律关闭。） | 带歧视短语、字段不足、过期的岗位发布失败或进人工复核（复核在机构端）。（verify:publish-expiry-completeness、verify:content-trust-publish-gate、verify:job-content-screening（扩充）、verify:job-review。） | 3.4 真实来源 | 10/26 – 11/6 | 你定发布规则（拍板第 9 条） |
| 3.2 线上平台目录下线 | C-01 | 按你的同意，下线一体机「线上招聘平台」入口（4 条商业招聘网站链接）；以后要恢复，只从已有带审核状态的目录表读取官方或书面授权站点。 | 一体机不再展示未授权的商业招聘网站。（pnpm verify:compliance-copy；一体机路由回归。） | 3.8 后台走查 | 10/19 – 10/23 |  |
...
| 3.4 真实来源全链路 | C-04 | 3.1、3.13、3.14 生效后，用一个愿意用自己账号发布的合作机构（学校或人社）走完：机构提交 → 机构审核发布 → 变更或过期撤回 → 我方紧急下架演练 → 机构修改后重新发布 → 只在该机构的终端展示（小程序不展示） → 审计日志；请求号、发布人、审计编号记入进度文档。到时还没有合作机构，就用测试机构账号走完，并如实标注「非真实来源」。 | 全链路证据入档。（请求号与审计编号入档；verify:job-review 作为夹具回归。） | 5.1 冻结 | 11/9 – 11/20 | 一个合作机构账号 |
