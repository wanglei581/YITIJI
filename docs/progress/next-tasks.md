...
- **前端：** Claude 单写者以用户指定的原项目 `docs/design/kiosk-redesign-2026-08/` 中原始编号 HTML 为一体机新版 UI 源稿，**把运行时页面改到源稿的布局、信息层级、组件与交互**；不得反向把源稿改成旧项目 UI，不得将旧页面换色、换壳或局部可读性修复计为新版落地。先以 `01-home.html` 对照实际 `/` 做结构级 1080×1920 首页样板和实屏评审，再逐页核对路由、状态、接口与操作；390 宽仅作兼容辅助。原始设计资产只读保留，线上旧首页不保留为目标版。稿 13 已合流的安全/等待/隐私修复仅算功能可用性进展，UI 整体仍 NO-GO。小程序按相同产品语言适配原生交互；最后做 Windows Edge/27 寸与微信真机触控及动画验收，Codex 复核与集成。
