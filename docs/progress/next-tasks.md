...
## 2026-09-17 R12：唯一下一步是单 PR 进入 CI，之后再做微信/后端/Windows 真机

**当前候选：** 分支 `codex/package-order-idempotency-r12-20260917`，运行时代码锚点
`f9a48684b201bb6746b2ee46261aeee1a1c3edff`，基线
`origin/main@50483cd28096780c5e6c4260dde86dec36e7d99f`。材料包耐久幂等、大小写唯一键绕过、
过期免费单回放、小程序本地键契约均已完成源码与本地验证。Grok / Agy 最终复审为 `GO`；
Hermes 因 DeepSeek 上游 502 为 `FAILED`，不是批准。**不要重做已关闭的 R10/R11 调查，也不要
再开并行 PR。**

| 顺序 | 负责人 / 层 | 唯一任务 | 达标判据 |
|---|---|---|---|
| 1 | Codex 集成 | 将本分支（含本次两份进度文档）push，只开一个 PR | PR diff 只含本候选；工作树干净；PR 记录 Grok/Agy GO 与 Hermes FAILED，不伪造五家一致 |
| 2 | GitHub CI | 等 final-head 必需检查完成，只处理第一条决定性失败 | `postgres-readiness`、`build-and-verify`、`kiosk-browser-smoke`、`release-bundle` 全绿；不得重用基线 run `34992685756` 证明新 head |
| 3 | 合并证明 | 合入后刷新 `origin/main` | `git merge-base --is-ancestor <PR-final-head> origin/main` 退出 `0`，记录新的 main SHA |
| 4 | Claude + 小程序/后端 | 在微信开发者工具与真实联调环境走材料包闭环 | 真实文件上传 → 隐私检查 → 多行报价 → 建单 → 故意丢/中断响应后同键找回 → 到机码 → 离开后从「我的打印订单」找回；A/B 账号、弱网、存储失败、大写键本地拦截均留证 |
| 5 | Windows / 奔图专用任务 | 只在合入后的精确 main SHA 上做材料包真机 | 到机核销 → 现场付款 → 多文件逐份出纸 → 状态回流 → 临时文件删除；记录订单/任务/文件 hash 与异常恢复。旧 unsigned `0.4.11@50483cd28` 不代表 R12 真机通过 |
| 6 | 运维 / 产品 | 具名窗口部署、微信发布、真实内容/UAT | 备份/回滚/容量/监控/生产 provenance 全证；小程序 Trial/提审/发布、真实岗位/招聘会/政策内容和客户签字分别过门禁 |

**本轮之后仍保留的非阻塞项：**

- 材料包当前没有页码范围选择；以后开放 `pageRange` 时必须同一刀贯通草稿指纹、确认页 data、
  报价 files、建单 payload、客户端/服务端指纹与页面测试，不能只改其中一层。
- `terminalId` 别名与 `pageRange` 空白的语义归一只影响外部客户端等价文本重试；当前小程序固定
  传终端 UUID、整份打印，不阻塞本 PR。
- 建单成功后跳转失败再立即杀进程的恢复路径仍需微信真机验证；不会重复建单，但用户体验可能要
  从「我的 · 打印订单」找回。
- 两个幂等 utility 已超过 500 行；下一次新增能力前先评估共享内核拆分。测试文件 702 行，仍低于
  800 行硬限制，禁止继续无界堆叠。
- 材料包仍无在线取消端点，小程序已如实提示未付款订单等有效期后失效；不要造一个必得 404 的按钮。

**证据边界：** 当前仅 `SOURCE / LOCAL: GO`。PR、final-head CI、合并、微信 DevTools/真机、
Windows/奔图、生产部署、商业 UAT 全部仍是 `NO-GO`。
