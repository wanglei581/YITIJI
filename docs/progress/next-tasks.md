...
| 顺序 | 负责人 / 层 | 必做事项 | 达标判据 |
|---|---|---|---|
| 1 | Claude + 小程序 | **小程序材料包 P0 候选已在本地完成（基线 `origin/main@ddef936de`），下一步是受控合入**：push → 开 PR → 等 CI 全绿 → 合入 | PR 必需检查全绿且无未处理 P0/P1；合入后 `git merge-base --is-ancestor <PR-final-head> origin/main` 留证并记录新的 main SHA；合并前先并一次 `main`（分支基线早于门禁落地时，PR 三绿合入后 `main` 才红） |
...
`docs/graph/**` 本轮未重跑（授权范围不含该目录）。合并前若要刷新，跑 `pnpm graph` 即可；
`ci.yml` 目前没有图谱新鲜度门禁，不构成合并阻塞。
