...
| 3.5 法务结论落地（拆成四件） | L-02、L-03、L-04、L-09 | a. 用后台「法务文档版本」发布定稿的隐私政策和协议，备案号放进一体机与小程序；b. 生产模型白名单（只放境内已备案模型）；c. 两端补齐 AI 生成标识和语音提示；d. 做账号注销的删除执行器，做好之前注销继续如实拒绝。 模型：DeepSeek 主用（与现有配置一致）、千问自动备用，两家都公示备案号。许可证由你办理。 | 法务要求在代码与配置里逐条落地；注销可用。（verify:legal-doc-version、verify:kiosk-ai-primitives、verify:legal-retention-copy；注销加一条「执行器就绪前仍拒绝」的断言。） | 5.4 提审 | a/b 10/26 – 11/6；c 11/2 – 11/13；d 11/2 – 11/20 | 0.8 的法务结论；定稿文本 |
