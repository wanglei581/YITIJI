...
- **容量治理：** 2026-09-25 本机约 45GB 项目根目录（其中 `.claude/worktrees` 30GB、`.worktrees` 7.2GB），Codex worktrees 另 49GB，Git 登记 205 个 worktree；根与候选的 `node_modules` 各约 1GB，单个大树不能按体积直接删。Agy 独立只读复核超时，尚无新的安全回收清单。先逐路径盘点未提交改动、独有提交、分支/任务所有权和真实占用，再给可审清理清单；不直接删除候选资产。此前百度云只读快照为 40GB 盘/约 21GB 可用、`/srv` 4.5GB；两份 runtime 备份各约 1.2GB，PG dump 可列目录但未恢复。需重新核对当前生产容量、保留期、恢复演练与发布峰值空间，未批准前不删线上备份/缓存。
