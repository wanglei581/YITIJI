...
- 卡纸 / 缺纸 / 缺粉的恢复路径
- 断网重连后打印队列是否保留
- Agent claim 领取循环出现 HTTP 429 时的限流窗口、退避和多终端高峰行为（2026-09-06 Windows 现场已观察到；本次最终完成但领取延迟约 4 分钟）
- Mac/Claude 发布 106 路由新版 UI 后，提供部署 SHA 与线上浏览器证据；Windows 侧只按该 SHA 复验打印/扫描入口，不能用旧线上 UI 判定能力缺失
