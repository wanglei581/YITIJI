...
**未决 1 项 —— `身份证号码`，需产品负责人拍板。** 小程序没有「身份证号」表单字段，
...
（已由后台截图确认）。倾向勾，用途建议写：「检测你上传材料中可能含有的证件号，
用于隐私遮挡与短期保存；不用于实名认证，不提供给企业」。**未拍板前本项留空。**
...
- **第 5 节「开发者对信息的存储」现填固定存储期限 0 天，必须改**，须与
  ...
  这是 E1 的显式子项，不是顺带提醒。**填什么见下方未决项第 2 条 —— 我算过，固定天数这个选项本身填不对。**
...
### 压在工程侧、需要产品负责人拍板的未决项（2026-09-03 一并上交）

1. **`身份证号码` 勾不勾** —— 见上方 E1 未决段，代码事实与两种判断已备齐。
...
2. **第 5 节存储期限填什么** —— 我算过了，**「固定存储期限 N 天」这个选项本身填不对**。
   ...
   两条路，请选一条：(a) 在指引里选「无法确定固定存储期限」类选项并写明「实现目的后删除或匿名化」；
   (b) 先把 `long_term` 从可选项里摘掉，再按最长固定档 **180 天** 声明。**不要留 0 天。**
