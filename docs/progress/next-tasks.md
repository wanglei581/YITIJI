...
**产品负责人决定（2026-09-25 晚）：** ① 人力资源服务许可证由产品负责人自行办理；一体机招聘会保留；小程序首发不上岗位、招聘会、找企业页面，按非招聘类目提审，拿证后再加回（一体机岗位列表是否保留待确认）。② 小程序与一体机打通成一个生态：小程序围绕线上功能，一体机保留线下定位与优势，不以小程序抢占、弱化一体机（见步骤 2.0 分工表）。③ 小程序可以付款，一体机交付实物；付款后 7 天未取自动整单退。④ 退款可在小程序、一体机「我的」、AI 客服自助申请；条件明确的即时自动退，其余人工审核；AI 客服不动钱。⑤ 取件码可分享给他人代取，也可本人取；一次性、7 天有效、可作废重发。⑥ 大模型 DeepSeek 或千问均可：默认 DeepSeek 主用、千问备用，公示备案号，做属地登记。⑦ 两个后台问题全部修复补全，三个空页做成真功能。⑧ 生产只读检查与安全处置由产品负责人自行负责，不在执行范围。⑨ 其余拍板事项按三方最佳推荐执行。⑩ 质量标准：像市面上正常的软件，每个功能、按钮、数据都正常；前端按 51 稿完成更新，不合理布局随做随改。
...
| 0.3 确定唯一候选与写入方，写入推进方案 | A-01、A-03 | Claude 会话分支快进到 ee5a6085c 作为唯一候选（Codex 分支保留不动，Codex 恢复后接在本分支之后）；本方案写进 next-tasks.md 与 current-progress.md；你把微信开发者工具改指向候选目录。 | 只有一个候选、一个写入方；进度文档和本方案一致。（git rev-parse 候选 = 本方案提交；图谱校验通过。） | 0.4 首次全量 CI | 9/26 | 你改开发者工具打开的目录 |
| 0.4 候选首次全量 CI | R-01、Q-02 | 把候选快进推到 #1042 的分支，跑三个必需作业；再手动触发一次 Windows 安装包流程（这次仍是测试签名，不进现场）。 | 一张红灯清单——第一批隐藏 bug，每个红灯标了归属模块。（gh run view 看 build-and-verify、postgres-readiness、kiosk-browser-smoke 三个作业。） | 0.5 修到全绿 | 9/27 – 9/28 |  |
| 0.5 修到三个必需作业全绿 | R-01、Q-02、R-09 | 按红灯所属模块交给对应分路修（见协作规则的分路表），Claude 逐个审；每个红灯记原因和修复提交。10/9 是目标，不是死线：红灯多就顺延，但不跳过。 | #1042 在候选上三作业全绿。（三个作业在同一个提交号上都是 success。） | 1.9 发布前提之一 | 9/28 – 10/9（按红灯数量浮动） |  |
| 0.6 开放 PR 收拢成 1 个 | A-03 | 核实 #1021、#1024、#937 的有用内容已在候选后关闭；#742、#1031、#606 的 PR 先关闭并注明「在 1.6 / 2.2 / 4.2 抽取」，分支保留到抽完。 | 开放 PR 只剩 #1042；每个关闭的 PR 都写明内容去向。（gh pr list --state open 只剩 #1042。） | 0.7 旧分支清理 | 9/29 – 10/2 |  |
| 0.7 远程旧分支清理 | A-07 | 逐个用只读接口确认没有开放 PR 在用、内容已进 main 或已明确放弃，再分批删；保留 main、整合分支、备份分支和待抽取的三支。 | 远程分支降到约 35 个。（git ls-remote --heads 计数；删除清单入档（已合并 PR 的分支可在 GitHub 恢复）。） | 收尾项，不阻塞阶段 1 | 10/2 – 10/9 | 需要你同意 |
...
| 1.2 已付款异常：堵住「收钱不出纸」 | F-03、F-04、F-06 | 分路「资金履约」：领任务跳过失效文件时写入「已付未履约、需退款」标记（不自动打款、不自动重派）；缺纸、断电、未确认分别写清去向，不和文件失效混成一条；只做整单退，删掉后台订单页和收银台里「部分退款」的误导文案。 | 注入「付款后文件失效」时订单进入待退款、不出纸；缺纸、断电、未确认三条各有断言；收银台和后台不再出现部分退款。（扩充 verify:payment-flow、verify:refund-idempotent、verify:admin-ops。） | 1.3 取件凭证（同一分路） | 10/8 – 10/20 | 你定退款规则（拍板第 3 条） |
...
| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 2.0 双端生态分工表（进行中） | E-01 | Claude、Grok、Agy 盘点小程序 64 页和一体机全部路由，按「一体机线下优势 / 小程序线上功能 / 两端打通点 / 重复需调整」出一张分工表，交你确认后作为 2.x 各步的依据。 | 一张经你确认的双端分工表。（分工表写进 next-tasks.md；后续页面改动都对照它。） | 2.1、2.6、2.7、2.9 | 9/26 – 10/9 | 你确认分工表 |
| 2.1 51 稿逐状态差距清单 | K-03 | 在现有 51 稿登记表上补列「空、等待、错误、禁用、结果」各状态的差距；1080×1920 下原稿与运行页逐状态截图对照。 | 每页每状态一行：差距、工作量、归哪一路。（现有截图脚本逐页出图；pnpm --filter @ai-job-print/kiosk test:browser:route-sweep。） | 2.4 逐页修正 | 10/12 – 10/17 |  |
...
| 2.3 6 个旧外壳页迁移 | K-01 | 按对应原稿迁到青序流光外壳；合同审查保持能力闸门关闭，只换外壳。51 稿之外的智慧校园 4 页、福利活动 2 页，按你的决定迁移或下线入口。 | 一体机不再出现旧界面。（pnpm --filter @ai-job-print/kiosk verify:fusion-w5、verify:fusion-w6。） | 2.4 逐页修正 | 10/20 – 11/3 | 拍板：稿外 6 页怎么处理 |
...
| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 3.1 内容发布关口 | C-03 | 按你定的规则，在现有发布逻辑里读取已有标记和质量快照，拒绝或转入现有审核队列；加虚假宣传关口；年龄等词表按你的规则补；后台审核列表加标记列。下架始终放行。 | 带歧视短语、字段不足、过期的岗位发布失败或进人工复核。（verify:publish-expiry-completeness、verify:content-trust-publish-gate、verify:job-content-screening（扩充）、verify:job-review。） | 3.4 真实来源 | 10/19 – 10/31 | 你定发布规则（拍板第 9 条） |
...
| 3.9 两个后台补全 | C-02、C-07 | 三个空页做成真功能：内部账号名册（含备用管理员）、外设真实状态、机构终端运营数据；16 处未完成点逐条做成真功能或如实下线入口。Grok 做接口，Claude 做页面。 | 两个后台没有空页、假数据和死按钮。（现有 Admin/Partner Playwright + route-sweep；机构数据字段白名单断言。） | 3.8 后台走查 | 10/19 – 11/13 | 你确认是否保留备用管理员 |
