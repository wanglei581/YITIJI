...
**历史副本只读盘点：** `apps/miniapp/.claude/worktrees/` 当前登记 15 棵工作树，8 棵有未提交改动；7 棵干净树中，`git cherry` 显示 4 棵的 tip 提交与当前商用候选有等价补丁，另 3 棵仍有非等价提交。后者包括 API 错误响应修复和历史小程序/设计评审分支；有未提交改动的树中另有内容链路验证候选。AI 费用门禁分支虽非祖先，但补丁等价，不能列作未迁入功能。2026-09-25 本机 `df` 约 252GB 可用，暂不按目录 `du` 大小直接删除。先逐棵判断独有提交/未提交改动是否已选择性迁入或明确废弃，再决定回收；工作树数量不是小程序发布版本数。
...
| 3. 报价、支付、履约、扫描与售后 | 一体机 10、14、18、32/37 有新版局部本地实现；15 仅打印中主态在隔离树；已付款任务自动重派防重有本地证据。 | 串起服务端报价→建单→真实支付/退款状态→到机码/取件凭证→Agent 下载→奔图出纸/扫描→回传→订单和本人资产，并处理异常、人工核纸与补偿。 | 33 号取件凭证核销合同、37 号支付逐态、真实支付/退款账、Windows Edge/Agent/奔图物理证据；未确认结果不得写成功。 |
...
| 5. 51 稿逐态产品化与原生小程序 | 部分 Kiosk 页面按原稿结构合流本地候选；01 原稿会话卡重叠和误导性语音承诺已本地修正；路由登记不等于视觉或功能完成；小程序按原生交互适配同一业务状态。 | Claude 按原稿逐页核布局、动作、等待/空/错/禁用和 AI 能力，Grok 修真实合同，逐条业务流程完成小程序与一体机一致性。原稿中打印能力等口径须按真实能力修正。 | 全部状态 1080×1920 实屏/触控、390 辅助屏、Windows Edge/27 寸现场与微信真机；01 几何门禁存量 13>0 字号基线红须核实，不能用静态 HTML 演示冒充运行时。 |
...
- **15 打印交付页：** Claude 的打印中主态隔离提交 `76d9344a1` 有本地 1080 实屏和浏览器 5/5，但未合流；先把 830 行页面按职责拆到阈值内，再逐态完成原稿其余八态与真实 Agent/退款合同，不用演示状态或虚构页级进度替代。逐态浏览器、Windows Edge/27 寸触控、奔图出纸与退款现场证据齐全后才能验收。
...
| 15-print-fulfill | `/print/progress` | Claude 隔离树仅完成纯展示模型抽取，typecheck/W2 静态门禁通过；原稿九态布局、真实 Agent/出纸、逐态实屏和真机仍待做，未合流，UI/商业 NO-GO。 |
