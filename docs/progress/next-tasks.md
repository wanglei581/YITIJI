...
## 2026-09-25：商业收口推进方案（定稿 50 步，取代下方同日「商业闭环逐步推进板」）

**来源：** Claude（Opus 5.5）起草并总指挥；Grok（`grok-4.7-build-fast`，xhigh）两路逐步对照候选 `ee5a6085c` 核实做法、工作量与验收命令；Agy（`gemini-3.8-flash-high`）两轮独立评审步骤链、依赖与排期；Codex 按产品负责人要求暂停。两路 Grok 结论矛盾处由 Claude 读代码裁决。产品负责人已认可方案方向，并追加三条硬要求：开放 PR 与远程分支要精简；之前赶工埋下的隐藏 bug 要系统排雷；不写垃圾文件和屎山代码。

**唯一候选：** 本分支 `claude/codex-task-history-progress-0de1dc`（由 `codex/commercial-closeout-active-20260924@ee5a6085c` 快进而来，Codex 分支保留不动；Codex 恢复后接在本分支之后工作）。远程备份分支 `backup/commercial-closeout-candidate-20260925`。任何时候只有一个开放的整合 PR（当前 #1042）。
...
**分路（每个模块一个写入方）：** Grok · 资金履约（订单、支付、退款、打印任务与取件，步骤 1.2、1.3、3.6b、4.1）；Grok · 文件与会员（上传会话、会员绑定、文件与导出，步骤 1.1、1.5）；Grok · 登录与身份（鉴权、终端身份、取件会话，步骤 1.4）；Grok · 发布运维（CI、发布脚本、回滚、证书、生产检查，步骤 0.5 协调、1.6、1.10、3.6d、5.x）；Grok · 内容与同步（岗位与招聘会、内容关口、同步，步骤 3.1 后端、3.3、3.4）；Grok · AI 与合规后端（AI 配置与降级、模型白名单、注销执行器、真实 AI 验收，步骤 3.5b/d、3.6a、4.7）；Grok · Agent（Windows Agent、安装包、更新器，步骤 1.7、4.2）；Claude · 共享层（主会话）（一体机外壳、动效、字阶；只有这一路能改共享样式，步骤 2.2、2.3 统筹）；Claude · 一体机三路子代理（打印链 / 简历链 / 岗位招聘会与我的及其余；只动自己的页面文件，步骤 2.3、2.4、2.5、2.8）；Claude · 小程序（专用工作目录，契约门禁，步骤 2.6、2.7）；Claude · 两个后台页面（管理员与合作机构后台的页面，步骤 3.1 页面、3.2、3.8）；Agy（合同与反例、每批独立复核；超时不算通过，步骤 贯穿）。
...
| 0.2 安全止血 | L-07、L-10 | 由产品负责人完成服务器与第三方凭据轮换、决定代码仓库可见性；Grok 只读扫描全部 Git 历史，结果只记路径和类型、不贴密钥。 | 需轮换的凭据全部失效；历史扫描结果入档，命中项各有轮换日期（旧凭据验证失败；扫描清单写进 current-progress.md） | 0.4（候选本次改动已扫过、可以先推） | 9/26 – 9/28 | 凭据轮换与仓库设置由产品负责人操作 |
...
| 0.9 生产只读体检 | R-03、R-08、R-12、R-13、R-15 | 授权后先手动触发现有的 Deploy Precheck 工作流（容量、备份目录、Redis、健康检查）；它不覆盖的项（证书续期、日志策略、Redis 持久化、生产闸门预检）用一次只读登录补齐。公开日志里不打印任何密钥。 | 体检报告入档；要修的项排进 1.6、1.10、5.2。（Deploy Precheck 运行记录 + 预检脚本输出 PREFLIGHT OK。） | 1.6 回滚演练；1.10 证书 | 授权后 1 – 2 天 | 需要你授权只读检查 |
...
| 1.6 可执行回滚与恢复演练 | R-02、R-12、F-13 | 分路「发布运维」：移植 #742 的恢复脚本，接进发布失败分支（自动恢复库和运行目录）；用生产备份在临时库恢复并执行全部新迁移；在临时环境演练一次失败回退。不在真生产上故意失败。 | 健康检查失败会自动回到旧版本；临时库恢复并迁移成功；有演练记录。（临时库 pg_restore 后 pnpm --filter @ai-job-print/api db:pg:deploy；verify:release-provenance、verify:deploy-data-safety-gate。） | 1.9 发布前提之一 | 10/9 – 10/23 | 一份生产备份（0.9 授权后取） |
...
| 1.10 HTTPS 证书续期确认 | R-10 | 授权后在服务器上跑一次续期演练；如果自动续期没在跑，修好并设到期提醒。 | 续期演练成功，续期后 nginx 自动重载，有到期提醒。（certbot renew --dry-run 成功；到期日检查。） | 5.2 发布前复核 | 10/19 – 10/30 | 需要你授权（修复属于生产写操作） |
...
| 2.6 小程序分包与隐私弹窗 | M-08、M-01 | 小程序专用工作目录里：四个 Tab 留在主包，其余按业务域分包；接入官方隐私弹窗（弹窗文案依赖 3.5a 发布的隐私政策）。 | 开发者工具上传时主包明显低于 2MB；未同意隐私时敏感接口被拦、同意后登录成功。（pnpm --filter @ai-job-print/miniapp verify:static（加一条断言：含分包与隐私检查）；开发者工具上传体积截图。） | 2.7 小程序逐页走查 | 10/12 – 10/26 |  |
...
| 3.2 线上平台目录与后台空页 | C-01、C-02 | 按你的决定：下线一体机入口（1 天），或补写接口 + 后台管理页并让一体机改读接口（5–8 天）；三处空页写清说明或标二期，不做成第二套权限或设备系统。 | 没有悬空入口，也没有假数据。（verify:recruitment-content-http；pnpm verify:compliance-copy。） | 3.8 后台走查 | 10/26 – 11/6 | 拍板第 7、8 条 |
...
| 3.5 法务结论落地（拆成四件） | L-02、L-03、L-04、L-09 | a. 用后台「法务文档版本」发布定稿的隐私政策和协议，备案号放进一体机与小程序；b. 生产模型白名单（只放境内已备案模型）；c. 两端补齐 AI 生成标识和语音提示；d. 做账号注销的删除执行器，做好之前注销继续如实拒绝。 | 法务要求在代码与配置里逐条落地；注销可用。（verify:legal-doc-version、verify:kiosk-ai-primitives、verify:legal-retention-copy；注销加一条「执行器就绪前仍拒绝」的断言。） | 5.4 提审 | a/b 10/26 – 11/6；c 11/2 – 11/13；d 11/2 – 11/20 | 0.8 的法务结论；定稿文本 |
...
| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 4.1 测试商户实单与对账 | F-05、L-06 | 3.6b 完成后，用测试商户跑：下单报价与回调金额一致 → 支付回调验签幂等 → 整单退款与迟到回调 → Agent 只领已付任务 → 出纸 → 订单沉淀；当天渠道账单与本地差异为 0。 | 真实资金链路证据；当天差异为 0。（商户平台记录 + 本地差异清单；verify:payment-flow 作为回归。） | 5.1 冻结 | 11/13 – 11/24 | 商户号与小额资金 |
...
| 4.4 跨端现场闭环 | F-01、F-02 | 拆成两条：上传领取在 1.1 合入后就能做；凭证出纸要等 1.3 和 4.1。都在现场测试日二执行。 | 两条主链路现场通过，各有录像。（现场录像；小程序 verify:static 中的上传用例作为预检。） | 5.1 冻结 | 11/18 – 11/27 | 现场人员与测试账号 |
...
**每周检查点：** W1（10/2）0.1–0.4、0.6、0.8 完成：基线确立，首次 CI 红灯清单拉出，外部依赖在节前全部发出，开放 PR 只剩 1 个。；W2（10/9）0.5 三作业全绿（或红灯剩余数与归属明确）；0.7 分支清理完成；0.9 体检入档（已授权时）。；W3（10/16）1.1 故障注入通过；1.6 临时库恢复成功；1.7 现场测试日一完成；2.1 差距清单出炉。；W4（10/23）1.2、1.4、1.5 通过；2.2 动效就位；2.6 分包完成；0.10 资产判定完成。；W5（10/30）1.3、1.8、1.10 完成；3.1、3.3 通过；1.9 发布条件全部满足。；W6（11/6）1.9 第一次受控发布完成；2.3 旧外壳清零；3.2、3.5a/b 完成；0.11 备案与类目有结论。；W7（11/13）2.4 打印链与简历链还原完成；2.8、3.4、3.5c 完成；3.6 四件完成。；W8（11/20）2.4 全部 51 稿、2.5、2.7、3.5d、3.7、3.8、4.7 完成。；W9（11/27）4.1 实单差异为 0；4.2–4.4 现场测试日二完成并留档。；W10（12/4）4.5 规程演练完成；4.6 现场 P0 清零；5.1 最终版本冻结。；W11（12/11）5.2 检查单通过；5.3 全量发布完成；5.4 小程序上线或按预案顺延。；W12–13（12/14 – 12/27）5.5 试点两周：每日对账差额 0，零资金事故。；W14（12/28）5.6 商业签收。

**问题编号索引（83 项，P0 22）：** F-01[P0] 会员手机上传：确认、取消、过期清扫三处数据不一致；F-02[P0] 付款后取件凭证：一体机现场单的凭证码无处核销；F-03[P0] 已付款但文件失效：任务被静默跳过，钱收了纸没出；F-04[P0] 已付款异常的状态机合同不完整；F-05[P0] 支付全链路从未真实跑通；F-06[P1] 部分退款实际不支持；F-07[P1] 与微信商户平台的日账单对账没做；F-08[P0] 取件会话安全只在桩上验过；F-09[P1] 收费简历导出：进程崩溃后文件与收费可能不一致；F-10[P1] AI 产物没用真实模型和真实存储验过；F-11[P1] AI 配置缺失会让整个 API 拒启，打印和支付一起停；F-12[P1] 内部账号禁用/降权后的旧缓存越权；F-13[P1] 候选迁上生产 PostgreSQL 的前向演练；K-01[P1] 6 个页面还是旧外壳；另有 6 个稿外旧页待定；K-02[P1] 02「全部服务」没有页面；52 号 AI 产物只有一种入口；K-03[P1] 约 30 张稿已挂新外壳，但没逐个状态对照原稿；K-04[P1] 已对齐的稿还有剩余状态；K-05[P1] 动效层缺失：「结构对、手感不对」；K-06[P1] 断网、弱网时的诚实状态；K-07[P2] 原稿里打印能力口径与真实能力不一致；K-08[P1] 没做过真机实屏验收；K-09[P1] AI 入口的降级完整性；M-01[P0] 缺微信隐私授权弹窗；M-02[P1] 求职进度：后端已做好，两端都不能手填；M-03[P1] 扫码登录、上传回执等只在本地夹具通过；M-04[P0] 小程序从没正式发布过：这次是首次提审；M-07[P1] 跨端同一会员验收；M-08[P0] 主包体积已顶到微信上限；M-09[P1] 小程序逐页界面与交互商用走查；M-10[P1] 手机号登录的资质与计费；C-01[P1] 线上平台目录：一体机有入口，后台没管理页；C-02[P2] 后台还有三个空页；C-03[P0] 内容发布规则有合规缺口；C-04[P1] 内容全链路没用真实来源跑过；C-05[P1] 岗位同步：任务号用法会挡住每小时的再同步；C-06[P2] 两个后台的商用化走查；L-01[P0] 小程序服务类目与人力资源服务许可；L-02[P1] 生成式 AI 的登记/备案，以及境外模型；L-03[P1] AI 生成内容标识不完整；L-04[P1] 隐私政策与实际数据处理不一致；L-05[P1] 数字人「小青」的肖像授权；L-06[P0] 微信支付商户与资金准备；L-07[P0] 凭据轮换（服务器登录与 OCR 密钥）；L-08[P2] 来源授权与岗位免责声明；L-09[P1] 账号注销没有开通；L-10[P0] 代码仓库可见性与 Git 历史密钥扫描；R-01[P0] 候选 274 个提交从没跑过 CI；R-02[P0] 发布没有可执行的回滚；R-03[P0] 百度云容量、备份恢复没重新核过；R-04[P1] 对象存储（COS）没有备份恢复方案；R-05[P0] 生产落后：一体机和后台还是旧版；R-06[P1] 自动发布开关一打开就是全量覆盖；R-07[P1] 15 个依赖安全漏洞；R-08[P1] 日志轮转与告警推送；R-09[P2] CI 小缺口；R-10[P1] HTTPS 证书 12 月 3 日到期；R-11[P1] 云资源到期与快照；R-12[P1] 数据库每日自动备份没确认在跑；R-13[P1] 生产配置没核对过；R-14[P1] 试运营数据怎么处理；R-15[P2] 服务器安全基线与资源；D-01[P0] 没有正式代码签名证书；D-02[P1] Agent 在线更新没进候选；D-03[P0] 现场硬件证据不足；D-04[P1] 安装后自启与系统锁定；D-05[P1] 彩色/双面开通前的价目与方向核对；D-06[P2] 耗材告警与出纸遗留提醒；D-07[P2] 奔图开放 API 彩色参数；D-08[P1] 会话超时与本地临时文件清理没在现场验过；Q-01[P0] 隐藏 bug 还没被系统地找过；Q-02[P1] 候选首次全量 CI 会暴露一批问题；Q-03[P1] 屎山代码只在修 bug 时顺手治理；A-01[P1] 项目主目录是「脏树」，还是微信开发者工具入口；A-02[P1] 4 棵最大的 Grok 工作目录也是同一个脏分支；A-03[P1] 开放 PR 从 7 个收拢成 1 个；A-04[P1] 13 个本地分支有独有补丁；A-05[P2] 本机空间；A-06[P2] 小程序历史工作目录；A-07[P1] 368 个远程分支，334 个已无用；O-01[P1] 人工核纸与退款处置规程；O-02[P1] 三方对账；O-03[P1] 客服、报障、巡检与耗材；O-04[P0] 受控试点与商业签收。

**待产品负责人拍板：** 1. 小程序类目与岗位信息怎么处理；2. 生成式 AI 的合规路径；3. 退款规则；4. 商务口径：小程序不能付款；5. 33 号取件凭证的用途；6. 线上平台目录；7. 后台三个空页；8. 内容发布规则；9. 51 稿外的 6 个旧页；10. 02 全部服务页；11. 生产只读检查授权；12. 仓库改为私有吗；13. 远程旧分支清理；14. 试运营数据怎么处理；15. 云资源与证书到期管理；16. 安全处置（今天就做）；17. 现场与资源；18. 旧资产保全方式；19. 指挥权。
