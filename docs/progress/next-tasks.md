...
- [x] **打印支付 claim 门禁代码侧收口**：`claimTasks` 已读取关联订单支付状态，免费订单或无订单任务可继续 claim；`amountCents > 0` 且 `payStatus != paid` 的付费订单保持 pending，不再分配给 Agent；`verify:print-jobs`、`verify:order`、`verify:print-scan-first-release` 和 API `typecheck` 已覆盖防回退。部署到目标环境后仍需用真实免费/付费试运营配置复验。
