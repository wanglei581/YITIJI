...
1. **先做候选验证：** 在可复现的隔离数据库环境修复/重跑 `verify:admin-print-scan` 的 schema engine 初始化问题；随后由 Grok 对本批 5 个后端文件做只读反证，重点检查 claim、retry 和材料包事务是否误拒合法 active 文件。
...
1. **Claude 前端：** `/resume/source`、`/resume/parse`、`/resume/materials` 与 390px `/scan?stage=result`
   ...
   还缺真实文件整屏预览、旧文件版本展示和单双面打印口径决策。前端只能由 Claude 修改；孤儿 CSS
   要有零引用与门禁证据后才能删除。
