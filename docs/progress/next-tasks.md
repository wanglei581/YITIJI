...
3. **`ASR_PROVIDER`** —— **已裁定（2026-09-04，产品负责人授权评审组直接定）：#756 合入时保持 `disabled`，不翻 `tencent`。**
   理由：转写端点无 `endUserId`、无按人按天额度，腾讯按次计费，翻 enabled = 打开一个没有身份维度的计费面；
   而 `disabled` 有 11 PASS 钉死的诚实降级，不阻塞合入也不阻塞 E2（E2 甲档第 4 步已写明
   `ASR_NOT_CONFIGURED` 是预期降级、不算失败）。
   **翻转前置条件，缺一不翻，按序**：① 按人按天额度先上线（要求先把 `endUserId` 绑到转写端点，
   是鉴权面改动，与方言切换同批，即本页「明确不进 #756」桶里那条）；② 翻转自带一次具名部署授权
   （额度必须在翻 enabled 的那次发布**之前**上线，不许在发布里顺手才想起）；③ E2 真机先跑完，
   让降级路径在 `disabled` 现实下被亲眼验掉，翻转后才有行为对照。
   执行人：`services/api` 负责人；翻转动作入 backlog，排在 #756 合入 + 部署 + E2 之后。
   **它不是 #756 的门禁，也不属于合入动作。** 原始背景：生产默认 `disabled`。
   **#756 合入 ≠ 语音简历可用**：合入后真机点语音，走到的是诚实但冰冷的整场手打。
   真转写的可用性挂在服务端 env 翻转 + 一次具名部署授权上，这件事目前不在任何人的清单里。

4. **`store-select` 的导航入口** —— **已裁定（2026-09-04）：藏，且已在本 PR 做掉。**
   `Terminal` 模型无坐标字段（迁移史只有 `displayName` / `macAddress` / `locationLabel` / `enabled`），
   后端从不下发，JS map 连 `lat`/`lng` key 都不设 —— 按钮 100% 走 toast，是必然失败的可点区域。
   藏是**删死入口不是加功能**，不违反「不再往 #756 加东西」（与订单详情 404 同类：不修必在走查复现）。
   做法：`store-select.wxml` 的按钮块包 `wx:if="{{item.lat && item.lng}}"`，JS 守卫保留作双保险；
   **刻意不在 JS map 里补 `lat`/`lng` 透传** —— 后端不发，那是死代码。
   坐标建模并下发后按钮自动回来，能力不丢。「补坐标」（两套 schema + 双库迁移 + Admin 编辑入口 + 端点透出）
   入 backlog，跨 `services/api` 与 `apps/admin`。

5. **「已知限制」清单** —— **已裁定（2026-09-04）：主载体是 PR #756 的描述**（PO 直接可见、
   reviewer 必读、合入后进 git 历史不可腐）；**E5 更新 `feature-scope.md` §1.2 时带一份原文副本**
   （§1.2 反正要改成 62 页 / 112 PASS，顺手带零成本）。**不写进本冻结节** —— 本节声明只接受整节重写，
   为一份清单重写冻结区，正是前几轮批评的 append 式打补丁。清单定稿 8 条见 PR 描述。
   维护规则：新增限制才追加、消除即删、每条带日期。
...
- **方言切换、转写按人按天额度** —— 都动 `services/api`。额度不是「约 15 行」：
  ...
  硬时点是 `ASR_PROVIDER` 翻 enabled 之前。
- **简历优化结果导出** —— 要后端端点，同一个桶。
