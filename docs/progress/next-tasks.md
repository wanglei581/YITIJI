...
**`身份证号码` —— 已拍板：勾（产品负责人 2026-09-03 决定）。** 小程序没有「身份证号」表单字段，
...
3. **`ASR_PROVIDER` 谁在什么时候翻 enabled** —— 生产默认 `disabled`。
   **#756 合入 ≠ 语音简历可用**：合入后真机点语音，走到的是诚实但冰冷的整场手打。
   真转写的可用性挂在服务端 env 翻转 + 一次具名部署授权上，这件事目前不在任何人的清单里。

4. **`store-select` 的导航入口是死的** —— `wx.openLocation` 活着，但 :145 有守卫且后端
   `getPublicTerminals` 从不下发坐标，永远到不了调用。「展示一个永远到不了目的地的导航入口」
   是补坐标数据源、还是把入口藏掉？这个产品决定目前只活在代码注释里。生产有终端之前它不咬人。

5. **合入说明须附一份「已知限制」清单**，免得走查时把设计如此当故障报：
   招聘会展位分布恒空、企业打印次数恒 0、`fair_company` 复制到本地记录的闭环未接。
   这些在提交信息里写了「待定」，但没进产品负责人看得见的地方。
