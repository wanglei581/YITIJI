...
- **PR 收敛与取件会话 P0：** 已按 ancestry、Git blob 和冲突证据关闭 #1039、#548、#1025，OPEN 13→10，分支未删除。保留 #1042 为唯一整合草稿，候选冻结后才更新 head 并跑该 SHA 的 CI。#1035 的离页/隐私清场/慢释放生命周期守卫与 20 个真实 Nest HTTP 场景在候选未等价覆盖；先定向移植、跑失败/修复测试，再决定旧 PR 去留，不能整包合并。#1031 动效、#1024 走查脚本和 #606 Agent 更新器继续按独有能力复核；不删远程分支。
...
- **AI 解析 P1：** 服务端持久意图账本、Redis 原子配额、CAS 单次模型调用、匿名令牌重放和 HTTP 双头分支已在本地主候选；会员删除意图已用 `revoked` 墓碑阻止重放。双端持久意图、同头重查、可信 429 与四种终态 4xx 退路已接入；小程序换材料冲突恢复 `e312e8e76` 已合流，生命周期 196/196。**下一项隐私竞态：** 原 POST 写好解析结果后若会员并发删除，`submission.complete()` 返回 `advanced=false/revoked` 目前被忽略，仍可能回传内存中的已删内容；须先修 runner 返回值检查并做确定性交错测试。`FILE_NOT_FOUND` 在已受理后删除竞态不能按码清意图。随后分类用途错误、空文件和损坏内容的费用口径，并验证真实 Redis/DB/对象存储、双账号微信/Windows 真机及费用核账。模型调用后未落库的崩溃窗口仍保守标未知。当前仅本地候选，P1 未关闭。
