...
Mac 和 Windows 同时开发时，任何新任务开工前必须先复制以下模板登记到本文件对应优先级小节。未登记的并行任务不应进入代码修改阶段。

```text
### TASK-YYYYMMDD-短名
- 负责人/设备：
- 分支：
- 状态：待开始 / 进行中 / 待验证 / 已完成 / 暂停 / 阻塞
- 对应真实功能闭环或上线阻塞：
- 允许修改：
- 禁止修改：
- 是否新增入口、页面、数据模型、服务或外部依赖：
...
- 不在 `main` 直接开发；每个任务从最新 `main` 独立建分支。
- 两台设备不得同时修改同一核心文件、同一路由、同一迁移、同一硬件链路或同一生产配置。
- Windows 真机、Terminal Agent、打印、扫描、驱动和出纸结论必须由 Windows 现场验证给出。
- Mac 可以收口 API、前端、预生产、文档和非硬件验证，但不能替代 Windows 真机通过结论。
- 任务完成、暂停、阻塞或移交时，同步 `current-progress.md` 与本文件。
...
- [ ] Windows 真机：Terminal Agent、奔图打印机、打印真实出纸、扫描链路、断网/重启恢复逐项记录。2026-07-06 只读复核已确认 G5 `ptask_kiosk_f05cd3c160ec55c6` 的系统链路 / Agent / Windows PrintService / 计数器旁证对齐；用户已确认 `G5 Second Print Probe` 照片纸张来自 `Pantum CM2800ADN Series / USB001` 出纸托盘，`PS-G3-PHYS-01` 人工物理出纸补证已完成；扫描链路、断网/重启恢复和完整 PS-G3 / PS-G4 演练仍需另行验收。
...
- [ ] **首期安全底座现场验收**：G0 证据包与 CI 防回退已建立，正式域名 / HTTPS 审批暂不作为当前阻塞项；按 `docs/acceptance/print-scan-field-execution-runbook.md` 和 `docs/acceptance/print-scan-first-release-acceptance-package.md` 在 Mac、候选服务器、Windows 一体机 + 奔图真机三段补齐 PS-G1~PS-G4 证据。2026-07-06 已登记 `PS-G3-RO-01`：G5 `ptask_kiosk_f05cd3c160ec55c6` 系统链路 / Agent / PrintService / 计数器旁证对齐；`PS-G3-PHYS-01` 已由用户确认照片纸张来自目标打印机出纸托盘，人工物理出纸补证完成。后续仍需迁移执行记录、完整真实出纸状态链路、Agent 降级时 Admin 可见且不领取任务、恢复后可继续领取、失败回传和卡住任务释放、敏感文件短 TTL / 删除审计。
