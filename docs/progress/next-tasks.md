...
> 最后更新：2026-06-20（06-17 P0 队列第①项 JobFair Admin 审核专项 verify 与第②项 Kiosk `/campus` 本校优先已补齐；机构类型矩阵仍待单项合入；套餐/支付/凭证仍后置）
...
- [ ] Linux 服务器准备：记录系统版本、Node.js/pnpm、时区、磁盘/内存/CPU、防火墙端口。
- [ ] 域名与 HTTPS：完成域名解析、证书签发、自动续期、nginx 反代与上传限制。
- [ ] PostgreSQL：生产实例/本机服务部署，`migrate deploy`、seed、核心 verify、备份与恢复演练通过。
- [ ] Redis：生产连接、队列/缓存配置、访问权限与内网隔离确认。
- [ ] COS：生产私有桶、CAM 最小权限、签名 URL、上传/下载/删除 live 冒烟通过。
- [ ] 部署守护：API / Worker / 前端静态资源、PM2/systemd、日志轮转、健康检查、回滚路径。

### 2. 真实服务接入

- [ ] 腾讯短信：签名/模板审核通过，拿到 SignName / TemplateId / SDKAppID；真实 CAM Key 进生产 env；真号登录 E2E 通过后再启用 `SMS_PROVIDER=tencent`。
- [ ] 百度 OCR：生产 Key 确认已轮换并写入服务器环境变量；线上 `verify:ocr-baidu` 或等价 live 冒烟通过。
- [ ] AI 大模型：生产专用 Key 配置到服务端/配置中心；AI 简历、模拟面试、职业规划等真实调用失败时不 fallback 假结果。
- [ ] TRTC / ASR / TTS：如试运营启用语音/数字人能力，完成生产 Key、权限、浏览器麦克风授权与失败兜底验证；未启用则保持文字兜底。

### 3. Windows 真机验收

- [ ] Terminal Agent：安装、服务自启、单实例、日志路径、token/凭据安全保存、终端注册与心跳。
- [ ] 奔图打印机驱动：记录 Windows 实际识别名，Agent 仅通过 `printerName` 配置指定，不硬编码型号。
- [ ] 打印真实出纸：PDF、图片、简历、份数、黑白、彩色、双面、失败回传、完成回传逐项验收。
- [ ] 扫描链路：确认 TWAIN/WIA 或扫描目录方案，扫描件生成、上传、进入我的文档；未接通前不得宣称闭环。
- [ ] 断网/重启恢复：断网离线、恢复上线、服务重启、Windows 重启、任务重试/幂等行为记录。

### 4. 法务与合规

- [ ] 用户协议：法务审定正式版本，不以试运营草稿冒充正式版本。
- [ ] 隐私政策：覆盖手机号、文件、简历、AI 输出、OCR/ASR/TTS、对象存储、日志、留存与删除。
- [ ] 文件留存说明：明确上传文件、生成文档、打印任务、签名 URL、清理周期与用户删除影响。
- [ ] AI 免责声明：说明 AI 结果仅供参考，不承诺录用、Offer、通过率或政策到账。
- [ ] 招聘信息来源免责声明：岗位/招聘会/政策仅为第三方/官方来源信息入口，投递/预约在来源平台完成。

### 5. 小范围试运营

- [ ] 仅 1 台终端 + 1 台打印机先行试跑。
- [ ] 选择少量真实用户，覆盖登录、AI 简历、OCR、文档、打印、岗位/招聘会/政策浏览与外部跳转。
- [ ] 建立问题记录表：时间、用户操作、终端、任务 ID、日志位置、复现步骤、处理状态。
- [ ] 试运营问题只做阻塞修复和必要体验修正；不借试运营新增业务功能。
...
4. 腾讯 SMS 签名/模板审核 **+ 真实密钥进 env + 真号 E2E 验收** → 草稿与落地步骤见 [launch-review-submissions.md](../compliance/launch-review-submissions.md) §A。**✅ 代码接入已完成（2026-06-14，分支 `feature/tencent-sms-send`）**：`TencentSmsSender.sendCode()` 已实现真实腾讯云 SendSms（TC3 签名，`verify:sms-send` 本地 stub 14/14 PASS）；剩余三项纯外部——签名/模板审核拿 SignName/TemplateId/SDKAppID、真实 CAM 密钥进生产 env、真号 E2E。三者齐备再置 `SMS_PROVIDER=tencent`（审核通过前服务端已强制禁假发送）
