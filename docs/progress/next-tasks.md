...
- **前端：** Claude 单写者以用户指定的原项目 `docs/design/kiosk-redesign-2026-08/` 中原始编号 HTML 为一体机新版 UI 源稿，**把运行时页面改到源稿的布局、信息层级、组件与交互**；不得反向把源稿改成旧项目 UI，不得将旧页面换色、换壳或局部可读性修复计为新版落地。原目录 51 份编号稿已与本地候选逐项比较并同步到同一哈希，候选 52 号补充稿保留；未搬入历史截图。`01-home.html` 的结构级首页样板已合流本地候选并通过 1080×1920 / 390 浏览器 W1 14/14；下一片按原稿 05 顾问内部状态，再逐页核对路由、状态、接口与操作。390 宽仅作兼容辅助。原始设计资产只读保留，线上旧首页不保留为目标版。稿 13 已合流的安全/等待/隐私修复仅算功能可用性进展，UI 整体仍 NO-GO。小程序按相同产品语言适配原生交互；最后做 Windows Edge/27 寸与微信真机触控及动画验收，Codex 复核与集成。
...
| 05-ai-cockpit | `/assistant` | QX 路由已登记；本地页壳/首屏通过，原目录内部状态和真机待验 |
