...
| 2. AI 简历与材料成为本人资产 | 解析、报告、优化/生成和材料包已有部分本地代码及门禁，不能等同真实模型交付。 | 逐项打通授权→上传/解析→AI 输出/失败退路→版本/材料包落库→本人小程序和一体机读取；核模型/OCR 能力闸门与费用。 | 受控真实 AI/OCR、结果与费用核账、跨端/账号隔离、文件清理和真实导出/下载。 |
...
| 4. 两后台与来源内容运营 | Admin/Partner 有现存审核、同步和运营能力；内部账号旧缓存越权已在本地候选修复并过隔离门禁。 | 核角色权限、岗位/招聘会来源→合作机构提交/同步→管理员审核/上下架→小程序和一体机展示、过期撤回与审计；不得形成平台内招聘闭环。 | 当前 SHA CI、真实数据库权限交错/负载、来源内容样本与两后台操作验收。 |
