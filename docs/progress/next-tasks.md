...
**当前结论：同版本显式恢复 CI 门禁 `PASS`；现场安装与硬件验收仍 `NO-GO`，不得直接覆盖安装。** exact-SHA 未签名候选 `0.4.11@1d7468c...` 已由 GitHub run `33876577638` 生成；run `33888282866@8e7f7a783` 已证明从冻结同版本前代制品 `0.4.11@7ffc10e...` 走“显式卸载、安装目标、失败时显式恢复前代”的隔离 runner 路径。该证据不是现场安装、安装器自动回滚或物理硬件验收。

- [x] 分支 `codex/windows-agent-same-version-gate` 已推送；`windows-agent-installer` run `33888282866@8e7f7a783` 全绿，一次性 `exact-field-same-version-transition` job 完整 PASS。
- [x] job 已验证历史 run `33376026528` 冻结 artifact identity 为 `0.4.11@7ffc10e`，并验证 run `33876577638` 目标 identity 为 `0.4.11@1d7468c`。**现场实际安装来源仍须在变更动作前只读比对**，未对应前不得称为现场精确前代。
- [x] 隔离 runner 已证明：完整卸载前代后安装冻结目标；MSI ProductCode、Burn 注册和卸载命令唯一且合法，installed manifest 全量文件哈希精确匹配，服务保持未 provisioning 的 `Stopped/Manual`。
- [x] ProgramData 在各自卸载/安装/显式恢复演练内哈希不变，DPAPI LocalMachine token 可解密，真实 SQLite `integrity_check` + 标记行可读，LKG 配置可解析。
- [x] 候选启动前失败注入后显式恢复前代并保留原始错误；正常路径卸载候选后重装前代的恢复演练 PASS。该注入不覆盖已注册、部分安装、MSI 事务中断、注册阶段失败或要求重启；不得称为安装器自动回滚，不得使用历史 `0.4.8` EXE。
