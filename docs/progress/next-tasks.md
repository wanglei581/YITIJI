...
**历史副本只读盘点：** `apps/miniapp/.claude/worktrees/` 当前登记 15 棵工作树，8 棵有未提交改动；7 棵干净树中，`git cherry` 显示 4 棵的 tip 提交与当前商用候选有等价补丁，另 3 棵有 patch-id 不等价提交、须按实际行为再审。旧 API 错误日志守卫已经由当前候选 squash `3a7457d273` 提供同样的 try/catch，不能重复移植；AI 费用门禁分支虽非祖先但补丁等价。有未提交改动的树中另有内容链路验证候选。2026-09-25 本机 `df` 约 252GB 可用，暂不按目录 `du` 大小直接删除。先逐棵判断独有提交/未提交改动是否已选择性迁入或明确废弃，再决定回收；工作树数量不是小程序发布版本数。
