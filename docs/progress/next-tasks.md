...
- [ ] **预生产 Gate 4 浏览器截图补齐**：仍需补 G4-01 / G4-02 / G4-05 / G4-10 浏览器截图和常规签名 URL 等待窗口截图；执行前必须先按 runbook 确认会话来源（真实短信 / Redis 测试验证码 / 受控 session 三选一）、仓库外证据目录、地址栏 / 签名 URL / COS XML / 手机号 / token 脱敏规则，以及 Admin 仅筛选本轮测试文件；腾讯短信审核通过后还需用真实短信完成会员登录 E2E，避免把 API/COS 级验收误写成完整浏览器验收。
