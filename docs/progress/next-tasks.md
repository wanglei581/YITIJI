...
- [x] **Task 3：Admin Alerts 真实化 + 告警处理 UI/UX** — 2026-06-09 完成，分支 `feature/sprint1-admin-alerts`（stacked 在 Task 2 上）。新建 `Alert` 模型 + migration + `alerts` 模块（list/detail/status，admin guard，处理写审计）+ dev 种子；前端 [alerts.ts](../../apps/admin/src/services/api/alerts.ts) 双 adapter + [alerts/index.tsx](../../apps/admin/src/routes/alerts/index.tsx) 重写（筛选/详情抽屉/处理·忽略/三态）+ 侧边栏 badge 接真实「待处理」数。**告警处理仅运营状态记录，不远程控制设备。** `verify:admin-alerts` 7 项 ALL PASS；回归 orders/order 全 PASS；typecheck/lint/build 全绿。浏览器实跑手验待办。
- [x] **Task 4：Partner Profile 可编辑 + 表单 UI/UX** — 2026-06-09 完成，分支 `feature/sprint1-partner-profile`（stacked 在 Task 3 上）。`Organization` 扩 6 可空列（contactName 复用 contact）+ migration；新模块 `partner-profile`（**partner 鉴权**，`GET/PATCH /partner/profile`，保存写审计 `partner.profile_update`）；前端 [profile.ts](../../apps/partner/src/services/api/profile.ts) 双 adapter + [profile/index.tsx](../../apps/partner/src/routes/profile/index.tsx) 重写（编辑态/校验/三态+未初始化空态/保存反馈）。本轮**只做机构资料维护**，不做 Partner Dashboard、不做岗位/招聘会导入、不涉招聘闭环。`verify:partner-profile` 7 项 ALL PASS；回归全 PASS；typecheck/lint/build 全绿。浏览器实跑手验待办。
- [x] **Task 5：Partner Dashboard 真实化 + 统计卡片 UI/UX** — 2026-06-09 完成，分支 `feature/sprint1-partner-dashboard`（stacked 在 Task 4 上）。新模块 `partner-dashboard`（**partner 鉴权**，`GET /partner/dashboard`，orgId-scoped 真实统计，只读无审计）；前端 [dashboard.ts](../../apps/partner/src/services/api/dashboard.ts) 双 adapter + [dashboard/index.tsx](../../apps/partner/src/routes/dashboard/index.tsx) 重写（真实统计卡/最近同步·岗位·招聘会/三态/刷新）。**删除原伪指标（近7天访问/展示/打印），不展示假增长率/趋势**；只统计岗位/招聘会/数据源/同步，不涉候选人/简历/投递/面试/Offer/招聘闭环。`verify:partner-dashboard` 7 项 ALL PASS（含 orgB 隔离 + 无伪造字段断言）；回归全 PASS；typecheck/lint/build 全绿。浏览器实跑手验待办。

**Sprint 1 收官 — 下一步建议（非编码）：** Sprint 1 总审；5 条 stacked 分支（`feature/sprint1-order-model` → `…-admin-orders` → `…-admin-alerts` → `…-partner-profile` → `…-partner-dashboard`）的**推送与 PR 策略**（逐条 PR 还是合并为一条 Sprint 分支）；各页面**浏览器实跑手验**（admin/partner 登录走真实后端，需起 API + 账号）；生产 Postgres 迁移（dev.db 经 db execute 注入的 5 个 migration 随 drift 统一重整）；后续自动告警生产逻辑（Task 3 后续）+ 打印计费/支付（Task 1 amountCents 真值）。

合规红线（全 Sprint）：退款不接真实资金、`payStatus` 默认 unpaid、Dashboard 缺数据指标不伪造、不触招聘闭环。

**Task 3 后续（自动告警生产逻辑，本轮未做）：** Task 3 只完成「Alert 表 + Admin 告警运营后台 + 处理状态」；当前告警仅 **dev/demo seed**（非生产环境），**生产不自动种子**。真实告警仍需后续接入告警**生产**逻辑：
- 终端心跳离线检测（基于 `TerminalHeartbeat` 超时）
- 打印机缺纸 / 卡纸 / 碳粉低（基于 WMI / Agent 上报的 `printerStatus`）
- 数据同步失败（基于 `SyncLog.result='failed'`）
- AI 服务调用失败
- 打印任务异常（`PrintTask` failed / 超时回收）
另：Admin「处理 / 忽略」仅运营状态记录，**不远程控制设备**，远程设备控制不在本项目当前范围。
