...
- [x] **客户岗位 Excel 本地导入链路验证**：已用用户提供的 `岗位数据导入模板_已填写.xlsx` 在本地 SQLite 隔离导入 100 条岗位，预览 / 确认 / 审核发布 / 公开查询 / 字段质量快照链路通过；本地备份为 `/tmp/ai-job-print-dev-before-job-sample-20260701114004.db`，并通过 `verify:job-customer-sample-readiness`、`verify:job-data-quality`、`verify:partner-excel-template`。该批数据不能作为客户真实样本验收通过：100 条岗位只有 10 个去重 `sourceUrl` 且均为首页级链接，不是逐条岗位详情页；2 条标题含“校招”的岗位源表 `workType=full_time`，需要客户或源表改为 `campus` / `校招` / `校园招聘`。
...
- [ ] **岗位信息页客户数据验收**：先修正本批 Excel 的源数据后再进入真实验收：`sourceUrl` 必须改为逐条岗位详情页或客户官方可定位到单岗位的落地页，标题含“校招 / 校园招聘 / 应届”的岗位 `workType` 必须由源表明确填为 `campus` / `校招` / `校园招聘`，不能依靠系统按标题猜测改分类。修正后用客户真实 API / Excel / Webhook 岗位样本复验标准字段映射、筛选、来源链接、外部编号、同步时间、岗位描述、职责要求、企业关联和收藏 / 浏览 / 外部跳转记录；验收只记录打开来源入口，不记录投递结果。
- [ ] **岗位信息 AI 商用闭环下一阶段**：补客户真实岗位样本验收、预生产公网浏览器验收和一体机真机验收。已解除预生产公网新端点 404 路由阻塞，并完成客户样本导入 readiness、Partner Excel 固定模板下载和一次本地 Excel 导入链路验证；下一步必须基于修正后的逐条岗位详情 URL 与正确 `workType` 重新导入或隔离当前演示岗位，执行 `verify:job-customer-sample-readiness`、审核发布样本岗位、复核 `JobDataQualitySnapshot`、Admin 质量摘要、Partner 机构隔离和 Kiosk 展示。通过客户样本 Gate 后，再用真实会员、真实已解析简历和真实 LLM/OCR 跑 `/jobs` AI 推荐、`/jobs/:id` AI 解读、岗位匹配参考、来源外跳记录、`/me/ai-records` 历史回看和 `/me/settings` 授权撤回。最后补 Windows 一体机、Terminal Agent、Pantum 真机出纸和断网恢复验收。推荐结果只能作为求职者参考，不得引入平台投递、候选人筛选、面试邀约、Offer 或向企业推荐候选人。未完成客户样本、预生产和真机验收前，不得对外宣称 AI 推荐或岗位匹配达到生产商用完成。
