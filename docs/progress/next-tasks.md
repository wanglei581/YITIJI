...
|---|---|---|
| C0 | 候选及旧 PR 收敛：Grok 核查、Codex 决策 | 进行中。核实本地领先 39 提交和各 open PR 的祖先/补丁关系；每项归为独有待修、已覆盖可关闭候选或待证实。保护 dirty 资产；先复核已有 Burn 修复，不重复实现。 |
| C1 | 已付款打印履约：Grok 主修、Agy 反证 | 进行中。2026-09-24：隔离 SQLite 与自建临时 PostgreSQL 16.15（`127.0.0.1:63312`，库 `verify_admin_ops`）上完整 `verify:admin-ops` 均退出 0。正常外键路径未复现悬空 fileId。CI、claim/retry/材料包竞态和生产库仍未做。不自动退款，也不关闭整个 C1。 |
| C2 | 四端功能及前端统一：Claude 主修、Grok 契约核查 | 审计中。按真实路由→API→权限→状态→数据沉淀推进；修剩余来源异常态、手机/小程序交接与旧壳混用。已合流的设置、文档、订单、来源、解析、材料库、扫描预览不重做。保留青序流光方向及诚实空/错/禁用状态。 |
| C3 | 完整业务回归：Grok 主验、Claude 修界面、Agy 复核 | 待 C1/C2。核验身份隔离、AI 简历/材料的真实生成与权益、本人文件生命周期、打印扫描、支付订单退款，以及 Partner→Admin→前台的授权内容闭环。按现有范围覆盖，不能用假数据或静态页面充当成功。 |
| C4 | 存储与历史资产：Grok 盘点、Codex 审核 | 已取得服务器容量快照，未清理。逐项证明文件/分支/worktree 无独有成果或运行依赖；给出清理对象、回滚/保留策略和预期回收量。建立活跃副本、日志、缓存、备份及用户文件增长预算；不凭目录大小删除。 |
...
| C6 | 真机与商业交付：全组按角色配合 | 待具备环境及具体授权。微信 DevTools/Trial 与至少两个受控账号隔离；Windows/Pantum 实际打印、扫描和故障恢复；真实支付退款对账、内容授权、运营客服、隐私/保留口径及 UAT。先完成受控试点，证据齐全才判商业 GO。 |
