...
## 2026-09-25：商业收口推进方案（定稿 55 步，取代下方同日「商业闭环逐步推进板」）
...
**分路（每个模块一个写入方）：** Grok · 资金履约（订单、支付、退款、打印任务与取件，步骤 1.2、1.3、1.11、2.9 后端、3.6b、4.1）；Grok · 文件与会员（上传会话、会员绑定、文件与导出，步骤 1.1、1.5）；Grok · 登录与身份（鉴权、终端身份、取件会话，步骤 1.4）；Grok · 发布运维（CI、发布脚本、回滚、证书、生产检查，步骤 0.5 协调、1.6、3.6d、5.x）；Grok · 内容与同步（岗位与招聘会、内容关口、同步，步骤 3.1 后端、3.3、3.4）；Grok · AI 与合规后端（AI 配置与降级、模型白名单、注销执行器、真实 AI 验收，步骤 3.5b/d、3.6a、4.7）；Grok · Agent（Windows Agent、安装包、更新器，步骤 1.7、4.2）；Claude · 共享层（主会话）（一体机外壳、动效、字阶；只有这一路能改共享样式，步骤 2.2、2.3 统筹）；Claude · 一体机三路子代理（打印链 / 简历链 / 岗位招聘会与我的及其余；只动自己的页面文件，步骤 2.3、2.4、2.5、2.8）；Claude · 小程序（专用工作目录，契约门禁，步骤 2.6、2.7、2.9 页面）；Claude · 两个后台页面（管理员与合作机构后台的页面，步骤 3.1 页面、3.2、3.8、3.9 页面）；Agy（合同与反例、每批独立复核；超时不算通过，步骤 贯穿）；你（产品负责人）（安全处置、生产检查、证书、云资源续费、许可证、商户与资源，步骤 0.2、0.8、0.9、1.10、5.2 生产部分）。
...
后台把内容关口和合规要求落进代码，AI 与对账补齐，隐藏 bug 第二批排雷。

| 步骤 | 解决 | 做法要点 | 结果（验收） | 下一步 | 时间 | 需要产品负责人 |
| --- | --- | --- | --- | --- | --- | --- |
| 3.1 内容发布关口 | C-03 | 按你定的规则，在现有发布逻辑里读取已有标记和质量快照，拒绝或转入现有审核队列；加虚假宣传关口；年龄等词表按你的规则补；后台审核列表加标记列。下架始终放行。 另加岗位板块开关（9/25 三方定）：终端配置里全局默认开、逐台可关、全局关优先；关闭后首页、服务台、帮助、深链一律不出现，且公开的 GET /jobs、/jobs/:id 一并拒绝。 | 带歧视短语、字段不足、过期的岗位发布失败或进人工复核。（verify:publish-expiry-completeness、verify:content-trust-publish-gate、verify:job-content-screening（扩充）、verify:job-review。） | 3.4 真实来源 | 10/19 – 10/31 | 你定发布规则（拍板第 9 条） |
...
| 3.4 真实来源全链路 | C-04 | 3.1 生效后，用一条已授权的真实来源走完：机构提交 → 审核发布 → 变更或过期撤回 → 两端展示 → 审计日志；请求号、审核人、审计编号记入进度文档。 | 全链路证据入档。（请求号与审计编号入档；verify:job-review 作为夹具回归。） | 5.1 冻结 | 11/2 – 11/13 | 一条授权来源 |
...
| 3.9 两个后台补全 | C-02、C-07 | 三个空页做成真功能：内部账号名册（含备用管理员）、外设真实状态、机构终端运营数据；16 处未完成点逐条做成真功能或如实下线入口。Grok 做接口，Claude 做页面。 备用管理员（9/25 三方定）：在名册里审计创建，绑定独立手机号，平时停用，只在主账号被锁、手机号丢失或人员变动时由你启用；后台显示最后登录时间。 | 两个后台没有空页、假数据和死按钮。（现有 Admin/Partner Playwright + route-sweep；机构数据字段白名单断言。） | 3.8 后台走查 | 10/19 – 11/13 | 你确认是否保留备用管理员 |
...
| 5.5 受控试点两周 | O-04 | 单网点、真实用户与资金；每天用 3.6b 的渠道对账核一次；事故当天记录。 | 资金错账 0、收钱未出纸未退款 0、崩溃未恢复 0；14 天渠道对账差额 0。（每日对账差异清单 + 事故记录。） | 5.6 商业签收 | 12/14 – 12/27 | 试点网点与运营人员 |
