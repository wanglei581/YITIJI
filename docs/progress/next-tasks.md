...
| 0.7 远程旧分支清理（进行中） | A-07 | 逐个用只读接口确认没有开放 PR 在用、内容已进 main 或已明确放弃，再分批删；保留 main、整合分支、备份分支和待抽取的四支。9/25 清单：368 个中可直接删 259（86 个提交已在 main，173 个 PR 已合并且之后未再提交），逐个判断 102（合并后又有提交 13、PR 关闭未合并 70、从未开 PR 19），保留 7。 | 远程分支降到约 35 个。（9/25 已删 259 个：86 个提交已在 main、173 个 PR 已合并且之后未再提交，368→109；清单与恢复方法见 docs/reviews/remote-branch-cleanup-2026-09-25.md。剩余 102 个逐个判断后再报你批。）（git ls-remote --heads 计数；删除清单入档（已合并 PR 的分支可在 GitHub 恢复）。） | 收尾项，不阻塞阶段 1 | 10/2 – 10/9 | 需要你同意 |
