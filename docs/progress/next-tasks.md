...
- **05 顾问与 51 稿 UI：** Claude 的原稿 05 结构与 12 态运行时增量已合流 `0fe0dd941`，1080×1920/390 本地截图、临时状态探针 5/5、合流后 W3 助手/解析 15/15、typecheck/视觉门禁通过；这仍只证明本地合成数据与浏览器。下一步核真实模型返回/费用、TRTC/ASR/麦克风权限与 Windows Edge/27 寸触控，按原始编号 HTML 继续 10 打印 Hub、收银、扫描、手机接力等实际页面逐态修复；#1031 的旧动效层须先对照本次 Claude 动效，不能直接叠加。原稿、能力闸门和真实路由不反向改旧版；UI/商用仍 NO-GO。
- **解析终态 4xx：** Kiosk/小程序本地候选已合流 `2b7d39a47`，本地 W3 解析 12/12、小程序生命周期 194/194、静态合同/typecheck 通过；Agy 复核提出的终态标题、二次确认文案与清场代际错误码已修。下一步由 Grok 复现并修正小程序 `INTENT_CONFLICT` 换文件后 `_intent` 为空仍显示可重提的无效出口，优先保证不清别人的意图、不自动产生第二次模型调用；之后用真实 HTTP/Redis/DB 故障注入与微信双账号真机核对。P1 未关闭。
- **PR 收敛：** 2026-09-25 已按 ancestry、Git blob 和冲突证据关闭 #1039、#548、#1025，OPEN 13→10，分支未删除。保留 #1042 为唯一整合草稿，待候选功能/验证冻结后再更新其 head 并跑该精确 SHA 的 CI；不要从旧 PR CI 推断当前候选通过。优先核 #1035 独有的取件鉴权两项验证、#1031 动效层是否与 Claude 05/51 稿设计兼容、#1024 走查脚本是否值得保留（不搬 55 张历史截图），以及 #606 Agent 更新器与 0.4.11 的实际差异；其余 5 个 OPEN PR 逐项核覆盖关系后再关闭，不删远程分支。
...
- **前端：** Claude 单写者以用户指定的原项目 `docs/design/kiosk-redesign-2026-08/` 中原始编号 HTML 为一体机新版 UI 源稿，**把运行时页面改到源稿的布局、信息层级、组件与交互**；不得反向把源稿改成旧项目 UI，不得将旧页面换色、换壳或局部可读性修复计为新版落地。原目录 51 份编号稿已与本地候选逐项比较并同步到同一哈希，候选 52 号补充稿保留；未搬入历史截图。`01-home.html` 结构级首页已合流并通过 W1 14/14；原稿 05 顾问舱与 12 态运行时增量已合流 `0fe0dd941`，合流 W3 相关 15/15。下一片按真实路由与原稿推进 10 打印 Hub、收银、扫描及手机接力，再逐页核对状态、接口与操作。390 宽仅作兼容辅助。原始设计资产只读保留，线上旧首页不保留为目标版。稿 13 已合流的安全/等待/隐私修复仅算功能可用性进展，UI 整体仍 NO-GO。小程序按相同产品语言适配原生交互；最后做 Windows Edge/27 寸与微信真机触控及动画验收，Codex 复核与集成。
- **AI 解析 P1：** 服务端持久意图账本、Redis 原子配额、CAS 单次模型调用、匿名令牌重放和 HTTP 双头分支已在本地主候选；会员列表/删除意图的重复模型调用漏洞已按 `revoked` 墓碑修复，隔离 SQLite 真实 HTTP 13/13 通过。原生小程序与 Kiosk 均在首次 POST 前安全保存 32 字节意图/证明；同头重放、2xx 失败后新意图、可信 public quota 429 安全释放及四种终态 4xx 的保守退路已有本地候选，Kiosk 解析 W3 浏览器 12/12，小程序生命周期 194/194。服务端带意图文件预检已合流 `054610d9b`，真实 Nest HTTP/隔离 SQLite+Redis 门禁通过；失效会员 Bearer 匿名降级已在 `4470ee5b3` 修复。下一步先修上述换材料无效出口；`FILE_NOT_FOUND` 存在已受理后删除竞态，不能仅按错误码清意图。再分类处理用途错误、空文件和损坏内容的费用口径，验证预检到读取之间的删除竞态、生产对象存储与数据库故障、双账号微信/Windows 真机及费用核账。上游模型已调用但结果未落库的崩溃窗口仍只能保守标为未知，不自动重跑/退款。当前仅本地候选，P1 未关闭。
