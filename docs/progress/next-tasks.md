...
## 2026-09-25：商业闭环逐步推进板（每步按证据更新）

**当前并行工作：** 一体机 15 号打印中与缺纸态已选择性合流本地候选；手机上传会员确认、取消、清扫的 PostgreSQL/Redis 故障交错仍在独立后端树，未合流。下表的「已解决」只指明已达到的证据层，不把本地候选、设计稿或旧生产样本当成当前线上验收。

**小程序只保留一个产品版本：** `apps/miniapp/` 是唯一源码与发布源；所有修复选择性合流到同一商用候选，最终只从该候选的精确 SHA 构建一个待验 Trial/发布包。不再建立并行的小程序项目、第二套原型或多套发布版本。历史 worktree/分支只是待复核资产，不能当作第二个版本；确认独有改动已迁移且工作树干净后，再按容量治理规则清理。

**历史副本只读盘点：** `apps/miniapp/.claude/worktrees/` 当前登记 15 棵工作树，8 棵有未提交改动；7 棵干净树中，`git cherry` 显示 4 棵的 tip 提交与当前商用候选有等价补丁，另 3 棵有 patch-id 不等价提交、须按实际行为再审。旧 API 错误日志守卫已经由当前候选 squash `3a7457d273` 提供同样的 try/catch，不能重复移植；AI 费用门禁分支虽非祖先但补丁等价。有未提交改动的树中另有内容链路验证候选。`project.config.json` 的 `packOptions.ignore` 已排除 `.claude`，这些历史树不进入小程序包；根工作区该目录 `du` 约 3GB，其中两棵旧树的 `node_modules` 各约 1GB，逻辑体积不等于实际可回收容量。2026-09-25 本机 `df` 约 252GB 可用，暂不按目录 `du` 大小直接删除。先逐棵判断独有提交/未提交改动是否已选择性迁入或明确废弃，再决定回收；工作树数量不是小程序发布版本数。

| 步骤 | 这一步解决什么、目前已证什么 | 下一步解决什么 | 仍缺的验收证据 |
| --- | --- | --- | --- |
| 0. 冻结真实基线与资产 | 已隔离干净候选；用户指定的 51 张编号原稿保持为一体机 UI 基准，52 号仅为补充；重复/过期 PR 已按补丁和差异证据收敛，当前开放 7 个。 | 冻结唯一候选 SHA、逐一处理剩余 PR 与受保护工作树，并复核生产部署 SHA、磁盘、备份和回滚。 | 当前 SHA 的 CI；百度云实时只读容量/备份恢复；未审资产的安全清理清单。 |
| 1. 身份、手机接力与文件入库 | 小程序扫码登录只确认手机动作，上传只在完整同会话回执后说已收到；两项均为本地候选和夹具证据。会员文件绑定恢复仍在隔离后端树。 | 验证小程序扫码/上传→API 会话→一体机领取的同一身份、同一文件、一次性票据与取消/过期恢复，合流通过审查的 P0 修复。 | 微信开发者工具和双账号真机、真实 PostgreSQL/Redis/对象存储故障注入、现场一体机回执。 |
| 2. AI 简历与材料成为本人资产 | 解析、报告、优化/生成和材料包已有部分本地代码及门禁，不能等同真实模型交付；收费简历导出并发失败路文件残留是已登记缺陷。 | 逐项打通授权→上传/解析→AI 输出/失败退路→版本/材料包落库→本人小程序和一体机读取；修复可恢复的收费导出文件与权益一致性，核模型/OCR 能力闸门与费用。 | 受控真实 AI/OCR、结果与费用核账、跨端/账号隔离、并发/崩溃/对象删除失败后的文件清理和真实导出/下载。 |
| 3. 报价、支付、履约、扫描与售后 | 一体机 10、14、18、32/37 有新版局部本地实现；15 打印中与缺纸态已合流本地候选，缺纸态 1080 浏览器 6/6、完成页真实状态 15/15；已付款任务自动重派防重有本地证据。 | 串起服务端报价→建单→真实支付/退款状态→到机码/取件凭证→Agent 下载→奔图出纸/扫描→回传→订单和本人资产，并处理异常、人工核纸与补偿。 | 15 其余结果/故障态、33 号取件凭证核销合同、37 号支付逐态、真实支付/退款账、Windows Edge/Agent/奔图物理证据；未确认结果不得写成功。 |
| 4. 两后台与来源内容运营 | Admin/Partner 有现存审核、同步和运营能力；内部账号旧缓存越权已在本地候选修复并过隔离门禁；来源字段、过期、宣传与歧视内容规则仍有合规缺口。 | 核角色权限、岗位/招聘会来源→合作机构提交/同步→管理员审核/上下架→小程序和一体机展示、过期撤回与审计；补齐来源内容审核与人工复核，不得形成平台内招聘闭环。 | 当前 SHA CI、真实数据库权限交错/负载、来源授权和内容样本、两后台操作、过期撤回及合规审计验收。 |
| 5. 51 稿逐态产品化与原生小程序 | 部分 Kiosk 页面按原稿结构合流本地候选；01 原稿会话卡、主按钮承诺与字号已本地修正，首页两态几何门禁通过；路由登记不等于视觉或功能完成；小程序按原生交互适配同一业务状态。 | Claude 按原稿逐页核布局、动作、等待/空/错/禁用和 AI 能力，Grok 修真实合同，逐条业务流程完成小程序与一体机一致性。原稿中打印能力等口径须按真实能力修正。 | 其余状态 1080×1920 实屏/触控、390 辅助屏、Windows Edge/27 寸现场与微信真机；不能用静态 HTML 演示冒充运行时。 |
| 6. 发布、试点与商业验收 | 当前仅本地候选；旧线上版本与新 UI 未完成对应发布。 | 合并唯一候选，跑该 SHA 的 CI；核备份可恢复、容量峰值、回滚；经明确发布条件后受控部署，再按跨端实际订单做试点签收。 | 精确部署 SHA、健康检查、真实资金/退款、物理出纸/扫描、运营对账、隐私清场、异常演练及试点签收，未齐前 NO-GO。 |

**每步交接格式：** 具体问题 → 本步改动与精确 SHA → 自动测试/实屏/真机/线上分别实测的结果 → 未解决的反例与风险 → 下一步责任人和依赖。每次只更新正式进度入口，不新增临时交接文档或重复 PR。

**排期口径（2026-09-25，只用于资源安排）：** 当前候选相对 `origin/main` 为 272 个提交、518 个文件差异，实时 OPEN PR 7。现有条目去重后有 18 项已知未关闭缺陷或证据关口；这不是所有潜在 bug 的上限。若 P0 合同/数据一致性 2–4 周、跨端/51 稿/后台并行 4–8 周、同 SHA CI/真机/资金/备份/试点 2–4 周，且设备、微信双账号、支付、来源样本和生产窗口及时可用，最快 2026-12-04 左右进入商业放行审查，完整商用目标窗口 2026-12-18 至 2027-01-15。任一真实验收发现新 P0 或外部条件延迟即顺延；时间估算不改变全部门禁未齐前的 NO-GO。

## 2026-09-24：当前新增阻塞与所有权

- **手机上传会话 P1：** Grok 已按旧版失败→修后通过的交错证据，修复场景码兑换盖掉已上传收据、锁过期删除后继锁、最终收据和开始上传普通写入覆盖胜出文件；主候选采用同一次 Redis Lua 比较锁/状态/无文件/TTL，既有上传门禁与 API 类型检查通过。Agy 对最终收据竞态确认 P1 并复核原子修复；确认/取消/过期清扫持锁慢操作仍在审查。2026-09-25 小程序 `kiosk-send` 已合流本地候选 `9a1606b95` 并收紧上传回执：空/不完整/跨会话/网络未知不再说已收到，成功后不再发第二份；本地行为测试 15/15 与完整静态门禁通过，微信开发者工具和真机未跑。下一步做真实 Redis 并发与 HTTP 交错、手机/一体机双端回执、微信扫码、Windows Agent 下载与奔图出纸；无这些证据不关闭 P1/商业门禁。
- **小程序扫码登录一体机诚实态：** 隔离提交 `9317154d7` 已选择性合流本地候选 `03e55d292`；`kiosk-login` 只在 `{ status: 'confirmed' }` 回执后说「已确认，请回一体机查看登录结果」，未知回执不再确认、只读重查，缺机器名不编造；隔离树行为测试 14/14 与完整 `verify:static` 通过。下一步在微信开发者工具与真机各扫一次一体机登录码，覆盖正常 claim、claim 失败重扫、断网未知三条；未做前不宣称扫码登录体验已验收，未推送、未部署。
- **会员确认/取消/清扫 P0：** Grok 隔离候选 `cc6081674` + `bc0c04358` 已补可恢复绑定阶段、对象 key 记账与逐条清扫；本地真实 Redis Lua、临时 SQLite/PostgreSQL 全量迁移（PG 76 个且 schema 无差异）和临时 PostgreSQL+Redis Nest HTTP 基本会员确认/取消通过。Agy 对最终候选复核多次超时，仍 **UNREVIEWED**，以上代码未合流。下一步独立审查最终 SHA，并用真实 PostgreSQL/Redis/本地存储故障注入验证进程中断、锁/会话过期、并发确认取消与对象删失败，再走微信手机→一体机双端和 Agent/出纸；未满足前不合流、不发布。
- **会员上传 HTTP 交错门禁可靠性：** 隔离树 `f76a88dde` 修正验证脚本在确认请求提前返回时遗留阻塞 FIFO 写端的问题；临时 PostgreSQL 16/Redis/Nest HTTP 正常流程与锁过期交错两轮 ALL PASS，强制早退反证为旧版 8 秒未退出、修后 2.5 秒按预期失败退出。此提交未合流，且只证明门禁能可靠退出，不等于会员绑定 P0 运行时已验收。下一步补进程中断、对象删除失败等各自有界故障场景并独立审查。
- **内部账号权限缓存 P1：** Grok 在隔离 HTTP 复现管理员禁用/降权后旧缓存继续放行，已修成每次数据库判定、更高 tokenVersion 冲突拒绝；旧实现 36/37 红，修复后隔离门禁 38/38 绿，主候选 API 类型检查通过。Agy 限定只读复核无具体 P0/P1。下一步跑当前整合 SHA 的 CI、真实 PostgreSQL 并发交错和内网负载压测，量测每个内部请求查 User、合作机构再查 Organization 的延迟与数据库读压；发布前核当前线上会话与回滚条件。未部署，商用仍 NO-GO。
- **已付款打印回执防重：** Grok 只读审计与隔离 SQLite 交错显示同一任务出纸后 ACK 丢失/租约超时不会自动重派，Agent 本地 `dispatching` 记录阻止重启后再打印；现有相关门禁退出 0，无代码改动。`PRINT_JOB_UNCONFIRMED` 必须现场核纸，不能当作成功或自动重试；人工明确失败重试与本地库丢失、Windows 假脱机在途、真实 PostgreSQL/奔图仍待专项验收，C1/商用不关闭。
- **05/10/18/32/37 与 51 稿 UI：** Claude 的原稿 05 顾问舱 `0fe0dd941`、10 打印 Hub `140bdedf5`、18 扫描工作台 `8a163930b`、32/37 收银支付 `9f90c9d56`，以及 51 手机 QR 登录 `5c9ed5d4f`/换票据串态修复 `ba32fdc5a`、手机上传 `cf12de49d` 已合流；本地 W3 助手/解析 15/15、打印 Hub 9/9、扫描 26/26、收银 1080 6/6+390 1/1+金额 3/3+W2 打印 55/55、手机 QR 390 浏览器 4/4、手机上传 390 浏览器 3+1 项与相关 typecheck/静态门禁通过。这只证明本地夹具与浏览器，不等于 23 个支付状态或 51 号两条手机链路的真实双端验收。下一步核真实模型/语音/支付/设备与 Windows Edge/27 寸触控，并按原始编号 HTML 继续其余稿逐态修复；#1031 旧动效层须先对照 Claude 动效。原稿、能力闸门和真实路由不反向改旧版；UI/商用仍 NO-GO。
- **15 打印交付页：** Claude 的打印中主态 `76d9344a1` 与展示组件拆分 `3793c4c34` 已合流 `e8e04d518` / `cdfc88efc`；结果页展示拆分 `76e048492` 与缺纸态 `c737c164b` 已合流 `795421e03` / `f0f374059`。缺纸态按原稿布局但不承诺自动续打或虚构出纸张数，重打只认服务端 `canRetry`。主候选 typecheck、局部 lint、W2 静态门禁、1080 浏览器 6/6、完成页真实状态 15/15 通过。继续逐态完成其余结果/故障态与真实 Agent/退款合同；Windows Edge/27 寸触控、奔图出纸与退款现场证据未验，UI/商业 NO-GO。
- **解析终态 4xx 与冲突：** Kiosk/小程序四种可信终态的退路已合流 `2b7d39a47`；小程序 `INTENT_CONFLICT` 换文件无效出口已按 owner/intent/载荷/任务快照和双重确认修复并合流 `e312e8e76`。本地 W3 解析 12/12、小程序完整静态门禁与生命周期 196/196 通过；极端存储恢复失败、真实 HTTP/Redis/DB 故障、微信双账号和费用核账待验。P1 未关闭。
- **PR 收敛与取件会话 P0：** 已按 ancestry、Git blob、完整差异与冲突证据关闭 #1039、#548、#1025、#1035；#1027 的旧交付 YAML 会写回过期路由/生产快照并把未独立验证的 OCR 密钥轮换标 CLOSED，已另行关闭；#1038 的四个代码提交在本地候选均有相同稳定 patch-id，唯一剩余文档提交是旧阶段口径，已关闭。远程分支和历史均保留，实时 OPEN 7。保留 #1042 为唯一整合草稿，候选冻结后才更新 head 并跑该 SHA 的 CI。#1035 的离页/隐私清场/慢释放生命周期守卫已选择性合流 `d38ea344a`；隔离分支 W2 打印 56/56、主候选生命周期 7/7 与 typecheck/静态合同通过。真实 Nest HTTP 鉴权场景已接入现有 `verify:terminal-identity` 门禁，两路共 20/20。非 mock 换票路径与少数辅助边界可按当前实现定向补强；真实终端、支付、其它调用方及当前 SHA CI 未验，P0 未关闭。#1031 动效、#1024 走查脚本和 #606 Agent 更新器继续按独有能力复核；不删远程分支。
- **小程序解析：** 已合流持久意图、原头同次重查、GET 404 暂未落库恢复、匿名令牌写后回读及漏回令牌保护；合流后整组静态门禁通过，隔离分支生命周期 192/192。下一步用真实 API/Redis/DB 故障注入、微信双账号真机核对页面退出和权限，做受控模型费用核账；这些本地证据不代替商用验收。
- **前端：** Claude 单写者以用户指定原项目 `docs/design/kiosk-redesign-2026-08/` 的原始编号 HTML 为新版 UI 源稿，**把运行时页面改到源稿的布局、信息层级、组件与交互**；不得反向改源稿或把旧页换色计为完成。原目录 51 份编号稿已同步同哈希，候选 52 号补充稿保留。01 首页、05 顾问舱、10 打印 Hub、18 扫描工作台、32/37 收银支付已结构级合流本地候选；10 七态截图和浏览器 9/9、18 扫描 26/26、收银通道已选 1080/390 截图与相关浏览器门禁仅为本地夹具，Windows 字体、现场触控与真实能力未验。下一片推进手机接力及其它未覆盖稿，再逐页核对状态、接口与操作。390 宽仅作兼容辅助；原始设计资产只读，线上旧页不作为目标。稿 13 的安全/等待/隐私修复仍只算功能可用性，UI 整体 NO-GO。小程序按相同产品语言适配原生交互；最后做 Windows Edge/27 寸与微信真机触控及动画验收。
- **AI 解析 P1：** 服务端持久意图账本、Redis 原子配额、CAS 单次模型调用、匿名令牌重放和 HTTP 双头分支已在本地主候选；会员删除意图用 `revoked` 墓碑阻止重放。双端持久意图、同头重查、可信 429 与四种终态 4xx 退路已接入；小程序换材料冲突恢复 `e312e8e76` 已合流，生命周期 196/196。会员删除与在途 POST 的内存结果回传竞态已在 `af134054d` 按完成状态与提交后回读修复，确定性交错夹具通过；真实 PostgreSQL 并发交错待验。`FILE_NOT_FOUND` 在已受理后删除竞态不能按码清意图。随后分类用途错误、空文件和损坏内容的费用口径，并验证真实 Redis/DB/对象存储、双账号微信/Windows 真机及费用核账。模型调用后未落库的崩溃窗口仍保守标未知。当前仅本地候选，P1 未关闭。
- **容量治理：** 2026-09-25 复核本机 `df` 可用 **249GB**，项目根目录 `du` 约 **44GB**（其中 `.claude/worktrees` **29GB**、`.worktrees` **7.2GB**），Codex worktrees 另 **49GB**，Git 登记 **205** 个 worktree；这些开发历史副本不是线上产品体积。根与候选的 `node_modules` 各约 1GB，单个大树不能按体积直接删；抽查最大的 `.claude/worktrees/elastic-hellman-1e7d34` 有四项未提交改动，不能删除。首棵干净旧树的可重装依赖缓存逻辑体积 1,140,124 KiB，实际 `df` 仅增加 16,140 KiB；暂停按 `du` 估算的批量清理，改按物理可用空间实测优先级。Agy 独立只读复核超时，尚无新的安全回收清单。先逐路径盘点未提交改动、独有提交、分支/任务所有权和真实占用，再给可审清理清单；不直接删除候选资产。此前百度云只读快照为 40GB 盘/约 21GB 可用、`/srv` 4.5GB；两份 runtime 备份各约 1.2GB，PG dump 可列目录但未恢复。需重新核对当前生产容量、保留期、恢复演练与发布峰值空间，未批准前不删线上备份/缓存。

## 2026-09-24：青序流光 51 稿逐页对照（UI 专项，本地候选）

- **分母：** 本隔离树的 `docs/design/kiosk-redesign-2026-08/` 顶层编号 HTML 实有 **52 份**：00–51 缺 17 共 51 张核心稿，另有后加的 `52-advisor-artifact.html`（已入库于 `9faa26c46`）。只读现场计数：用户指定的原根工作区 51 份且无 52 号，当前隔离树 52 份，不能混用 checkout 数字。51 张核心稿均在隔离树实际渲染，HTTP 200、无 pageerror；52 号也单独实拍了无产物态，截图在忽略的 `test-results/kiosk-51-reference-016bcf3b/`。核心稿的 33/37 是已有路由的状态版式，36 是设计索引；剩余 48 个产品宿主中，当前主候选 38 个主入口在 `KioskRoot.tsx` 的 QX 集合或精确模式登记，9 个未在该集合，02 没有独立的“全部服务”路由。含 52 号则是 49 个产品宿主、39 个 QX 登记。**38/48 与 39/49 只是壳层路由登记率，不是视觉或业务完成率。**
...
- **实屏判断：** 基线 `016bcf3b` 对照运行时 `/`、`/print-scan`、`/assistant`、`/help`、`/login` 时，`/assistant`、`/help` 仍是旧壳，打印 Hub 有新版内容但页头/字阶仍有差异，首页主打印卡说明曾被挤到几乎不可读。后续已修首页并在 1080×1920 与 390×844 复验，也将 06 帮助页和 05 AI 顾问页迁入 QX 壳；05 目前仅页壳与首屏通过本地浏览器 3/3，其内部多状态未按用户原目录完整验收。其余稿仍未逐状态验收。手机 `/upload/phone` 只拍到无效链接态，不能代表正常接力。
...
| 01-home | `/` | 原项目源稿结构级首页已合流本地候选；1080×1920 / 390 W1 14/14。真实终端、现场触控与线上待验 |
| 02-services | — | 缺独立全部服务路由 |
| 03-login-gate | `/login` | QX 路由已登记；视觉待验 |
| 04-session-guard | `/session-timeout` | QX 路由已登记；视觉待验 |
| 05-ai-cockpit | `/assistant` | 原稿结构与 12 态运行时增量已合流；本地截图/临时探针和正式 W3 助手 3/3，真实模型/语音/真机待验 |
| 06-help | `/help` | 本地已迁入 QX 壳、双视口实屏与分类问答通过；真机待验 |
...
| 15-print-fulfill | `/print/progress`、`/print/done` | 打印中主态与展示组件已合流本地候选，1080 浏览器 5/5；其余结果/故障态布局、真实 Agent/出纸、逐态实屏和真机仍待做，UI/商业 NO-GO。 |
...
| 36-index | — | 设计索引，不算产品页 |
| 37-pay-states | `/print/cashier` | 23 态文案模型与 32 共用真实路由；已有状态门禁仅覆盖部分场景，真实支付与逐态实屏待验 |
...
| 51-phone-relay | `/member/qr-login`、`/upload/phone` | 两条手机分屏已按原稿结构合流；QR 登录 390 宽 4 项、上传 390 宽 3+1 项夹具浏览器用例通过；真实短信、手机文件上传、跨设备 claim/到机确认与 Windows 真机未验 |
| 52-advisor-artifact（补充稿） | `/ai/plan` | QX 路由已登记；仅无产物态实屏对照，其余产物态与发起入口待验 |

**下一批优先级：** P0：05 顾问剩余状态、10 打印 Hub 与 11 到机码剩余真实状态，以及收银、扫描、手机接力的实际路由/状态与目标稿差异；首页上方主入口的 27 寸竖屏触达需现场验证。稿 06 帮助页本地已迁入 QX，仍需 Windows Edge/现场复核。P1：其余 51 稿逐页真实路由→等待/空/错/禁用→结果实屏验收，优先打通使用频次高的打印与简历路径。P2：其余页视觉细节与真实状态反馈继续收口，遵守 reduced-motion；首页一次性入场、按钮内流光、按压/焦点反馈已有本地候选与实屏录屏，不能替代现场触控验收。AI 简历上传→解析→报告→优化/生成、求职材料与顾问必须逐项证明真实 API、授权、失败退路、结果落库及非 AI 回退；现有合成夹具只证明本地合同，不证明生产模型或第三方服务。
...
**最终完成条件：** 约定范围内严重阻塞全部关闭；关键流程真实端到端验收；源码/CI/部署/设备证据对应同一候选；资金、权限和文件归属正确；备份可恢复、发布可回滚、容量可持续；其余遗留项有明确影响、负责人和处置。实现、合成测试、CI 或多模型同意均不能单独替代商业验收。
...
1. **完成真实库验证：** 覆盖 active 对照、过期/删除/隔离/上传中/缺失关联、重复告警稳定身份、消警/审计、不泄露 signed URL；隔离 PostgreSQL/SQLite 与 CI 需要实际通过，不能用 health-only mock 代替。当时记录为完整 `verify:admin-ops` 被本机 Prisma schema engine `undefined` 阻断。
   **2026-09-24 更正：** 该阻断来自 schema engine 初始化空库，不是业务断言失败。本轮用既有 `prisma migrate deploy` 在隔离 SQLite `file:/tmp/verify-admin-ops-TAXgSR/verify.db` 上跑通完整门禁，退出 0。命令：`pnpm install --frozen-lockfile --filter @ai-job-print/api...` 退出 0；`pnpm exec prisma generate` 退出 0（client 7.8.0）；`pnpm exec prisma migrate deploy` 退出 0（103 个迁移）；未设置 `ADMIN_OPS_ALERT_HEALTH_ONLY` 的 `pnpm exec node -r @swc-node/register scripts/verify-admin-ops.ts` 最终退出 0。真实行覆盖 active、未到期、过期、软删除、隔离、上传中、paid/unpaid/refunded、两台终端、episode/确认/关闭/恢复后再故障，以及不含签名 URL、storageKey、哈希、支付字段和本人标识。正常删除走 `ON DELETE SET NULL` 并清空 `fileId`，清空后不告警。已撤掉「第二条连接 `PRAGMA foreign_keys=OFF` 制造悬空 fileId」的 helper 和强造断言。本轮正常外键路径没有复现悬空引用；关掉外键的夹具不算生产证据；历史异常数据能否留下非空 fileId 且文件行缺失，本次未证实。该形状仍只由内存 mock 覆盖。未改业务代码。随后用已有 Homebrew PostgreSQL 16.15 二进制在 `/tmp/verify-admin-ops-pg-lcVrTa` 初始化私有集群（`initdb` / `pg_ctl` / `createdb` 均退出 0），只监听 `127.0.0.1:63312`，库名 `verify_admin_ops`，未连接 5432 或任何现成库。`DOTENV_CONFIG_PATH=/dev/null`、`NODE_ENV=test` 下 `prisma generate --config prisma.postgres.config.ts` 与 `prisma migrate deploy --config prisma.postgres.config.ts` 均退出 0。探针确认 adapter `kind=postgres` 且 `current_database()` 为该库、端口 63312 后，同一环境完整 `verify:admin-ops`（未设置 health-only / skip-http，虚构 JWT）退出 0，日志 `DB connected` 指向该库。有一条 `pg` 重叠 query 弃用警告，断言未失败，未改 API。CI 未跑。这不关闭整个 C1。
...
材料页稿 25 的真实文件预览已由 `c81a1930b` 合流并通过 W3 3/3；后续只需补 390px/Windows Edge 嵌入确认，不得重复实现。当前仍未完成的是稿 21 扫描结果的 `rs-pv-*` 预览工具条、来源异常态和全站旧 UI 统一。

## 2026-09-23：以 `d88814280` 为当前唯一集成候选继续收口

1. **Claude 前端：** `/resume/source`、`/resume/parse`、`/resume/materials` 与 390px `/scan?stage=result`
   入口遮挡已本地合流，不再重做。继续按稿 21 验收来源异常态、预览工具条与真实后端交接；稿 25
   真实文件整屏预览已由 `c81a1930b` 完成，旧文件版本展示和单双面打印口径仍需产品/页面证据确认。前端只能由 Claude 修改；孤儿 CSS
   要有零引用与门禁证据后才能删除。
2. **Grok 后端：** `dd44e3cf9` 的收费导出暂存/同事务激活与 `ce83d4cb7` 的非 active 文件打印拒绝
   均已本地合流，`b3085bb7b` 的 catch-only 候选不再合入。当前 HEAD 对抗审查在途；若发现竞态或
   合法路径误拒，先修复再判源码通过。随后用 PostgreSQL 并发、真实进程退出与 COS 删除失败验证
   权益、本人资产和物理对象最终一致。隔离 SQLite 绿测不替代这些证据。
...
`/resume/materials`（稿 25）当时仍是旧壳；该项现已由 `09c93dece` 本地合流关闭，
不再按这条历史记录重复施工。
...
`1a1292102` 的支付 217/41/70、API 类型检查及收银页目标用例已实际通过；
签名合流 `7d8f007ca` 的安装输入/签名静态双门禁通过，下一步必须在最终发布 SHA 运行
Windows 签名/验签、负测、证书清理与安装生命周期 CI，之后才进入专用窗口的 Pantum 验收。
小程序 `9ec68f58` 暂未合入，继续修冷启动存储部分写入造成身份不一致；Claude 继续岗位匹配最终视觉验收。
...
不要重复实现该映射。继续验收 Grok 支付候选的互斥、迟到成功、退款取源及查单恢复，
以及 Claude 小程序候选的补签/登出/换账号/存储失败时序。后端候选未合流前不得关闭支付阻塞。
定价与会员权益、AI计费、机构权限隔离、文件归属、打印扫描状态、数据库迁移与生产回滚
仍属于商业收口范围；逐项复用现有实现，以真实缺口和测试证据决定修改，禁止另建重复系统。
Windows 签名资产选择性提取必须保持干净基线；工作区初始化异常不得混入提交。

## 2026-09-22：先修支付与会话阻塞，再验收前端候选

业务基线 `55893b515904fac32375eacb429e651b8d7de6fa`。以下事项优先于下方历史批次计划；
本地历史通过不消除新发现。`SOURCE / LOCAL: PARTIAL`，其余商业放行层继续 `NO-GO`。

| 优先级 | 负责人 | 范围与完成判据 |
|---|---|---|
| P1/P2 | Grok，独立 payment-callback-race 工作区 | 未知出码结果保持互斥；迟到成功必须落可退款流水；受理回填不得降级 success；覆盖两种回调顺序、持久化失败和重复回调，反向变异必须判红 |
| P1 | Claude，独立 miniapp-resignin-logout 工作区 | 补签中登出/换账号后旧成功不能写回或重放，旧失败不能清掉新账号；请求/上传、自然过期、single-flight 和法律版本业务码均有时序回归 |
| F2 | Claude 前端 owner | 接续 job-fit 现有 WIP；先修390x844下整体缩放造成23px返回键和过小正文，再补分析/授权/打印流程及1080x1920视觉验收；基础route-sweep 2/2不能充当全状态验收；未提交候选不计迁移完成 |
| 合流 | Codex | 核对隔离分支提交和实际 diff，保留既有改动；当前集成 SHA 重跑受影响门禁；记录独立审查来源与缺失证据 |
| Windows 候选资产 | Grok / 专用验收任务 | `windows-agent-signing-gate-20260917` 仍停在 `50483cd28` 且有15个已修改/未跟踪文件；签名脚本未进入本集成候选。保留该WIP，后续按当前基线审查提取、验证Windows行为CI，再进入专用窗口真机验收；历史静态通过不等于已合入 |

Agy 本轮跨端调用超时为 `UNREVIEWED`。需要确认任务在执行时，轮询实际工具句柄；
...
Agy 已确认本批没有后端契约漂移，但其“CI GO”只代表具备 CI 条件，不是当前 SHA 的 GitHub run；按治理口径仍为 `CI: NO-GO`。
Grok 绑定 `ad154ac1a` 后本轮返回 `UNREVIEWED`，没有批准也没有可确认 P0/P1/P2。下一批必须继续用本地命令、exact SHA、真机和生产证据分别验收，不能把模型沉默、超时或能力声明算作共识。

## 2026-09-22：招聘会 F1 已完成，进入简历次级能力批次

招聘会八条活路由已合入 `a856aa7aa`，图谱 gate 边已由 `c7297adfd` 登记；语义修复和目标门禁/浏览器
证据见 `current-progress.md` 顶部。F1 的招聘会项可以划掉，不能把本地夹具证据外推为真实内容、CI、
真机或生产通过。

| 顺序 | 负责人 / 层 | 必做事项 | 达标判据 |
|---|---|---|---|
| 1 | Claude | F2 简历次级能力：source/parse/templates/materials、job-fit/actions、career-plan；不得修改已合入的自评与招聘会文件 | 按真实路由分组复用 Qx 工作台；文件预览、AI 结果、保存/打印、错误/权限/清场状态完整；先报文件预算和图谱影响 |
| 2 | Grok `grok-4.7-build-fast xhigh` | 只读对抗审查当前 `a856aa7aa`，重点查来源阻断、打印 URL、路由出口、旧壳残留和反向变异；可派不重叠子代理 | exact SHA 绑定报告；不重复实现前端；发现项给文件/行号/退出码，timeout 不算 GO |
| 3 | Agy | 重跑跨端身份/订单/文件/支付/打印/扫描/状态/本人资产契约审查 | 输出契约矩阵与 SOURCE/LOCAL/CI/DEVICE/PRODUCTION 分层；任何缺真机/生产证据保持 NO-GO |
| 4 | Codex 合流 | 检查 `a856aa7aa` 与 `8feaadfac` 祖先和路径范围，跑图谱、repository integrity、targeted gates | 只有一个集成候选，既有 `.gitignore` 未提交改动不回滚；删除项有零引用证据 |
| 5 | 后置收口 | CI、Windows/Pantum、小程序 Trial/真机、生产备份迁移/回滚、真实支付退款对账和 UAT | 对当前最终 SHA 重新取得真实证据；未授权不得 push、部署、真实支付或操作硬件 |

前端全站目标仍未改变：所有活路由最终只用青序流光，招聘会和自我探索只是已完成批次；剩余旧壳页面
必须按不重叠 worktree 继续，禁止用占位页面、假数据或重复入口快速“补齐”。
...
`/policy-service`）已迁入青序流光并完成合入前定向修复，详见 `current-progress.md` 顶部条目。
**F1 lane 的「五个服务台统一」这一项可以划掉；同一行里的「招聘会 8 路由」仍未开始。**

下一位接手者注意三条，都是本轮踩出来的：

1. **稿里的字段要全抽出来，别在页面上猜。** `serviceHubSpecs.ts` 第一版漏了稿的 `quick`（15 条「我的」台账入口
   静默消失），第二版漏了 `icon`（页面改用标题正则猜图标，三处与稿不符）。新迁的页如果也从稿机械抽取，
   把稿里那条记录的**每一位**都带出来，抽不动的字段宁可显式抛错，不要留兜底。
2. **`:first-of-type` 按标签名算，不按类名。** 六卡页底部 400px 死白就是这么来的，而且 typecheck / lint /
   既有门禁全绿。布局类「吸收余量」的规则一律用显式类名，并在 1080×1920 实截确认。
3. **共享 hook 的「停用档」返回值不等于「正在加载」。** `useTerminalDeviceStatus(false)` 的 `loading` 会停在
   初始 `true`；照字面读就会让四个没有设备能力的服务台永久显示「正在确认本机设备」。调用方要自己
   乘上「我到底要不要这个信号」。

剩余前端分母已变（对照 `35d02eb96` 重算，方法见 `current-progress.md` 2026-09-20 条）：
109 条生产路由模式 − 18 条兼容重定向 − 3 条 contract-review 生产默认 fail-closed 重定向 = **88** 条页面落点仍成立。
五个服务台迁入后是 **49** 条 Qx / **39** 条旧壳，不是 44/44。剩余工作台约 **21–25** 个（原 22–26 减去已完成的「五个服务台 5→1」）。
2026-09-18 表仍是当时快照：F1 行的「五个服务台统一」以本条为准划掉；「招聘会 8 路由」仍未开始。
...
**总判定：`SOURCE: GO`；`LOCAL: GO`（隔离 SQLite + 空库 migrate + 本机 Homebrew PostgreSQL 16.15 完整 `postgres-readiness` 预演，绑定 HEAD `cd19fb47a`，见 `current-progress.md` 顶部）；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。**
不得把任一来源候选的旧 CI 绿或旧现场证据外推到本合流树。本机预演已按 job 顺序跑通升级库 / 招聘 Wave2 / seed / 后半 103 条（`103/103`），这只是 `LOCAL postgres-readiness rehearsal: GO`，**不是** GitHub CI。macOS / Homebrew / Redis 8 / 既有 `node_modules` 不能写成 CI 等价。CI 待办仍要求对 **final-head** 真实跑 `build-and-verify`、`postgres-readiness`、`kiosk-browser-smoke`。渠道已受理但本地回填失败的订单，运营走 Admin `opsAttention` / 对账 `attention.unconfirmedCollections`，按 `PaymentAttempt.id` 查渠道账本；不要重新出码。
...
`verify:scan-tasks` 失败，根因是扫描升级测试沙箱误包含本 PR 新增的、更晚的打印幂等迁移。
夹具已改为只复制到扫描硬化的明确前置迁移；目标测试已完成“修复前红 → 修复后绿 → 删除修复再红
→ 恢复后再绿”。不要重查小程序 R10，也不要让 Claude 重做前端审查。

**下一阶段唯一任务：** 把 R11 修复与本段进度记录提交到现有 PR #1037，等待新 head 的三个必需
CI job 全绿；全绿后再合并，并用 `git merge-base --is-ancestor <R11-SHA> origin/main` 证明进入主线。
若新 CI 仍红，只处理新日志中的第一条决定性失败，不重跑或弱化已通过门禁。

达标线：`SOURCE / LOCAL / CI: GO`，PR 合并且祖先关系退出码 `0`。这仍不等于微信真机、
Windows/奔图、生产部署或商业验收完成；合并后的下一阶段才是精确 main SHA 的真机链路。

## 2026-09-15 R10：撤回 `f7486bbe2` 的 GO —— Grok 复审复现出两条 P1，已修

**撤回的是什么：** `f7486bbe2ed0124da68350aaba1c859f846522ce`（R9，取件页归属绑定）当时给出的
`SOURCE / LOCAL: GO` **不成立**。Grok 独立复审（session `76417146-b557-4787-9f0a-8e90a6e430f4`）
在那个提交上复现出两条 P1，两条我都在本机独立复现过：

1. **R9 的归属绑定仍可被冒认，且不需要任何生命周期回调。** R9 允许在"本页自己那一发的
   回调里、页面仍可见"时把当前身份认作开页那位 —— 可页面根本不知道服务端是按谁放行的，
   `auth.saveSession` 不经过 onHide / onShow。过期开页 + 请求在途 + B 静默登录 →
   A 的到机码画给 B。R9 的测试全靠 onShow / onHide 驱动，一条都没照见这个形态；
   我当时把它写成了"应当不可达"的推理，那条推理是错的。
2. **`storage.read` 只把"抛异常"算读失败。** `null` / `{}` / `'bad'` / `42` 都落进
   `loadAll` 的 `return []`，三个读-改-写回全量的入口照样覆盖全表（本机实测 3 次写）。

**本轮做了什么：** 归属改由**服务端**认 —— 只有"发出时就带着确定账号"的确认请求拿到 200
才建立归属，且只认令牌上那个账号；发出时归属未定的那一发永不建立归属、永不写屏。
`storage.read` 改成 `{ok, found, value}` 三分，只有"key 确实不存在"才是空表。
R9 的"归属存疑（粘性）"与"登录回程一次性绑定窗口"**整个撤掉**（前者没堵住 P1-1，
还会把本人永久锁死）。只改前端 6 个文件，`services/**` 一行未动。
测试 142 → **148 全绿**（含 Grok 点名的 a–f 六条）；反向变异 **16 条全部判红**。
**`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。** 未 push、未开 PR。

**下一步顺序不变，达标判据在 R9 那一节的基础上再加两条：**

| 顺序 | 负责人 / 层 | 增加的判据 |
|---|---|---|
| 三方只读复审 | Claude / Grok / Agy | 对**包含本提交的新 tip** 复审；不得沿用 `f7486bbe2` 的绿灯（它已被本节撤回）。重点：① 归属现在只有 `onLoad` 与 `_confirmOwner` 两条来源，是否真的穷尽 —— 有没有第三条路能在没有服务端 200 的情况下写 `_openerAccount`；② `_ownerDeniedFor` 按账号记，换人之后能不能再问一次，会不会被用来反复刷服务端 |
| 微信开发者工具真机链路 | Claude + 小程序 | 追加两条：① **过期开页现在要走两个来回才出码**（触发补签的那一发 + 确认那一发），慢网 / 弱网下的观感与等待文案必须在真机上看一遍；② 确认请求被拒时的 404 判据钉的是 `statusCode === 404 && code === 'PRINT_ORDER_NOT_FOUND'` 这一对，要在真实网络上确认服务端确实这么回 |

**登记的已知遗留（不阻塞，但不得当成已解决）：**

- **两个来回是实打实的代价。** 过期开页这条路（取件链最常见的一条）现在比 R9 多一次
  往返。换来的是"归属不靠猜"。是否值得在真机上再压一压（例如让服务端在补签响应里
  直接带回 memberId），要等真机数据。
- **`_confirmOwner` 里有两条判据今天是冗余的**（`state !== 'ok' && state !== 'resignable'`、
  `this._account !== token.account`）：`'changed'` 与 `'unusable'` 会先一步在
  `_resolveIdentity` 里被截掉。它们只有门禁红、没有测试红。保留是因为它们是契约的一部分，
  押在另一条判据的当前写法上，那边一改这边就静默放行。**不为了让变异转红而编造断言。**
- **`print-pickup.js` 已经 827 行**（CLAUDE.md §8：800 行以上不得继续堆新功能）。
  本轮是安全修复，没有拆分。**下一次要给这一页加任何新能力之前必须先拆**；候选仍是把
  归属绑定 + 代次 + 令牌 + 确认请求抽成 `utils/page-guard.js` 里的纯工厂，
  `package-code` 可以复用。
- **小程序至今从未发布。** 本轮没有改变这一点。

## 2026-09-15 R9：取件页归属绑定的 P0 已修，锚点前移到本提交

**修的是什么：** R8（`b2fa6bad1`）证明的是"响应回来那一刻身份对不对"；R9 打的是它前面
那一步 ——「这一页到底是谁的」什么时候才算数。核心是一条 P0：打开取件页时 A 的 JWT 已经
自然过期（三个账号键全是空串，请求照常带补签资格发出去），在途期间 B 登录并回到本页，
**B 被记成开页那位**，A 的迟到 200 于是被 `_ownsResponse` 里那句
`if (state === 'ok') return true` 无条件放行，A 的到机码画到了 B 的屏幕上。
连同修掉的还有四条：`'ok'` 无条件放行本身、身份变化不作废在途代次且旧 `_polling` 锁死恢复、
画码 `exec` 回调不重认码/归属/代次、幂等记录在一次存储读失败后会把别人那条未落定的记录抹掉
（`clearRecord` 会把整张表写空）。细节与证据见 `docs/progress/current-progress.md` 本日 R9 条。

**本轮做了什么：** 只改前端六个文件（取件页 + 幂等模块 + storage + 生命周期测试 +
两个小程序门禁），`services/**` 一行未动。测试 134 → **142 全绿**（原 134 条一条没删；
`R8-B3` 因签名变更逐条改写、断言含义一条不减）。反向变异 **19 条全部判红**，还原用 sha256
逐字节比对，未用 `git checkout` / `reset`。
**`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。** 未 push、未开 PR。
...
| 顺序 | 负责人 / 层 | 增加的判据 |
|---|---|---|
| 三方只读复审 | Claude / Grok / Agy | 必须对**包含本提交的新 tip** 复审；不得沿用 `b2fa6bad1` 的绿灯。重点看两处：① 归属绑定的两个合法时刻是否真的穷尽了（`onLoad` / 本页自己那一发的回调且页面可见 / 一次性登录回程），有没有第三条路能让别人被认作主人；② 归属存疑的**粘性**是不是过严（见下面遗留第 1 条） |
| 微信开发者工具真机链路 | Claude + 小程序 | 追加三条本机证不了的：① 「开页时已过期 → 补签成功 → 码正常出现」这条最常见的路在真机上确实走通（本机是沙箱里的 `auth` 替身补签）；② 归属存疑那一页的文案在小屏上完整可读、且用户照着它能走到「我的 · 打印订单」并成功重进；③ 存储读失败（`wx.getStorageSync` 抛异常）在真机上到底以什么形态出现 —— 本机是手工让它抛 |
| 后端联调（只读后端） | Claude | 本轮**没有改任何后端文件**，`verify:miniapp-cloud-print-m2` 仍 ALL PASS；联调时只需确认 `GET /me/print-orders/:orderId` 的 `requireOwned` 404 仍是前端 fail-closed 的最后一道，不需要新端点 |

**登记的已知遗留（不阻塞，但不要当成已解决）：**

- **归属存疑是粘性的，代价是一次误伤。** "开页时 JWT 已过期、且恰好在那第一发请求飞着的
  时候被切走（`onHide`）或被别人登录"会让本页永久 fail-closed，用户要照提示回
  「我的 · 打印订单」重新进一次（新的一页实例绑定是说得清的，已由 R9-A2 钉住）。
  这是刻意选的那一侧：另一侧是"把一张取件凭证画给一个认不出来的会话"。
  **但它在真机上多久触发一次没有数据**，真机链路跑通后应当回看一次。
- **有一条路本机推不到，也没有证据说它不存在。** 若在"别人登录"与"A 的响应落定"之间
  **一个生命周期回调都不触发**，本页就没有机会发现身份变了。按微信的导航模型，去登录页
  必然经过 `onHide` / `onShow`，而页面内的静默补签（`request.js` 走 `wx.login`）拿到的
  是**同一个微信用户**的 code —— 所以这条路应当不可达。**这是推理，不是实测**，
  真机上要专门走一遍"另一个账号登录"的完整路径确认。
- **`print-pickup.js` 已经 745 行**（CLAUDE.md §8：500 行以上新增功能前必须评估拆分）。
  本轮是安全修复，没有做拆分 —— 在一次凭证归属的修复里顺手重构身份状态机，正是 §8.1
  禁止的那种"大范围重写"。**下一次要给这一页加新能力之前必须先评估拆分**（候选：把
  归属绑定 + 代次 + 令牌那一套抽成 `utils/page-guard.js` 里的一个纯工厂，
  `package-code` 也能复用 —— 它今天有同一组问题的另一半解法）。
- **`M16` 这条变异至今只有门禁红、测试绿，而这是对的。** `ensureKey` 的第一道读失败判断
  与铸键前那道 `const base = loadAll(); if (!base) throw` 是**真的冗余**（后者排在任何
  一次写入之前），删掉第一道在行为上没有差别。登记它是为了下次有人看见"这条变异没红"
  时不要去编一条测不出真差别的断言。
- **小程序至今从未发布。** 本轮没有改变这一点。

## 2026-09-15 R8：撤回「前端幂等链已可进最终复审」的结论，锚点前移

**撤回的是什么：** `638ea71baeb8556d4327241e7a9221bee4ee15e1`（前端幂等键耐久化）与
`103adb9e7899f620104f6abf3443e666a076fe8d`（后端回放收口）各自的结论都成立，但此前据此
形成的「这条链可以进最终复审了」不成立 —— 六个缺陷活过了它们，全部在本机独立复现：
取件页在途请求会把凭证清场推迟到网络之后（P1-A）、请求发出后才自然过期的 200 被当成登录失效
而清掉一张仍然有效的码（P1-B）、`clearRecord` 不读回也不返回于是页面被"假成功"解锁（P1-C）、
淘汰保护挂在内存里进程重启就没了（P1-D）、`'live'` 结论被永久缓存（P2-E）、
时钟回跳让本机自己写的键当场作废（P2-F）、指纹字段集跨层无人比对（P2-G）。
细节与证据见 `docs/progress/current-progress.md` 本日 R8 条。

**本轮做了什么：** 只改前端六个文件（两个页面 + 幂等模块 + 生命周期测试 + 两个小程序门禁），
`services/**` 一行未动。测试 123 → 134 全绿（原 123 条一条没删）；反向变异 15 条 + M3 家族
4 条全部判红，还原用 sha256 逐字节比对，未用 `git checkout` / `reset`。
**`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。** 未 push、未开 PR。
...
| 顺序 | 负责人 / 层 | 增加的判据 |
|---|---|---|
| 三方只读复审 | Claude / Grok / Agy | 必须对**包含本前端提交的新 tip** 复审；不得沿用 `638ea71ba` 或 `103adb9e7` 的绿灯。重点看两处放宽是否越界：`'resignable'` 成功响应的放行条件（`_ownsResponse` 三条同时成立）、未来时间戳一律按未过期处理 |
| 微信开发者工具真机链路 | Claude + 小程序 | 追加两条本机证不了的：① `onHide` **不清凭证**是权衡后的选择（回前台由 `onShow` 同步判身份清场），必须在真机上走一遍「码显示 → 切后台 → 回前台」与「码显示 → 切后台 → 另一页换账号 → 回本页」两条路，确认后者在回到本页的那一帧就没有码；② 未落定名额用尽时的拒绝文案在小屏上完整可读、且用户照着它能走到「我的 · 打印订单」 |

**登记的已知遗留（不阻塞，但不要当成已解决）：**

- **本轮六条全部只有静态执行与沙箱证据。** 沙箱真跑的是页面源码本身（`node:vm` + 真实
  `page-guard` / `auth` / `storage`），不是正则断言；但它替代不了 WXSS 编译、真实 `wx` 宿主
  行为与真机存储压力。存储写失败那两条（`clearRecord` / `rememberOrderId`）在真机上到底
  以哪种形态出现（抛异常 / 静默 no-op / 部分写入），本机只能两种都挡住，挡对了没有真机佐证。
- **`MAX_PENDING_RECORDS = 20` 这个上界没有真实数据支撑。** 它要被填满需要同一台设备在 7 天内
  发生 20 次"提交出去了但从来没拿到过 orderId"。取值是推理出来的，不是量出来的；
  真机链路跑通后应当回看一次实际的未落定记录条数分布。
- **未来时间戳按"无上界地视为未过期"处理。** 这是最保守的一侧（宁可留着一个键，不可凭空
  多一张订单），代价是被恶意/异常写入的远期 `createdAt` 会一直占着一个未落定名额直到被清掉。
  当前由条数上界兜住，不会无限堆积；若真机上出现异常占用，再收紧成"有界容忍"。
- **小程序至今从未发布。** 本轮没有改变这一点。

## 2026-09-15 R3 已合入后的唯一推进顺序（取代下面 R3 候选那一节）

**当前总判定：R3 已进入 `main`（merge commit `ddef936def46e9220a25e44ffe33dcc3458ed00b`，
`main` CI run `34917887442` 四 job 全 success，含 `release-bundle` 产物生成）。
`SOURCE / LOCAL / CI: GO；DEVICE / PRODUCTION / COMMERCIAL: NO-GO。`**

本机独立复核过，不是转述 PR 页面：`git merge-base --is-ancestor ddef936def46e9220a25e44ffe33dcc3458ed00b origin/main`
退出码 0，`git rev-parse origin/main` 当前等于该 SHA；`gh run view 34917887442` 为
`completed / success`，`headSha` 即 `ddef936de`。**`release-bundle` 只生成产物，没有任何部署动作**，
不得据此宣称已上线。下面 R3 候选那一节里「只剩最终 head CI 与合并」的第 1 项到此关闭。

| 顺序 | 负责人 / 层 | 必做事项 | 达标判据 |
|---|---|---|---|
| 1 | Codex 集成 + Claude / Grok / Agy 复审 | **小程序材料包 R4 运行时代码锚点 `70bc85f10` 已本地完成；先对图谱 / 文档收尾后的 final head 做三方只读复审，再只开一个 PR**：push → PR → final-head CI 全绿 → 合入 | 工作树干净；final head 无 P0/P1；PR 必需检查全绿；合入后 `git merge-base --is-ancestor <PR-final-head> origin/main` 留证并记录新的 main SHA。不得把 `70bc85f10` 的本地绿灯继承给后继文档 SHA |
| 2 | Claude + 小程序 | **在微信开发者工具里真的打开一遍**这条链：材料包四页 + 「打印订单」材料包分区 | 逐页截图；确认 WXSS 能编译（不白屏）、六态可见、48px 触控、下拉刷新 / 触底 / 返回重进不重复不丢游标；A/B 两个账号切换后上一位的列表与到机码不残留 |
| 3 | Claude + 后端联调（只读后端） | 材料包端到端跑通一次：真实上传 → 隐私检查 → 服务端多行报价 → 建单 → 拿到机码 → 离开再从「我的打印订单」找回 | 记录每一步真实响应；重点验四个 fail-closed 路径确实按预期拒绝并给出可执行下一步：终端离线、隐私检查未过、价目未配置、订单非本人 |
| 4 | Windows / 奔图专用任务 | 在合入后的精确 SHA 上完成材料包到机核销 → 现场付款 → 多文件出纸 → 状态回流 | 记录任务/订单/文件 hash/状态回传/临时文件删除；重点证明多文件材料包逐份派生任务正确，且旧文件不交给后来的用户 |
| 5 | 运维 / 生产 | 具名维护窗口按精确 SHA 完成备份、迁移、部署、监控、回滚 | 同下面 R2 那一节第 4 项，口径不变 |
| 6 | 产品负责人 / 运营 / 客户 | 微信类目与提审、授权内容冷启动、真实用户 UAT 与签字交付 | 同下面 R2 那一节第 5 项，口径不变 |

执行边界：第 1 项前不宣称已合入，第 2-3 项完成前不宣称小程序可用，第 4 项完成前不宣称真机通过，
第 5 项前不宣称生产可用，第 6 项前不宣称商用收口。

材料包候选的已知遗留（登记，不阻塞第 1 项）：

- **材料包没有在线取消端点。** 服务端 `PackageOrdersController` 只有 `@Post()`、`@Get()`、`@Get(':id')`，
  未付款订单只能等有效期（7 天）结束自动失效。这一点已如实写在下单页与到机码页上，**不做成能撤单的假象**。
  要开放取消，必须先在服务端补端点 —— 前端不得自己造一个必得 404 的调用。
- **材料包分区与单件打印分区各自分页，没有合成一条时间线。** 这是刻意的：两者是两套 id 空间和两个独立
  游标，合成一页就必须在触底时二选一，推错会静默丢掉一整段订单。要做统一时间线得服务端先给一个合并端点。
- **视觉刻度棘轮比基线低 17（字阶）/ 12（圆角阶）**，未触发门禁要求调低的阈值（20），故未动 `BASELINE`。
...
三条 P1 都不需要换人、不需要竞态，一次普通下单就会踩到。锚点前移到本分支 tip。

- **落盘从来没被确认过**：`utils/storage.js` 的 `set()` 会吞掉 `wx.setStorageSync` 的异常
  返回 `false`，而这个返回值当时被整个忽略 —— 键没落本机、POST 照发，响应一丢就是第二张订单。
  现在 `persist()` 既查返回值、又把记录**读回来逐字核对** `account/fingerprint/key`；
  核不上就 `reject`，一个 POST 都不许发。
- **同一格能并发铸出两个键**（取随机数是异步的）：模块级按 `account+fingerprint` 串行化，
  重叠调用共用同一个在途 Promise。
- **淘汰会把"正在飞"的那条挤掉**（只按 `createdAt` 留最新 20 条）：铸键时把这一格钉住
  （最长 30 分钟，落定即释放），钉住的不参与淘汰；名额只在没钉住的那批里按价值回收。
  存储总量仍然有界。
- **`wx.getRandomValues` 两个回调一个都不来时页面永远停在「正在提交…」**：加有界超时
  （8s）按失败处理，并接住"只回 `complete`"那种实现。仍 fail-closed，仍不退回 `Math.random`。
- **换人只认得 A→B 这一种跳变**：真实链路更常见的是 A 登出（快照清成 `''`）之后 B 才登录，
  那一跳在状态机眼里是正常补签升级 = `'ok'`。`package-code` / `print-pickup` 两页判据统一
  换成**当前这位是不是开页那位**（独立判据，不是 `changed` 分支里的一个 `if`）；登出仍不粘。
- **一条七天前的恢复记录会把这一组参数锁死七天**：服务端幂等键永久挂在 Order 行上
  （`verify:miniapp-cloud-print-m2` 的 T10 = 取消态回放原单），订单早就取消/过期/打完了，
  用户却只有一个按不动的按钮。现在恢复出锁后用既有端点 `GET /me/print-orders/:orderId`
  核一次真实状态：**终态才给出用户自己点的「重新下单」**（点了才清记录、下一次才铸新键），
  **核不上一律保持锁定**，页面自始至终不自动换键。

123/123 真执行测试（比 R6 多 15 条）；**13 条行为变异 + 12 条门禁变异共 25 条全部判红**
（按被测命令的退出码判定，不是数 FAIL 行；还原用 sha256 逐文件比对确认字节一致，
未用 `git checkout` / `reset`）。其中 `M8` 第一次是 GREEN ——
那是我把变异写成了测试替身根本不调的 `complete` 回调（变异本身无效，不是测试漏了），
重新表述成"在 `redirectTo` 之前就清记录"后判红。门禁新增 `⑭` 一段共 12 条静态断言；
并**收紧**（不是放宽）`package-code` 那条旧锚点：
原本只钉"开页那位回来能解除"，证明不了"A 登出→B 登录也会被挡住"。

**边界不变且必须照写：`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。**
未 push / 未 PR / 未合并 / 未跑 CI / 未进开发者工具 / 未接真实 API / 未真机 / 未部署。
本轮**没有改任何后端行为**（`services/**`、Prisma、迁移一个字节都没动）。
上表第 1 项的达标判据原样有效，三方只读复审要对**这个新 tip** 做。

R7 新增遗留（不阻塞上表第 1 项，但不得当作已修）：恢复记录核状态会多打一发请求，慢网观感未在
真机看过；`PIN_TTL_MS = 30min` 与 `RANDOM_TIMEOUT_MS = 8s` 都是照推理取的、未在真机标定；
requireOwned 的 404 被当成"这张订单没了"是本轮唯一一条把服务端错误当状态证据的判据，
只认 `404 + PRINT_ORDER_NOT_FOUND` 这一对，真实网络上没触发过。

2026-09-15 **幂等建单接线：撤回 `cf2e12d93` 的「重复下单已收口」结论。**

该 SHA 靠的全是页面内的一把内存锁，挡得住"同一个页面实例里的并发点击"，挡不住真正会
多扣一笔钱的那一种 —— **200 丢在路上**：订单已建成、响应没回来、页面的锁随失败分支解开，
用户再点一次就是第二张订单。那需要服务端先有一个可回放的键。
`ed576f3cb`（Grok，服务端）已经给 `POST /me/print-orders` 加上必填 `Idempotency-Key`
与按 `(endUserId, key)` 的耐久回放；本轮把小程序接上去，并修掉冷审的三个页面问题。
锚点前移到本分支 tip。

- 新增独立 utility `apps/miniapp/utils/print-order-idempotency.js`：
  `wx.getRandomValues` 出 UUID v4（**禁用 `Math.random`**，碰撞=两个人共用一个键）、
  按 `会员 id + fingerprint(fileId,terminalId,copies,colorMode,duplex)` 持久化、TTL 7 天、
  **不存到机码 / 文件名 / 金额**、B 不得读或复用 A 的记录。指纹字段集与服务端逐字同一组。
- **先拿键、先落盘，再 POST**；**拿到 orderId 先写进发起那位的恢复记录，再判页面是否接收**；
  **确实跳走了才清记录**（跳转失败要留着，否则唯一的线索就没了）。
- 服务端回放已 cancelled / expired 的原单时按返回的 orderId 处理，不自己另铸新键
  （那等于伪造一次"重新下单"）。
- 材料包码页换到 B 后**第二次及以后**的 `onShow` 也不再用 B 的 token 查 A 的 orderId
  （粘性 `_foreignBlocked` + 不随清场销毁的 `_openerAccount`；**登出不粘**，A 重登可恢复）。
- 报价 fail-closed 给出**去登录**按钮（状态字段 `quoteRecover`，不按中文文案判断）。

108/108 真执行测试，含一条用真 `utils/api.js` + `utils/request.js` 截 `wx.request` 入参、
断言键在 Header 不在 body。8 条反向变异全部判红 —— 其中 `N6` 第一次是 GREEN
（测试直接调 `page.toLogin()` 绕过了模板接线），改成从模板读函数名再调才判红，如实记录。
门禁修了四处按单行字面量 / 定长窗口匹配的断言（重构暴露出来的，不是放宽），
并新增一条跨端断言：小程序把键放 Header，与服务端 `@Headers('idempotency-key')` 一致。

**边界不变且必须照写：`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。**
未 push / 未 PR / 未合并 / 未跑 CI / 未进开发者工具 / 未接真实 API / 未真机 / 未部署。
**「幂等」尤其不要外推**：前端复用键与服务端回放各自被证明过，**两端拼起来在真实网络上
一次都没跑过**。上表第 1 项的达标判据原样有效，三方只读复审要对**这个新 tip** 做。

本轮新增遗留：取键是异步的（`wx.getRandomValues` 无同步形态），慢设备上的观感未在真机验过；
该 API 不可用时本页 fail-closed 拒绝下单，最低基础库版本未核对；恢复记录只在本机，
换手机 / 清缓存后只能从「我的 · 打印订单」找回。

2026-09-15 **R5 收口：撤回 `c42818c11` 的「可进入三方复审 / 开 PR」结论。** 最终只读复审在
该 SHA 上又判出 1 个 P1 + 3 个 P2，全部与「补签**失败**之后没有人写终态」这同一个成因有关，
已在本地修完并追加一个提交（不 amend）。锚点前移到本分支 tip。

- **P1** 材料包到机码页：过期 → 发请求 → 补签失败（`auth.logout()` 撤销资格）→ 401 →
  `_accepts` 因 `'unusable'` 返回 false → 只 `return`，`loading:true` 永远留在屏幕上。
  这一跳里快照始终是 `''`，**不算换人**，所以 R5 主体的 changed 清场分支根本不会执行。
- **P2** 确认支付页报价同一形态；停在 `'loading'` 还会同时锁死「重新核价」（模板只在
  `error` 时给按钮，`retryQuote` 又只在非 loading 时才动）。
- **P2** 材料包到机码页 `onShow` 身份判两次：判成换人后仍调 `loadOrder`，会用 B 的 token
  去要 A 的 orderId（必然 404）并把「账号已切换」覆盖成 loading。
- **P2** 确认支付页 `wx.hideLoading()` 排在归属判定之前：它无条件掀掉当前那张遮罩，
  A 的迟到回调会掀掉 B 正在进行的提交遮罩。

四条**先写测试证明缺陷、再修**（修前 `not ok 95/96/97/98`，修后 98/98 全绿）。8 条反向变异
全部判红；R5 主体里因本轮改动失配的 `M6` / `M11` 按新代码重新表述后重跑仍红，其中 `M11'`
从「只被门禁判红」升级为被可执行测试判红。门禁只做补强（含把又一处定长窗口换成配对闭合提取），
没有替代行为测试。`docs/graph/**` 本轮无漂移、未改动。

**边界不变且必须照写：`SOURCE / LOCAL: GO`；`CI / DEVICE / PRODUCTION / COMMERCIAL: NO-GO`。**
未 push / 未 PR / 未合并 / 未跑 CI / 未进开发者工具 / 未接真实 API / 未真机 / 未部署。
上表第 1 项的达标判据原样有效 —— 三方只读复审要对**这个新 tip** 做，不得沿用 `c42818c11`
的本地绿灯。`utils/auth.js`、`utils/request.js`、`orders.js` 本轮一行未改。

2026-09-15 **R5 修正（取代上表第 1 项里「R4 已本地完成」的说法）：R4 的四态身份判定只修对了
一半，且它的回调清场测试是假覆盖。两件事都已在本地收口。**

- R4 声明「取件页四态判定，不再永久卡在 loading」。该声明在「**打开时就已过期**」成立，在
  「**码已显示出来之后才过期**」不成立 —— 后者才是这条链最常见的形态（中午下单、下午到机器前
  打开）。`auth.getToken()` 过期时先 `clearSession()`，`'u:A' → ''` 被判成主动登出：清掉一张
  服务端仍然认的码、一个请求都不发、静默补签永远跑不到。材料包码页、确认支付页各重现一次；
  确认支付页那一次的后果是**重复下单重复扣款**（订单已建成却不锁 orderId 还解开按钮）。
- R4 的 `R4-2` 回调清场测试先登出再调 `_load()`，入口守卫就把场清了，回调里那次判定根本没执行。
  实测：把 `orders.js` 四个回调的 `_enforceIdentity()` 全删掉，三条 `R4-2` 全绿。已补真正的
  延迟场景（请求发出时还登录着 → 之后 request.js 才 `logout()` → 再让 promise 落定）。
- 判据收敛进 `apps/miniapp/utils/page-guard.js` 的 `resolveAccountState` / `sameAccount` /
  `adoptIdentity`；**`utils/auth.js` 与 `orders.js` 的运行时代码本轮一行未改**（前者早就提供了
  `RESIGNIN_ELIGIBLE` 这个持久判据，后者改的是证明它的方式）。
- 本机全部退出码 0：小程序 `verify:static`（132 PASS，生命周期 **94/94**）、
  `verify:package-chain`（301 PASS）、API `verify:miniapp-cloud-print-m2`、根三条治理门禁、
  `git diff --check`、`graph:check`（`pnpm graph` 标准重生成后）。11 条反向变异全部判红
  （其中 10 条被可执行测试判红，1 条因纵深防御在行为上不可观测、只被门禁判红，如实登记）。
- **边界不变：`SOURCE / LOCAL: GO`；未 push / 未 PR / 未合并 / 未跑 CI / 未进开发者工具 /
  未接真实 API / 未真机 / 未部署。** 上表第 1 项的达标判据原样有效，只是运行时代码锚点从
  `70bc85f10` 前移到本分支 tip。

R5 登记的遗留（不阻塞上表第 1 项，但不得当作已修）：

- **`orders.js` 的自然过期仍然按清场处理**（空列表 + 请登录），没有放行静默补签。fail-closed、
  不是安全缺口，但确实是能力退化；改它要动 `isLoggedIn` 的模板分支与两个分区的加载入口，
  列为独立任务。
- **账号快照的销毁依赖页面下一次判定**，仓库没有全局会话事件总线。前台三页都会当场发现
  （取件页 3 秒轮询、另两页在 `onShow` 与每个回调上判），但切到后台且无在途请求的页面要到
  切回前台才清。

2026-09-15 R4 补充：三方复审在 `faae6c6b` 上发现的前台会话失效清场、未登录取件页永久 loading、
隐私扫描代次、跨身份建单锁、协议 label、真实双面 / 彩色能力码和单件 URL / 重复建单问题，已在
运行时代码提交 `70bc85f10a11dbeb62206a055fcdd2c500654060` 收口。小程序 `verify:static` 0
（132 PASS，生命周期 77/77）、API `verify:miniapp-cloud-print-m2` 0、三条根治理门禁均 0。
新增门禁引起的 3 个 `docs/graph/**` 漂移已经 `pnpm graph` 重生成。该结论仍只属于本地候选；
final head 的独立复审、GitHub CI、微信开发者工具、真实 API、真机和生产全部待做。

2026-09-15 再补：**Grok 对抗审查（冻结点 `23089517756d103994ecd535298a1d1c874d2c8b`，结论 NO-GO）
判出的 1 个 P0 + 10 个 P1 已全部在本地修完**，同分支再追加一个修复提交（不 amend）。
仍未 push / 未开 PR / 未合并 / 未跑 CI / 未进微信开发者工具 / 未真机 / 未部署。

P0 是**双面材料包从来没有真正工作过**：UI 的 `duplex: 'double'` 被报价与建单原样发出，
而服务端两个 `@IsIn` 白名单都不含该取值，选了双面必然在报价那一步 400；而且
`verify-package-chain` 有一条断言把这个缺陷钉死了。现已统一经
`pkg.toWireDuplex` 映射到 `duplex_long_edge`（口径取自 `packages/shared` 既有的
`normalizeDuplex`），报价与建单逐字同源。本机 6 条门禁全 0，22 类反向变异全部判红后还原复跑通过。
细节见 [current-progress.md](current-progress.md) 当日条目。

**这一条 P0 直接改变了上表第 3 项的验收内容**：材料包端到端联调必须**单独跑一遍双面**，
而不只是默认的黑白单面。要确认的是三件事，缺一不可：

- 选双面 → 服务端报价返回真实金额（此前这一步必然 400，等于双面从没被验过）；
- 目标终端**未登记** `duplex_print` 能力时，报价按 `CAPABILITY_NOT_CONFIGURED` 拒绝并
  给出「换一个服务点」，**不得静默按单面出纸**；
- 终端已登记该能力时，建单 → 到机核销 → 真机出纸确实是双面。
  注意 CLAUDE.md §3 记着「长边 / 短边翻页方向仍未核对」—— 本轮按仓库既有口径取长边，
  真机验收时要当场确认翻页方向是否符合竖排材料的预期；不符则属产品口径问题，不是前端映射问题。

另外要在第 2 项（微信开发者工具）里一并看的：

- 未勾选《打印服务协议》时「确认下单」的拦截提示是否清楚（默认已改为不勾选）；
- 单件取件页现在**只带 orderId**，首屏必然要等一次服务端回读 —— 确认加载态不刺眼、
  失败时给的是可执行的下一步，而不是一片空白；
- `package-create` 文件列表「加载更多」失败时，已加载的文件与已勾选状态是否真的留在屏幕上。

`docs/graph/**` 已在 R4 收尾按标准命令 `pnpm graph` 重生成；最终是否无漂移以 PR final head 的
`pnpm graph:check` 退出码为准。

2026-09-15 补：**材料包候选的身份生命周期与异步竞态已在本地收口**（同一分支上的追加修复提交，
不 amend 原提交；仍未 push / 未开 PR / 未合并 / 未跑 CI / 未进微信开发者工具 / 未真机 / 未部署）。
修的是 P1-01 跨用户与登出时在途请求回写、P1-02 `package-code` 迟到响应复活到机码、
P1-03 重复加载乱序覆盖终态、P2-01 `temp_package_data` 未绑定用户、P2-02 未登录先解析草稿、
P2-03 `temp_selected_store` 跨草稿残留。机制落在新增的 `apps/miniapp/utils/page-guard.js`
（身份快照 + 代次 + 逐通道 latest-wins），并新增真执行测试
`apps/miniapp/scripts/tests/page-lifecycle.test.mjs`（18 例，已串进 `verify:static`）。
本机 6 条门禁全 0，5 类反向变异全部判红后还原复跑通过。细节见
[current-progress.md](current-progress.md) 当日条目。

这**不替代**上表第 2、3 项：静态门禁与 node 沙箱证明不了真机行为。上面第 2 项的
「A/B 两个账号切换后上一位的列表与到机码不残留」现在有了可执行的回归，但仍必须在
**微信开发者工具真机调试**里再走一遍 —— 沙箱里的 `onHide/onShow` 是测试自己调的，
真机上的触发时机（后台、锁屏、返回、`navigateBack` 层级）由微信决定，两者不是一回事。

还需要在第 2 项里一并看的两点（本轮改动引入，只有真机能确认）：

- 建单成功后清草稿的时机从 `redirectTo` 的 `success` 回调**提前到跳转之前**。
  要确认跳转仍正常落到到机码页，且返回后不会出现"草稿已清、页面还停在确认页"的空窗。
- `package-create` 进入时会清 `temp_selected_store`（新草稿开始）。要确认从
  `store-select` / `package-confirm` 用 `navigateBack` 返回第一步再继续时，
  用户**不会**无故丢掉刚选好的服务点（返回不经过 `createPackage()`，理论上不受影响）。

`docs/graph/**` 已在 R4 收尾按标准命令 `pnpm graph` 重生成；旧候选阶段“未重跑”的描述只保留为
历史过程，不再代表当前工作树。

## 2026-09-15 R3 候选后的推进顺序（已完成，保留作为历史；实时口径见上一节）
...
| # | 事项 | 为什么别人做不了 | 判据（做完怎么验） |
|---|---|---|---|
| 1 | **提审微信小程序** | 需要小程序管理员账号；lane 能上传版本，不能提交审核 | 用 `GET /miniapp/code` 打首页不再回「小程序尚未发布」 |
| 2 | **接入第一家真来源机构**（BL-06） | 需要管理员后台建机构 + 内容信任激活，没有后门 | `GET /api/v1/jobs` 的 `pagination.total > 0` |
| 3 | **先改彩色价目描述，再开通 `color_print`** | 只能在管理员后台改；**顺序不能反**（CLAUDE.md §3） | 改完 `GET /admin/billing/price-config` 里描述与 `unitCents` 自洽；#1026 的写入闸门会拦住不自洽的新写入 |

第 2 条还有个当前后果值得先知道：三个板块 `total=0` 时，**小程序四个主 Tab 里的「求职」是一张「暂无岗位」空页**
（链路：`app.json` → `pages/jobs/jobs.js:48` → `GET /jobs` → `jobs.wxml:73`）。
...
- **一期（Windows 主机 + 打印机 + 扫码枪）**：打印首单已出纸；**面板扫描**与**扫码枪付款码**两项未实测。
- **二期（27 寸一体机整机）**：整机到货后全套重跑。
- 现场脚本已备齐，不用再写：顺利路径 [`../device/onsite-acceptance-runsheet-2026-09.md`](../device/onsite-acceptance-runsheet-2026-09.md)、
  故障恢复 `onsite-failure-recovery-runsheet-2026-09.md`（7 场景）。
  故障恢复那份的三条硬结论先记住，免得现场把「驱动不报」误判成「系统坏了」：
  **这台奔图缺纸时不出缺纸文案**（会落到 `PRINT_JOB_UNCONFIRMED`）、**缺粉无自动处置**、**已付款未出纸不会自动退款**。

### 出发前必做（否则到现场才发现）

1. 服务器上 `grep PAYMENT_PROVIDER`（外部探不出来：订单查找排在渠道判断之前）。
2. 桥接令牌值随身带。
3. 跑一次 `bash scripts/collect-production-evidence.sh`（59 项只读，密钥只报有无与长度）。
4. BL-05：百度 OCR 密钥曾在聊天暴露，控制台重建应用轮换。

### 顺手可做的一条（有证据，没做，留给接手的人判断要不要做）

首页「就业政策」磁贴和岗位卡片是**同一个毛病**：生产 `GET /policies` 实测 `total=0`，
而磁贴写死「资格与办理条件以官方核验为准」，**空不空都一样**。

- 岗位那颗已按招聘会同构改成四态（#1034）；政策这颗没动。
- 技术上不难：`HomeTile` 本来就支持 `badge`（百宝箱磁贴 `QxHomeView.tsx:274` 就在用），
  照 `useHomeJobHighlight` 再来一个即可。
- **没顺手做的理由有两条，都不是「忘了」**：①会给首页加第三个网络请求；
  ②政策内容更像官方/自建而非合作机构来源，「暂无政策」是不是该说，
  和岗位不一定同一个判断。**这是产品判断，不是工程判断**，所以留着。

### 代码侧还开着的（都不是上线阻塞）

青序流光迁移余量（BL-01，已迁部分线上生效、其余页仍旧版式）、旧页下架、动效层补齐 —— 见各 lane 的 PR。
...
jobs / job-fairs / policies 均 0，companies 3）。**微信审核对「功能不完整 / 空白页面」
是明确驳回项**，所以这不只是运营缺内容，是提交审核的硬阻塞。

门槛数字与来源候选清单已写进
[docs/product/content-onboarding-runbook.md](../product/content-onboarding-runbook.md)
的 §5A / §5B / §5C（补在既有手册里，没另起文档）。下面只列**工程侧做不了、
必须你本人出面**的事：

### 一、拿授权（这一项不解决，后面全部无从谈起）

- [ ] **本地高校就业指导中心**：校方书面合作函 / 共建协议。
      这是唯一能同时供岗位 + 招聘会 + 政策的机构类型。
- [ ] **市/区级公共就业服务机构**（人才交流中心 / 就业促进中心）：官方合作文件或数据使用授权。
      **政策内容的唯一权威来源** —— 系统里只有这类和高校能发政策，商业机构一律拒写。
- [ ] **本地招聘会主办方**：单场次授权即可，门槛最低。
      一场招聘会能一次带来几十家参展企业与展位信息，**冷启动性价比最高的一次性投入**。
- [ ] 如与持证人力资源服务机构合作：**必须查验对方的人力资源服务许可证**并留存复印件。
      我方无证，借道无证机构等于自己无证经营。

> 拿到授权后由工程侧在 Admin 把该机构标为「内容可信」，**标记时必须填核验依据
> （授权书 / 合同 / 公开声明编号），这条依据会进审计日志**。未标记时任何发布动作
> 一律被 `ORG_CONTENT_TRUST_REQUIRED` 拒绝 —— 这是设计如此，不是故障。

### 二、微信公众平台侧（工程侧无账号，且已明确不索取口令）

- [ ] 小程序**类目**确认：岗位 / 招聘会 / 政策信息展示对应的服务类目是否已选全。
      类目不符会在审核阶段直接驳回，与内容多少无关。
- [ ] `WECHAT_MINIAPP_APPID` / `WECHAT_MINIAPP_APPSECRET` 提供给运维配进生产 env。
      **appSecret 只进服务端**（CLAUDE.md §12），不要发在聊天里。
      未配置时「微信一键登录」返回 `WX_CONFIG_MISSING`，用户只能走短信登录。
- [ ] 小程序码 `getwxacodeunlimit` 生成的是永久码，**但只对已发布版本有效**；
      未发布时微信返回 `errcode 41030`（本项目 AppID 实测值）。
      新页面上线前可先把 `WECHAT_MINIAPP_ENV_VERSION` 设为 `trial` 走体验版验链路。
- [ ] 体验版成员名单：把需要验收的人加进去。

### 三、内容录入的量（拿到授权后，运营动作）

提交审核门槛（依据见 runbook §5B）：**岗位 ≥ 30 条**（覆盖 ≥3 区域 ×  ≥3 行业）、
**招聘会 ≥ 3 场且 ≥1 场在未来 14 天内**、**政策 ≥ 8 条且 4 个人群分类每类 ≥1 条**、
**≥1 条政策录入真实申领条件**。

一体机现场门槛更高：**岗位 ≥ 50 条**（现场按「区域 + 行业」两级筛完仍要有结果）。

### 四、不要做的事（工程侧已在代码里堵死，写在这里免得对外承诺）

- 不抓取第三方招聘平台列表页。没有授权的抓取在岗位数据上同时踩数据权益与不正当竞争。
...
**发布管道不发小程序。** `deploy.yml` 只拷 `apps/kiosk/dist`、`apps/admin/dist`、
`apps/partner/dist` 加 API —— 全文 `miniapp` 出现 **0 次**（已核实）。
小程序只有在 **mp.weixin.qq.com 重新上传 → 提审 → 发布** 才会到用户手里。

由此产生一条时序约束，**发布前必须确认**：

### 必须先合入 main，再重新发布小程序

- [x] **#972 登录失败说不出原因** —— 已合入 `main`（2026-09-08）

  #953 给小程序补了「服务端机器码不外泄」的收敛层，但认证链路的错误码一个都没登记，
  导致**登录失败说不出原因**。最严重的是协议版本更新时用户只看到「登录失败，请重试」，
  **永远不知道要重新勾协议，只会一直重试一直失败**。

  #953 已在 `main` 上。**如果先重新发布、后合 #972，那一版就是坏的，
  而且要再走一轮提审才能修回来。**

### 发布前逐项确认

- [ ] 确认 `main` 上已包含 #972（`git log origin/main --oneline | grep 972`）
- [ ] 用开发者工具打开当前 `apps/miniapp`，**实际走一遍短信登录**，
      并故意用错验证码，确认屏幕上显示的是「验证码不正确，请重新输入」
      而不是「登录失败，请重试」
- [ ] 上传前跑 `pnpm --filter @ai-job-print/miniapp verify:static`
      （WXSS 比 CSS 严，`*/;` 这类会整站白屏而所有门禁保持绿色 —— 门禁绿不等于能渲染，
      必须用开发者工具真开一次）

### 还有一条只有重新发布才能解开的：小程序码

生产实测（2026-09-09，按控制器源码取的真实路径，不是猜的）：

```
POST /api/v1/miniapp/code/capabilities → {"available":true,"envVersion":"release"}
POST /api/v1/miniapp/code              → 端点存在
POST /api/v1/upload-sessions/scene/resolve → 端点存在，乱码统一回 UPLOAD_SCENE_UNUSABLE
```

服务端三段已经齐了（生成小程序码 → 场景码兑换 → 既有上传端点），但

> `getwxacodeunlimit` 生成的是**永久码，只对已发布版本有效**。
> 当前 `envVersion=release`，而 `pages/kiosk-send/kiosk-send` 是新页面 ——
> **在重新发布之前，扫出来的码对这个页面会拿到 `errcode 41030`。**

所以「微信扫一体机的码直接进小程序」这条路，**要等重新发布才真的通**。

想在发布前先验链路：把生产的 `WECHAT_MINIAPP_ENV_VERSION` 设成 `trial` 走体验版
（需要先把验收人加进体验成员）。

> ⚠️ 在重新发布并实测通过之前，**不要在任何验收材料里写「扫码进小程序已通」**。
> 代码在 main 上 ≠ 用户能用 —— 这正是本节存在的理由。

### 一个容易混淆的地方

产品负责人若报「小程序登录有问题」，先问清是**线上版本**还是**开发者工具里的本地版本**：

- **线上版本** → 跑的是更早的已发布版本，与 #953 无关，要另查
- **开发者工具本地版本** → 那就是 #953，已由 #972 修复
...
- 原型 `refund-info` 在完成页落成「费用与订单边界」：一体机反馈门禁禁止完成页出现退款字样，本机仍不提供自助退款，费用处理以工作人员核查为准。
- 稿里 cancelled / cancel-race / session-lost 停留屏：运行时仍按已验证闭环跳转（取消成功回 `/scan?stage=start`，完成带文件进结果阶段，没有扫描会话回起点），避免拆掉 w2/w6 契约。
## 2026-09-07 会员记录青序迁移残留（39 号稿）

- 求职进度手填入口按稿标注「待建设」：后端 `POST /me/job-applications` 已在，一体机本批只读列表，不新增表单页或路由。
- 「岗位匹配」从简历行出去不再带 `taskId`（对齐 39 号稿：落到 `/resume/job-fit` 的未选目标岗位态）。
- `/renshi` 政策列表仍是旧壳；收藏/足迹里的政策出口仍走现有路由，不假装青序宿主已迁完。
...
- 手机通道已改用 `useUploadSession` + 青序 `PhoneQrSlot`，冻结组件 `UploadSessionQrPanel.tsx` 未改（哈希仍是 PR #598 的 `6e9fdb90…f3c0`）。其它页（简历来源 / 图片转 PDF / 签名盖章等）仍用冻结面板的 `catch { /* best-effort only */ }` 取消；后续可收敛到同一 hook，本轮不动冻结文件。
- 完整预览用真实 `FileContentPreview`，不渲染原型里那份「王雷」演示简历。
- `FileSourceView.tsx` 约 751 行，低于 800 停写线，超过 500 行拆分建议；后续可按通道把 screen body 再拆。

## 2026-09-07 青序首页运行时缺口

- [ ] 原型 `02-services.html` 对应的 `/services` 全部服务目录尚无运行时路由。首页迁移保留“更多服务”和“查看全部服务”两个控件并标注“目录迁移中”、保持禁用；目录页真实接入后再改为可导航入口，当前不得临时跳到单一业务域冒充全部服务。

## 2026-09-07 收口记录：P2 D 桶已清零（11 个 PR 全部合入）

产品负责人拍板「剩下的 5 件全部做完」后：R1 小程序标签 / 段选 / 步骤条 ≥48px（#852，全站复量可点元素 <48px 从 60 → 0）；R2 ADM-C15 服务端分页（#862）；R3 D 桶约 35 条按目录合入 #854 / #856 / #858 / #859 / #863 / #864 / #867 / #869；R4 后端已由受控发布到最新 main（体验版、体验成员、`downloadFile` 域名、E2 真机第 3 / 5 步仍需产品负责人在公众平台 / 真机操作）；R5 商用收口最后一段中代码侧可做的（PG 生产实例、最新 main 上线、三端入口 200、法务文档已发布、Windows 现场复验）已具备，**线上岗位 / 政策 total=0（内容录入需授权来源）、支付商户、密钥轮换、法务正文确认、TRTC / 扫描现场、两周试运营**仍需产品负责人 / 外部方，NO-GO 判定不变。残留见 `current-progress.md` 2026-09-07 收口条目。

## 2026-09-06 收口记录：上线前体检 P2 三桶已清零

体检清单（`docs/reviews/launch-audit-2026-09-05.md`）A 不伪造 / B 合规 / C 资损三桶的 27 条真实缺口按四条线全部合入 `main`：⑨a API（#836）、⑨b Admin（#834）、⑩ 一体机 + 合作机构（#831）、⑪ 小程序（#830）；尾项 JOB-15 / API-36 由 #835 落地；架构级 P0「可信终端身份总闸门」由 #833 落地并经 #837 修正 main 转红。**仍未做**：D 桶 87 条「其它」未清（不在本轮清零范围）；ADM-C15 三个来源页只做了「仅显示前 N 条」诚实降级，服务端分页未做；#833 的 Windows 真机换票 / 吊销 / 看门狗自愈复验待部署后执行（见下方「先做这一单」）；小程序登录态按钮分支测试待开发者工具空闲后跑（方案已定：本机 API + `SMS_PROVIDER=log` + 注入真 token）。

## 2026-09-06 并行主线：AI 结果层「诚实交付门」（与 51 页迁移并行，产品负责人已拍板）

产品负责人 2026-09-06 提出「每个功能跑完用户到底拿到什么」，五家联合评审（[docs/reviews/2026-09-06-result-layer-review.md](../reviews/2026-09-06-result-layer-review.md)）
给出 14 条 P0 与「半实现全部做完整」的要求，负责人同日拍板 8 项（评审 §6）：改口径为「换模板出新稿 / 只转格式不改字」两模式并废止 PNG；
**Word → PDF 转换进本次上线承诺（服务器装引擎，一体机不装）**；优化导出收费由 Admin 计费管理的价目开关决定；小青保留 TRTC 语音通话并新增长按语音转文字接大模型；
旧版 .doc 三端接收后服务端转换；高敏结果可保存不打印；所有半实现项做完整；五家协同实现、Claude 收货。

- 任务包与跨包契约：[docs/reviews/result-layer-2026-09-06-packets.md](../reviews/result-layer-2026-09-06-packets.md)。第一波 5 包（A 诊断报告导出 / B 收费开关与模板双源 / C 小程序第一批 / D 文档转换引擎 / E 一体机报告页 22 迁移）并行；第二波 7 包；第三波 P1 补全。
- 与 51 页迁移的关系：包 E / F 就是 22 / 23 / 24 三页的青序流光迁移，登记进 `QX_MIGRATED_ROUTES`，按下方「单页验收标准」六条验收；不是第二套口径。
- 口径已落笔：`feature-scope.md §2.2`（高敏可保存不打印、简历上传 Word 转换）、`§2.2.1`（两模式、PNG 废止、收费开关）、`§2.3`（Word 打印）、`§2.6`（小青语音、本次要点）。
- **2026-09-07 状态**：第一波 5 包与第二波 9 包（C2 / E2 / F / G / H / I / J / K1 / K2）已全部合入 main，squash 提交号见 [current-progress.md](current-progress.md) 2026-09-07「结果层 14 条 P0 收口」条目；每包的允许改 / 禁改 / 验收与执行记录以 packets 文档为准。第三波（≥3 套模板与 DOCX 版式、具名版本与跨端续做、PDF→图片、gotenberg 队列化、Puppeteer 评估）未开工，属 P1，不进本次上线承诺。
...
### 稿待订正（运行时为准，将来反画稿时按这里改）

迁移过程中发现的「稿与运行时不一致，且判定以运行时为准」的条目。
**不改稿**（稿是只读的视觉真值），只在这里登记，将来有人反画稿时照此订正。

| 稿 | 位置 | 稿里写的 | 运行时真值 | 判据 |
|---|---|---|---|---|
| `23-resume-optimize.html` | 第 962 行 | 「打开我的简历」的 `data-route` = `/me/ai-records` | `/me/resumes` | 稿**内部就不自洽**：同一行的 href 指向 `39-member-records.html?view=resumes`（简历视图），与 `data-route` 矛盾；且运行时保存版本的路由确为 `/me/resumes`（包 L6 订正 24 页时已按运行时改过）。 |

另记一条**不算稿缺陷、但照抄会坏页**的：`23-resume-optimize.html:1061` 给出口行用
`grid-template-rows:repeat(3,minmax(0,1fr))`。该写法的前提是 grid 处在**有界容器**内；
运行时优化页空态没有同样的高度上界，照抄后三条出口各涨到约 350px，把下面
「不用 AI，也能自己改」的 8 步清单挤出视口（1080×1920 实测只剩第 1、2 步可见）。
故运行时保持出口行自然高度 —— 这与稿 :1059「本页不会为了把版面填满而编几条建议出来」
是同一个态度。理由已写进 `optimize-empty-state-qx.css` 注释。

**禁止事项：** 不删原型上的接口名与字段名标注（那是给开发看的功能说明，不是冗余文案）；不为降低工作量删减入口；缺失能力写进本文件并保留入口标注真实状态。

**到机码 hid 指引（2026-09-07，分支 `claude/qx-b1-arrival-hid`）**：`/print/pickup-claim` 已补稿 11 `rHid()` 扫码指引态（未扫码可见机身扫码区位置 + 亮度调高；250ms 静默窗口与 hid-guard 判据未改）。不要当缺失页重做。

**51 页迁移缺口（打印扫描 Hub / 10-print-hub，2026-09-07）**

- U 盘导入：Hub 已放真实入口 `/print/upload?source=document&tab=usb&mode=transfer`，卡面标注「本地网桥已实现 · Windows 真机未验收」，未假装现场已验收。
- 证件照：只有说明页 + 「照片打印 / 材料扫描」两条真实替代路径；无排版、换底、成品生成。
- 复印：明示请在奔图机器面板操作，本机网页没有复印流程、也不代收费。

**签名盖章页（`/print-scan/sign`，2026-09-07）未接线能力（控件保留、禁用 + 原因）：**

- 从 U 盘选 PDF / 从 U 盘选图片：U 盘导入能用来打印，还没通到签名盖章。
- 在屏幕上手写签名：触屏手写要先做压感校准，未开放。
- 服务端仍允许匿名 inspect/compose（RG-1）；本页前端 fail-closed 要求登录，服务端收口另开。
**图片转 PDF（`/print-scan/convert`，原型 19）已迁青序流光后仍未接线的控件（保留入口 + 禁用说明，不删）：**

- **旋转 90°**：转换接口 `ConvertImagesDto` 没有旋转字段，服务端按原图像素写入 PDF。按钮在选中操作条上保持禁用，并写明「不会制造已经旋转但导出没变化的假操作」。
- **U 盘导入图片**：本页只接本机上传与手机扫码。U 盘入口进入能力边界说明，不把文件带进待合并列表；要拼 PDF 需先拷到手机或本机再在本页上传。通用 U 盘打印仍走 `/print/upload?tab=usb`。
...
#### 迁移账（2026-09-07 重算，对 `origin/main@f0aa458e5`）

2026-09-03/04 已按主题拆出 8 个 PR 合入 main：#777 「我的」游客免登录出口（`456cd668d`）、
#778 岗位来源四要素停发外跳（`1e568fb3a`）、#779 Partner 驳回原因 + 侧栏能力投影（`ee65ed9e3`）、
#780 线下机构资质核验 + 绑定来源机构（`8be9c6c20` + `61fd5b093`）、#781 政策审核申领条件三态（`fcb6d0b04`）、
#782 无设计稿体检四缺陷（`d4cb2efc1`）；另有同伴会话的 #783 / #786（W4 范围守卫修复与退役）。

**冻结分支仍有 42 个提交未按祖先关系进 main，其中 35 个仍有文件待搬。**

> ⚠️ **「待搬」的口径，先看清楚再用。** 下表按「文件内容是否与冻结分支逐字节相同」统计。
> 这个口径**会随 main 前进而衰减**：main 自己再改一次某文件，identity 就破了，
> 即使冻结分支那部分活确实已经在 main 里。所以**已合入 PR 对应的提交在表里仍有残留数**，
> 那不等于"没做"。2026-09-04 曾按同口径算出「已吸收 209 / 待搬 55」，
> 三天后同一算法得「158 / 106」——不是倒退，是口径衰减。
> **判断某项是否真的没做，看它对应的提交有没有被上面那批 PR 带走，不要只看这个数。**

| 冻结提交 | 主题 | 可直接取最终态 | **须三方合并** | 冻结新增文件 |
|---|---|---|---|---|
| `6d74c2f17` | fix(graph): 补齐 lazy 路由与多控制器解析 | 3 | 6 | 0 |
| `23464350f` | feat(kiosk): 取件码页接入虚拟数字键盘 | 1 | 3 | 0 |
| `2f92deb44` | docs(delivery): 建立 kiosk-redesign-r1 交付治理包 | 0 | 1 | 0 |
| `2d9f73c1b` | docs(design): kiosk-redesign-2026-08 入库（51 页上线口径原型） | 0 | 13 | 0 |
| `1135e251d` | feat(kiosk): 建立青序流光设计地基 + 可复用路由截图链路 | 2 | 0 | 1 |
| `f9d1890ea` | fix(contract): 外露 duplex；更正字段审计里两条误判 | 0 | 5 | 0 |
| `e8a468fca` | feat(kiosk): 取件码页迁移到青序流光（51 页替换的第一块样板）+ duplex 收尾 | 0 | 11 | 0 |
| `8492b02f7` | feat(kiosk): 地基加余量吸收，消除定高屏底部空白 | 0 | 3 | 0 |
| `d73abd98c` | fix(print): 按奔图开放打印 API V1.0 官方文档对齐取值，修三处会被拒的 bug | 0 | 1 | 0 |
| `a72bcdbde` | feat(ai): 简历报告加内容结构与问题证据；补手抄契约漂移门禁 | 0 | 1 | 0 |
| `6a850d74e` | fix(kiosk): 打印订单显示优惠额与退款额；清掉三处编造默认值 | 0 | 1 | 0 |
| `afdbc1df9` | docs(progress): 记录分支拆分要求、6 项待裁决与真机验证清单 | 0 | 1 | 0 |
| `b65645b05` | docs: 清理 65 份过期文档；统一彩色口径，把"事实"与"未知"分开 | 0 | 3 | 0 |
| `35cd4f843` | docs(device): 记录彩色与自动双面真机验证通过（产品负责人 2026-09-02） | 0 | 2 | 0 |
| `456cd668d` | fix(kiosk): 「我的」9 页给游客免登录出口，单块焦点态改居中 | 1 | 3 | 0 |
| `ae972b364` | fix(orgs): 新建机构写入显式 pending，不再留 null | 0 | 1 | 0 |
| `14e1b342c` | fix(kiosk): 操作条一体机视口补吸底；修伪造「已保存」；地基裸色令牌化 | 1 | 3 | 0 |
| `ee65ed9e3` | fix(partner): 驳回原因回传给机构；侧栏按能力投影；规则收敛成一份 | 0 | 12 | 0 |
| `230e3d28c` | feat(admin): 终端用户停用/恢复（完成 feature-scope 里后置的 P1 封禁开关） | 0 | 5 | 0 |
| `c79100812` | feat(ai): 配置变更补审计；日志端点开服务端筛选 | 0 | 2 | 0 |
| `36d646e48` | fix(gates): 壳层遮蔽断言改为判据，修复 isQxRoute 触发的两条误红 | 0 | 2 | 0 |
| `d4cb2efc1` | fix(kiosk): 修复无设计稿页面体检查出的四个真实缺陷 | 0 | 5 | 0 |
| `1e568fb3a` | feat(jobs): 岗位详情来源四要素缺失时停发外跳与扫码 | 0 | 3 | 0 |
| `2977a85a1` | docs(progress): 记录 2026-09-02 页面体检修复与壳层遮蔽门禁判据化 | 0 | 1 | 0 |
| `8213eebb9` | fix(fairs): 活动资料份数改为真实计数，不再恒显「0 份」 | 0 | 4 | 0 |
| `b24ca161e` | fix(gates): 撤销 verify-fusion-w4 里 6 条无效豁免 | 0 | 1 | 0 |
| `3af16d78c` | feat(partner): 数据源凭证可轮换、可归档，修掉教人做不存在操作的文案 | 0 | 10 | 0 |
| `389b0863e` | feat(admin): 订单页接上线下收款入账，并修订过期的门禁禁令 | 0 | 3 | 0 |
| `fcb6d0b04` | feat(admin): 政策审核页可查看申领条件，三种状态严格可分辨 | 0 | 2 | 0 |
| `8be9c6c20` | feat(admin): 线下机构页接入资质与网点核验，取证走强制留痕路径 | 0 | 2 | 0 |
| `7c5e0d23c` | fix(payment): 堵住线下入账的三条资损/泄漏路径（对抗性审查实证） | 0 | 2 | 0 |
| `a1d44e1ea` | fix(sources,payment): 归档不再被 Admin 打穿；轮换加 CAS；入账文案随来源变化 | 0 | 5 | 0 |
| `8ab523f30` | fix(admin,docs): 门禁断言收紧为整行匹配；0 元不再谎称未计费；权限矩阵回填 | 0 | 3 | 0 |
| `856be87d9` | fix(admin): 停用机构时明示「内容仍可被发布」，不改发布闸门 | 0 | 1 | 0 |
| `61fd5b093` | feat(admin): 线下机构可绑定来源机构，资质核验链路对新建行不再空转 | 0 | 2 | 0 |

**合计 132 项：可直接取 8 / 须三方合并 123 / 冻结新增 1。**（同一文件被多个提交碰过会重复计数，
这正是取 hunk 时需要的定位信息。）

**须三方合并的占比从 2026-09-04 的 37/55 涨到 123/132。** 原因是 main 在这几天继续演进，
越来越多文件两侧都被改过。**这条债务的性质会随时间变化：放得越久，可整取的越少、须逐 hunk 合并的越多。**

复现本表（只读，不改工作区）：

```bash
FZ=claude/project-readiness-review-959ffe; MB=$(git merge-base origin/main $FZ)
for c in $(git rev-list --reverse origin/main..$FZ); do
  for f in $(git show --format= --name-only "$c"); do
    git cat-file -e "${FZ}:$f" 2>/dev/null || continue
    git cat-file -e "origin/main:$f" 2>/dev/null || { git cat-file -e "${MB}:$f" 2>/dev/null || echo "NEW  $c $f"; continue; }
    [ "$(git rev-parse origin/main:$f)" = "$(git rev-parse ${FZ}:$f)" ] && continue
    [ "$(git rev-list --count ${MB}..origin/main -- "$f")" -gt 0 ] && echo "MERGE $c $f" || echo "TAKE  $c $f"
  done
done
```

**搬迁纪律（2026-09-03/04 八刀实践得出，每条都有实证）：**

1. **判断 main 有没有动过某文件必须用 merge-base**，不能用两点 diff。两点 diff 会把
   main 自己的活报成"待搬"——同一天里这个坑踩过五次，其中一次差点把同伴的活当成删除。
2. **须三方合并的文件禁止 `git checkout <冻结分支> -- <file>`**。已有实证：整取会退掉
   main 上的 `isTokenDefinitionFile` 令牌豁免（`verify-kiosk-visual-unity.mjs`）。
   正确做法是先列「main 侧必须存活清单」（`git log $MB..origin/main -- <file>` 的每个提交及其意图），
   合完逐条 grep 自证两边都在。
3. **动手前先查这个文件是不是早就在 main 上了**。`verify-kiosk-visual-unity.mjs` 那次，
   冻结侧要搬的内容已由另一条 PR 先进了 main，整取只会造成回退。
   判据：`git cat-file -e origin/main:<path>` 存在且 main 行数更多 → 默认跳过，
   确认冻结侧有独有行为才取 hunk。
4. **§9 类缺陷（适配层伪造 + 页面渲染伪造值）天生跨层，不可拆分**。已有实证：
   把页面文件退回 main、只留类型与适配层，kiosk typecheck 报 3 个 TS2538
   （字段改可选后不能再当索引）。类型系统强制同批，硬拆会先死在 typecheck 上。
5. **跑 verify 看退出码，不要 grep 输出文字**；**调用方式与 CI 一致**
   （`pnpm --filter <pkg> run verify:xxx`），从仓库根直接 `node apps/xxx/scripts/xxx.mjs`
   会因相对路径读错 `package.json` 而假红。这两条同一天各栽过一次。
6. **每次 rebase 后重跑全部验证**，不要沿用 rebase 前的结果。
7. 六个 slice 分支代码零重叠，但都改 `docs/progress/current-progress.md` 同一位置，
   合一个后其余必冲突——两条记录都保留即可。
...
- [x] **Windows Agent 升级 + Phase F F1/F2/F3（2026-09-06，现场分支 `field/windows-phase-f-2026-09`）**：[执行单](../device/windows-agent-upgrade-and-phase-f-recheck-2026-09.md) 已完成安装包 `main@5bff1bc42`（CI run `34026906605`）、重新绑定、启动票、看门狗、生产 Kiosk 1 页黑白单面真实出纸和队列回空复验。F4/F5/F6/F7、B3 和扫描/U 盘/断网等仍待后续现场窗口，不得据此宣称整体验收完成。

- **双面长边/短边的实际翻页方向** —— 名字只是名字，搞反了用户拿到的装订方向就是错的
- 本地驱动能否真正控制彩色打印（CLAUDE.md §3 标着待验证）
- 卡纸 / 缺纸 / 缺粉的恢复路径
- 断网重连后打印队列是否保留
- [~] Agent claim 429：根因是安装脚本默认 1 秒 claim（60 次/分钟）撞上 claim 端点每台 30 次/分钟限流；修复分支 `fix/claim-rate-limit-429`（安装脚本默认 5s + Agent 识别 429 退避 + API 心跳下发 `claimIntervalMs`）。合入部署后 Windows 侧只需看 Agent 日志不再出现 `HTTP 429 [RATE_LIMITED]`，以及后台心跳后 30 秒内日志出现 `claim interval updated by server — 5000ms`。多终端高峰行为仍待多机现场。
- 线上前端就是 main（`891492396`，deploy run `34024303135`，kiosk bundle `assets/index-CC6VWakW.js`），**不存在待部署的「106 路由新版」**——那是 51 页青序流光迁移，当前 1/51。Windows 侧按 `891492396` 复验打印/扫描入口即可；迁移页合入并部署后再给新 SHA。
- 连续不同订单打印：候选 Agent 已有 `task-runner-control` 单飞控制，但尚未完成 5 单顺序、10 秒突发、10–20 单耐久和异常恢复真机证据；完成前不得承诺不会卡纸、卡顿、漏打或串单
- 打印能力按钮按证据分层：彩色/扫描/复印的硬件能力与系统/API 已接通能力分开展示；彩色 wire value 未获厂家确认前不得发送 `mode="color"`，扫描/复印不得造假任务
...
- 禁止：改动既有 `POST /me/print-orders` 单文件语义；改 `claimableWhere` / 每轮 `maxTasks`（现为 1）/
  「`claimed` 只由 Agent 写」；把 `Order.pickupStatus=claimed` 与 `PrintTask.status=claimed` 合成一个状态机；
  任何投递 / 企业侧字段；本刀不改 `apps/kiosk/**` 工作台（那是 A3）。

### 是否新增数据模型 —— 是；形态与 A3 拆开（第 16 轮三家一致，grok 设计）

**v1 形态：** 新 `OrderItem`（`orderId` + `seq` + `fileId` + 行参数 + `billablePages` + `amountCents` + `status` + `printTaskId?`，
`@@unique([orderId, seq])`）。**`Order.printTaskId` 仍是 1:1，语义改为「当前那一条 PrintTask」。**
到机 `release` 只为 `seq=0` 建 PrintTask（现有 `pickup-order.service.ts:134-157` 的路径）；`PrintTask.status=completed`
回调后由服务端为下一行建 `pending` 任务，并把 `Order.printTaskId` / `Order.taskStatus` 拨回 `pending`。
**Agent claim 合同一字不改**：仍每轮最多领 1 条（`terminals-agent.service.ts:394`）、仍靠 `order.is` + `printTaskId` CAS
（`:410-434`）、`claimed` 仍只由 Agent 写。`PrintTask` 只加**可空非唯一** `orderId` 做血缘，**不把 Prisma 关系改成 1:N**，
否则 `claimableWhere.order.is` 会吃到已完成行。单文件路径继续走 `sourceFileId`；package 路径 `sourceFileId` 可空、以 OrderItem 为准。
**不复用 `itemsJson`**（那是计价快照）。

**这个形态的代价（写明，免得下一个人「顺手」改成 1:N）：**
1. 不能按行独立 claim / 并行出纸 —— 一台机一次仍一条任务。
2. 不能在 Agent 层按行重试 —— 行失败后由服务端再插一条 PrintTask；「只重打第 2 份」是应用层。PRD 用户故事 5 降级，v1 不承诺。
3. 部分出纸的退款是产品规则不是 A3 工作台 —— v1：已出纸行不自动退，剩余行 Admin 退或整单人工。不在本刀做用户侧退款入口。
4. 历史任务暂时脱离 `Order.printTaskId` —— 靠 `PrintTask.orderId` 血缘补。
5. 将来真要并行 / 按行租约时，第二次迁移把 claim 挂到 OrderItem。现在做是预支 A3。

**与 A3 的关系：拆开。** A3（quote / reservation / 七阶段 / 扫描工作台 / 用户退款 / 故障码）不进本刀。
A3 评审本立项只审三条产品规则：部分失败怎么算、退款怎么算、`pickupStatus` 与 `PrintTask.status` 不混用。
（此前写「须与 A3 同批评审、不得单独拍」与本节「禁止改 claim 语义」自相矛盾，第 16 轮指出后改正。）
...
v1 是**用户自选文件 + 行参数**，不做模板。PRD 的「招聘会标配包」与 `hall_paper` 的「大厅材料包」都是将来的模板目录（`PrintMaterialPack` 空壳表正是为此），依赖扫描真机与点位，**不在 v1**。
**排期：在 #756 合入 + E2 真机走查之后另开分支，不进 #756。**

## 生活圈判定（2026-09-05 评审 + 代码核；**产品负责人同日拍板「同意换形状」，已改写 `miniapp-life-circle-plan-v0.1.md` 至 v1.0**）
...
| 方案 | 对象（已有？） | 出口（已有？） | 判定 |
|---|---|---|---|
| **面试出门清单**（产品负责人 09-05 补充：仪容仪表 / 服装 / 面试过程） | 模拟面试报告 `checklist` ✓ 后端已生成、PDF 第十节已出纸；一体机 `/interview/tips` 六条含形象着装 ✓ | 报告 PDF 打印链 ✓ | **能做，零新页零新 API** —— 三处已有，只缺合成成「出门前一页」。边界：着装只一句不导流；`hall_paper` 面试要凭证型提示词变体。 |
...
| 福利中心 | `benefit-activities` ✓ | 券核销到打印页 / AI 次数 ✓（`RedemptionRecord`） | **留，但收窄**：活动类型是**四种**（`coupon` / `free_quota` / `package_entitlement` / `subsidy_eligibility_hint`），全部保留 —— 第四种是 info-only 政策资格提示，是窗口材料单的亲戚。**收窄的是文案**：禁异业（外卖 / 打车 / 电影 / 红包）、禁行为换券。 |
...
**订阅消息**是出门清单类提醒的出口，全仓 0 处，要微信后台模板 + 服务端发送，周期以周计 —— **独立立项，不绑进材料包第一刀**。
已改写为 v1.0（文件名保留）。三个先做模块的动工声明按 §8.1 另写，排期在 #756 合入 + E2 之后。

## 小程序 PR #756 合入退出标准与真机走查（2026-09-03 终版，本节已冻结）

> **本节是一次性重写的终版，不再按评审意见打补丁。** 前三版是在原文上追加更正段，
> 结果新段和旧段并存、互相矛盾（例如身份证号码同时出现在「未决项」和「不该勾」两处）。
> 如需改动，整节重写并把版本说明写进本注记，不要 append。
>
> 分支 `claude/miniapp-integrate-main`，本节只管这一个 PR。E1/E2 是它的放行前置。
>
> **2026-09-04 修订（产品负责人授权评审组直接定第 3/4/5 项）。** 只改「拍板项」
> 与下方「已知限制」子节，E1–E5 退出标准和 E2 三档动线一字未改。前一笔
> `613d6465b` 把 store-select 导航藏进了本 PR，并声称「清单 8 条见 PR 描述」——
> 当时 PR 描述里没有这 8 条，且 `store-select` 被 `guardPackageChain()` 整页拦截、
> E2 走不到，不构成走查必现缺陷。本修订：甲维持 disabled；乙裁定仍是藏，但
> **撤出本 PR、入合入后 backlog**；丙把清单写进本节，PR 描述只留锚点。

### 放行标准（全部满足才 `gh pr ready`，缺一不合）

- [x] **E1** 微信公众平台「用户隐私保护指引」按下方清单勾完，且第 5 节存储期限填成与代码承诺一致。
      **2026-09-05 产品负责人报告已配置**（经 Codex 按执行单操作；截图未附，E2 第 4 步真录音能进即为实证）。
- [ ] **E2** 真机走查一遍（约 20 分钟），按下方三档动线，留一份笔记。
- [ ] **E3** 走查中未出现阻断级问题；出现则修完重跑 E2。
- [x] **E4** **合入当天**再合一次 `origin/main`，且合并树 CI 三 job 全绿。**2026-09-06 达成**：#756 合入当天 rebase 到 `main`（六处冲突人工解），合并树 build-and-verify / postgres-readiness / kiosk-browser-smoke 三 job 全绿后合入（`main@79c9c13cd`）。
      这条按定义只能在合入当天达成 —— 某个提交今天绿不等于合入那天的树绿；
      `main` 每天多笔合入，绿灯是易腐的。
- [x] **E5** 合入同时更新 [feature-scope.md](../product/feature-scope.md) §1.2（**2026-09-06 随 #756 同 PR 更新为 63 页 / 113 PASS**）：
      该行现写「53 个注册页、98 PASS」，那是 `origin/main` 的事实（已核，当前正确）；
      本分支是 62 页 / 112 PASS，**合入那一刻 §1.2 就变成陈述旧数字**。

### E2 首轮真机结果（2026-09-05 23:2x，产品负责人本人，预览码 = integrate 树 62 页 × **生产 API 771d53e2**）

> **先记一个比缺陷更要紧的事实**：预览码里的小程序是 #756 的，但所有请求打的是生产 `https://zyidai.cn`，
> 后端是 208+ 笔前的旧代码。**E2 验的是「新小程序 × 旧后端」。** 任何 API 侧修复，E2 重跑都看不到，直到部署。
> 好在 E2 甲档五步的判定对象都在小程序侧或后台配置侧，这个限制不改变 E2 的有效性；但 API 侧缺陷只能记入 backlog。

| 步 | 结果 | 根因 / 处置 |
|---|---|---|
| 1 登录 | 过（能走到第 2、4 步即证） | — |
| 2 上传 → 我的文档 → 预览 | **没过：预览看不到文件** | 预览链接是同源相对路径 `/api/v1/files/:id/content`（`file-url.js` 头注释），端点 `res.send(buffer)` 流式出流不跳转（`files.controller.ts:314`），生产同样有此端点与相对路径签名。**剩下唯一解释：`zyidai.cn` 没进 downloadFile 合法域名**（与 request / uploadFile 是三张分开的名单）。代码已备好该情形的提示文案。**待产品负责人确认手机上看到的错误文案**：若是「暂时无法打开原文：该功能需要在小程序后台配置下载域名」即坐实 → 后台加 `https://zyidai.cn` 到 downloadFile 合法域名后重测第 2 步。E1 同类，不是代码。 |
| 3 语音不勾同意 | （未单独报告） | — |
| 4 真录音 | **E1 生效实证**：麦克风授权通过、转写连续跑了 9 题 → 隐私指引已接住；同时证明**生产 `ASR_PROVIDER` 是开着的**（此前按默认 disabled 假设，与生产实况不符）。**第 10 题报「尝试过于频繁，请稍后再试」** | 服务端 `@PaidAiThrottle(6)` = 每客户端每分钟 6 次（`THROTTLE_WINDOW_MS=60_000`），12 道语音题 + 试音必撞。小程序侧处理**正确**：`transcribeFail` 把 429 归为 `degrade:false`、提示等一会再录，**没有**把人踢去整场手打。缺陷在 API 限值：**改 `@PaidAiThrottle(20)`**（人说话物理上到不了 20 次/分；IP 每小时 300 天花板仍在）。跨 lane，独立小 PR 打 main；**部署前 E2 重跑仍会撞 6 次**，不算 E3 阻断。 |
| 5 材料包 | （未单独报告） | — |

**E3 现状：未通过。** 第 2 步待确认根因（大概率后台配置）；第 4 步小程序侧通过、API 侧 backlog。

### E1 · 用户隐私保护指引

位置：微信公众平台 → 设置 → 服务内容声明 → 用户隐私保护指引 →「使用用户信息类型」。
下列结论按 `apps/miniapp` 里**实际调用的接口**逐个枚举得出，不是按功能推测。

**该勾（10 项）**

| 项 | 依据 |
|---|---|
| 麦克风 | `wx.getRecorderManager`（`utils/voice-recorder.js`）；`app.json` 已配 `permission['scope.record'].desc` |
| 摄像头 | `wx.scanCode({onlyFromCamera:true})`（`pages/kiosk-login/kiosk-login.js:46`，扫码登录一体机）；另 `pages/resume-upload/resume-upload.js:91` 是 `sourceType:['camera']` **纯拍摄无相册兜底**，`contract-review` / `documents` 的 `chooseMedia` 也含 camera |
| 选中的照片或视频 | `wx.chooseMedia` 3 处：`contract-review:243` / `resume-upload:88` / `documents:323` |
| 选中的文件 | `wx.chooseMessageFile` 4 处：`resume-upload` / `documents` / `contract-review` / `package-create` |
| 手机号 | `launch.wxml` 的 `open-type="getPhoneNumber"`；短信登录是表单采集，同归此项 |
| 设备信息 | `wx.getSystemInfoSync()` 3 处兜底：`app.js` / `assistant.js` / `launch.js`（全仓无 `wx.getDeviceInfo`） |
| 剪切板 | `wx.setClipboardData` 10 处（写）、`getClipboardData` 0 处（读） |
| 邮箱 | `utils/resume-build-model.js` 的简历表单字段，落库 |
| 订单信息 | 打印订单、取件码 |
| 操作日志 | `utils/history.js` 登录态上报 `POST /activity/browse` 与 `/activity/external-jump`，30 天 TTL |

**摄像头与剪切板的判据说明。** 三家评审对这两项的**微信映射口径有分歧**（一家认为 `wx.scanCode`
不走 `scope.camera`、`setClipboardData` 只是写入不算采集，故两项都不勾；另一家举出 2023 / 2025
两次真机实测报告称只声明「选中的照片或视频」时拍照会 `errno 112`、`setClipboardData` 未声明
同样会 `errno 112`）。代码事实三家一致，**判据取代价不对称**：漏勾 → 真机功能直接断
（扫码登录、纯拍摄上传、10 个复制按钮）；多勾 → 审核多问一句，而两项都有真实用途答得上来。
故两项都勾。注：这两条本想在 E2 里实测掉，但空库下复制按钮全部不可达（见 E2 丙档），实测不成。

**`身份证号码` —— 已拍板：勾（产品负责人 2026-09-03 决定，2026-09-04 记录进本文档）。** 小程序没有「身份证号」表单字段，
但后端 `services/api/src/materials/pii-scan.util.ts:239` 会把扫描结果标成 `type:'id_card'` 并掩码，
`id_scan` 是正式 `FilePurpose`，`documents` 页也对用户写着「身份证等敏感文件已设置有效期」。
一家认为无字段即非采集，一家认为既然识别并标记了证件号就该声明。该项在后台**确实是可勾的枚举项**
（已由后台截图确认）。用途按拍板结论填：「检测你上传材料中可能含有的证件号，
用于隐私遮挡与短期保存；不用于实名认证，不提供给企业」。至此 E1 的勾选项为 **11 项**，无未决。

**不该勾，及依据**

- **用户信息（微信昵称、头像）** —— 全仓 `getUserProfile` / `getUserInfo` / `chooseAvatar` /
  `type="nickname"` 命中均为 0；登录是手机号 + 短信 + 静默 `wx.login`。**若后台已勾，需取消。**
- **位置信息** —— `wx.getLocation` 只在 `store-select.js:78` 的注释里；`store-select.js:149`
  有活的 `wx.openLocation`，但 :145 有守卫（`lat`/`lng` 非 number 即 toast 返回）且后端
  `getPublicTerminals` 从不下发坐标，永远到不了调用；且 `openLocation` 是显示调用方给的坐标，不读用户位置。
- **相册（仅写入）/ 通讯录 / 日历** —— 依据是**代码里没有这个接口**：
  `saveImageToPhotosAlbum` / `saveVideoToPhotosAlbum` / `addPhoneContact` / `addPhoneCalendar`
  全仓命中均为 0。写入相册在微信体系里确实要授权，不勾是因为没这功能，不是「写入不用勾」。
- **选择的位置信息、地址、发票信息、微信运动、蓝牙、车牌号、发布内容、所关注账号、
  加速度 / 磁场 / 方向 / 陀螺仪四个传感器** —— 对应接口全仓命中均为 0。

**另外两件**

- **自定义加一条「求职简历信息」**：姓名、联系方式、求职意向、教育经历、工作经历、项目、
  技能、证书、自我介绍，用途写「用于生成、优化与打印求职材料」。清单里「邮箱」照勾，
  但只勾邮箱会显得平台只采了个邮箱，简历才是主要采集对象。
- **第 5 节「开发者对信息的存储」现填固定存储期限 0 天，必须改**，须与
...
但生产库**终端 0 台、招聘会 0 场、政策 0 条、岗位 0 个**。上一版动线有 4 步在这个前提下
跑不起来或勾了等于没测，故按可跑性重分档。**没有前置条件的步骤，勾了不算数。**

**E1 未勾完之前，语音只走「用手打 / 拒绝授权」，不要点录音。**

**甲档 —— 现在就能真验（自带数据，不依赖生产库）**

1. 登录（微信一键；失败走短信降级）。这步不过后面全废。
2. **先上传一个文件**，再去「我的文档」看列表出现它并**点开预览**。
   新账号名下是空的，直接看列表只会看到空态、什么都没测到。
   这是全场唯一能验 `downloadFile` 合法域名的动作 —— 开发者工具勾了「不校验合法域名」，测不出来。
3. 语音简历：不勾录音同意 → 应立刻进整场手打，且已确认内容不丢。不需要任何数据。
4. E1 勾完后再来一次：**先去微信「设置 → 小程序 → 本小程序 → 麦克风」把权限清掉**，
   否则手机记着上次授权会直接开录，授权分支一行都不会执行。
   **判定**：仓里 `requirePrivacyAuthorize` / `onNeedPrivacyAuthorization` / `getPrivacySetting` /
   `__usePrivacyCheck__` 命中均为 0，即**零隐私弹窗代码**，走的是微信默认弹窗 ——
   弹窗出现 → 同意 → 能录，才算通过；**没弹窗、直接约 5.6 秒后降级成手打 = 隐私弹窗没接住**，
   不是「授权被拒」，两者要分清。
5. 材料包：入口应明写「未开放」，点进去不应出到机码。

**乙档 —— 只能验「空态是不是诚实的」**

6. 发起打印：0 台终端 → 应是诚实空态。**不要为了走通而造数据。**
7. 岗位 / 招聘会 / 政策三个列表：全空。看空态措辞是否说得清「暂无」而不是「加载失败」。

> 这一档能验的只有一件事：空态和「加载失败」长得不一样，且不是白屏、不是假数据、不是错误被吞掉。
> 已按类核过：全部列表页的 `catch` 都设了错误字段（`loadError` / `error` / `showToast` 之一），
> 没有把错误吞成空数组的，所以本档预期是过的。真机上若看到白屏或含糊文案，那是新问题，记下来。

**丙档 —— 这次验不了，别勾也别当成通过**

- **打印订单详情 404 修复**（`7a1a4133c`）：只在**一体机产生的 `PrintTask` 行**上出现，
  终端数 0，新账号造不出这种行。已在开发者工具用注入数据验过（一条 Order 行有按钮、
  一条 PrintTask 行换成说明文字）。真机验证要等有一台真终端跑过一单之后。
- **剪切板 `errno 112`**：全仓 10 个复制按钮，9 个在需要后端数据的详情页
  （招聘会 / 政策 / 岗位 / 企业 / 到机码 / 逛展计划），第 10 个在隐私页但要先跑完整套数据导出。
  **空库下一个都不可达**，这条分歧本次实测不掉。
- **AI服务记录「加载更多」死锁修复**（`a0beccb3c`）：要触发 `onReachBottom`，
  账号名下得有十几二十条记录把列表撑出滚动条；只有两三条时手指划两下就到底，
  修复代码一行都不会执行。本次不具备前置条件。
- **简历优化「结果带不走」**：要先有简历并跑完一次优化才看得到那个断头路。想验就单独留时间。

**本次完全不要碰**：职业生活圈、今日早报、方言切换（服务端还没有这个能力）。

### 需要产品负责人拍板的项（2026-09-03 上交，第 1、2 项当日已决）

1. ~~**`身份证号码` 勾不勾**~~ —— **已拍板：勾**（2026-09-03）。用途措辞见 E1 段。

2. **第 5 节存储期限填什么** —— 我算过了，**「固定存储期限 N 天」这个选项本身填不对**。
   实际留存档位（`services/api/src/files/retention-policy.ts` + `file.types.ts`）：

   | 档 | 期限 | 谁 |
   |---|---|---|
   | `highly_sensitive` | 1 小时 | 证件照 / 签名 |
   | 合同审查 | 2 小时 | `CONTRACT_REVIEW_TTL_MS` |
   | `sensitive` | 6 小时 | 敏感件 |
   | `normal` | 24 小时 | 普通件 |
   | `months_3` | 90 天 | 用户自选延长 |
   | `months_6` | 180 天 | 用户自选延长，**需单独同意** |
   | `long_term` | **无期限**（`expiresAt` 返回 `null`） | 用户自选长期保存，**需单独同意**，仅部分派生文件可选 |

   小程序 `pages/documents/documents.js:236` 会调 `updateFileRetention`，可选项由服务端
   `/me/documents` 的 `allowedRetentionPolicies` 下发（证件照 / 签名 / 合同类锁死 `system_short`，
   原始文件不允许 `long_term`）。**因为 `long_term` 可达，声明一个固定天数就是不准确的。**
   **已拍板（2026-09-03）：选 (a)** —— 在指引里选「无法确定固定存储期限」类选项，
   写明「按文件敏感级与用户选择分档保存，实现目的后删除或匿名化」。
   这是对当前代码最准确的描述，**不改代码**；`long_term` 保留为用户可选项。
   （被否的 (b)：先摘掉 `long_term` 再按最长固定档 180 天声明 —— 那要改
   `services/api` 的 `allowedRetentionPolicies`，跨 lane，且会移除用户已有的一个选择权。）

3. **`ASR_PROVIDER`** —— **已拍板（2026-09-04 评审组）：#756 合入时保持 `disabled`，不翻 `tencent`。**
   理由：`POST /resume/voice/transcribe` 无 Guard、`endUserId` 恒记 `null`，腾讯按次计费；
   现有闸只是 `@PaidAiThrottle(6)` + 每 IP 每小时 `AI_IP_HOURLY_CEILING`（默认 300），
   那是防突发不是成本预算。合入不部署，生产仍是 `771d53e2` 且 runbook 已写 `disabled`，
   翻 env 根本不是合入动作。`disabled` 有 `voice-degrade-falsify.mjs` 11 PASS 钉死的整场手打。
   **按人按天额度必须先做完，否则不许翻。** 额度不是约 15 行：得先把身份绑上转写端点，
   是鉴权面，动 `services/api`。另外两件也是硬前置、缺一不翻：生产 `TENCENT_SECRET_ID/KEY`
   （或 `TENCENT_ASR_*`）在位且有一句话识别权限；一次具名部署授权。
   **E2 不是翻转前置**——它是 #756 合入门禁，别把两道闸混成一道。
   执行：额度 PR 由 Claude 在 `services/api` 于 #756 合入后单独立项；env 翻转由产品负责人
   在额度进 `main` 之后的那次具名部署里改，运维执行。现在不做，入 backlog。
   **#756 合入 ≠ 语音简历可用。**

4. **`store-select` 的导航入口** —— **已拍板（2026-09-04 评审组）：藏；不进 #756，合入后做。**
   理由：按钮 100% toast（`Terminal` 无坐标字段，`getPublicTerminals` 不下发，JS map
   不设 `lat`/`lng`），藏是对的产品决定，坐标补上后 `wx:if` 会自动回来。
   **算违反「冻结 diff、CI 红了才动」**，不算违反「不加功能」——以冻结 diff 为准。
   它也不是走查必现缺陷：`guardPackageChain()` 在 `onLoad` 首行恒拦截，E2 甲档材料包
   标「未开放」，生产终端 0 台。前一笔 `613d6465b` 以「不修必在走查复现」把 2 行藏进
   本 PR，事实不成立，已撤回 wxml。
   执行：#756 合入后 Claude 小程序 lane 第一刀，只改 `store-select.wxml` 加
   `wx:if="{{store.lat && store.lng}}"`（或 `item.lat && item.lng`），JS 守卫保留；
   **不许**在 map 里补空坐标透传。补坐标（两套 schema + 双库迁移 + Admin 编辑 + 端点透出）
   另立，跨 `services/api` 与 `apps/admin`，更后。现在不做。

5. **「已知限制」清单** —— **已拍板（2026-09-04 评审组）：写在本节下方「已知限制」子节，
   作为 E2 走查对照表。** **PR #756 描述里贴同一份正文**（产品负责人 2026-09-04 批准；PR 描述是 PO 直接可见、
   reviewer 必读、合入后进 git 历史不可腐的合入审计面）。两处内容必须一致，以 PR 描述为准；
   本节这份是 E2 走查时手边的对照表。`feature-scope.md` §1.2 **现在不改** —— #756 未合入，
   把未合入分支的限制写进 `origin/main` 真值是把未来当现在。
   维护：合入前本轮一次性写入，只改事实错误不改结论；合入当天 E5 执行人（合入 #756 的人，
   默认 Claude）把「合入后仍成立」的条目迁入 `feature-scope.md` §1.2 小程序行，本节不再改。

> 复制按钮的判定补一条（本次虽不可达，将来验时照此）：**以粘贴到微信任意聊天框确认有内容为准**，
> 不以应用自己的 toast 为准 —— `setClipboardData` 静默失败时，用户看到的是「没反应」。

### 已知限制（E2 走查对照，2026-09-04 评审组定稿）

走查时下列现象按「设计如此」记，不报阻断、不因此开 #756 的修复刀。空库进不去的页面，勾了也不算测过。

**E2 会撞上的**

| # | 现象 | 不要当成 |
|---|---|---|
| 1 | 语音简历：`ASR_NOT_CONFIGURED` → 整场手打 | 录音坏了 / 转写没接 |
| 2 | 岗位 / 招聘会 / 政策 / 打印门店全空 | 列表接口挂了（先看是不是空态文案；白屏或「加载失败」才记新问题） |
| 3 | 材料包入口「未开放」，四页 `guardPackageChain()` 回首页 | 残页、死链 |
| 4 | 体验版若打生产 API：后端仍是 `771d53e2`，本分支多出的 3 个 api 文件不在生产上 | 小程序 404 = 前端 bug |
| 5 | 岗位 / 招聘会 / 企业 / 参会企业主 CTA 是「复制来源链接」 | 应该「去来源平台投递 / 打开网页」（小程序打不开任意外部网页） |
| 6 | 招聘会提醒：不打开小程序不会响 | 订阅消息没配（全仓 `requestSubscribeMessage` = 0，是本地存储） |
| 7 | 打印上传彩色标「暂未开放」 | 彩色打印已上线 |
| 8 | 隐私页账号注销：服务端 `ACCOUNT_CLOSURE_NOT_AVAILABLE` | 注销按钮坏了 |

**E2 走不到、合入后仍成立（E5 迁入 `feature-scope.md` §1.2）**

| # | 限制 | 代码事实 |
|---|---|---|
| 9 | 招聘会展位统计恒无数字 | `FairZone` 不下发 `boothCount` / `checkedInCount`；页面写「暂无数据」，不兜成 0。平面图没上传整块不渲染 |
| 10 | 活动资料「累计被打印 N 次」不出现 | 出纸完成后没有递增 `printCount` 的写路径，恒 0；页面 `>0` 才显示 |
| 11 | 参会企业「复制来源链接」后，浏览记录点不回企业详情 | `recordJump('fair_company')` 已上报；`browse-history` 的 `ROUTES` 无 `fair_company`，本机记录 toast「无法打开该记录」；服务端行被折成招聘会详情。回顾态「你在本机留下的记录」读服务端 `localRecords`，空库验不了 |
| 12 | `store-select` 导航按钮 100% toast「暂未提供地图坐标」 | 见拍板项 4；材料包守卫拆除前走不到。合入后第一刀藏掉 |
| 13 | 职业生活圈 / 今日早报 / 方言切换 | 后端能力未上；E2 写明本次完全不要碰 |

### 等待期开工三条测试（2026-09-05 第 16 轮定，可复用）

「不往 #756 加」≠「仓库停工」。但把等待 E1 / E2 的焦虑变成工具箱忙碌（第 10 轮批评）也不行。
**任何「趁合入前做点东西」都过这三条，全过才开，一条不过就是忙碌：**

1. **不改变 #756 的 diff 和合入日成本。** 另一条分支；`merge-tree` 与已冻的 `apps/miniapp` 产品文件交集为空，或明确永不合进 #756。
2. **不依赖 E2 尚未给出的真机事实。** 开工所需的对象和出口已经在仓里。
3. **合入后仍成立，且不给本轮 E2 制造新的必现缺陷。** 不注册新入口、不让走查员走进半成品、E2 写明「不要碰」的面保持不要碰。

**套到当前：** A′ 面试出门清单**过**（对象与出口全在、零 API、不改 `app.json`、E2 本就不碰生活圈）—— 但收窄：只合成已有字段 + 静态着装一句 + 城市，
**不做凭证型提示词变体**（那是后端分支，生产岗位 0，过不了第 2 条）；一个人、小预算、基于 #756 的独立分支、合入后 rebase。
材料包 / 拆守卫 / A / B **不过**（开放四页让 E2 第 5 步变半成品；生产终端 / 招聘会 / 政策为 0）。再写战略文档**不过**。
**A′ 是等待期唯一过三条的产品活，不是许可证** —— 变成「生活圈四个一起开」就回到工具箱。
...
### 语音简历的完整链路现状（2026-09-05 产品负责人问「是不是可以这样」，对代码核）

负责人期望：语音把简历字段变成问题 → 回答填进模板 → 字段可手动改 → 可选不同模板样式 → 可选 AI 优化 / 诊断 → 或直接导出保存。

| 环节 | 现状 | 缺口 |
|---|---|---|
| 字段 → 问题 | ✓ 23 题（12 道语音），`resume-voice-questions.js` | — |
| 回答填进表单、可手改 | ✓ `_finish` → `redirectTo resume-build?from=voice`，表单全可编辑 | — |
| 选模板样式 | **一半**：版式参数可选（字号 / 行距 / 边距 / 单双栏 / 强调色，`resume-build.js:84,361-371`，后端 `layout` 已收）；**命名模板 `templateId` 后端 DTO 已收（生产也有）、一体机有模板库页，但小程序不发也无列表端点** | 小程序加模板选择 + 需要一个简历模板列表端点（`job-materials/templates` 明确拒 `resume_template`） |
| AI 优化 / 诊断 | 工具都在（`resume-diagnose` / `resume-optimize`），但**生成后落到「我的文档」，没有直达入口**，要回工具格子 | 生成成功弹窗加「去诊断 / 去优化」两个按钮，纯小程序 |
| 直接导出保存 | ✓ `exportGeneratedResume` → pdf / docx / txt / md → 我的文档 | — |

**结论：可以，而且大半已经是这样。** 缺的两小块都在小程序侧（模板选择 UI、生成后直达），模板列表端点跨 lane。
排期：#756 合入后，与 A′ 同一批小切片，不进 #756。

### 落后 main 的处理规则（2026-09-04 评审组定，可复用）

**非合入日容忍「受控 DIRTY」，不为刷绿而预防性合 `main`。** 理由：E4 要求合入当天再合一次，
任何非合入日达成的 CLEAN 到那天都要推倒重做；而「冲突面有没有扩大」这个信号用
`git merge-tree --write-tree origin/main <head>` 秒级就能拿到，不必烧 25 分钟 CI 去换。

成本阶梯：探针（秒）< 本地在合并树上跑四条门禁（分钟）< 全量 CI（25 分钟）。日常只走最便宜的档。

| 状态 | 处理 |
|---|---|
| CLEAN 且落后 | **不合**。健康态。 |
| DIRTY，冲突集 ⊆ 已知机械文件（`docs/progress/current-progress.md`），且与本 PR 产品文件交集 = 0 | **维持**，但必须是「已知 DIRTY」：把落后数与冲突集记进 PR 描述。 |
| DIRTY，冲突集**超出**已知集合（运行时 / 门禁 / schema / `apps/miniapp`） | **当天立即合**并重跑被交集碰到的验证。未知冲突在新鲜时处理最便宜。 |
| 要开始准备 `gh pr ready` / 看合并树 CI / 进 E4 | 先合成 CLEAN 再动。E1/E2 本身不依赖合并按钮，进行中可以留着 DIRTY。 |
| 合入当天 | 按 E4：只合一次、CI 三 job 全绿即合。**日常任何绿都不算 E4。** |

**当前记录（2026-09-04）**：3 落后（#777 kiosk「我的」9 页 / #776 / #775 纯文档），
`merge-tree` 冲突集 = `{docs/progress/current-progress.md}`，与 `apps/miniapp` 交集 = 0 → 按规则维持，不合。

**合入日前的准备**：`current-progress.md` 已连续 5 次以「双方条目都保留」解冲突，可能攒了语义重复。
建议在合入日**之前**（不是当天）单独整理本分支这半边，让 E4 当天的解决是机械的。

### 评审模型的护栏（2026-09-04 评审组定）

**评审的产出物是判断，不是改动。** 规则不是「评审一律只读」，而是**「评审不改评审对象」**——
动手分三层，只禁第三层：

1. **读码、跑门禁、跑测试、复现** —— 不改任何源文件。必须做，永远允许。
2. **为验证假设临时改文件** —— 允许，但只能在独立 worktree / 临时 clone 里做，不留改动。
3. **向目标分支 commit / push** —— **禁止**，无论内容对错。

为什么内容对也不豁免：评审直接落地会让①独立性坍缩（自己给自己背书，四家互审的设置就空了）；
②审计账本污染（未经派发方核验的提交移动了后续所有状态陈述的基准）；
③评审对象变移动靶（报告写的是 SHA X，改完成 X′，报告与代码不再一一对应）。
护栏的价值在可预测性，不在单次结果好坏。

执行：派发时任务书写明边界与交付物形态（发现列表 + 建议 patch，不是已落地的修改）；
评审 agent 指向独立 worktree 而非工作树；派发前记 HEAD sha / `git status --porcelain` /
`git ls-remote`，返回时先校验再读结论，不一致即该轮作废。

**已发生一次（2026-09-04，第 12 轮）**：评审 agent grok 在工作树直接提交 `19c6cb8a5`
（撤回误塞进本 PR 的 store-select 藏入口 + 改写本节三项裁定）。已核对：只改它声称的 3 个文件、
wxml 为逐字节纯撤回、无夹带；**内容经派发方核验后保留**，不为证明原则而 revert 一个正确提交。
上述护栏自第 13 轮起生效。

### 明确不进 #756 的（backlog，不是 blocker）

- **藏 `store-select` 导航** —— #756 合入后 Claude 小程序 lane 第一刀，约 2 行 wxml。材料包守卫拆除前必须已经藏上。
- **Terminal 坐标建模** —— 两套 Prisma + 双库迁移 + Admin 编辑 + `getPublicTerminals` 透出。跨 `services/api` 与 `apps/admin`。藏入口之后、开放材料包之前不阻塞。
- **方言切换、转写按人按天额度** —— 都动 `services/api`。额度不是「约 15 行」：
  `POST /resume/voice/transcribe` 没有任何 Guard，拿不到 `endUserId`，按人限额得先把身份绑上，
  那是鉴权面。现有闸是 6 次/分/客户端 + 每 IP 每小时 `AI_IP_HOURLY_CEILING`（默认 300）。
  硬时点是 `ASR_PROVIDER` 翻 enabled 之前。额度进 `main` 之前生产 env 不许翻 `tencent`。
- **`ASR_PROVIDER=tencent` 生产翻转** —— 额度 + 腾讯凭证权限 + 具名部署授权，三件齐了由产品负责人下令、运维改 env。不是合入动作。
- **简历优化结果导出** —— 要后端端点，同一个桶。
- **版式第 6 类全量扫描** —— 合入后只扫新 diff，不在合入前做。
- **fair_company 浏览记录回跳参会企业详情** —— `browse-history.js` 的 `ROUTES` / 筛选项补 `fair_company` → `fair-company-detail`。合入后、有招聘会数据再做。

### 已澄清的误判（免得后来人照旧结论办事）

- 招聘会提醒**不是**「等微信订阅消息模板」的断头路：全仓 `requestSubscribeMessage` 命中 0，
  `utils/reminders.js` 文件头写明「不依赖后端，不需要微信订阅消息模板」，是 `wx.Storage`
  本地提醒、App 启动时检查 24h 内临近项。真实局限是**用户不打开小程序就不会被提醒**，
  这是设计选择不是待办。
- #756 **已经包含 `services/api` 改动**：3 个文件 +55/−0 纯增量（self-assessment controller +25、
  policies controller / service 各 +15），来自更早的提交。「小程序 lane 不动后端」是操作纪律
  不是分支事实，不要拿它当挡箭牌，也不要反过来当成可以再开一刀的许可。
...
- [x] **扫码上传后端生命周期独立收口（2026-09-06 主体完成，分支 `fix/upload-lifecycle`）** —— 会话创建登记 Redis 到期索引，复用 `FilesCleanupTask` 每分钟回收自然过期且无人再读的未确认 `FileObject`（走 `systemDelete`，保留删除日志）；上传/确认/取消/清理共用会话锁，确认后即撤销索引；Redis 故障本轮跳过并 warn；手机端错误信封不含 token / 终端名 / 堆栈。**未做、待拍板**：匿名文件绑定会员后对象仍在 `tmp/` 前缀（DB 归属已切换、权限与留存按 DB 走），安全搬迁需可恢复账本（新字段或迁移表），建议择期。原文：当前公共 UI 已保证刷新先撤销旧码、手机成功态不可伪造重传，但 Redis 会话自然过期后未确认 `FileObject` 的删除仍不能只靠 UI 证明。需独立设计并验证：自然超时、上传/确认/取消竞态、匿名与会员归属、确认后进入打印履约的短留存；不得将旧会话 token、终端展示名或原始错误带入手机页。

## 平台可靠性遗留（FIX-CONSOLE-P0 拆出，2026-08-17）

两条都在修「Redis 故障时后台全 500 + 异常零日志」时查实，但不在该批范围内，需另立任务：

- [x] **`EndUserAuthGuard` 的 Redis 等待未加界（2026-09-06 #841）** —— 会话读取走 `tryRedis` 有界等待（500ms），失联 → 503 `MEMBER_SESSION_STORE_UNAVAILABLE`；拒绝判定未改。原文：C 端会员端点在 Redis 不可达时单请求实测 **23.6 秒**后返回 500。它的 fail-closed 判定本身是**正确**的（Redis 就是会员会话真源，读不到就该拒绝，绝不能改成放行），问题只在耗时与错误码不诚实：应收敛为有界等待 + 明确的 503/`MEMBER_SESSION_STORE_UNAVAILABLE`，而不是让每个请求把连接占满 23 秒再塌成通用 500。修的时候**不得改变「拒绝」这个判定**。
- [x] **`admin-orgs.service.ts` 的 `invalidateAccountSession` 在 Redis 故障时造成「看起来失败其实成功了」（2026-09-06 #841）** —— 响应带 `sessionInvalidation` 与 `staleWindowSeconds=60`，写 warn 审计，状态已是目标值时仍重做失效。原文：账号启停 / 改密 / 换邮箱的数据库更新已提交后，缓存失效调用抛错 → 端点返回 500，管理员以为没生效；重试时因为状态已经是目标值会跳过整个分支，反而**连缓存失效也不做了**。当前只在 `/health` 里如实声明为 `internal-console-redis-actions=unavailable`，代码未改。修的时候要一并想清楚「部分 Redis 故障（写失败读成功）」下最长 60s 的陈旧缓存窗口如何处置。
- [x] **`admin-ops.service.ts:203` 告警 `take: 50` 硬截断（2026-09-06 #841）** —— `limit`（50/上限 200）、`total`、`viewTotal`、按视图判断的 `truncated`。原文：响应体无 `total`、无 `truncated` 标记，被截掉的条数在接口层无法察觉（来源批次实测 73 条只回 50 条）。批量故障恰恰是最需要看全的场景。本批未碰。
...
- [ ] **正式 AppID 真机回归与体验版候选**：先用真实会员账号覆盖微信一键登录/短信降级、本人文档、报价、隐私确认、终端选择、建单、二维码显示、扫码后撤码和 AI 历史回看；确认生产 API 与候选契约一致后，再取得“精确提交 SHA + 指定体验版版本号”的新授权上传，未授权前不上传、不直接发布正式版。
...
- [x] **同一提交受控发布（2026-09-06 完成：`6ee09fcc0` 已上线）** —— 负责人授权「重新部署到最新 main」后按 runbook 执行；首发 `35af2263b` 因部署脚本漏持久化 `PRINT_REQUIRE_PRINTER_ONLINE` 在 PM2 重启后失败（修前进未回滚，#829 已堵根因），随后重跑目标 CI 完整重发 `6ee09fcc0`：备份、additive migration、API + Kiosk/Admin/Partner 三前台同步发布、公网按域名复验 bundle 一致，`DEPLOY_API_ENABLED` 已恢复 `false`。记录见 `docs/device/production-server-cleanup-2026-09-06.md` §八。原文：先获得新候选精确 SHA 的具名部署授权，备份 PostgreSQL/运行目录，执行双库 additive migration，同步发布 API + Kiosk，保持 `PRINT_REQUIRE_PII_SCAN=true`，发布后恢复 `DEPLOY_API_ENABLED=false`。小程序在后端上线前不得单独上传，否则会再次出现前后端断层。
...
- [~] **M2 手机下单到到机打印**：第一片本地代码候选已完成 Order-only 预提交、到机码、终端核验、现场支付后原子释放唯一 `PrintTask`、到期/CAS/限流、服务端页数与报价快照，服务点只返回 `enabled + active + 近 5 分钟心跳 + 本地任务库可用` 的真实终端。**本期不接 JSAPI 远程支付，只开放 A4 黑白单面**。当前仍为进行中，因为 API/Kiosk 尚未部署，小程序未上传，Windows + 真实奔图还没有完成“未付款绝不出纸、已付款只出一次、失败不重复扣款/重打”的现场证据。
...
- [x] **A-1 隐私工单页去重（2026-09-06 本地候选）**：按产品负责人 2026-09-05 最新拍板，保留功能更完整的 `/privacy-requests`，删除 `/member-privacy` 路由、导航、页面及孤立适配器，**不保留旧 URL 重定向**；`verify:data-request-ui` 已改为守住保留页、游标分页、真实端点及重复入口退役事实。该项仅代表本分支源码候选，未合入、未部署。
