...
| 证件复印/证件照/~~云打印~~/格式转换/签名盖章/U盘 | 首页占位·`/print-scan` | 占位 | — | 安全 | **待真机·诚实禁用/隐藏**（**2026-07-12 更新**：「云打印」已按正式取舍决策从首页删除，能力归位文档打印+手机扫码上传，见 `docs/reviews/2026-07-12-cloud-print-decision.md`；另更正：「格式转换」已于 2026-07-11 点亮图片→PDF MVP、「U盘」已于 2026-07-11 在 PrintUploadPage `usb` tab 代码级接线，两者仅待真机/生产验收，不再是纯占位，见 matrix §3.4 与 next-tasks） |
