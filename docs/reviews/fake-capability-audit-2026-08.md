...
### 补充：代码注释同样只是「意图记录」，不是现状

> 2026-08-16 由小程序线在跨会话协作中提炼，比本节原表述更完整。

本节原本只说「计划文档记录的是当时打算怎么做」。实际上**代码注释是同一类东西**：

**实例**：`pricing.service.ts` 的 `billingEnabled` 注释自称
「政企 E1 整机免费模式**预留位**，当前恒 true」，小程序 `utils/print-pricing.js`
也据此写了 `billingEnabled === false → 全免费` 的分支。

但产品所有者 2026-08-11 已**否决**整机免费方案
（原话：「机器里有打印功能也可以当普通打印机用，我不想所有的全部都是免费」），
改用按场景+功能限定的补贴策略。

→ **那个注释记录的是写代码时的意图，说明不了产品后来的决策。**
把它当「预留能力」会导致下游继续围绕一个已废弃的方案建设。

**判断某能力是否该建设，依据优先级**：
```
产品当前决策（progress / 方案文档的裁定）
  > 实现代码的实际行为
    > 代码注释与计划文档所述意图   ← 最弱，只说明「当时打算怎么做」
```
