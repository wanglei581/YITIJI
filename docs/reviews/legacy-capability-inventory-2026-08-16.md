...
### H-1 · 小程序到机码核销（PR-5）—— **P0，必须在 V6 里新设计**

**为什么 V6 没有**：**纯冻结时点问题。** V6 能力冻结在 `ee4bc267f`（2026-08-11），该能力首次落地是 `c61b7e06f`（2026-08-12），差一天。不是产品有意不要 —— 恰恰相反，`67145a855`（08-16）还在为它补「小程序与一体机共用后台裁定 + M1 渠道字段」。

**丢掉的后果**：用户在小程序下单付了钱（或选了到店付），拿着 10 位码或二维码走到机器前，**在整机上找不到任何输入口**。`Order.pickupStatus` 停在 `pending`，到 `pickupCodeExpiresAt` 后被惰性置 `expired`、`payStatus` 置 `closed`。这条链会**静默断在最后一步**，而且断得毫无提示 —— 小程序侧一切正常，机器侧根本不知道有这回事。

**额外的埋雷（比"丢失"更危险）**：如果改版时有人看到 V6 P41 的「取件码」就顺手接线，会把 10 位字母数字码接到一个 `maxlength="4"` + `inputmode="numeric"` 的输入框上 —— **结构上无法输入生产的码**，还会连带把 P41「出纸口无人认领」的语义（认领别人落下的纸）和 PR-5 的语义（核销自己的订单并付款）混成一个。两者背后是两张不同的状态机：P41 那一支不写 `Order.pickupStatus`，PR-5 必须写。
...
| 处置 | 条数 | 编号 |
|---|---|---|
| **在 V6 原型里新设计** | 8 | H-1(P0)、H-2、H-3/H-4/H-5(合一页)、H-6、H-7、H-8、H-11 |
| **留位并标明，不做完整设计** | 1 | H-10 |
