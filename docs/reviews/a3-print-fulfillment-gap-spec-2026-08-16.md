...
1. ~~**用户侧退款申请端点缺失。**~~ **⛔ 本条与一条已封板的产品决定冲突，暂停据此派单（2026-08-16 订正）。**
   ...
   ### ✅ 已裁决（2026-08-16，产品负责人）

   **结论：不做一体机自助退款；退款裁决权在后台，一体机只提供「反馈问题」入口。**

   | 维度 | 定案 |
   |---|---|
   | 触发条件 | ①用户上报 ②功能使用失败 ③没有实际出纸 |
   | 裁决方式 | **混合** —— 系统能确定「没出纸」的单**自动退**（打印任务 failed、超时未取、终端上报故障）；用户主观上报的（如「打出来是花的」）进**后台人工队列** |
   | 一体机前台 | 只出现**「反馈问题」**入口，**不提退款字样**。避免用户误以为点一下就能拿到钱 |
   | `PrintCashierPage.tsx:494`「本机不提供自助退款」 | **不改，它继续为真** —— 一体机确实不执行退款，它提供的是上报 |

   **对 PR #632 的处置：不按原样合入。** 它建的用户侧端点 `POST /me/print-orders/:orderId/refund` 与本定案冲突（定案下不应存在任何用户侧退款端点）。但它的两项产出**必须保留复用**：
   - **72 格状态矩阵中放行的那 5 格**（`paid ×` `pending`/`pending_release`/`awaiting_payment`/`expired`/`failed`，共同判据「这单不可能已经产出过纸」）—— 这正是**自动退档的判据**，只需把触发方从用户侧改到系统侧；
   - **那道静态门禁** —— 一旦门禁层出现 `refund.create` / `order.update` / `$transaction` / 自造 `refundNo` / 直连 provider 就构建失败，防止有人绕开 `RefundService` 的幂等与 CAS 另写一套。

   **两样已存在但闲置的东西要接上，不要新造：**
   - `POST /kiosk/feedback` —— 后端 08-16 已上线且实现完整，**kiosk 前端 0 引用**，异常反馈仍跳登录墙（见能力盘点「反向事实」①）。「反馈问题」入口就用它。
   - `RefundService.refund()` —— 幂等键 `RFD-<orderNo>`、CAS、provider、收敛任务、失败重试俱全。自动退与人工退都必须走它，不得平行实现。

   **仍然成立的事实**：生产单价为 0（免费试运营）时订单创建即结清，0 元单无款可退，前台应显示「本单免费，无款可退」而不是任何退款入口。
