...
| 序 | 动作 | 风险 |
|---|---|---|
| 1 | `/print/material-check` + `/print/preview` → 一张打印台（稿 13） | 低，稿已画好；**已实现，见 `/print/desk`** |
| 2 | `/print/confirm` 去掉与收银台重复的价目明细 | 低 |
| 3 | 自我探索 4 → 1（稿 34） | 低，状态键逐字对应；**归「简历优化功能完善」会话的 lane** |
| 4 | 扫描 4 → 1（稿 18） | 中，涉硬件轮询 |
| 5 | 面试训练 5 → 1（稿 29） | 中 |
| 6 | `/print/progress` + `/print/done` → 一张交付页（稿 15） | 低，但 PR #938 已在改这两页，需协调 |
