...
### 发现⑤：「云打印」卡片语义与「文档打印」重叠 —— 已加澄清备注，决策留待点亮前

- **已执行**：`user-data-flow-matrix.md` §3.4"云打印"行已补充范围澄清备注：该卡片当前定义的"缺口/下一步"与「文档打印」已实现架构完全相同，真正点亮前必须先定义清楚增量能力（如第三方网盘拉取打印），否则应走与 job-master 同等的正式取舍流程（合并/隐藏），而非直接点亮。
- **未做**：不涉及代码改动；是否点亮/重新定义/合并留待该待办真正排期时决策，本次仅记录风险供后续窗口参考。
