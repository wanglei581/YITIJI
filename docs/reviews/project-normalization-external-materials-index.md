...
| 路径类别 | Git 状态 | 处置 |
| --- | --- | --- |
| `docs/business/AI求职打印服务终端-B2G-B2B2C线下就业服务终端解决方案.pdf` | 已跟踪 | 不能在 T5 中假装仓库没有 PDF；后续如需清理，另起任务执行 `git rm --cached` + 仓库外归档确认 |
...
| 来源 | 状态 | 处置 |
| --- | --- | --- |
| 已跟踪旧方案 PDF | 仓库历史中已经存在 | 另起清理任务；T5 不执行 `git rm`，不移动文件 |
...
| 后续任务 | 输入 | 边界 |
| --- | --- | --- |
| D1 已跟踪 PDF 清理 | 已跟踪旧方案 PDF | 先确认外部归档位置，再独立分支执行 `git rm --cached`，不删除用户本地文件 |
