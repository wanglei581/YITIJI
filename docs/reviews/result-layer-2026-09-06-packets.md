...
#### 包 L2 · 一体机优化页：草稿自动保存 / 已确认版本 / 逐条「回到原文」（消费包 H 的服务端）

- **已合入 main**：#888（squash `e10794379`，2026-09-07）。
- **条目**：P0-12 的用户可见部分（包 H 只做了服务端）。
- **目标**：登录会员进入 `/resume/optimize` 先 GET draft；有草稿则青序提示条「继续上次编辑 / 从优化结果重新开始」，选择前不渲染编辑区。编辑区 / 版式 / 逐条裁决变化后防抖 ≥2s PUT draft，状态诚实（已保存 / 失败仍在本机 / 未登录不保存）。每组建议可切回原文（`decisions` 记 `{[moduleKey]:'original'|'optimized'}`），导出按 decisions 组装。已确认版本面板 GET versions，点击按 fileId 走既有 `GET /files/:id/preview-url` 预览 / 二维码。匿名不调 draft / versions，只写「登录后可自动保存草稿与保留多版本」。

- **执行记录**：
  > 2026-09-07 **包 L2 · 一体机优化页草稿自动保存 / 已确认版本 / 回到原文**（分支 `claude/rl-l2-kiosk-draft`，本地工作区，未 commit、未 push、未开 PR、未部署）。消费包 H 的 `PUT/GET /resume/records/:taskId/draft`、`GET .../versions`。① kiosk `ai.ts` + http/mock 适配器新增 4 个方法；mock 对 GET draft / versions 给空结果，PUT draft / fact-check 拒绝，不伪装已保存。② 登录会员先 GET draft；有草稿则青序提示条「继续上次编辑（更新于 hh:mm）/ 从优化结果重新开始」，选择前不渲染编辑区。③ 内容 / 版式 / 逐条裁决变化后防抖 2s PUT draft；状态「草稿已保存 hh:mm:ss」/「保存失败，改动仍在本机」/「未登录，草稿不会保存」。未保存改动仍走 `requestLeave`。④ 每组建议可「回到原文 / 使用优化稿」，`decisions` 随草稿保存，导出前按裁决把已回退模块换回原文。⑤ 「已确认版本」GET versions，点击用既有 `GET /files/:id/preview-url`（`fetchAccessUrl`）打开 `FilePreviewDialog`；找不到链接写明「文件可能已到期或被清理」。导出成功后 `refreshKey` 刷新列表。⑥ 匿名不调 draft / versions，无假按钮。`ResumeOptimizePage.tsx` 316 行。夹具 `api-router` 对未登记的 draft/versions 默认 404。Playwright：会员草稿 `updatedAt=2099-03-15T08:30:00.000Z`。**未做**：小程序接线、服务端 / Prisma / 多版本对比、匿名草稿。**验证（实跑）**：kiosk `tsc --noEmit` 0；改动文件 eslint `--max-warnings=0` 0。`verify:fusion-w3` / `kiosk-runtime-error-boundary` / `lightflow-k2b-ai-resume` / `profile-documents-inkpaper` / `resume-diagnosis-flow-ui` / `resume-phone-upload-ui` / `kiosk-visual-unity` / `ai-down-fallbacks` / `member-session-closure` / `kiosk-frontend-debt` / `no-raw-error-render` / `llm-timeout-concurrency` / `toolbox-ai-skill-intents` 全绿。Playwright：`resume-optimize-draft.spec.ts` 1 passed；`kiosk-visible-actions-truth.spec.ts` 4 passed；`fusion-w3.spec.ts` 11 passed（含优化稿页内预览）；`fusion-w6-routes.spec.ts` `/resume/optimize` 与 `/resume/export` 2 passed。未部署。
#### 包 L3 · 小程序优化页：草稿自动保存 / 已确认版本 / 导出前事实核对 —— grok

- **已合入 main**：#889（squash `ad8e4d970`，2026-09-07）。
- **条目**：P0-12 小程序侧、P0-6 用户可见部分（消费包 H 服务端；包 F 记录写明小程序事实核对未接）
- **事实**：端点与类型同包 L2 / 包 H（`PUT/GET /resume/records/:taskId/draft`、`GET .../versions`、`POST .../fact-check`；导出 DTO 可选 `factsConfirmedAt`，缺失或无效 → 400 `RESUME_FACTS_NOT_CONFIRMED`）。页面 `apps/miniapp/pages/resume-optimize/` 已接四格式导出与收费三态（包 C2）。
- **要做**：① `utils/api.js` 只加 `putResumeDraft` / `getResumeDraft` / `getResumeVersions` / `factCheckResume`（needAuth true）。② 登录会员进页 GET draft，有草稿弹「继续编辑 / 重新开始」；编辑后防抖 ≥2s PUT，状态写「草稿已保存 hh:mm:ss / 保存失败 / 未登录不保存」。③ 已确认版本列出服务端 `items`（confirmedAt、事实核对时间、打开 fileId）。④ 导出前 POST fact-check，`foundInOriginal=false` 逐项勾选后才带 `factsConfirmedAt`；原文不可得不伪造已核对。
- **允许改**：`apps/miniapp/pages/resume-optimize/**`、`apps/miniapp/utils/api.js`（只加）、`apps/miniapp/scripts/api-contract.json`。
- **禁改**：`services/api/**`、`packages/shared/**`、`apps/kiosk/**`、`apps/miniapp/app.json` 的 tabBar、`.github/**`。
- **验收**：`pnpm --dir apps/miniapp verify:api-contract`、`verify:static`、`verify:visual-scale`；图谱列出的门禁；WXSS 无 `*/;`。

- **执行记录**：
  > 2026-09-07 **包 L3 · 小程序优化页草稿 / 版本 / 事实核对**（worktree `rl-l3-miniapp-draft`，本地工作区，未 commit、未 push、未部署）。消费包 H 已合入的 draft / versions / fact-check 端点。① `utils/api.js` 新增四个封装，needAuth true，mock 模式诚实拒绝。② 登录进页 `GET draft`：有 `draft.resume` 弹「继续编辑 / 重新开始」；编辑区改的是即将导出的结构化优化稿（对照区只读）；防抖 2s `PUT draft`，状态为「草稿已保存 hh:mm:ss / 保存失败 / 未登录不保存 / 当前账号无法保存这份任务的草稿」。匿名不调草稿接口。③ 已确认版本按服务端 `items` 列出（当前实现只回最新确认快照，不臆造 v1…vN-1）；有 `fileId` 走既有 `GET /files/:id/preview-url` + `wx.downloadFile` / `wx.openDocument`（与「我的文档」相同）；无 fileId 写明没有其它 signedUrl 字段。导出成功后刷新版本。④ 导出前 `POST fact-check`，先 flush 草稿；`foundInOriginal=false` 必须勾选；全部确认后把 ISO `factsConfirmedAt` 放进既有 `exportGeneratedResume` body。原文 404/503/`originalAvailable` 异常按服务端返回展示，不写「已核对」，并禁止导出。400 `RESUME_FACTS_NOT_CONFIRMED` 用人话提示。新逻辑在 `draft.js` / `fact-check.js`，页面只接线。格式段选 min-height 提到 96rpx。**验证（实跑）**：`contract:update` 后快照补 4 个端点；`verify:api-contract` 绿（131 调用 / 1 个已知缺口仍是材料包取消）；`verify:visual-scale` 绿（字号偏离 426≤439，圆角 171≤181）；`verify:static` 123 PASS / 0 FAIL（含优化导出结果层与价格三态、WXSS 无空声明）；图谱列出的 `verify:assistant-voice` ALL PASS、`verify:miniapp-cloud-print-m2` ALL PASS。**未做**：微信开发者工具 automator 截图（本机有 DevTools cli，但未安装 `miniprogram-automator`、开发者工具未在跑、本页需要真实 taskId，未拍三张图）；未改服务端 / Prisma / kiosk / tabBar / workflows；未 commit / push / 部署。服务端 versions 目前只返回最新确认快照，面板按返回值列出，不补历史版本。

- **执行记录（第二轮）**：
  > 2026-09-07 **包 L3 第二轮（收货返工，同一工作区，未 commit / push）**。不重做第一轮，只补两处。① **PUT draft 不得冲掉一体机 layout / decisions**：`GET draft` 后把 `draft.layout`、`draft.decisions` 缓存在页面实例 `_draftExtras`（不进 data、不渲染）。「继续编辑」和「重新开始」都保留这份缓存——裁决属于同一 taskId 的优化结果，不随重新开始丢弃。每次 PUT 经 `buildDraftPutBody(resume, extras)` 原样带回；服务端没返回这两项时不带键，不发 `null` 或 `{}`。PUT 成功若响应里带 `draft.layout` / `draft.decisions` 则刷新缓存（当前服务端 PUT 只回 `{taskId,updatedAt,saved}`，缓存保持 GET 到的原值）。**小程序端不解释 decisions 语义，只透传。** ② **事实核对「剔除该修改」**：每个 `needsConfirm` 项增加「删掉这条，回到原文」（`.btn.ghost.block`，全局 `.btn` 高 52px ≥48px）。标量 path（`basic.phone` / `basic.email` / `basic.city` / `intention.*` / `summary`）走 `draft.applyInput` 清空编辑区字段，该项 `removed: true` 不再计入 `pending`，面板顶部「已从优化稿删除 N 项，导出内容以编辑区为准」。数组段落（`experience[i].company` / `education[i].period` 等，含 `experience[i].description` 一类）不自动改文本；关面板、`wx.pageScrollTo` 到对应编辑区、toast「请在编辑区修改：\<label\>」，该项保持待确认。编辑或剔除后 `_factsNeedRecheck=true`，再次点「生成并打开文件」必须重新 `POST fact-check`，不得复用上一次结果。初始仍全部 unchecked、确认按钮 disabled。③ **顺带核一遍（已满足，未改口径）**：`restoreDraft` 仍只在 `auth.isLoggedIn()` 为真时调用；匿名不发 draft / versions / fact-check。页面 js 498 行（≤500），新逻辑在 `draft.js` / `fact-check.js`。**验证（实跑）**：`verify:api-contract` 绿（131 调用 / 1 个已知缺口仍是材料包取消）；`verify:visual-scale` 绿（字号偏离 426≤439，圆角 171≤181）；`verify:static` 123 PASS / 0 FAIL；图谱 `apps/miniapp/utils/api.js` 列出的 `verify:assistant-voice` ALL PASS、`verify:miniapp-cloud-print-m2` ALL PASS。**未做**：未改 `utils/api.js` / 契约快照 / 服务端 / kiosk / tabBar / workflows；未 commit / push / 部署；未跑微信开发者工具或真机。
#### 包 L1 · 「我的文档」Word 转 PDF 入口（两端）+「重新打印」按 reprintable 接线 —— grok

- **已合入 main**：#890（squash `b30adea5a`，2026-09-07）。
- **条目**：补包 D / K2 留下的缺口：引擎与三端接收已在 main，但「我的文档」页两端没有转 PDF 按钮；`reprintable=false` 服务端已回传，前台未接线。
- **目标**：两端 Word 文件显示「转 PDF」（`wordToPdf=true` 才可点，关闭/探测失败置灰并写 `WORD_CONVERSION_UNAVAILABLE_COPY`，不隐藏）；点击 convert 有进行中状态，成功刷新列表并展示 filename / pageCount / sizeBytes / 有效期倒计时 + `WORD_CONVERSION_DISCLOSURE` + warnings；失败按错误码给人话。`reprintable=false` 的「重新打印」`aria-disabled` + 「该报告仅可查看，不可打印」。小程序新增 `convertDocumentToPdf`。
- **禁改**：`services/api/**`、`packages/shared/**`、`apps/kiosk/src/pages/profile/profileEntries.ts`、`apps/kiosk/src/pages/auth/**`、`.github/**`。

- **执行记录**：
  > 2026-09-07 **包 L1（分支 `rl-l1-docs-convert`，本地工作区，未 commit / 未 push / 未部署）**。① 一体机 `/me/documents` 对 Word 文件接线「转 PDF」：复用 `documentConversion.ts` 的 `useDocumentConversionCapabilities` / `isWordDocument` / `convertDocumentToPdf` / 固定文案；能力关闭或探测失败按钮 `aria-disabled` 并常驻 `WORD_CONVERSION_UNAVAILABLE_COPY`（不隐藏）；进行中写「转换中」；成功刷新列表（新 FileObject）并展示 filename / pageCount / sizeBytes / `useRemainingSeconds` 倒计时，附 `WORD_CONVERSION_DISCLOSURE` 与非空 `warnings`。`AUTH_REQUIRED` / 401 →「请先登录」；`CONVERSION_*` / `UNSUPPORTED_FILE_TYPE` 只透传可展示的服务端中文，不原样渲染 `err.message`。主文件已到预算上限，转换入口拆到 `DocumentConvertAction.tsx`，保存期限确认弹层拆到 `RetentionConfirmOverlay.tsx`，reprintable 判定在 `documentReprint.ts`。② `reprintable=false`（及 `purpose==='contract_review_report'` fail-closed）：打印键 `aria-disabled` + 常驻「该报告仅可查看，不可打印」，onClick 短路，不再点进打印吃 400。③ 小程序 `utils/api.js` 只加 `convertDocumentToPdf(fileId)`（POST `/files/:id/convert`，needAuth true，timeout 65s）；列表可见「转 PDF / 重新打印」，关闭态同样置灰写原因；结果卡展示文件名/页数/大小/倒计时/披露/warnings；样式只用 `--fs-*` / `--r-*`。`api-contract.json` 补 `POST /files/:p/convert`。④ 批次守卫只加行：`verify-profile-documents-inkpaper` / `verify-lightflow-profile-entry` / `verify-profile-inkpaper-home` / `verify-file-retention-ui` / `verify-word-conversion-ui`（新增 documents convert-guard 与 reprintable 变异）。**未改** `services/api/**`、`packages/shared/**`、`profileEntries.ts`、`apps/kiosk/src/pages/auth/**`、`.github/**`、`docs/progress/current-progress.md`、`docs/graph/**`。**未做**：Playwright 真点 convert（夹具已有 capabilities 默认应答，本包未新增会触发 convert 的用例）；微信开发者工具 / 真机；生产部署。**验证（实跑）**：kiosk `tsc --noEmit` 0；改动文件 eslint 0 error；图谱 8 条 + `verify:word-conversion-ui`（含 10 项变异各红后恢复）+ `verify:file-display-truth` + `verify:kiosk-visual-unity` 全绿；`verify:kiosk-touch-targets` 不存在故跳过；小程序 `verify:api-contract`（128 端点）/ `verify:visual-scale` / `verify:static`（123 PASS）全绿；图谱交叉 `verify:no-raw-error-render` / `verify:assistant-voice` / `verify:miniapp-cloud-print-m2`（isolated）全绿。未部署。
...
#### 包 L4 · `/resume/optimize/compare` 逐条对照页迁入青序流光 —— codex

- **已合入 main**：#893（squash `a753cfa7c`，2026-09-07）。
- **执行记录**：
  > 2026-09-07 **包 L4（分支 `claude/rl-l4-compare-qx`，本地工作区，未 commit、未 push、未部署）完成 `/resume/optimize/compare` 青序流光迁移**：① 页面改用 `QxPageFrame` 并登记 `QX_MIGRATED_ROUTES`，主文件由 410 行拆至 196 行；27 寸竖屏一次只裁决一条，顶部显示「第 n / N 条」，底部 `用改写 / 保留原文 / 下一条` 使用青序 `qx-btn`，主操作高 ≥56px、可点宽 ≥48px，当前裁决以 `aria-pressed` + 双线轮廓表达；事实确认复选框本体使用 `--qx-tap-min`，不依赖外层标签补足可点区。② 复用既有 `react-diff-viewer-continued`，新增内容用下划线加粗、删除内容用删除线，不只靠颜色；左右原文/改写完整展示，并提供整份裁决草稿预览。③ 抽出 `detectUnconfirmedTextAdditions(after, before)`，与 `resume-deliver/facts.ts` 既有数字及 `主导 / 负责 / 带领 / 季度之星` 规则同源；改写中新增项逐项高亮、勾选确认前禁用「用改写」，撤销确认会同时撤销该条 optimized 裁决。④ 保留 `taskId / accessToken` 读取、匿名请求头与返回优化页行为；当前 `resume-deliver` 未出现包 L2 的已定型 decisions 类型，故按约定通过路由 state 回传 `Record<string, 'original' | 'optimized'>`，键为 `module-1`、`module-2` 等，供 L2 对齐。未自动改写优化稿、未落库、未生成文件；常驻文案明确裁决仅为本次草稿。⑤ 请求切换或重试前清空旧模块、事实确认与裁决，并只展示与当前 `taskId` 匹配的响应，避免跨任务串数据；缺上下文、读取中、无候选、普通读取失败和 AI outage 均保留诚实状态与重试/返回出口。⑥ W6 fixture 增加真实形状的两条候选；W6 路由面收集 compare 的 1080×1920 与 390×844 两断点，按 `.kiosk-stage` scale 换算断言底部三键尺寸；`kiosk-visible-actions-truth` 增加匿名 access token、事实确认门、逐条裁决及 state 回传流程。**实跑通过**：Node 22.23.2；kiosk `tsc --noEmit`；完整 `src` ESLint 0 error（10 条既有 Fast Refresh warning）；带 `VITE_API_MODE=http`、TRTC 与终端变量的生产 build；`verify:ai-down-fallbacks`、`verify:resume-diagnosis-flow-ui`、`verify:kiosk-visual-unity`、`verify:visible-actions-truth`、`verify:fusion-w3/w4/w6`、`verify:fusion-shell`、`verify:profile-documents-inkpaper`、`verify:device-status-honest`、`verify:fusion-home`、`verify:kiosk-runtime-error-boundary`、`verify:kiosk-shell-active-nav`、`verify:lightflow-k1-public-entry`、`verify:lightflow-k2a-ai-career`、`verify:lightflow-k2b-ai-resume`、`verify:lightflow-profile-entry`、`verify:print-parameter-capability`、`verify:profile-inkpaper-home`、`verify:runtime-terminal-identity`、`verify:print-done-truth`、根级 `verify:compliance-copy` 等价四段及 `verify:fixture-time-bombs`；Playwright `--list` 收集 W6 107 条（含 compare 两断点）与 visible-actions 6 条。仓库无 `verify:kiosk-density`。**环境阻塞 / 未通过浏览器验收**：沙箱禁止监听 `127.0.0.1`，`verify:qingxu-proto-geometry` 与 Vite preview 均报 `listen EPERM`；Chromium 启动报 `MachPortRendezvousServer ... Permission denied (1100)`，因此 W6 compare、`kiosk-visible-actions-truth.spec.ts` 真跑及截图均为 **UNREVIEWED**，不得写成 Playwright 通过。**明确未做**：未改 `services/api/**`、`packages/shared/**`、`apps/miniapp/**`、Prisma、workflow、`docs/progress/current-progress.md` 或 `docs/graph/**`；未新增依赖或服务端端点。

#### 包 L6 · 文档与原型标注补漏（交付标准评审仍开着的文档项）

- **已合入 main**：#895（squash `501f76fde`，2026-09-07）。
...
- **已合入 main**：#896（squash `a6b9598ba`，2026-09-07）。
- **条目**：交付标准评审 A 组仍开着的四条 + 一条注释（`wx-resignin`、终端两把密钥进启动门禁、Kiosk AI 能力端点、admin-orgs 缓存失效容错、取件码注释）。
- **执行记录**：
  > 2026-09-07 **包 L5 · API 侧补漏**（worktree `rl-l5-api-gaps`，本地工作区，未 commit、未 push、未开 PR、未部署）。① `POST /member/auth/wx-resignin`：复用 `fetchWxOpenId`，按 `wxOpenId` 找已绑定会员；找到且未禁用 → `issueLoginForUser` 同形状 `{ token, user }`；未绑定 401 `MEMBER_NOT_BOUND`；禁用 403 `ACCOUNT_UNAVAILABLE`。不建号、不绑手机、不重签法务。限流与 wx-login 同款 `@Throttle({ ttl: 60_000, limit: 10 })`；wx-login 本身无 AuditLog，本端点同款不加。② `TERMINAL_ADMIN_SECRET` / `TERMINAL_ACTION_TOKEN_SECRET` 确被 `terminals-agent.service.ts:165-166` `requireEnv` 读取（管理凭证与动作令牌签名）。已按 `MIN_LONG_SECRET_LENGTH=32` + `SAMPLE_SECRET_MARKERS` 进 `production-runtime-gates.ts`。`services/api/.env.example` 样值本就以 `replace-with-` 开头，未改。③ `GET /api/v1/kiosk/ai/capabilities` 匿名可读、`@TerminalScopedThrottle(30)`，返回 `{ items: [{ key, status, reason?, providerName? }] }`，只读 llm-config 视图，不调 `getApiKey`、不发出站请求。`verify:ai-throttle-dimension` 扫描 522 条 HTTP 路由派生 33 条计费路由，本端点不在其中（非计费判定正确）。④ admin-orgs 缓存失效真实位置是 `services/api/src/orgs/admin-orgs.service.ts`（不是 `src/admin-orgs/`）：改前 `invalidateAccountSession` ~817、`publishDeletedSessionState` ~848、账号启停/改密/换邮箱提交后调用 ~519/550/609。已改走 `tryRedis`，失败 warn + 既有 `sessionInvalidation: 'failed'` 形状，不回滚已提交结果。⑤ `claim-pickup.dto.ts` 注释「6 位新码」改为「8 位新码」，正则与逻辑未动。**未做**：小程序/一体机接线、Prisma、`docs/graph`、`current-progress.md`、commit/push/部署；`verify:partner-account-action:postgres` 未跑（需 PG）。**验证（实跑）**：api/shared `tsc --noEmit` 0；改动文件 eslint 0。`verify:kiosk-ai-capabilities` 全绿，变异（list 丢掉 `assistant_chat`）→「items 数量 14」红，恢复后绿。`verify:production-runtime-gates`（含两键缺失/过短/样值）/ `production-real-services` / `boot-resilience` 44/44 / `throttle-dimension` / `ai-throttle-dimension` / `llm-timeout-concurrency` 78 / isolated `admin-orgs` / `print-jobs` / `member-auth` / `payment-flow` / miniapp `verify:api-contract`（127 端点，无需加快照）/ `verify:repository-integrity` / `verify:ci-gate-coverage` / kiosk `verify:member-session-closure` 及图谱列出的 member-account-status、member-step-up、wave2、member-data-retention、cjk-font、print-rollout-config、backend-p0-contracts、ai-contract-mirror、deploy-gates-in-sync、legal-doc-version、admin-orgs-delete-schema/concurrency、partner-account-action(+schema)、partner-org-self、partner-email-login-alias、partner-smart-campus、miniapp-cloud-print-m2、assess-isolation、toolbox-ai-skill-intents、profile-commercial-first-batch、ai-down-fallbacks、ai-artifact-print-url-contract、profile-documents-inkpaper、profile-feedback-inkpaper、job-ai-history-privacy-ui、job-fit-m1-5-ui 全绿。未部署。

#### 包 M1 · 对照页裁决语义改为「明确询问后再应用」+ 批量动作 + 裁决草稿预览 —— grok

- **条目**：产品负责人 2026-09-07 裁定。对照页只做阅读裁决；回优化页时弹确认，用户确认后才写入编辑区。批量动作与裁决草稿按 `23-resume-optimize.html` 第 1176–1180 / 1854–1878 / 975–999 行。
- **事实**：对照页 `ResumeOptimizeComparePage.tsx` + `resume-compare/*`；裁决键与优化页同源 `moduleKeyOf` → `m<序号>:<标题>`。旧实现 `useCompareDecisionsReturn` 回传后自动 `apply()` 改写简历。模块契约只有 `{title, before, after}`，无字段路径 / reason / 单条重生成 / 逐条落库端点。
- **执行记录**：
  > 2026-09-07 **包 M1**（worktree `rl-m1-compare-semantics`，本地工作区，未 commit、未 push、未开 PR、未部署）。① `useCompareDecisionsReturn` 不再自动 `apply`：回传 `decisions` 后把待应用裁决交给优化页；新建 `CompareDecisionsApplyDialog`（标题「对照页有 N 条裁决」，正文「应用后会改写下面编辑区的对应文字；只有编辑区的内容会进入导出」，按钮「应用到编辑区」/「暂不应用」）。确认才走原来的 `applyDecisionChanges`；「暂不应用」丢弃、不落草稿。对照页常驻说明裁决是阅读决策，回优化页再选是否应用。`ResumeOptimizePage.tsx` 332→335 行（+3，限额 +10），只加接线与弹层，未动空态 / `ResumeStatePanel` / `ctabar` / `QxPageFrame` 签名。② `replaceResumeText` 由全对象 `split(from).join(to)` 改为按对象遍历顺序只替换**第一处命中**（命中第一个包含该文本的字符串字段后停止；同一字段内也只改第一处）。行为变化：若 `before/after` 在多个字段或同一字段多次出现，不再一并改写。新增 `verify:resume-decisions`（静态 + 逻辑断言），挂在既有 `verify:resume-diagnosis-flow-ui` 之后，进 CI 闭包。③ 对照页顶部三按钮逐字照稿：「可采纳的全部采纳」（跳过有待确认事实）/「其余保留原文」/「清空全部裁决」`data-tone="danger"`（连同 `confirmedByModule` 与自己写文本一起清）。提示区用稿原文。按钮用 `qx-btn` + `--qx-*` 令牌，无裸 hex；高 ≥56px、可点宽 ≥48px。④ 可展开「裁决草稿」：逐条标 已采纳 / 保留原文 / 自己写 / 待定；顶部写明「这是阅读草稿，不是简历最终稿；只有优化页编辑区的内容会进入导出。」有待确认事实且未确认完的条目草稿退回原文并标注原因。⑤ 第四态「自己写」做到阅读层：可输入自己的写法，只进草稿与统计，刷新即丢；不参与「应用到编辑区」；弹层写「自己写的 N 条不会被应用，请在编辑区自行修改」。无字段路径，不能自动套进编辑区。`verify-resume-diagnosis-flow-ui` 把「已采纳」负向断言改指向「已保存」（阅读层标签现为稿要求），并保留 `aria-pressed={decisions[keyAt(currentIndex)]` 与「未保存」。**未做**：服务端 / Prisma / 共享类型 / 小程序 / 逐条落库 / 单条重生成 / 字段路径套用；未动 `useResumeDraftAutosave` / `useOptimizeSession` / workflow / `docs/graph` / `current-progress.md`。未 commit / push / 部署。**验证（实跑）**：kiosk `tsc --noEmit` 0；改动文件 eslint `--max-warnings=0` 0。`verify:resume-diagnosis-flow-ui`（含新 `verify:resume-decisions` first-hit）/ `fusion-w3` / `fusion-w6` / `fusion-shell` / `ai-down-fallbacks` / `kiosk-visual-unity` / `visible-actions-truth` / `qingxu-proto-geometry`（51 页 / 713 变体）/ `kiosk-runtime-error-boundary` / `lightflow-k2b-ai-resume` / 图谱列出的 `profile-documents-inkpaper` / `resume-phone-upload-ui` / 根 `verify:fixture-time-bombs` / `verify:ci-gate-coverage`（408/413 闭包，新门禁经 diagnosis-flow-ui 间接执行）/ `verify:repository-integrity` 全绿。Playwright：`kiosk-visible-actions-truth.spec.ts` + `resume-optimize-draft.spec.ts` 8 passed（含回传后确认弹层、「暂不应用」不改写编辑区、批量采纳跳过未确认事实、清空回待定、手机断点触控）；w6 config `-g resume/optimize` 3 passed（`/resume/optimize`、`/resume/optimize/compare` 1080 与 compare 手机断点）。

#### 包 N3 · `/smart-campus/freshman-insights` 改为「迎新服务导览」（零新数据源）—— grok

- **主持人收货补记（2026-09-07）**：门禁 `verify-smart-campus-ui.mjs` 的 A2 `mustNotContain` 不但保留还加严了（原有 `getFreshmanInsights` / `MOCK_FRESHMAN` / `isMock` / `示例数据` / `topMajors` / `ageDistribution` / `conic-gradient` 之外，新增 `校园大数据` / `聚合统计` / `报到人数`），假统计回流仍被钉死。`verify-fusion-w4.mjs` 里被移除的「学校书面授权 / 数据处理协议 / 聚合脱敏统计」三条断言不构成合规防线流失：该前置条件原文记在[decisions/2026-06-17-smart-campus-jobfair-delivery-rules.md](../decisions/2026-06-17-smart-campus-jobfair-delivery-rules.md) 的 P2 第 1 条（「校园大数据解冻。前置条件是学校书面授权、数据处理协议、聚合脱敏模型、小样本保护、审核发布和法务确认」）。**将来若要解冻校园大数据，按该决策文档执行，不得只改页面。**

- **条目**：产品负责人 2026-09-07 裁定。本平台没有迎新报到数据，这一页不做统计，改成新生到校后用得上的服务入口导览，全部指向已有真实能力。
- **事实**：`apps/kiosk/src/pages/smart-campus/FreshmanInsightsPage.tsx` 原为「校园大数据 / 迎新报到聚合统计 · 暂未开放」空态；同域 `SmartCampusHomePage` / `SmartCampusServicePage` 已有 `sc-mod` / `kproto-*` / `W4Presentation` 版式。`/campus/freshman-insights` 归包 N2，本包不碰。

- **执行记录**：
  > 2026-09-07 **包 N3**（worktree `rl-n3-freshman-guide`，本地工作区，未 commit、未 push、未开 PR、未部署）。① 本页标题改为「迎新服务导览」，副标题「新生到校后可在本机使用的服务入口」，去掉「大数据 / 聚合统计」。复用智慧校园 `KioskPageFrame`（W4 wheat）+ `kproto` / `sc-mod` 卡片网格，129 行。② 列出五项已用 `git grep` 核对、非占位的真实路由：`/print/upload`（上传报到表等 PDF/图片本机预览打印；Word 需转换引擎开放）、`/resume-service`（诊断 / 优化 / 生成草稿并可打印）、`/campus`（校园主题招聘会参展企业与导览；投递请去来源平台）、`/policy-service`（已审核政策与材料指引，不代办）、`/me/documents`（登录后查看/预览/重打本人文件）。③ **未列**：证件照（`/print-scan/feature/id-photo` 自身写明「尚未开放 / 没有可用流程」，已有照片打印并入打印材料）；行李帮运 / VR校园 / 校园卡 / 一卡通 / 校园网（服务页徽章「办理指引 · 未接线上办理」，占位不链）。④ 无统计数字、无报到人数、无院系分布；常驻「本平台没有迎新报到数据」。⑤ 触控复用 `.sc-mod`（min-height 220px）与 `.kproto-btn`（64px），无裸 hex。⑥ 门禁断言只改文案、不删：W6 `featureText` / marker、`verify-fusion-w4` 诚实性、`verify-smart-campus-ui` A1/A2、`fusion-w4.spec.ts` 三处旧标题。**未改** `services/api/**`、`packages/shared/**`、`apps/miniapp/**`、`pages/placeholders/FreshmanInsightsPage.tsx`、`SmartCampusGuard`、`.github/**`、`docs/graph/**`、`docs/progress/current-progress.md`、Prisma、外部依赖。**未做**：未把本页加进 `SmartCampusHomePage` / 首页入口（任务未允许新增入口；C/D 仍钉死无 freshman-insights 链接）；未跑 Playwright 浏览器真点（验收列出的是静态门禁；沙箱未必允许听 127.0.0.1）；未 commit / push / 部署。**验证（实跑）**：kiosk `tsc --noEmit` 0；改动 TS/TSX eslint `--max-warnings=0` 0。图谱 `FreshmanInsightsPage.tsx` → `verify:fusion-w4` / `verify:smart-campus-ui` 全绿；`fusion-w6-route-cases.ts` → `verify:fusion-w6` / `verify:fusion-shell` / `verify:print-done-truth` 全绿；`packets.md` → `verify:profile-documents-inkpaper` 全绿（本包未触碰 `/me/documents` 明细页，守卫按设计跳过范围 allowlist）。任务包点名的 `verify:kiosk-visual-unity` / `verify:visible-actions-truth` / `verify:ai-down-fallbacks` / 根 `verify:fixture-time-bombs`（无新增夹具日期）全绿。
#### 包 N1 · 「求职方案」四栏落在已存在的 `/resume/career-plan` 页（不新开路由）—— grok

- **条目**：产品负责人 2026-09-07 两次裁定的合并结果。求职方案与顾问成果各一页；求职方案四栏里三栏已在 `/resume/career-plan`，只补缺的「已有材料」，保住 106 条基线。
- **执行记录**：
  > 2026-09-07 **包 N1（改版）**（worktree `rl-n1-job-plan`，本地工作区，未 commit / 未 push / 未部署）。
  >
  > **为什么没新开路由**：核查 `CareerPlanPage` 已渲染 `currentSnapshot` / `directions` / `skillPlan` / `actionChecklist`。新开 `/ai/plan` 或第二条求职方案路由会把 W6 从 106 条基线撑破，也和「求职方案、顾问成果各一页」冲突。本包只在既有 `/resume/career-plan` 补第四栏。
  > **为什么没碰 `/ai/plan`**：`apps/kiosk/src/pages/ai-plan/**` 与 `/ai/plan` 属于青序流光迁移主线（顾问成果页，设计稿 `52-advisor-artifact.html`），本包一行未改。
  > **四栏数据来源**：① 已有材料 = 会员 `GET /me/resumes` + `GET /me/documents`（复用 kiosk `getMyResumes` / `getMyDocuments`，不新写 fetch；事实清单，不经模型）。匿名不调会员端点，文案「登录后可看到你已保存的材料」，不出假按钮、不显示 0 份冒充空。读取失败可重试，不拖垮其余三栏。② 目标与方向 = career-plan 的 `currentSnapshot` + `directions`（AI）。③ 尚需准备 = `skillPlan`（AI）。④ 执行计划 = `actionChecklist`（AI）。页头 `basedOn`（「依据：本人简历 + …」）保留，说明这份规划根据什么生成；已有材料栏写明那是手上已保存文件，两者不混。
  > **做了什么**：页头收口为「求职方案」；新栏拆到 `pages/resume/components/career-plan/`（`CareerPlanExistingMaterials` + 抽出 `CareerPlanSection`）。主文件 579→565 行（净减 14，限额净增 ≤15）。打印仍走 `POST resume/career-plan/:taskId/print`，继续消费 `variant === 'degraded'`，无规划时文案仍是「打印求职参考单（未含 AI 规划）」。W6 `/resume/career-plan` featureText 改为「求职方案」，并补 390×844 断点。`api-router.ts` 为 `/me/resumes`、`/me/documents` 给默认空页。新增 Playwright：匿名不打会员端点 / 材料失败不拖垮三栏。
  > **重构（单列）**：把页内 `Section` 抽到 `CareerPlanSection.tsx`，主文件才能在加栏后继续变短；未再拆 AI 三栏正文。
  > **未做**：未改 `apps/kiosk/src/pages/ai-plan/**`、`/ai/plan` 路由、`services/api/**`、`packages/shared/**`、`apps/miniapp/**`、路由表、Prisma、`.github/**`、`docs/graph/**`、`docs/progress/current-progress.md`；未加首页/服务入口；未 commit / push / 开 PR / 部署。
  > **验证（实跑）**：kiosk `tsc --noEmit` 0；改动源文件 eslint `--max-warnings=0` 0。图谱列出的 `verify:ai-artifact-print-url-contract` / `verify:ai-down-fallbacks` / `verify:fusion-w3` / `verify:kiosk-frontend-debt` / `verify:lightflow-k2a-career`（80 checks）/ `verify:profile-commercial-first-batch` / `verify:profile-documents-inkpaper` 全绿。包点名另跑 `verify:fusion-w6`（106/106）/ `verify:fusion-shell` / `verify:print-done-truth` / `verify:kiosk-visual-unity`（新 CSS 无裸 hex）/ `verify:visible-actions-truth` / 根 `verify:no-raw-error-render` / `verify:fixture-time-bombs`（无新增夹具日期欠账）全绿。Playwright `--list` 能解析；`career-plan-materials.spec.ts` 2 passed；w6 `/resume/career-plan` 1080×1920 与 390×844 两断点 2 passed（390 按 `.kiosk-stage` scale 换算触控尺寸）。未部署。
#### 包 N2 · `/campus/freshman-insights` 校园招聘数据 —— grok

- **条目**：产品负责人 2026-09-07 裁定。两个数据源都要：先上「聚合已审核来源数据」保证页面有真实内容，同时开「合作机构上传官方统计」作为第二数据源。
- **执行记录**：
