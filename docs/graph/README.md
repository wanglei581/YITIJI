...
flowchart LR
  ...
  api["services/api<br/>NestJS<br/>546 端点"]
  db[("Prisma<br/>106 模型")]
  gates{{"verify 门禁<br/>487 个脚本"}}

  kiosk -->|"/api/v1"| api
  admin -->|"/api/v1"| api
  partner -->|"/api/v1"| api
  api --> db
  gates -.->|"断言 1520 个文件"| kiosk
  gates -.-> admin
  gates -.-> partner
  gates -.-> api
```
