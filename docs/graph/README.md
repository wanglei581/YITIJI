...
| 应用 | 目录 | 路由数 | 源文件 | 入口可达 |
| --- | --- | --- | --- | --- |
| kiosk | `apps/kiosk` | 88 | 545 | 532 |
| admin | `apps/admin` | 36 | 150 | 145 |
| partner | `apps/partner` | 13 | 50 | 49 |

| 维度 | 数量 |
| --- | --- |
| HTTP 端点（services/api） | 527 |
| Prisma 模型 | 101 |
| 门禁脚本文件 | 460 |
| ├ 其中辅助库（被别的门禁 import） | 121 |
| ├ 已在 package.json 里有脚本名 | 422 |
| ├ 在 CI 执行闭包里 | 410 |
| └ **无脚本名，从未被执行** | 0 |
| 被至少一条门禁断言的文件 | 1418 |
...
flowchart LR
  ...
  api["services/api<br/>NestJS<br/>527 端点"]
  db[("Prisma<br/>101 模型")]
  gates{{"verify 门禁<br/>460 个脚本"}}

  kiosk -->|"/api/v1"| api
  admin -->|"/api/v1"| api
  partner -->|"/api/v1"| api
  api --> db
  gates -.->|"断言 1418 个文件"| kiosk
  gates -.-> admin
  gates -.-> partner
  gates -.-> api
```
