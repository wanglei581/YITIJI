...
| 数据类别 | 计划存储 | 默认留存 | 清理 / 删除 | 边界 |
| --- | --- | --- | --- | --- |
| 简历智能上下文会话建议 | React 内存 | 当前公共终端会话 | 退出、切换账号、待机、硬隐私超时、BFCache 恢复失败或来源删除时清空 | 不写浏览器持久存储、URL、埋点或日志；Gate 0 已批准，功能未开发 |
| `ResumeProfileSnapshot`（长期复用候选） | 服务端加密字段，具体模型待单独评审 | 匿名或无专用同意最长 24 小时；会员专用同意后最长 90 天；均不得晚于来源简历到期 | 撤回同意、删除来源简历、删除关联 AI 记录、账号删除或 TTL 到期时级联硬删 | 不提供无限期保存；只服务本人；Admin、Partner、企业、校方和第三方工具不可读；当前无此模型 |

上述两项的正式字段、同意和删除契约见 [简历智能上下文与 AI 辅助填写契约](../product/resume-context-ai-assist-spec.md)。实现后必须把真实模型、常量和专项 verify 补入“当前验证门禁”，在此之前不得把规划行描述为已上线能力。
