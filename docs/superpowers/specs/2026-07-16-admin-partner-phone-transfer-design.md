...
> 状态：用户已批准“保留机构账号、一次性安全转移手机号”的方案方向与可用模型切换；双模型针对性复审均 READY，允许进入 TDD。
...
2. 完全镜像严格初绑的密码失败预算：先预约一个共享额度，执行 bcrypt；bcrypt 异常立即释放后统一失败，密码错误保留额度并失败，密码正确立即释放额度后才继续。成功 start 和后续短信失败均不得占用密码失败额度。
...
7. 发码成功后 best-effort 写 `auth.phone_transfer_start`：`actorId=Admin`、`targetId=Partner`、空 payload；审计失败不改变发码结果。
8. 返回 ticket、冷却/过期时间，以及 Admin 本来有权查看的最小来源摘要：Partner 用户名、机构名称、脱敏手机号。不得返回内部手机号字段或其他机构数据。
...
6. 执行单个 Prisma 事务，按固定顺序：
   ...
   - 在同一事务写入 Admin 完成事件和 Partner 释放事件；审计不得含手机号明文、hash、密文、密码、OTP 或 ticket。Admin 完成事件使用 `actorId=Admin`、`targetId=Admin`，payload 只含脱敏手机号与 `sourcePartnerId`；Partner 释放事件使用 `actorId=Admin`、`targetId=Partner`，payload 保持为空。
   - 两条审计写入后，在同一事务内通过 `tx.user.findUniqueOrThrow` 读取 Partner 及其机构启用状态，返回新会话状态：`userId`、`role`、`orgId`、`enabled`、递增后的 `tokenVersion` 与 `orgEnabled`；不得误以为 `updateMany` 会返回记录。
7. 事务提交后使用现有 `setJsonIfVersionNotOlder` 把新版本 Partner 会话状态写入 `internal:session-state:<partnerId>`，TTL 与 `JwtAuthGuard` 保持一致。这样旧版本在途请求无论先写还是后写都不能覆盖新版本，旧 JWT 在缓存命中时也会因 `payload.ver !== state.tokenVersion` 被拒绝。缓存刷新失败只记录不含敏感数据的 warn，不把已提交成功伪装为失败；此时数据库 `tokenVersion` 是最终真值，旧缓存残余窗口以既有 60 秒 TTL 为上限。
...
取消只使用 CAS 清理当前 Admin 对应的 ticket、活动 ticket 和验证锁，不修改任何数据库字段；成功消费活动 ticket 时写 `auth.phone_transfer_cancel` 空 payload 审计。审计失败不得记录手机号、账号名、ticket 或 Redis payload。取消结果不确定时，前端沿用严格初绑的保守策略，清空内存状态并要求重新登录。

## 5. 原子性、并发与失败恢复

- PostgreSQL 非延迟唯一约束在每条写语句执行时检查，因此事务中必须先清 Partner，再绑 Admin；顺序相反会触发唯一冲突。
...
- Partner 新版本会话状态必须在事务后写入缓存，且使用原子“版本不倒退”语义；专项测试必须证明新版本覆盖旧缓存、并发旧版本回填被拒绝。缓存刷新失败时，专项测试还必须证明既有 TTL 到期回源后旧 Partner token 被拒绝。
...
- 密码正确或 bcrypt 异常必须立即释放已预约额度；密码错误保留额度。短信发送发生在密码额度释放之后，不能因短信供应商失败重复释放或泄漏额度。
...
- 正常转移与字段结果；
- 使用隔离 SQLite 的静态临时 trigger，在 Partner 第一步更新后递增目标 Admin tokenVersion，真实触发事务第二步 `updateMany count=0`，并断言 Partner 清空与 trigger 更新整体回滚；测试后在 `finally` 删除 trigger；
- 先清后绑不触发手机号唯一冲突；
- Partner 旧 token 失效、Admin 当前 token 保持有效；
- 新版本 Partner 会话状态覆盖旧缓存、旧版本并发回填被 `setJsonIfVersionNotOlder` 拒绝、旧 JWT 立即失败；缓存刷新失败后数据库真值在 TTL 到期回源时拒绝旧 token，并产生脱敏 warn；
...
- start/complete/cancel/released_by_admin 四类审计全部存在，actor/target 正确，且不包含手机号明文、hash、密文、密码、OTP 或 ticket；
...
- `services/api/src/common/guards/jwt-auth.guard.ts`（仅导出既有 60 秒缓存 TTL 常量，行为不变）；
...
- Claude 只读审查：实际模型为 Claude Opus 4.8（1M context）；初审要求补齐密码额度释放、四类审计写入点、事务第二步真实失败测试和并发旧缓存回填防线，针对性复审结论 `READY`、Critical 0、Warning 0，允许进入 TDD。
- Antigravity：通用 wrapper 默认走 Gemini 且该模型个人额度耗尽；显式 Opus 4.6 长审查返回服务端无容量。经用户同意切换后，`--backend antigravity` 实际使用 Claude Sonnet 4.6（Thinking）；初审 `84/100 REQUEST_CHANGES`，收紧后针对性复审 `READY 96/100`。代码完成后仍须对真实 diff 再做双模型终审。
