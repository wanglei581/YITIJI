...
> **状态（2026-07-17）：Slice 1 已由 PR #275 squash 合入 `main@0ae51289`，合并前与合并后 `build-and-verify`、`postgres-readiness` 均成功；未部署。Slice 2 仍为方案，不含运行时代码。**
>
...
- `services/api/src/member-privacy/member-data-request.service.ts` 已负责 export/revoke_consent；`delete` 在访问 Prisma、Redis、BullMQ、文件、审计、账户状态或 step-up 前稳定返回 `ACCOUNT_CLOSURE_NOT_AVAILABLE`，不会落账本。
...
- 双 Prisma schema 的 `UserDataRequest` 已有 idempotency、activeKey、executionVersion、workerJobId、exportFileId、expiry 与失败字段；文件、审计、BullMQ、Redis 均已有可复用基础。
...
1. 在 `AuditService` 新增 `writeRequired(tx, args)`：复用现有 payload 限长/安全序列化，但使用调用方 transaction client，写失败必须抛出。旧 `write()` 保持兼容；grant 成功后的队列入列确认、Admin decision/retry、artifact ready/expired/cleanup 统一改为必需审计。grant 消费前创建、且在 grant 失败时会物理删除的暂存 request 不写 required audit，避免留下审计孤儿。
...
**2026-07-17 Slice 2 强制修订：** 本切片必须同时落地 `pending/handling` 的恢复 sweep，不能整体推迟到 Slice 3。worker 在 `pending -> handling` 的 CAS 中预留 `FileObject(status='uploading')` 与 `exportFileId`，使 put 后崩溃可由账本确定性定位并物理清理半成品；恢复 sweep 必须覆盖「grant 已消费但 enqueue 前崩溃」与「put 成功但 ready transaction 前崩溃」两类窗口，并以相同 `${requestId}:${executionVersion}` job id 幂等重投。`ready` 审计必须与 FileObject 激活、账本 ready CAS 在同一 Prisma transaction 成功；审计或账本写失败时不得保留 `ready` 假象。详见 `docs/superpowers/plans/2026-07-17-user-center-wave1b-slice2-export-artifact.md`。
