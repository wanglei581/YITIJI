...
- [ ] **Step 1: 在核销 verify 中暂停提交后的 `benefit.redeem` 审计**
...
- [ ] **Step 1: 对齐付款码支付已使用的 reservation 模式**

在 `createPayAttempt` 的未完成尝试检查后、`paymentAttempt.create` 与 `provider.createQrPayment` 之前，执行：
...
仅在本地 `paymentAttempt.create` 失败且尚未触达 Provider 时释放该 reservation；Provider 调用后任何异常保持 `created/pending + paying` 并交由既有惰性过期/查单收敛，不得回退为可核销状态。删除旧的“Provider 返回后才 `unpaid → paying` 且忽略 CAS 结果”写法。
