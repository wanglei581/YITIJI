...
- [x] **Step 4: 再跑 RED 守卫，确认失败只来自尚未迁移的三页**
...
- [x] **Step 2: 在不改对话状态机的前提下替换 JSX 骨架**
...
- [x] **Step 1: 先把两条 Profile 守卫改成目标合同并确认 RED**
...
- [x] **Step 3: 保留 ProfileHeader 真实数据，替换视觉骨架**
...
- [x] **Step 4: 将 969 行 Profile CSS 变为本地聚合入口并拆分**
...
> 当前状态（2026-07-14）：未完成。本地预览浏览器连接受当前环境的 `Cannot redefine property: process` 阻塞；需用户在 Firefox 的 5174 本地预览手工目检上述三个视口，不能以静态或构建结果替代浏览器验收。
...
> 当前状态（2026-07-14）：双模型审查已完成。Antigravity 与 Claude 均为 `APPROVE`，Critical=0、Warning=0；仅有三项不阻塞 Info：首页 ripple 覆盖、权益活动同名、旧动画死 CSS，均列为后续可选项，本批不处理。候选提交尚未执行。
