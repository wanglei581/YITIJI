...
**Outcome:** 百宝箱应用从“管理员直接改终端 items”升级为“应用目录 + 审核版本 + host 白名单 + 终端发布投影”。
...
- `ToolboxAppVersion`
- `ToolboxAllowedHost`
- `ToolboxDeployment` 可先延后，短期继续投影到 `TerminalToolboxConfig.itemsJson`
...
- [ ] 提交人与审核人不能相同。
- [ ] host 过期、应用熔断、版本未审核、命中禁用词时不得发布。
- [ ] 所有审核、发布、下架、回滚写 AuditLog。
- [ ] Kiosk 公开配置只包含 published 且未熔断应用。
