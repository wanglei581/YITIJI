...
> 版本：2026-07-16 审阅稿
>
> 状态：方案已由 PR #259 进入正式基线；Wave 0（PR #261）、授权撤回终态保护（PR #263）、Wave 1-A 账户安全底座（PR #265）及追加安全加固（PR #270，`88e940cd`）均已合入 `origin/main`，但尚未部署。Wave 1-B 数据权利执行器、Wave 1-C Kiosk/Admin 隐私运营 UI、真实导出与不可逆注销执行仍未开始；注销继续受法务分类留存矩阵、冷静期与执行开关的 fail-closed 门禁约束。
>
