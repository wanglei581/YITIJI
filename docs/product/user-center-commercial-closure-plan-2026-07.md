...
> 版本：2026-07-16 审阅稿
>
> 状态：方案已由 PR #259 进入正式基线；Wave 0、授权撤回终态保护与 Wave 1-A 账户安全底座已合入 `origin/main@f69bf1b7`，但均未部署。Wave 1-B 数据权利执行器、Wave 1-C Kiosk/Admin 隐私运营 UI、真实导出与不可逆注销执行仍未开始；注销继续受法务分类留存矩阵、冷静期与执行开关的 fail-closed 门禁约束。

> 状态（2026-07-17，`origin/main@d4101fcc`）：方案已由 PR #259 进入正式基线；Wave 0（PR #261）、授权撤回终态保护（PR #263）、Wave 1-A 账户安全底座（PR #265）及追加安全加固（PR #270，`88e940cd`）均已合入主线，尚未部署。Wave 1-B Slice 1 已由 PR #275 合入 `main@0ae51289`；Slice 2 的恢复型导出方案已进入主线文档，但尚未有运行时代码合入。Wave 1-C Kiosk/Admin 隐私运营 UI、真实下载与不可逆注销执行仍未开始；注销继续受法务版本化分类留存矩阵、冷静期与执行开关的 fail-closed 门禁约束。
>
...
- 重复的第二个“权益活动”。
- 独立“身份切换”入口在最新 main 已不存在；用真相守卫禁止回归，设置页内真实“切换账号”操作继续保留。
- 不可用的邮箱登录方式。
...
1. 删除重复权益活动和邮箱按钮；最新 main 已无独立“身份切换”入口，只补防回归断言。
2. 隐藏扫码凭证、打印套餐、AI 套餐占位卡。
3. 同步清理设置页关于“身份切换”的遗留注释/文案，避免入口删除后口径漂移。
4. 保留当前二维码实现；最新 main 的 `verify:qr-login-ui` 已与现行文案一致并通过，本波只在干净基线复验，不制造无效改动。
...
3. **W1-C 异步导出**：worker 聚合、加密包、短期 FileObject、手机一次性应用 ticket、带租约的下载 claim、HTTP `finish` 后物理清理与过期 reconciler；不向客户端下发对象存储 URL。
