...
`contract-review` 自 2026-08-09 起仅保留为归档治理记录：用户入口固定在「AI简历服务 → 签约与权益」，名称为「AI签约风险提示」，不得再通过百宝箱发布投影形成重复入口。
...
- 法律风险、试卷打印涉及法律、版权和责任边界，必须先有免责声明、文件留存策略和审核门；合同风险能力已移出百宝箱，执行其独立发布门禁。
...
- 明确法律风险审查、试卷打印、英语模拟练习仍需法务、版权、品牌授权和真机验收，不进入低风险首批；`contract-review` 已归档并移出百宝箱投放范围。
...
- 预生产 TAS-G2 真实模型边界探针已通过但仍带注意事项；本阶段不代表真实 Kiosk 浏览器验收、公共终端隐私验收或微应用商用上线完成。
...
- 新增 `docs/acceptance/toolbox-ai-skill-real-acceptance.md`，把首批 AI skill 真实验收拆为 TAS-G0 至 TAS-G5：
  - TAS-G0：本地静态门禁。
  - TAS-G1：预生产只读预检。
  ...
  - TAS-G4：公共终端隐私与竞态验收。
  - TAS-G5：证据复核与上线阻断项。
- 新增 `docs/acceptance/toolbox-ai-skill-real-execution-record.md`，记录 TAS-G0 / TAS-G1 / TAS-G2 脱敏执行摘要；执行包创建时 TAS-G3 至 TAS-G5 继续保持 `PENDING`，本轮已补充 TAS-G3 / TAS-G4 PASS 和 TAS-G5 PASS_WITH_NOTES 的脱敏摘要。
- 新增 `verify:toolbox-ai-skill-real-acceptance`，检查执行包、执行记录、产品文档、进度文档、停止条件和过度宣称。

仍未完成、不得误宣称：

- 未用 Windows 一体机真机完成三类 skill 问答验收。
- 未完成 Windows 真机场景切换、真实触控、断网/重启恢复与隐私验收。
- 未完成正式自有域名 HTTPS、Windows 一体机或试运营验收。
- 未开放第三方 JS / WASM / 任意外部 skill 包执行。
- 下一步仅剩正式自有域名 HTTPS、Windows 一体机真机和试运营证据；正式商用前必须切换到自有域名 HTTPS，不能使用 `sslip.io` 作为品牌生产入口。
