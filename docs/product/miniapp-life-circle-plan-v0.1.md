# 微信小程序 · 「职业生活圈」规划 v1.0

> **方案快照（2026-09-26 标注）：** 本文写于 （2026-09），功能实现以代码为准；与 2026-09-26「设备与软件供应方、托管 a」口径冲突的地方，以 docs/product/feature-scope.md 为准。

> **状态**：v1.0，产品负责人 2026-09-05 拍板「同意生活圈换形状」。取代 v0.1 草案（v0.1 从未定版，
> 其提出的 5 个页面 / 3 个 API / 1 张表**均未实施**，本次改写不删除任何代码）。文件名保留以维持既有引用。
> **范围**：仅 `apps/miniapp` 的「职业生活圈」Tab（`pages/ai`）内容方向。不新增底部 Tab，不改首页 / 求职 / 我的。
> **从属**：[feature-scope.md](./feature-scope.md)、[compliance-boundary.md](../compliance/compliance-boundary.md)、
> [audience-growth-space.md](./audience-growth-space.md)。冲突时它们赢。
> **性质**：这是方向与范围，不是实施。每个模块动工前按 `CLAUDE.md §8.1` 另写任务声明。排期在 #756 合入 + E2 真机走查之后。

---

## 0. 决策记录

| 日期 | 决定 | 依据 |
|---|---|---|
| 2026-09-04 | 第 14 轮评审判定 v0.1（学习视频 / 福利中心 / 邀请有礼 / 每日签到）是贴皮：四样里没有一样从「求职 + 打印 + 人社场景」长出来，换成任何 App 都能贴 | 合成页「求职材料服务网」问三 |
| 2026-09-05 | 替代方案逐个对代码核对象与出口（见 `next-tasks.md` 生活圈判定表） | 代码核 |
| 2026-09-05 | 产品负责人：**同意换形状** —— 从「内容 + 裂变」换成「出门前 + 去窗口前」 | 本文 |
| 2026-09-05 | 产品负责人补充：面试前的准备（仪容仪表、服装穿着、面试过程）纳入生态圈 → 定为模块 A′「面试出门清单」，与招聘会出门清单并列 | 本文 §3.1′ |
| 2026-09-05 | 第 16 轮评审：「四个先做」是没有顺序 → 改为 A′ 等待期可做、材料包合入后第一刀、C 随时 sidecar、A / B 等目录与供数；C 误砍第四种活动类型，改正 | 本文 §2 |

**一句话**：生活 = 求职 / 办事这件事在下班后和出门前的那几小时，不是用户生活的全部。

---

## 1. 判定标准（四条全过才进本 Tab）

1. **同一周、同一场求职或办事。** 招聘会前夜、去窗口前、合同不敢签、家里人要打印证明。不是「用户也要娱乐」。
2. **消耗我们已有的对象。** 简历版本、到机码、订单、已审核政策、场馆、合同任务、设备在线。不新开内容品类。
3. **出口是我们能履约的。** 出纸、扫描、来源平台跳转、到机核销。不是播放完成、签到成功、积分到账。
4. **不做第二品类 App。** 不做社交、课表、商城、出行、通用福利。人社场地也讲不清为什么要它。

---

## 2. 处置清单

| v0.1 模块 | 处置 | 理由 |
|---|---|---|
| 📹 学习视频 | **撤** | 官方公开课也是内容产品，播放完成不是我们的履约（违反标准 2、3） |
| 🔗 邀请有礼 | **撤** | 增长黑客挂在求职终端上，场地甲方无法解释；不发奖励的邀请码没有对象也没有出口（违反 2、3、4） |
| 📅 每日签到 | **撤** | 草案自己写成纯预告、不发奖励 —— 承认没有对象没有出口。留入口等于教用户我们会做游戏（违反 2、3） |
| 🎁 福利中心 | **留，收窄** | 券只在核销到打印页 / AI 次数 / 材料包时才是我们的履约（见 §3.3） |
| 职业圈动态 / 今日早报（现网 pending） | **保持未开放** | 无接口；且社交 feed 与资讯早报不消耗打印 / 政策 / 到机码。早报只有改成「你收藏的那场会明天开始、你的到机码 4 小时过期」才过标准 —— 那已是「出门清单」 |

**新增（按能不能真做排序）**

| 模块 | 对象（已有？） | 出口（已有？） | 判定 |
|---|---|---|---|
| A · 招聘会出门清单 | 逛展计划 ✓ | 「生成准备单去打印」✓ 已在 `fair-visit-plan.js` | **材料包最小形态之后；生产有场次再做**（到机码行依赖材料包，招聘会目前 0 场） |
| A′ · 面试出门清单 | 模拟面试报告 `checklist` ✓（后端 LLM 已生成、PDF 第十节已打印）、一体机 `/interview/tips` 六条静态 ✓、目标岗位 ✓ | 报告 PDF 出纸 ✓ | **等待期可做**（过 `next-tasks.md` 三条测试；零新页零新 API，纯合成；不做凭证型提示词变体） |
| B · 窗口材料单 | 政策核对 ✓（kiosk `EligibilityPanel` + miniapp `policy-check`） | 出纸 ✓ | **有政策库存再做，不上代码充数**（生产政策 0 条） |
| C · 福利中心（收窄） | `BenefitActivity` / `RedemptionRecord` ✓ | 券核销到 Order / AI 配额 ✓（C5-4 设计） | **随时 sidecar**：只收文案（禁异业、禁行为换券），**四种活动类型全部保留**（含 `subsidy_eligibility_hint`） |
| D · 家人代出示到机码 | 到机码 ✓ | 出纸 ✓ | **后置**：委托模型全仓 0 处，且是隐私面，须过合规评审 |
| ~~E · 证件到期雷达~~ | 无用户证件模型 | 无（订阅消息 0） | **撤或后置**：两头都没有 |

---

## 3. 先做的三个模块

### 3.1 A · 招聘会出门清单

**用户在什么时候用**：明天 9 点要出门去招聘会。

**内容**（全部是已有对象的聚合，不生成新内容）：
- 我的逛展计划（`fair-visit-plan` 已有）—— 目标展位、顺序、要问的问题
- 简历带几份（从「我的简历」取最近版本 + 用户填份数）
- 到机码（**依赖材料包立项**；材料包上线前此行显示「到场后在一体机打印」而不是空格子）
- 哪台一体机在会场 / 在线（`GET /terminals/public` 已有；生产终端 0 台时显示诚实空态）
- 出口：**「生成准备单去打印」**（已有）→ 打印链

**不做**：出门前提醒。它依赖微信订阅消息（全仓当前 0 处），那是独立立项（要微信后台模板 + 服务端发送），
不在小程序 lane。v1 出门清单**没有提醒**，用户自己打开看。写清楚，别让入口暗示会响。

**页面**：不新建页。在 `pages/fair-visit-plan` 顶部加「出门清单」聚合区，或在 `pages/ai` 的生活圈区加一张
「最近的招聘会」卡直达它。**二选一，动工时按 §8.1 定，禁止两处都加**（同义入口，违反 `user-data-flow-matrix.md`）。

**API**：无新增。复用 `fair-visit-plan`、`resumes`、`terminals/public`；到机码那行等材料包端点。

**合规**：招聘会仍只做第三方 / 官方来源入口；清单里的「去预约」仍是「去来源平台预约 / 扫码预约」。

### 3.1′ A′ · 面试出门清单

**用户在什么时候用**：明天要去面试，今晚看一眼：穿什么、带什么、几点到、怎么走、他们可能问什么。

**已经在的三处（本模块是合成，不是新建）**：
- 后端 `mock-interview-llm.service.ts` 的报告已生成 `checklist`（提示词：「面试前准备清单 5–8 条：公司岗位调研 / 自我介绍 /
  代表性经历 / 反问问题 / 材料 / 设备路线」），`interview-report-pdf.service.ts` 第十节「面试前准备清单」已出纸
- 一体机 `InterviewTipsPage.tsx` 六条静态：背景调研 / 岗位匹配 / 基础演练 / **形象着装**（按公司性质：互联网偏休闲、金融国企偏正装）/
  面试材料（可在本终端打印简历）/ 面试安排（时间地点路线）
- 小程序 `pages/interview-result` 已渲染报告 `checklist`

**合成内容**（一页，可打印）：
1. 报告里的个性化清单（已有）
2. 着装一句 —— **只按目标岗位 / 公司性质给一句话**（复用一体机静态六条里「形象着装」的口径），不展开
3. 带什么 —— 简历几份（到机码，**依赖材料包立项**；上线前显示「到场后在一体机打印」）、证件、作品集
4. 几点到怎么走 —— **岗位模型只有 `city`，没有街道地址**（`address` 只在招聘会上）。此行只写「城市 + 面试地点以来源平台通知为准」，**不做导航、不显示假地址**（`store-select` 的死导航教训在前）。若将来岗位源提供地址再补，不在本模块范围
5. 他们可能问什么 —— 报告里的高频问题（已有）

**出口**：报告 PDF 已有打印链 → 加「面试出门清单」为报告的附页，或独立一页；小程序看、一体机打。

**两条边界（写死）**：
- 着装只做「按公司性质一句话」。**不推商品、不做穿搭内容、不导电商、不放图片流**。一放就违反 §1 第 4 条，且是「导流」。
- `hall_paper` 的面试不是 STAR。进厂 / 保洁 / 安保面试问的是能否倒班、有无工伤病史、健康证带没带。
  现有提示词是 `phone_digital` 形状；当目标岗位是凭证型时，checklist 提示词须切换为「证件 / 体检 / 工作服 / 倒班」变体。
  这是提示词分支，不是新模型。见 `audience-growth-space.md §1`。

**页面**：不新建。`pages/interview-result` 加「生成出门清单」动作（对已有报告），或 `pages/ai` 生活圈区加
「最近的面试」卡直达。**与 A 同一规则：二选一，禁止两处都加。**

**API**：无新增。报告 `checklist` 与 `predictedQuestions` 已在；着装一句按公司性质取（一体机 `InterviewTipsPage` 的六条目前只在 kiosk `.tsx`，小程序用须抽到 `packages/shared` 或复制一份常量 —— 动工时定，不算 API）；城市从岗位取，无地址。
凭证型提示词变体是后端 `mock-interview-llm.service.ts` 内的分支，动工时按 §8.1 声明。

**合规**：面试练习只做备考侧；不做实时提词（已定性为作弊，第 13 轮调研）；报告只给本人；不回传企业。

### 3.2 B · 窗口材料单

**用户在什么时候用**：明天去人社窗口办事，该带什么。

**内容**：
- 政策核对的结果（`policy-check` 已有，**资格判定继续走确定性引擎，不换 LLM**）
- 把「符合」的政策正文收成材料清单 + 官方办理码（AI 只做「把已发布政策正文收成清单」这一步）
- 出口：小程序行前看；**一体机当场打**（打印链已有）

**空库纪律**：库里没有政策时先探空、不收身份证、不展示假清单。生产政策当前 0 条 —— 此模块上线前提是内容库存，
不是代码。

**页面**：`pages/policy-check` 结果页加「生成材料单」动作；不新建页。

**API**：可能需要一个小端点把政策正文 → 材料清单（服务端做，避免前端自行解析政策文本漂移）。动工时按 §8.1 声明，
跨 `services/api`。

**合规**：补贴只做指引，不承诺到账、不代申请；材料单上的码必须是官方源。`compliance-boundary.md` §8.2。

### 3.3 C · 福利中心（收窄）

**现状**：`membership` 页已调 `getMyBenefits`；活动类型 `BENEFIT_ACTIVITY_TYPES` 是**四种**：`coupon | free_quota | package_entitlement | subsidy_eligibility_hint`
（第四种是 info-only 政策资格提示，`schema.prisma:2000`）；`RedemptionRecord.kind` 是前三种的可核销子集；`orderId` 回填抵扣是 C5-4 设计。

**收窄规则（写进 Admin 与合规文案）**：**四种活动类型全部保留** —— 打印券（`coupon`）、AI 次卡（`free_quota`）、材料包权益（`package_entitlement`）、
政策资格提示（`subsidy_eligibility_hint`，info-only，是窗口材料单的亲戚，**不许砍**）。**收窄的是文案**：不得出现电影票、外卖、打车、
异业券、现金红包；不得以签到 / 邀请等行为换券。（v1.0 初稿误写「只允许三类」，第 16 轮指出后改正。）

**要做的**：
- Admin 创建福利活动时类型仍是四选一；文案门禁加禁词（外卖 / 打车 / 电影 / 红包 / 异业）
- 小程序 `membership` 页展示时按 `kind` 分三组，每张券写清「在哪核销」（打印页 / AI 服务 / 材料包）
- 不做：签到送券、邀请送券、任何以行为换券的机制

**API**：无新增。

---

## 4. 后置与撤销的说明

**D · 家人代出示到机码**（后置）。到机码的分享 / 委托模型全仓 0 处，且是隐私面（本人资产不对代办者展开简历原文）。
生产终端 0 台、无大厅观察前只留这一行；详见 `audience-growth-space.md §2.1`，不在两处各写设计。

**E · 证件到期雷达**（撤或后置）。没有用户证件模型（`validUntil` 只在机构资质 `QualificationRecord` 上），
没有订阅消息出口。两头都没有，做出来就是又一条断头路。若将来做，两个前置都不在小程序 lane。

**视频 / 邀请 / 签到**（撤）。见 §2。v0.1 §2.1 / §2.3 / §2.4 及其 API、表、页面规划**全部作废**，未实施故无需回滚。

---

## 5. 文件影响（方向层面，动工时按 §8.1 精确声明）

| 模块 | 小程序 | 后端 | 数据库 |
|---|---|---|---|
| A 出门清单 | `pages/fair-visit-plan` 或 `pages/ai` 加聚合区（二选一） | 无 | 无 |
| A′ 面试出门清单 | `pages/interview-result` 加动作，或 `pages/ai` 加卡（二选一） | `mock-interview-llm.service.ts` 凭证型提示词分支 | 无 |
| B 窗口材料单 | `pages/policy-check` 加动作 | 可能 1 个「政策正文 → 材料清单」端点 | 无 |
| C 福利收窄 | `pages/membership` 分组展示 | Admin 侧 `kind` 白名单 + 文案禁词 | 无（枚举已存在） |

**不动**：`app.json` 不新增页面（v0.1 要加 5 页，本版 0 页）；Kiosk / Partner 不动；`packages/shared` 不动。

---

## 6. 验证

- 静态：`verify-miniapp-static` 现 112 条须保持；福利文案禁词进 `verify:compliance-copy`
- 运行时：开发者工具走 A / B 的生成 → 打印链；C 的三组分类展示
- **真机**：与 E2 同批，不单独开

---

## 7. 合规审查

| 模块 | 检查 | 结论 |
|---|---|---|
| A 出门清单 | 只聚合本人已有对象；招聘会仍是来源入口；不预约、不签到 | ✅ |
| A′ 面试出门清单 | 着装只一句不导流不放图；备考侧不提词；报告只给本人 | ✅ |
| B 窗口材料单 | 资格判定确定性引擎；只指引不代办不承诺到账；空库不收身份证 | ✅（内容库存是前提） |
| C 福利收窄 | 三类 `kind` 白名单；无行为换券；无异业 | ✅ |
| D 家人代出示 | 隐私面，须评审后才动 | ⏸ |
| 整体 | 未触碰岗位投递 / 简历给企业 / 硬件链路 | ✅ |

---

## 附录：v0.1 → v1.0 变更摘要

| 项 | v0.1 | v1.0 |
|---|---|---|
| 形状 | 内容 + 裂变（视频 / 福利 / 邀请 / 签到） | 出门前 + 去窗口前（招聘会出门清单 / 面试出门清单 / 材料单 / 福利收窄） |
| 新页面 | 5 | 0 |
| 新 API | 3 | 0～1 |
| 新表 | 1（邀请关系） | 0 |
| 判定标准 | 无 | 四条（§1） |
| 顶部 Tab 名 | 「保持 AI百宝箱」（已过期，Tab 现为「职业生活圈」） | 不改 |
