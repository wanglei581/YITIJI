...
> 同日晚产品负责人拍板托管选 a：**我方云上不存岗位、招聘会、企业资料**，机构在我方云上只维护官方渠道二维码与政策；完整招聘功能只在客户私有化部署（b）里由客户运营。
...
- 扫码跳走第三方平台投递 / 预约
- 上传简历 / 扫描纸质简历 → AI 诊断 / 优化 → 打印（未登录/证件/系统文件短期清理；登录会员简历按 90 天 / 180 天 / 成果物长期保存规则管理）
- 求职信 / 推荐信 / 简历模板下载 + 打印
...
1. **Kiosk 简历上传页** → 绿色"隐私保护"横幅 → "未登录和证件类短期自动清理；登录后默认保存 90 天，可在我的文档调整期限"
2. **Kiosk 招聘列表** → 橙色 → "投递请前往来源平台办理"
3. **CTA 按钮** → 全场"查看岗位 / 扫码投递",**无"一键投递"**
4. **Admin 岗位信息源** → 蓝色 → "未取得人力资源服务许可证,禁止设计一键投递"(法务最爱)
5. **Admin 文件管理** → 展示保存策略、到期时间、长期保存和待清理统计 → 红色"强制清理过期"
...
**Q3:"你们存了简历 PDF,算什么?"**
A:未登录上传和证件/系统处理文件按短期策略自动清理；登录会员上传的原始简历默认保存 90 天，可由本人延长至 180 天，优化后或派生成果物可在确认保存条款后长期保存。所有文件只服务本人查看、下载、打印，访问使用短期签名 URL；管理员运维访问必须落审计。我方不向企业或合作机构提供简历，不做候选人筛选、面试邀约或 Offer 管理。
