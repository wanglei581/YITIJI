...
智慧校园是一条**贯穿 4 个端**的链路，不是单端功能。各端只做自己该做的：

```
┌──────────────────────────────────────────────────────────────┐
│  一体机前台 Kiosk        —— 学生 / 家长看到的展示面             │
│   · 迎新 / 大数据 / 行李 / 全景 展示                            │
│   · 读「开关」决定模块显隐（出校园即整张隐藏）                  │
│   · 走免鉴权读端点；返回体不含学生明细                          │
└──────────────▲───────────────────────────────┬───────────────┘
               │ 读 {enabled, modules}          │ 取已审核聚合 / 外链
               │                                 ▼
┌──────────────┴────────────────────────────────────────────────┐
│  后端 services/api + DB  —— 单一数据源                          │
│   · TerminalSmartCampusConfig(开关) / CampusStatistic(聚合)     │
│   · Terminal.orgId(机构归属)                                    │
│   · 免鉴权读端点(白名单) + 鉴权管理端点(service 层 orgId 隔离)  │
└──────▲─────────────────────────────────────▲──────────────────┘
       │ 学校配置 / 录入(仅本校)              │ 平台审核 / 归属 / 代配
       │                                      │
┌──────┴───────────────────┐     ┌────────────┴───────────────────┐
│ 合作机构后台 Partner       │     │ 管理员后台 Admin                 │
│ （学校账号，按 orgId 隔离）│     │ （平台运营，全局视角）           │
│ · 终端开关(本校)           │     │ · 内容审核 / 终端归属指派         │
│ · 迎新内容录入             │     │ · 凭证 / 审计 / k-匿名阈值兜底    │
│ · 校园大数据录入           │     │ · 一期开关由运营在此代配置        │
│ · 看本校使用统计           │     │                                  │
└────────────────────────────┘     └──────────────────────────────────┘

Terminal Agent：不参与智慧校园（除打印复用现有链路）
```

| 端 | 承担 | 为什么放这里 |
|----|------|--------------|
| 一体机前台 Kiosk | 学生侧展示 + 按开关显隐 | 产品对用户的唯一触点；"出校园即消失"靠 Kiosk 读开关 |
| 合作机构后台 Partner | 学校自管：开关 / 迎新 / 大数据录入 / 使用统计 | 学校是外部机构，partner 后台天生按 `orgId` 隔离，只看本校 |
| 管理员后台 Admin | 平台监管：审核 / 归属 / 凭证 / 审计；一期代配开关 | admin 是平台全局控制台；**不给学校开 admin 账号** |
| 后端 services/api + DB | 开关/聚合数据、归属、免鉴权读端点 + 鉴权管理端点 | 单一数据源，三个前端围着它转 |
| Terminal Agent | 不参与 | 智慧校园无新硬件交互 |

> 详细：后台功能区见 §九；账号归属 / 菜单显隐 / 数据隔离的实现判定见 §十三；终端展示边界见 §八。
