...
- ~~`/print/params`：零运行时导航~~ ✅ **已收口（2026-08-18）**：该页下线为指向 `/print/preview` 的兼容重定向，页面本体（462 行）删除，七步条收成六步（`上传/材料检查/预览/确认/支付/打印`）。
  **关于「独占真实估价能力」的口径修正**：本条此前判定必须先把估价迁进 preview 再删，复核后**不迁**。理由是预览页 `PrintPreviewPage` 的「费用说明」卡自己写着「本页只设置打印参数与估算用纸……**不在此页展示本地估算金额，避免与最终计费不一致**」——把本地估价搬进预览页会直接推翻该页既定口径。而应付金额的真值本来就在确认页：`PrintConfirmPage` 走 `POST /orders/quote` 由服务端出价。params 页那三行只是「预览之后、确认之前」的第二个本地估价点，删掉它等于价格真相源更单一，没有任何用户可达能力丢失（该页零运行时导航，用户本来就到不了）。`verify-price-single-source` 已相应改为守「预览页不得自行本地估价」。
...
| ~~**P2**~~ ✅ | ~~迁移 `/print/params` 估价能力进 preview 后删除~~ 已于 2026-08-18 下线为兼容重定向（估价不迁，理由见上）；合并 `/resume` 重复重定向仍待办 | 中，动主链路，须补测试 |
