...
| AI简历优化 | `/resume/source?intent=optimize` → `/resume/optimize` | `AiResumeResult(kind=optimize)`、优化版简历/PDF FileObject | 我的简历、我的文档、AI服务记录、打印订单 | 部分打通：kind=optimize 进 AI记录，导出 PDF 进我的文档；Kiosk「我的简历」独立页待恢复，`resume_upload` 1h TTL 短留存属隐私设计而非缺口 | 新增 `/me/resumes` 后只展示安全元数据和下一步动作；保持事实校验、防编造、PDF/打印链路 |
