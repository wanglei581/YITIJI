...
| 岗位大师 | 首页既有卡片 → `/resume/job-fit`（无简历时由既有页面引导上传诊断） | 复用 2D 岗位匹配参考、定向优化建议与报告打印 | AI服务记录、我的文档、打印订单 | 🔶 PR #200 已合入 `main` 且仅复用 2D；未新增路由、记录类型或同义入口 | 保持唯一入口；尚未部署预生产，不得宣称已完成预生产 / 真机验收 |
...
合规口径：岗位链路只能是「查看 → 收藏 → 匹配参考 → 去来源平台投递/扫码投递 →（可选）本人回来记录一次投递」，不得出现平台内投递，也不得出现由平台或第三方写入的投递结果。

> 末段那一步是 2026-09-02 经产品负责人具名授权新增的：用户**本人自填**的求职进度（独立模型 `JobApplication`，尚未实现）。它与「投递结果记录」的区别是写入方 —— 本人手填是个人求职记事本，第三方回流才是招聘闭环数据。`BrowseLog` / `ExternalJumpLog` / `Favorite` **不得**因此扩展任何状态字段，上表「不记录投递结果」保持不变。六条硬边界见 [compliance-boundary.md §4.4A](../compliance/compliance-boundary.md)。
