...
| 纸质扫描 | `/scan/start` | 扫描 PDF/图片 FileObject | 我的文档 | ❌ 未打通（核查 2026-06-12：整条链路为前端流程演示——定时器+假文件元数据，页面已诚实标注；不产生真实 FileObject） | 真实扫描依赖 Terminal Agent 扫描扩展与真机验收，打通前扫描结果不得进「我的」（**注**：本仓库另有 2026-07-11 的纸质扫描状态更正，随不同分支收口，不在本分支范围内，见 `docs/reviews/home-entry-closure-plan-2026-07-11.md`） |
| 云打印 | 当前首页已有，占位 | FileObject、PrintTask | 我的文档、打印订单 | 未打通 | 云端任务队列 → Windows Agent claim，本机驱动打印（**注**：本仓库另有 2026-07-11 的云打印范围澄清备注，随不同分支收口，不在本分支范围内，见 `docs/reviews/home-entry-closure-plan-2026-07-11.md`） |
