...
>
> **托管 a（2026-09-26 标注）：** 我们云上不存岗位、招聘会、企业资料。下表的岗位（全职 / 实习 / 兼职 / 全部）、找企业，以及收藏与浏览记录里的岗位和招聘会，按 next-tasks 3.13、3.14 在我们云上关闭，代码保留给私有化部署（b）；岗位匹配改为只接用户自填的岗位要求（3.14）。表中「已打通状态」记的是代码实现事实，不代表我们云上继续开放；切换完成前代码行为不变。
...
| 岗位大师 / 岗位匹配 | 作为既有入口真实化；3.14 后只接用户自填岗位要求，对照本人简历 | 新增「目标岗位分析」同义入口 |
...
| 岗位匹配（2D；诊断报告 CTA 与首页既有「岗位大师」同指向，不新增路由） | `/resume/report` →「目标岗位匹配参考」→ `/resume/job-fit`；首页「岗位大师」→同一路由 | `AiResumeResult(kind=job_fit)`、关联 `JobAiSession(operation=match)`、报告 PDF `FileObject`、后续 `PrintTask` | AI服务记录（仅展示一条 `job_fit` 成功结果；同任务已完成 match 会话去重）、我的文档、打印订单 | 🔶 2026-07-12 PR #200 已合入 `main` 且 CI 通过：已复用 consent / 配额 / 会话 / 用量治理，结果页只展示真实摘要、匹配依据 / 可选关键词、差距与简历改写；内部 HMAC `printFileUrl` 进入既有打印确认，且仅「查看岗位」到 `/jobs/:id` | 尚未部署预生产；仍需真实会员、LLM、受控无出纸打印和 Windows 真机验收；3.14：只接用户自填岗位要求，去掉「查看岗位」到 `/jobs/:id` |
...
| 已有入口 | 当前路由/状态 | 产生数据 | 「我的」归属 | 已打通状态 | 缺口/下一步 |
|---|---|---|---|---|---|
| 全职岗位 | `/jobs?category=fulltime` | 浏览记录、收藏、外部跳转记录 | 我的收藏、浏览与跳转记录 | ✅ 已打通（2026-06-12：`BrowseLog(targetType=job)` + `ExternalJumpLog(action=external_apply)`；详情加载后记录浏览，打开「去来源平台投递 / 扫码投递」入口时记录外部跳转） | 托管 a：按 3.13 在我们云上关闭（代码保留给 b）；保持来源平台边界；不记录投递结果 |
...
| 岗位大师 | 首页既有卡片 → `/resume/job-fit`（无简历时由既有页面引导上传诊断） | 复用 2D 岗位匹配参考、定向优化建议与报告打印 | AI服务记录、我的文档、打印订单 | 🔶 PR #200 已合入 `main` 且仅复用 2D；未新增路由、记录类型或同义入口 | 保持唯一入口；3.14 后只接用户自填岗位要求；尚未部署预生产，不得宣称已完成预生产 / 真机验收 |
| 找企业 / 企业展示（岗位信息页内入口，非新增首页卡片） | `/jobs` 页内入口 → `/companies` → `/companies/:id` | 企业浏览记录、企业来源页跳转记录、经企业进入岗位的浏览/投递入口记录 | 我的·浏览与跳转记录（企业 Tab） | ✅ 已打通（2026-06-12：CompanyProfile 真实数据底座；列表/统计/筛选全真实聚合；详情指标受 Admin 开关控制；岗位联动既有 /jobs/:id 与来源投递链路） | 托管 a：按 3.13 在我们云上关闭（代码保留给 b）；定位「来源企业导览」非招聘平台；不收简历、无平台内投递（长期红线见 compliance §4.5） |
