...
> 最后更新：2026-07-16（对齐用户中心 Wave 0 本地候选：Profile 只保留 22 个已接真目的地；隐藏套餐、凭证等未开放入口；邮箱登录不再展示；数据导出/注销没有真实执行器时必须 fail closed）
...
> 2026-07-16 Wave 0 本地候选复核：Profile 固定为 22 个已接真目的地，删除重复招聘会活动入口并隐藏招聘会扫码凭证、求职打印套餐、AI 服务套餐；登录页只保留手机与扫码；账号设置明确换绑、注销、数据导出尚未开放。后端在真实导出/注销执行器上线前拒绝把 `export/delete` 工单标为 `completed`，也拒绝用普通 `rejected` 结束 `delete`，且失败尝试不改变状态、不写完成审计；`revoke_consent` 继续可同步完成。SQLite 57 个正式 migration 与 PostgreSQL 29 个正式 migration 均在一次性空库通过，临时库已删除；详细证据见 [user-center-wave0-acceptance.md](../acceptance/user-center-wave0-acceptance.md)。该结果仅属于本地分支，未推送、合并、部署或运行 GitHub CI。
