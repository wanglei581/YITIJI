...
## 附：岗位大师 M1 入口登记（2026-07-02，`feature/job-master`，尚未合入 `main`）

- **入口**：首页「岗位信息」组「岗位大师」磁贴（点亮既有 `disabled` 占位，**非新增同义入口**）→ `/jobs/master`。与 `/resume/job-fit`（AI 简历服务内的岗位匹配能力）能力有交叠但定位不同：岗位大师是「岗位组决策台」（适配度＋薪资＋晋升路径＋风险＋可打印决策报告），job-fit 是简历服务内的定向匹配。M2 完成岗位详情页「分析这个岗位」入口改线到 `/jobs/master?jobId=` 前，两入口并存需复查本矩阵不堆同义入口。
- **操作**：选站内已发布岗位或手填目标岗位 → 单次分析 → 竖屏四段结果卡 → 打印《岗位决策参考报告》。
- **产生数据 ↔ 归属**：`AiResumeResult(kind='job_master')` → AI服务记录分类（承载于「我的」AI记录概览「岗位决策分析」条目与简历/AI服务相关页）；报告 PDF（`purpose='print_doc'`, `createdBy='job_master'`）→ 我的文档；打印 → 打印订单。均按本人（会员 `endUserId` / 匿名 `accessToken`）归属，24h TTL；删除走既有硬删链路。
- **合规**：适配度仅三档参考等级（无百分比/录用概率）；薪资只透传来源方文本（`Job.salary`），缺失显示「来源平台未提供」；投递只引导「去来源平台投递」；LLM 失败诚实提示不产假结果。
- **M1.5「决策台深化」（2026-07-03，`feature/job-master`，尚未合入 `main`）**：仅**加深既有结果**，**入口不变、数据归属不变、AI 记录类型不变**（仍 `AiResumeResult(kind='job_master')`，**不新增入口 / 同义卡片 / 新 AI 记录类型 / 新数据模型**）。结果视图由四段扩为决策台分段：决策摘要条、技能地图＋关键词覆盖（命中/待补，**无比例**）、差距行动（学习方向＋第一步）、面试准备参考（问题＋为什么问＋准备，不承诺通过）、简历改写要点、晋升路径（依据＋第一步）、薪资、风险、底部操作；PDF 同步新增面试准备/简历改写/关键词覆盖区块。合规红线不变（三档参考、无百分比/录用概率/通过率、手填薪资「来源平台未提供」、只引导「去来源平台投递」、evidence 防编造）。真实 DeepSeek 匿名＋会员双路径已本地验证归属一致（`/me/ai-records` 见 job_master、`/me/documents` 见岗位决策参考报告）；**1080×1920 竖屏 UI 截图待补**。M2 才做：与 `JobAiSession` 入口收敛、岗位详情页「分析这个岗位」改线、多岗对比、站内薪资统计、找企业。
