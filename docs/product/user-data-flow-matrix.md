...
> 最后更新：2026-07-16（Wave 0 基线已由 PR #261 合入主线：Profile 只保留 22 个已接真目的地；隐藏套餐、凭证等未开放入口；邮箱登录不再展示；数据导出/注销没有真实执行器时必须 fail closed）
...
- 简历 / AI 简历记录 → 简历服务相关页面
- 文档 / 打印订单 → 打印扫描相关页面
- 收藏 / 浏览 / 外部跳转记录 → 岗位、招聘会（按 3.14 在两端隐藏）、政策等对应页面
- 权益 / 套餐 → 权益活动 / 服务套餐页面
...
| 岗位大师 / 岗位匹配 | 用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位 | 新增「目标岗位分析」同义入口 |
...
| 岗位匹配（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位。诊断报告 CTA 与首页既有「岗位大师」同指向，不新增路由） | `/resume/report` →「目标岗位匹配参考」→ `/resume/job-fit`；首页「岗位大师」→同一路由 | `AiResumeResult(kind=job_fit)`、关联 `JobAiSession(operation=match)`、报告 PDF `FileObject`、后续 `PrintTask` | AI服务记录（仅展示一条 `job_fit` 成功结果；同任务已完成 match 会话去重）、我的文档、打印订单 | 🔶 2026-07-12 PR #200 已合入 `main` 且 CI 通过：已复用 consent / 配额 / 会话 / 用量治理，结果页只展示真实摘要、匹配依据 / 可选关键词、差距与简历改写；内部 HMAC `printFileUrl` 进入既有打印确认。用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位 | 尚未部署预生产；仍需真实会员、LLM、受控无出纸打印和 Windows 真机验收 |
...
| 已有入口 | 当前路由/状态 | 产生数据 | 「我的」归属 | 已打通状态 | 缺口/下一步 |
|---|---|---|---|---|---|
| 全职岗位 | `/jobs?category=fulltime`。我们云上暂停（托管 a），b 版本可恢复 | 浏览记录、收藏、外部跳转记录 | 我的收藏、浏览与跳转记录 | 我们云上暂停（托管 a），b 版本可恢复 | 我们云上暂停（托管 a），b 版本可恢复 |
| 实习岗位 | `/jobs?category=intern`。我们云上暂停（托管 a），b 版本可恢复 | 浏览记录、收藏、外部跳转记录 | 我的收藏、浏览与跳转记录 | 我们云上暂停（托管 a），b 版本可恢复 | 我们云上暂停（托管 a），b 版本可恢复 |
| 兼职信息 | `/jobs?category=parttime`。我们云上暂停（托管 a），b 版本可恢复 | 浏览记录、收藏、外部跳转记录 | 我的收藏、浏览与跳转记录 | 我们云上暂停（托管 a），b 版本可恢复 | 我们云上暂停（托管 a），b 版本可恢复 |
| 全部岗位 | `/jobs`。我们云上暂停（托管 a），b 版本可恢复 | 浏览记录、收藏、外部跳转记录 | 我的收藏、浏览与跳转记录 | 我们云上暂停（托管 a），b 版本可恢复 | 我们云上暂停（托管 a），b 版本可恢复 |
| 岗位大师 | 首页既有卡片 → `/resume/job-fit`（无简历时由既有页面引导上传诊断）。用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位 | 复用简历对照、定向优化建议与报告打印 | AI服务记录、我的文档、打印订单 | 🔶 PR #200 已合入 `main` 且仅复用 2D；未新增路由、记录类型或同义入口 | 保持唯一入口；尚未部署预生产，不得宣称已完成预生产 / 真机验收 |
...
| 纸质扫描 | `/scan/start` | 扫描 PDF/图片 FileObject | 我的文档 | 🔶 **2026-07-11 更正**：`feature/real-scan` 已于 2026-07-10 完成全部 21 个任务并合并进 `main`（新增 `ScanTask` 模型 + Agent 端 `scan-watcher.ts` 监听打印机 SMB 共享目录投递 + Kiosk 四页面全部接真，不再是 `mockFile()`），本条已从「未打通/演示」升级为**代码级真实闭环**；上一条 2026-06-12 核查结论已过期。 | 仅剩 Windows 真机物理验收（真实打印机→SMB→Agent 端到端硬件链路从未上机跑过）；打通前该子项不得单独宣称生产验收完成 |
| ~~云打印~~（已删除） | **2026-07-12 已按正式取舍决策从首页删除**（原为 disabled 占位） | — | — | 已裁定 | **2026-07-12 用户拍板（D1=b 删除磁贴）**：经与「岗位大师」（§3⑧）同等流程的正式取舍决策——该磁贴描述的"云端任务队列 → Agent claim → 本机驱动打印"架构与「文档打印」已实现链路重叠，"其他联网设备上传"语义已被「手机扫码上传」覆盖（2026-07-10 云上传范围收窄声明），定义不出叫"云打印"的增量能力。能力归位「文档打印+手机扫码上传」；真增量方向「远程提交·到店取件 / 跨终端释放」已记入 next-tasks 商用二期候选（候选未立项，含释放状态机等准入门槛）。完整评估与两轮外部 Codex 评审见 [docs/reviews/2026-07-12-cloud-print-decision.md](../reviews/2026-07-12-cloud-print-decision.md)。**不得以"云打印"名义重建同义入口。** |
| 格式转换 | `/print-scan/convert` | 转换后 FileObject（`assetCategory=derived`） | 我的文档 | ✅ **2026-07-11 已打通**：图片→PDF MVP 已代码级真实闭环——新增后端 `print-conversion` 模块（归属校验+guest capability 校验+pdfkit 合并+Redis 幂等，`verify:print-conversion` 16 断言进双 CI）+ Kiosk `ConvertImagesPage`（本机上传/手机扫码均"一次一张、可继续添加"）；首页磁贴与 `/print-scan` 服务中心卡片均已点亮，生成后直接进 `/print/confirm` 复用既有打印链路。范围仅图片（JPG/PNG）→PDF；PDF→图片、Word/Excel/PPT→PDF、证件复印/证件照仍是占位，未开工；签名盖章见下一行（已打通）。 | 仅代码 + 本地 verify + mock 模式浏览器走查级；未预生产、未真机、未接入真实 COS/PostgreSQL 环境验收 |
| 签名盖章 | `/print-scan/sign`；`我的文档` PDF 条目「签名盖章」动作可直达（跳过选文档） | 合成 PDF FileObject（`assetCategory=derived, sourceFileId=源文档`）、签名/印章图片（新增 `signature_image` purpose，highly_sensitive，锁定系统短期，不进「我的文档」列表） | 我的文档、打印订单 | ✅ **2026-07-13 已打通**：PDF + PNG/JPG 签名或印章图片经 pdf-lib 在指定页/九宫格位置合成新 PDF（`useObjectStreams:false` 保证既有打印计费器可数页），服务端 `signature_stamp` 能力门禁 + 数字签名 PDF 检测拒绝 + 授权确认勾选 + 幂等（owner-token compare-and-delete）；`verify:print-sign`（110 断言）进双 CI；`/print-scan` 服务中心卡片已点亮，`PrintScanFeatureInfoPage` 的 sign 说明页分支已移除（旧 `/print-scan/feature/sign` URL 现走「未找到该功能」优雅兜底）。真实浏览器走查（真实登录会员 + 真实后端）确认：上传 PDF → 上传印章图 → 九宫格/页码/大小 → 授权勾选门控生成按钮 → `/me/documents` PDF 条目显示「签名盖章」按钮 → 点击直达第 2 步，均按预期工作。走查中发现的 `kiosk-upload-options.dto.ts` purpose 白名单缺陷已修复（`ee239460`，附 class-validator 回归断言）。 | 走查中一次生成合成 PDF 遇到 500 + 后端进程退出，已专项调查（真实环境 45+ 次冷启动/并发/边界值复现尝试均未复现），当前最可能假设是走查环境内存压力导致 OOM（未确认，非 print-sign 代码缺陷结论）；仅代码 + 本地 verify + 浏览器走查级；未预生产、未真机；触屏拖放定位/手写签名板/EXIF 方向归一化留二期 |
...
### 3.9 「我的」账户与服务入口（P0a/P0b/P1 + 权益活动 P2 真实化；Wave 0 真值收口至 2026-07-16）

> 「我的」页当前按「我的资产 / 常用服务 / 招聘会与活动 / 权益与政策 / 账户与支持」分组，只展示已接真的目的地。P0a 做低风险前端真实化与既有页面跳转；P0b 复用现有 `BenefitGrant` 底座打通「我的权益」只读页与 Admin 手动发放/撤销；P1 新增消息通知与意见反馈域；P2 在同一权益底座上打通权益活动领取闭环。Wave 0 不引入支付、套餐购买、招聘会扫码凭证、活动核销、现场签到或 Partner 自助配置，也不为这些未开放能力保留可点击占位。
...
| 账号设置 | `/me/settings`（原建设中，2026-06-18 新增轻量版；2026-07-16 Wave 0 对齐文案） | 无（只读账号状态 + 协议入口 + 退出/切换账号） | — | ✅ 现有能力已打通（展示脱敏手机号 / 登录态 / 公共终端会话说明 / 协议·隐私入口 / 退出登录 / 切换账号；切换账号是退出后重新登录，不是多角色“身份切换”） | 手机号换绑、账号注销和数据导出尚未开放；须完成 step-up、安全执行器、审计和运营闭环后再在本页提供 |
...
| 权益活动 | Kiosk `/activities`、`/activities/:id`；Admin `/benefit-activities` | `BenefitActivity`、`BenefitClaim`、`BenefitGrant`、AuditLog | 我的权益 | ✅ 已打通（Admin 创建草稿、发布、结束、查看领取记录；Kiosk 活动列表/详情/登录后领取；领取成功写入 `BenefitClaim` 并生成 `BenefitGrant`，可在 `/me/benefits` 查看；Profile 只保留一个 `/activities` 入口） | 暂不做支付、套餐购买、活动核销、自动资格审核、Partner 自助配置、招聘会签到或凭证 |
| 求职打印套餐 / AI服务套餐 / 招聘会扫码凭证 | 当前不展示、未进入 Wave 0 范围 | — | — | ❌ 未开放（无可点击入口、无生产 API、无真实数据或后台闭环） | 后续只有在套餐 / 支付 / 凭证域分别满足产品、合规、审计和运营门禁后才能重新评审；不得先恢复占位 |
...
> 2026-07-16 Wave 0 基线已由 PR #261 合入 `origin/main@0c4cdd57`：Profile 固定为 22 个已接真目的地，删除重复招聘会活动入口并隐藏招聘会扫码凭证、求职打印套餐、AI 服务套餐；登录页只保留手机与扫码；账号设置明确换绑、注销、数据导出尚未开放。后端在真实导出/注销执行器上线前拒绝把 `export/delete` 工单标为 `completed`，也拒绝用普通 `rejected` 结束 `delete`，且失败尝试不改变状态、不写完成审计；`revoke_consent` 继续可同步完成。当前本地补丁还拒绝把创建时已经实际撤回的 `revoke_consent` 工单写为 `rejected`，避免操作结果与终态不一致。SQLite 57 个正式 migration 与 PostgreSQL 29 个正式 migration 均在一次性空库通过，临时库已删除；详细证据见 [user-center-wave0-acceptance.md](../acceptance/user-center-wave0-acceptance.md)。当前补丁尚未推送、合并、部署或运行 GitHub CI。

> Wave 0 临时运营边界：`delete` 工单在 Wave 1 前只能停留于 `pending/handling`，没有完成、驳回、取消或 SLA，且当前 Admin 前端没有隐私工单处理页；API 会明确返回执行器未开放，运营不得把线下动作写成系统终态。旧的会员级 Job-AI 批量删除只覆盖两张 AI 表和同意记录，不能代表完整注销，现已暂停；本人逐条删除 AI 记录与过期清理仍保留。Wave 1 须补分类留存矩阵、综合执行器、服务端审计凭证和失败补偿。
...
| `/profile` 求职打印套餐 / AI 服务套餐 / 招聘会扫码凭证 | 不展示，无可点击入口 | 无生产 API | 无 | 无 | 无 | Wave 0 守卫要求保持隐藏；后续套餐/支付/凭证域通过独立评审与真实闭环后才可开放 |
...
| 分组 | 应收纳的数据 | 必须支持的操作 |
|---|---|---|
| 我的简历 | 上传简历、诊断简历、生成简历、优化简历版本 | 查看、继续优化、简历对照（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位）、下载/打印、删除 |
| 我的文档 | 上传文件、扫描文件、AI 生成 PDF、报告 PDF、政策/招聘会资料 | 预览、下载、打印/再打印、删除 |
| AI服务记录 | 简历诊断、简历优化、简历生成、简历对照（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位）、模拟面试、职业规划、招聘会参会准备单、AI助手问答 | 查看结果、继续下一步、打印报告、删除 |
| 打印订单 | 所有打印任务，包括简历、扫描件、报告、政策/招聘会资料 | 查看状态、查看参数、再次打印、异常反馈 |
| 我的收藏 | 岗位、招聘会（按 3.14 在两端隐藏）、政策 | 查看、取消收藏、继续匹配/打印/来源跳转 |
| 我的权益 | 优惠券、免费次数、套餐权益、补贴资格提示 | 查看、使用、过期说明；不得承诺补贴到账 |
| 浏览与跳转记录 | 岗位 / 招聘会（按 3.14 在两端隐藏）/ 政策浏览记录；去来源平台投递 / 扫码投递 / 去来源平台预约 / 扫码预约 / 扫码前往来源平台签到 / 官方入口打开记录 | 查看岗位 / 查看招聘会 / 查看政策、再次打开来源平台 / 官方入口、删除；不得记录投递/预约/签到结果 |
...
诊断 / 优化 / 生成 / 简历对照（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位）四条 AI 链路 → AI服务记录（含删除级联）；职业规划 → AI服务记录 / 我的文档 / 打印订单；文档上传与 AI 导出 PDF → 我的文档（预览/下载/再打印/删除）；打印任务 → 打印订单（状态/参数）；三类收藏（岗位/招聘会/政策）→ 我的收藏（含本机合并；岗位与招聘会按 3.14 在两端隐藏）；岗位 / 招聘会 / 政策浏览与外部入口打开记录 →「我的」浏览与跳转记录（岗位与招聘会按 3.14 在两端隐藏）；招聘会资料打印 → 我的文档+打印订单；模拟面试 → 自有历史页（查看/删除/报告打印）。
...
| ~~P1~~ ✅ 已完成（2026-07-04） | 打印状态实时追踪 UI | `/me/print-orders` 已对进行中订单做 5 秒条件轮询，按 `id` 合并并保留已加载分页；`/print/progress` 保持真实任务页 2 秒轮询 | 保持回归；失败原因本人可见范围仍需产品确认后独立小步 |
| P2 | AI 助手会话不落库 | 无会话模型 | 隐私先行设计（TTL/脱敏/本人可删）后再建模，落 AI服务记录 |
| 待真机（**2026-07-11 更正**：不再是占位/演示） | 纸质扫描 | `feature/real-scan` 已合并 `main`（2026-07-10），代码级真实闭环（`ScanTask` + Agent `scan-watcher.ts` + Kiosk 四页面接真） | 仅剩 Windows 真机物理验收，通过前不得单独宣称生产验收完成 |
| 依赖硬件（待真机） | U盘导入/证件复印/证件照 | 证件复印/证件照：首页 disabled 占位，无后端、无路由；U盘导入：**2026-07-12 更正**——已于 2026-07-11 在 `PrintUploadPage.tsx` `usb` tab 代码级接线（Agent 枚举 + 本地网桥，见 next-tasks「首期 U 盘导入」条目），仅剩 Windows 真机验收，不再是"无后端、无路由" | 对应硬件扩展链路完成真机验收前不接「我的」（不造假数据）；已在 `next-tasks.md` P0 打印扫描首期逐项排期。（原并列的「云打印」已于 2026-07-12 按正式取舍决策从首页删除，见 §3.4） |
...
```text
职业规划入口 → 选择/确认简历与目标方向 → 生成职业规划建议 → 结果页 → 存入 AI服务记录 → 可导出/打印为我的文档和打印订单 → 可继续简历优化/模拟面试/简历对照（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位）
```
...
- 后续 CTA：继续简历优化、简历对照（用户自填岗位要求，对照本人简历（简历对照）；不接系统内岗位）、模拟面试；这些是结果页链路，不新增首页入口。
