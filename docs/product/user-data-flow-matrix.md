...
| 云打印 | 当前首页已有，占位 | FileObject、PrintTask | 我的文档、打印订单 | 未打通 | **2026-07-11 范围澄清（尚未决策，仅记录待办）**：本行原「缺口/下一步」写的"云端任务队列 → Windows Agent claim，本机驱动打印"，与「文档打印」入口已实现的架构完全相同（见 CLAUDE.md §3"云打印架构"）。在真正点亮这张卡片前，必须先明确它相对「文档打印」的**增量能力**是什么（例如：从第三方网盘/其他联网设备拉取文件后打印），否则会违反"不新增同义入口"的入口稳定规则（§二）。若定义不出增量能力，应按 §3⑧ job-master 同等流程走一次正式取舍决策（合并/隐藏），而不是直接点亮。 |
...
| 依赖硬件（未开工） | U盘导入/云打印/证件复印/证件照 | 首页 disabled 占位，无后端、无路由 | 对应硬件扩展链路完成真机验收前不接「我的」（不造假数据）；已在 `next-tasks.md` P0 打印扫描首期逐项排期 |
