...
| 岗位匹配（2D；诊断报告 CTA 与首页既有「岗位大师」同指向，不新增路由） | `/resume/report` →「目标岗位匹配参考」→ `/resume/job-fit`；首页「岗位大师」→同一路由 | `AiResumeResult(kind=job_fit)`、关联 `JobAiSession(operation=match)`、报告 PDF `FileObject`、后续 `PrintTask` | AI服务记录（仅展示一条 `job_fit` 成功结果；同任务已完成 match 会话去重）、我的文档、打印订单 | 🔶 2026-07-12 本地 M1.5 候选：已复用 consent / 配额 / 会话 / 用量治理，结果页只展示真实摘要、匹配依据 / 可选关键词、差距与简历改写；内部 HMAC `printFileUrl` 进入既有打印确认，且仅「查看岗位」到 `/jobs/:id` | 候选尚未合入 / 部署；仍需真实会员、LLM、预生产打印和 Windows 真机验收 |
...
| 岗位大师 | 首页既有卡片 → `/resume/job-fit`（无简历时由既有页面引导上传诊断） | 复用 2D 岗位匹配参考、定向优化建议与报告打印 | AI服务记录、我的文档、打印订单 | 🔶 仅复用 2D；本地 M1.5 候选未新增路由、记录类型或同义入口 | 保持唯一入口；候选合入前不得宣称已完成预生产 / 真机验收 |
