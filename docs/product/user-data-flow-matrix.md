...
| 证件照打印 | `/print-scan/id-photo` | 证件照裁剪产物 FileObject（`purpose=id_scan`）、排版 PDF FileObject（`purpose=id_photo_print`）、`PrintTask` | 我的文档（1 小时可见窗口）、打印订单 | ✅ **2026-07-13 代码级闭环**（前端浏览器裁剪 + 服务端排版 + 打印参数契约强校验 + 建单后自动删源 + 全链路审计，详见 `docs/superpowers/specs/2026-07-12-id-photo-design.md`）；能力开关默认不可用，待真机彩色验收点亮 | 真机彩色出纸 + `noscale` 实物量尺 + 隐私链路演练（三项均见 `docs/device/production-deployment-and-windows-host-checklist.md` 证件照真机验收小节） |
