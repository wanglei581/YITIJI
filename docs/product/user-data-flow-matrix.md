...
| 签名盖章 | `/print-scan/sign`；`我的文档` PDF 条目「签名盖章」动作可直达（跳过选文档） | 合成 PDF FileObject（`assetCategory=derived, sourceFileId=源文档`）、签名/印章图片（新增 `signature_image` purpose，highly_sensitive，锁定系统短期，不进「我的文档」列表） | 我的文档、打印订单 | ✅ **2026-07-13 已打通**：PDF + PNG/JPG 签名或印章图片经 pdf-lib 在指定页/九宫格位置合成新 PDF（`useObjectStreams:false` 保证既有打印计费器可数页），服务端 `signature_stamp` 能力门禁 + 数字签名 PDF 检测拒绝 + 授权确认勾选 + 幂等（owner-token compare-and-delete）；`verify:print-sign`（108 断言）进双 CI；`/print-scan` 服务中心卡片已点亮，`PrintScanFeatureInfoPage` 的 sign 说明页分支已移除（旧 `/print-scan/feature/sign` URL 现走「未找到该功能」优雅兜底）。真实浏览器走查（真实登录会员 + 真实后端）确认：上传 PDF → 上传印章图 → 九宫格/页码/大小 → 授权勾选门控生成按钮 → `/me/documents` PDF 条目显示「签名盖章」按钮 → 点击直达第 2 步，均按预期工作。 | 走查中发现真实缺陷：`kiosk-upload-options.dto.ts` 的 purpose 白名单未包含 `signature_image`，导致 Kiosk「本机上传图片」上传签名/印章图 400（手机扫码上传路径不受影响，已正确包含该值），本任务未修复；生成合成 PDF 一次遇到 500 + 后端进程退出（相同请求 curl 直接重放到全新后端可干净复现成功，根因未定位）；仅代码 + 本地 verify + 浏览器走查级；未预生产、未真机；触屏拖放定位/手写签名板/EXIF 方向归一化留二期 |
