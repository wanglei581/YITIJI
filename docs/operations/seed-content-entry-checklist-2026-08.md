...
> ⚠️ **验政策时不要只看 `?tab=policy`。**
> 该面板把后端数据和一批**内置的硬编码指引**合并展示
> （`RenshiPage.tsx:69-70` 合并 `builtinData.ts`）。
> 也就是说：**即使一条后端政策都没进来，这个页面看上去也是有内容的。**
> 要证明数据真的到了，用 `?tab=notice`（纯后端数据），
> 或直接看 5.2 那个接口的 `pagination.total`。
...
| 政策接口 total 对，但一体机列表条数更多 | `?tab=policy` 混入了内置指引 | 改看 `?tab=notice` 或接口 total |
...
**C. `/renshi?tab=policy` 把后端数据和内置硬编码指引合并展示。**
`RenshiPage.tsx:69-70`。后果是政策库为空时页面看起来也是满的——
这与 CLAUDE.md §9「不伪造能力」的取向相冲突（用户无法区分哪条是真实政策库内容）。
**影响**：既误导用户，也让验收失去判别力（见 5.3 的警告）。
**建议**：至少给内置条目加来源标识，或在政策库为空时如实显示空态。
