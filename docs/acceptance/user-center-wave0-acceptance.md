...
- 真实导出/注销执行器上线前，`export→completed`、`delete→completed`、`delete→rejected` 均 fail closed；失败尝试保持工单 `pending`，不写完成/拒绝审计。`revoke_consent` 在创建时已同步实际撤回授权，因此只允许记录为 `completed`，拒绝写入伪造的 `rejected` 终态。
...
| `revoke_consent` | `rejected` | 拒绝，`DATA_REQUEST_ALREADY_EXECUTED` | 工单保持 `pending`；授权已在创建时撤回；不写拒绝审计 |
