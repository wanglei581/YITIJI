...
- `PS-G3-ADMIN-01-LP`：2026-07-06 只读盘点 `C:\ai-job-print-evidence`、`%LOCALAPPDATA%\Temp\ai-job-print-evidence` 和 `C:\tmp\ai-job-print-evidence` 后，未发现 Admin 页面截图；现有图片文件仅为 G5 纸张照片。源码静态复核确认 Admin 终端页在 `agentStatus === 'agent_degraded'` 或 `localTaskDatabaseAvailable === false` 时展示“本地任务库不可用，已暂停领取打印任务”，`GET /admin/terminals` 契约与后端 `listTerminalsForAdmin()` 均返回相关字段；`PS-G3-DEG-02-memory-probe-20260706163452` 日志包含 `PASS Admin terminal view exposes agent_degraded + localTaskDatabaseAvailable=false`。该项只证明 Admin 降级可视化代码 / 内存级门禁对齐，不替代现场 Admin 截图；现场截图补证步骤已收敛到 `docs/acceptance/print-scan-field-execution-runbook.md` §8.1，未获现场授权前保持 pending。
- `PS-G3-BIND-01-LP`：2026-07-08 本地 `verify:print-jobs` / `verify:print-scan-first-release` 已覆盖 PrintTask 创建必须绑定目标 `terminalId`，Terminal Agent claim 按自身 `terminalId` 过滤，claim 更新时仍以 `id + status=pending + terminalId` 做原子守卫，且不会把目标 `terminalId` 改写成 claimedBy。该项只证明本地 service / static guard，不替代现场两终端真实 API / DB 隔离演练。
- `PS-G3-STATUS-01-LP`：2026-07-08 本地 `verify:print-jobs` 已覆盖 service 级 `pending -> claimed -> printing -> completed` 状态链、`completedAt` 写入、终态幂等和终态后再请求 `printing` 不回退；该项不证明 Windows 真机完整状态链或公开 Kiosk / Admin 轮询截图。
...
- `PS-G3-REC-01-LP`：2026-07-08 本地 `verify:print-jobs` 新增恢复后 paid-before-claim 代码门禁：临时设置 `PRINT_REQUIRE_PAID_BEFORE_CLAIM=true`，验证恢复 `online` 后付费 `unpaid/pending` 任务仍保持 `pending`、不被领取；线下 `markPaid(... paymentSource='offline')` 后错终端仍不可领取，原目标终端可 claim，`Order.taskStatus` 同步为 `claimed`。该项只证明 service 级防回退，不替代现场 paid / allowed 真实领单、出纸和状态一致性证据。
...
- `PS-G4-01-LP`：2026-07-08 本地 `verify:print-scan-agent` 新增临时文件清理断言：任务下载文件 `task_*` 在完成 / 失败后进入 `finally` 立即删除；Agent 启动和空闲 claim 周期会按 1 小时 TTL 清理崩溃残留的 `task_*` 下载文件和 `print_*.pdf` 图片转换文件。该项只证明本地 Agent 代码防回退，不替代现场临时目录检查和删除审计。
...
- `PS-G4-04-LP`：2026-07-08 本地 `verify:admin-print-ops-visibility-ui` 新增 Admin 运维可见性静态断言：终端页必须展示在线 / 离线 / `agent_degraded` / `localTaskDatabaseAvailable=false` 与“本地任务库不可用，已暂停领取打印任务”降级文案；订单页必须展示失败状态、错误码、状态流转日志、pending 取消 / 重分配人工处理入口，并保持只读订单接口不暴露文件 URL / 签名 / hash。该项只证明 Admin UI / 契约防回退，不替代现场截图和人工处置记录。
...
| PS-G0-02 | Passed Locally | `pnpm --filter @ai-job-print/api verify:print-jobs` 覆盖降级不可领取、恢复后兼容领单、paid-before-claim 下 unpaid 不领 / paid 后原终端可领 | 不记录真实文件 URL 或签名参数 |
...
| 证据 ID | 状态 | 通过标准 | 仓库外证据 |
| --- | --- | --- | --- |
| PS-G3-BIND-01 | Code Guard Passed; Field API/DB Pending | 本地 `verify:print-jobs` / `verify:print-scan-first-release` 已覆盖任务创建绑定目标 `terminalId`、claim 按 `terminalId` 过滤且原子更新守卫不改写目标终端；尚未在现场两终端真实 API / DB 中复验 | `services/api verify:print-jobs` / `services/api verify:print-scan-first-release` / `<PRIVATE_EVIDENCE_DIR>/PS-G3-BIND-01-terminal-isolation-<timestamp>.log` |
| PS-G3-STATUS-01 | Partial; Service Guard Passed, Field UI/Admin Pending | 本地 `verify:print-jobs` 已覆盖 service 级 `pending -> claimed -> printing -> completed`、`completedAt`、终态幂等和完成后不回退；现场证据 ID `physical-print-20260706101346` 记录正式端点链路 `pending -> printing -> completed`，但公开状态轮询未单独捕获 `claimed`，Kiosk / Admin 状态一致性仍待补证 | `services/api verify:print-jobs` / `physical-print-20260706101346` / `<PRIVATE_EVIDENCE_DIR>/PS-G3-STATUS-01-status-chain-<timestamp>.md` |
...
| PS-G3-ADMIN-01 | Runbook Ready; Field Screenshot Pending | `print-scan-field-execution-runbook.md` §8.1 已定义受控 Admin 降级截图补证步骤：先只读确认无 active 打印任务、队列 / spool 为空、页面可脱敏，再经现场授权制造降级并截图 Admin 终端页“本地任务库不可用，已暂停领取打印任务”；未授权前不得为了截图停 Agent 或替换 `agent.db` | `<PRIVATE_EVIDENCE_DIR>/PS-G3-ADMIN-01-degraded-terminal-view-<timestamp>.png` |
...
| PS-G3-REC-01 | Partial; Recovery Service + Paid-Claim Code Guard Passed, Field Paid/Allowed Claim Pending | 恢复原 `agent.db` 后，Agent 日志显示 `db: opened`、`agent ready`、`task-runner: starting` 和恢复心跳 acknowledged；公开只读 `printer-status` 返回 `printerStatus=ready` / `isOnline=true`，现场恢复后 `AIJobPrintAgent` 为 Running / Automatic、打印机 ready、队列和 spool 为空。`ptask_kiosk_046e67e6bbb917bd` 为 `unpaid/pending`，在 paid-before-claim 门禁下未被领取，不能据此判定恢复后真实领单 / 出纸通过；本地 `verify:print-jobs` 已补 `PRINT_REQUIRE_PAID_BEFORE_CLAIM=true` 恢复语义，证明 unpaid 任务保持 pending、线下 mark-paid 后错终端不可领、原目标终端可 claim。尚未在现场真实 paid / allowed 任务中复验领单、出纸和 Kiosk / Admin 状态一致性 | `PS-G3-DEGREC-20260706155258` / `PS-G3-DEG-02-memory-probe-20260706163452` / `PS-G3-DEG-02-field-20260706165759` / `services/api verify:print-jobs` |
...
已确认：物理出纸最小硬证据已通过，G5 只读证据链与 PS-G3-PHYS-01 照片补证已对齐；终端绑定 claim 和 service 级状态链路已有本地防回退；Agent 本地任务库不可用时 fail-closed 和恢复后心跳正常已有运行证据；后端 claim 二道闸门已有内存级探针与现场真实 API / DB 降级领单补证，降级态真实 claim 返回 0 且任务保持 pending；恢复后 paid-before-claim 本地代码门禁已证明 unpaid 不领、paid 后原目标终端可领；Admin 降级可视化已有源码静态复核和内存级探针断言，受控截图步骤已写入 Runbook，但没有现场截图；错终端状态回传拒绝已有本地 service 级防回退；Admin pending 打印订单取消 / 重分配已有本地代码和 verify 级防回退。
阻塞项：终端隔离、完整状态链路、Admin 降级截图、恢复后 paid / allowed 任务真实领单、现场真实 API / DB 错终端回传拒绝、Admin 运维动作候选部署鉴权复验等现场演练仍未全部完成。
```
...
| 证据 ID | 状态 | 通过标准 | 仓库外证据 |
| --- | --- | --- | --- |
| PS-G4-01 | Code Guard Passed; Field Pending | 本地 `verify:print-scan-agent` 已覆盖任务完成 / 失败后 `finally` 删除 `task_*` 下载文件，并在 Agent 启动和空闲 claim 周期按 1 小时 TTL 清理崩溃残留的 `task_*` / `print_*.pdf`；尚未在现场临时目录中复验 | `terminal-agent verify:print-scan-agent` / `<PRIVATE_EVIDENCE_DIR>/PS-G4-01-local-cache-cleanup-<timestamp>.log` |
...
| PS-G4-04 | Code Guard Passed; Field Screenshot/Manual Record Pending | 本地 `verify:admin-print-ops-visibility-ui` 已覆盖 Admin 终端页在线 / 离线 / 降级 / 恢复状态展示、降级文案、订单页失败状态 / 错误码 / 状态流转日志 / pending 取消与重分配人工处理入口，并确认只读订单接口不暴露文件 URL / 签名 / hash；尚未取得现场 Admin 截图和人工处置记录 | `admin verify:admin-print-ops-visibility-ui` / `<PRIVATE_EVIDENCE_DIR>/PS-G4-04-admin-ops-visibility-<timestamp>.png` |

判定：

```text
Gate 4 Privacy And Recovery: Not Passed Yet
已确认：Agent 任务下载文件和图片转换 PDF 的立即删除 / 1 小时 TTL 清理已有本地静态门禁；stuck printing 任务释放保留目标 terminalId、错误终端不能领取、原目标终端可重新领取已有本地 service 级防回退；Agent spooled 重启补偿和 Pantum Retained 超时均已由本地静态门禁锁定为 PRINT_JOB_UNCONFIRMED，不误报 completed；Admin 运维可见性已有本地 UI / 契约静态门禁，覆盖降级状态、恢复状态、失败错误码、状态流转和 pending 人工处理入口。
阻塞项：尚未执行高敏文件 TTL 删除、卡住任务释放、断网 / 断电恢复和 Admin 运维可见性现场验收；PS-G4-01 / PS-G4-02 / PS-G4-03 仍需现场真实 Windows / API / DB 证据后才能判通过，PS-G4-04 仍需现场 Admin 截图和人工处置记录后才能判通过。
```
