...
- [ ] 身份证复印：证件放置 → 复印/扫描出纸或成档全链路可用；复印产物不落长期存储，完成后按敏感文件策略清理并有删除日志。
- [ ] 证件照：该能力当前未上线（无数据模型）。上线前验收 = Kiosk 卡片不可进入正式流程且终端能力开关（Admin「打印扫描运维 → 设备能力」）中 `id_photo` 不为「可用」；能力真实上线后须补真机排版打印与敏感照片清理验收，再更新本条。
