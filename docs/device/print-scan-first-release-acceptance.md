...
| 缺运行时终端身份 | Kiosk 必须通过本机 Agent `http://127.0.0.1:9527/local/identity` 取得身份；生产构建禁止读取或回退到 `VITE_TERMINAL_ID`，避免克隆机器共用终端号；文字助手模式不豁免该运行时身份要求 | `verify:runtime-terminal-identity` 通过；本机 identity 返回已绑定终端 |
