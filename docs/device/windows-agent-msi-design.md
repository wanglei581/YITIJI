...
Windows Actions run [`31157191672`](https://github.com/wanglei581/YITIJI/actions/runs/31157191672) 已对提交 `12b523b9` 全绿：输出 `PROVISIONING_ORIGIN_COLLECTION_PASS`、两次 `INSTALLED_RUNTIME_ACL_PASS installedMode=installed`（安装后与 Repair 后）、`EXE_UPGRADE_PASS from=0.3.3 to=0.3.4`、`EXE_LIFECYCLE_PASS` 和 `MSI_LIFECYCLE_PASS`。EXE repair 的 MSI 日志记录 `REINSTALLMODE=cmuse` 与 `ShortcutCreate`；独立 MSI `/fcmuse` repair 被规范化为 `REINSTALLMODE=ecmus`，同样执行 `ShortcutCreate`。最终未签名 `AIJobPrintTerminalSetup.exe` 为 45,307,221 bytes，SHA-256 `A5ABB4D508C77C04238CC1374C1BB1A1EAF5B30F17F7CDAE342CDD90C9AE56DE`，已保存为本机 `Downloads/AIJobPrintTerminalSetup-0.3.4-unsigned.exe`。该文件仍未签名，CI 全绿不替代真实 BindCode、UAC、DPAPI、Automatic/Running、新心跳和奔图打印扫描验收。

### 10.3 `0.3.5` 兑换失败可诊断化（2026-08-07）

`0.3.4` 真机激活现场“BindCode exchange failed”经生产 nginx/数据库只读核验为 `HTTP 400 MAC_ALREADY_BOUND`：Windows 主机 `DESKTOP-DDD6FCO`（MAC `1C:83:41:D1:22:37`）已绑定 active 的 `KSK-001`，而本次绑定码生成在 planned 的 `new01`。正确换机流程：后台将 KSK-001 进入维护 → 生成 KSK-001 换机绑定码 → Windows 激活 → 新心跳确认后恢复运行。

`0.3.5` 不再把兑换失败压成一句泛化提示：错误消息只展示 `HTTP <status>` 与服务器结构化错误代码（如 `MAC_ALREADY_BOUND` / `TERMINAL_MAINTENANCE_REQUIRED`），不透传响应正文、不包含绑定码，并针对两类常见原因给出可执行提示。诊断脚本 `Get-RuntimeRootAclStatus` 与激活校验同口径：TrustedInstaller / CREATOR OWNER 继承 ACE 视为可信，普通用户真实写权限仍拒绝；修复后“运行时 ACL: too_permissive”不再误报。固定升级基线调整为 `0.3.4@12b523b9 → 0.3.5`。新版 Windows CI 全绿与真机激活证据补齐前，`0.3.4` 仅保留历史证据。

Windows Actions run [`31160510941`](https://github.com/wanglei581/YITIJI/actions/runs/31160510941) 已对提交 `cf24e226` 全绿：输出 `PROVISIONING_ORIGIN_COLLECTION_PASS`、两次 `INSTALLED_RUNTIME_ACL_PASS installedMode=installed`、`EXE_UPGRADE_PASS from=0.3.4 to=0.3.5`、`EXE_LIFECYCLE_PASS` 和 `MSI_LIFECYCLE_PASS`。最终未签名 `AIJobPrintTerminalSetup.exe` 为 45,306,519 bytes，SHA-256 `327A8BE31EA5D684C2C9AAB9B561FE7F38A6C2E4223A22E4CAD7206D6EC372AC`，已保存为本机 `Downloads/AIJobPrintTerminalSetup-0.3.5-unsigned.exe`。该文件仍未签名，CI 全绿不替代真实 BindCode、UAC、DPAPI、Automatic/Running、新心跳和奔图打印扫描验收。
...
### 10.5 `0.3.7` 配置 ACL 自修复（2026-08-07）

真机 0.3.6 现场：浏览器访问 `127.0.0.1:9527/local/terminal-identity` 返回 `ERR_CONNECTION_REFUSED`；配置窗口诊断为服务 `Stopped/Manual`、配置 JSON 有效、ProgramData 目录与 token ACL 正常，但“已保存凭据重新配置”报“Existing Agent config is not protected or valid JSON”。根因是 `%ProgramData%\AIJobPrintAgent\agent-config.json` 文件 ACL 失去保护，旧诊断未检查该文件。`0.3.7` 在激活参数固定携带 `RepairProgramDataAcl`：`Get-PreservedLocalSettings` 前先把目录、配置文件、token 统一修复为受保护的 SYSTEM + Administrators；诊断新增 `configFileAclStatus` 与 `localApiStatus`（127.0.0.1 监听探测）。固定升级基线调整为 `0.3.6@134f352a → 0.3.7`。新版 Windows CI 全绿与真机“一键重配启动 + 本机接口 listening + 前端在线”证据补齐前，`0.3.6` 仅保留历史证据。

Windows Actions run [`31166100094`](https://github.com/wanglei581/YITIJI/actions/runs/31166100094) 已对提交 `82186e23` 全绿：输出 `PROVISIONING_ORIGIN_COLLECTION_PASS`、两次 `INSTALLED_RUNTIME_ACL_PASS installedMode=installed`、`EXE_UPGRADE_PASS from=0.3.6 to=0.3.7`、`EXE_LIFECYCLE_PASS` 和 `MSI_LIFECYCLE_PASS`。最终未签名 `AIJobPrintTerminalSetup.exe` 为 45,310,987 bytes，SHA-256 `33AD1CC8AD8D1FCA4F2EF32BF7C6024C26D5E7965AD08FA8AA94E58AA2FF4546`，已保存为本机 `Downloads/AIJobPrintTerminalSetup-0.3.7-unsigned.exe`。该文件仍未签名，CI 全绿不替代真实 BindCode、UAC、DPAPI、Automatic/Running、新心跳、本机接口、前端在线、奔图打印自检与打印扫描验收。
