...
### B1 独立 managed 主机与固定端点

| 字段 ID | 非秘密输入 | 当前值 | 状态 | 只读关闭条件 |
| --- | --- | --- | --- | --- |
| `B1.1` | managed 主机/隔离实例标识 | `<UNSET>` | `UNSET` | 正式资料能唯一定位，且与 legacy 主机独立或具备等价隔离。 |
| `B1.2` | managed health 端点 | `http://127.0.0.1:3010/api/v1/health` | `NOT_VERIFIED` | 代码固定值已知；仍须证明 managed 实例独占该 loopback 端口。 |
| `B1.3` | 与 legacy 的隔离证明引用 | `<UNSET>` | `UNSET` | 证明没有在 legacy 主机私增端口或复用现役进程。 |

### B2 独立 managed PM2 名称
...
| 字段 ID | 非秘密输入 | 当前值 | 状态 | 只读关闭条件 |
| --- | --- | --- | --- | --- |
| `B9.1` | 部署账户角色 | `<UNSET>` | `UNSET` | 可写批准的 release/artifact/control/launcher 目标。 |
| `B9.2` | API 运行账户角色 | `<UNSET>` | `UNSET` | 对上述目标只读，且与部署角色分离。 |
...
| 门槛 | 当前状态 |
| --- | --- |
| B1 独立 managed 主机/端点 | `NO-GO / UNSET` |
| B2 独立 managed PM2 | `NO-GO / UNSET` |
