...
> 版本：v1.3（Phase 8 设计文档）  
> 创建时间：2026-05-26  
> 最后更新：2026-05-26（v1.1：双进程架构/Token安全/claim lease/单实例锁/Phase 8.0验证清单；v1.2：actionToken签发接口/lease续租/Named Pipe ACL；v1.3：actionTokenSecret每终端独立+可吊销轮换/9528双向鉴权+service-helper-shared-token/actionToken语义明确：Agent只校验与taskId绑定的token不主动申请）  
> 状态：设计文档已定稿，待 Phase 8 实现  
> 关联文档：[pantum-cm2820adn.md](./pantum-cm2820adn.md) | [api-v1-design.md](../api/api-v1-design.md) | [CLAUDE.md](../../CLAUDE.md)
...
| 设备 | 型号 / 标准 | 接入进程 | 接入方式 |
|------|------------|---------|---------|
| 打印机 | 奔图 Pantum CM2820ADN（有线网络） | Service | Windows GDI Print API |
| 扫描仪 | CM2820ADN 内置扫描（ADF 50 页） | Helper | TWAIN / WIA 驱动（优先）；SMB 目录监听（备用） |
...
> 重要约束（CLAUDE.md §3）：CM2820ADN 无 WiFi 仅有线网络；不支持 A3；无云端远程发起扫描的开放 API。
...
每 5 秒调用 `POST /api/v1/terminals/:id/tasks/claim`（原 GET + PATCH 方案，改为原子 claim）：
...
每次状态变更 `PATCH /api/v1/print-tasks/:id/status`，`completed` 只上报一次（幂等）。API 失败时保留"待上报"状态，网络恢复后重试，不伪造成功。
...
- 每 5 秒 POST /tasks/claim，分发给 printer-service 或 scanner-service
...
### 4.3 POST /api/v1/terminals/:id/tasks/claim
...
### 4.4 PATCH /api/v1/print-tasks/:id/status
...
（Kiosk 可选：调用 POST /local/print + actionToken 触发 Agent 立即轮询，加速响应）
       │
       ▼
Agent task-runner 每 5s 调用 POST /api/v1/terminals/:id/tasks/claim
       │
       ▼
...
|--------|----------------------|---------|
| 打印机识别 | `Get-Printer -Name "Pantum CM2820ADN"` | 返回对象，不抛 error |
| 活动打印任务 | `Get-PrintJob -PrinterName "Pantum CM2820ADN"` | 打印进行中时返回 1+ 行 |
...
- `agentToken`、`actionTokenSecret`、`localAuthToken` 均使用 **Windows DPAPI** 加密保存，绑定当前用户账号和机器，拷贝到其他机器无法解密
- 加密存储路径：`%ProgramData%\AIJobPrintAgent\agent.token`（仅 Agent 服务账号/管理员可读）
- 代码层：`node-dpapi` 或 PowerShell `CryptProtectData`
...
- 后端 `PATCH /print-tasks/:id/status` 设计为幂等（相同状态重复 PATCH 返回 200）
...
| 依赖 | 奔图 CM2820ADN Windows 驱动 V3.x+ 已安装 |
...
| 依赖项 | 安装要求 | 验证方式 |
|--------|---------|---------|
| 奔图 CM2820ADN Windows 驱动 | 部署前必须已安装，V3.x+ | 控制面板 → 设备和打印机中可见 |
...
| V11 | **DPAPI 加密存储（node-dpapi）** | 加密写入 agent.token，换账号/拷贝到其他机器尝试解密 | 原机原账号成功解密，换机/换账号解密失败 |
...
| 能力 | 说明 |
|------|------|
| 终端注册 | 注册获取 terminalId + agentToken + actionTokenSecret |
| 单实例 Mutex | 启动时加锁，重复启动自动退出 |
| 心跳上报 | 每 30s，携带打印机基本状态 |
| 打印任务 Claim | POST /tasks/claim，lease 防重复 |
| 打印任务执行 | 下载 → MD5 校验 → GDI 打印 → 回传状态 |
| 扫描任务执行 | Named Pipe 触发 Helper → TWAIN → PDF 合并 → 上传 → 回传 |
| 文件上传 | multipart 上传扫描文件 |
| 状态回传 | 打印/扫描结果 PATCH，幂等上报 |
| 临时文件清理 | 任务结束立即删除 + 每小时兜底 |
| Windows 服务 | 开机自启，崩溃重启，后台运行 |
| Session Helper | Named Pipe 通信，TWAIN 扫描 |
| local-api-server | 127.0.0.1:9527，localAuthToken + actionToken 全部鉴权 |
| DPAPI Token 加密 | agentToken / actionTokenSecret / localAuthToken 加密存储 |
...
| 离线任务队列 | 断网时本地缓存，恢复后自动补发 |
| SMB 扫描备用方案 | TWAIN 不可用时监听 SMB 目录 |
...
| # | 风险 | 可能性 | 影响 | 应对措施 |
|---|------|--------|------|---------|
| R1 | 奔图 CM2820ADN 无开放云端打印 API | 已确认 | 高 | Windows GDI Print API，不依赖云端 |
