...
apps/terminal-agent/
  ...
  config/
    agent-config.example.json # 仅作为模板和一次性迁移源，不保存运行时状态
...
| Windows 服务安装 | `node dist/index.js install-service / uninstall-service`（node-windows） | ✅ Phase 8.2C 基线完成；新 Windows 主机需复验 |
...
| lease 续租 | `PATCH /terminal-tasks/:id/lease` | ✅ Phase 8.2C 基线完成；新 Windows 主机需复验 |
