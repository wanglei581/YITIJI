...
> 版本：v1.4（Phase 8.1C 实现记录更新）  
> 创建时间：2026-05-26  
> 最后更新：2026-05-28（v1.4：Phase 8.1C 已实现 — DPAPI/SQLite/PID锁/断网重试/Windows服务；Phase 8.1D Windows真机验证待执行；v1.3：actionTokenSecret每终端独立+可吊销轮换/9528双向鉴权+service-helper-shared-token/actionToken语义明确：Agent只校验与taskId绑定的token不主动申请）  
> 状态：Phase 8.0–8.1C 已实现；Phase 8.1D（Windows 真机验证）待执行；Phase 8.2+ 待开发  
...
| Windows 服务安装 | `node dist/index.js install-service / uninstall-service`（node-windows） | ✅ Phase 8.1C（代码完成，真机验证⏳） |
...
| local-api-server | 127.0.0.1:9527，localAuthToken + actionToken 全部鉴权 | 📋 Phase 8.1D |
| actionToken HMAC | HMAC 签名校验（当前 base64 占位） | 📋 Phase 8.1D |
| lease 续租 | `PATCH /terminal-tasks/:id/lease` | 📋 Phase 8.1D |
