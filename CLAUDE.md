...
核心定位（2026-09-26 起，产品负责人拍板「按推荐执行，托管选 a」）：

AI求职打印服务终端 = AI 求职操作系统（AI 简历、模拟面试、职业规划等 AI 工具，AI 是驱动层）+ 打印扫描与材料履约 + 运营机构的官方渠道与政策入口 + 设备运维后台。

- 公司是**设备与软件供应方**：卖终端、随机交付软件、按年收维保；不运营招聘信息，不承接要持证的就业服务外包。收费只有设备款与维保费，不收与岗位、招聘会、浏览、投递、入职挂钩的费用。
- **托管 a（目标口径，代码与存量数据切换中）**：我们云上不存岗位、招聘会、企业资料；一体机只在运营机构自己的终端上放该机构官方渠道的二维码；政策不属于招聘信息，由机构在我们云上自己审核发布，我们只留紧急下架。完整招聘功能只在客户私有化部署（b）里由客户运营。代码切换见 `docs/progress/next-tasks.md` 3.13–3.15，**切换前代码行为不变，但不得在我们云上新增岗位、招聘会、企业类功能**；切换与存量清理（3.15）完成前，对外材料不得宣称「我们云上已不存这些数据」。
- 定位、用户与客户、AI 求职操作系统分层和 AI 设计原则见 [feature-scope.md §零](docs/product/feature-scope.md)；角色边界见 [role-boundary.md](docs/product/role-boundary.md)；法规与资质总表见 [compliance-boundary.md §1.1–§1.2](docs/compliance/compliance-boundary.md)。
...
岗位和招聘会只能作为第三方/官方来源信息入口。托管 a 下我们云上连这个入口也不做（见 §1）：一体机与小程序不得新增岗位卡片或投递、预约按钮；下面的按钮文案白名单只约束现有代码（切换前）与客户私有化部署（b）版本。
...
- 去来源平台预约
- 扫码预约
- 复制来源链接（仅小程序：小程序不能打开外部网页，只能复制来源平台链接让用户自行前往）
...
> 历史章节：秒哒截图目录已移出仓库（见 §6 注）。本节只保留「参考 / 不照搬」的结论，不作为开发依据。
...
- NestJS（`services/api`，已定；早期写的「或 FastAPI」不再适用）
- PostgreSQL
- Redis
- BullMQ / 任务队列
...
> **2026-09-03 裁决（产品负责人）：一体机前台的正式上线视觉真值是
> `docs/design/kiosk-redesign-2026-08/`（青序流光，51 页）。**
> 它取代了此前「V6 / `kiosk-ai-os-v3-2026-08`」的口径。三套设计稿目录
> （`kiosk-proto-2026-07` / `kiosk-ai-os-v3-2026-08` / `kiosk-redesign-2026-08`）
> **全部保留，一个都不删**；后两者降为设计参考。
> 完整裁决与三套稿的处置见 [docs/progress/next-tasks.md](docs/progress/next-tasks.md) 顶部
> 「2026-09-03 视觉真值裁决」。
>
> 下面这段「不设固定风格限制」仍适用于**管理员后台与合作机构后台**，以及一体机上
> 青序流光未覆盖到的页面；一体机凡是青序流光有对应稿的页面，以稿为准。

项目不再设置固定页面视觉风格限制。`docs/design/` 中的视觉规范、动效规范、历史预览和旧秒哒参考只作为可选设计参考，不再作为“必须使用 / 禁止使用”的硬性标准。

页面 UI 可以根据当前任务重新设计，包括但不限于配色、渐变、毛玻璃、圆角、阴影、动效、插画、图标体系、版式密度和设计系统组织方式。是否先建立 design system、是否复用既有公共组件，由当前任务的交付目标、文件预算和实现风险决定。

仍然必须保留的硬约束：

- 合规边界不变：托管 a 下我们云上不做岗位、招聘会、校招、企业展示；私有化部署（b）里也只能做第三方 / 官方来源信息入口，不得出现平台内投递、收简历、筛选、邀约或 Offer 闭环。
- 用户可见按钮文案不越界：投递 / 预约相关文案继续使用合规白名单，禁止“一键投递 / 立即投递 / 平台投递”等表述。
- 不伪造能力：没有真实数据、真实接口、真实硬件状态或真实保存结果时，页面不得展示已完成、已保存、已投递、已打印、设备正常等结论。
...
> **2026-09-26 托管 a 口径：** 下面「岗位信息」「招聘会信息」两块，以及管理员后台的岗位 / 招聘会信息源、合作机构后台的岗位 / 招聘会信息管理、数据源管理、同步日志，在我们云上按 next-tasks 3.13 关闭（入口隐藏、接口不返回、写入停止），代码保留给私有化部署（b）；机构终端上的这两个入口改指「本机构官方渠道」页（3.14），自营点位隐藏。管理员对招聘类内容只保留紧急下架。功能清单的现行版本以 [feature-scope.md](docs/product/feature-scope.md) §二–§四 为准。
...
- AI简历服务
- 打印扫描
- 岗位信息（托管 a：机构终端改为本机构官方渠道；自营点位隐藏）
- 招聘会信息（同上）
- 政策服务
- 我的记录
- 设备状态
...
**岗位信息**（托管 a：我们云上关闭，只在 b 版本；机构终端改为「本机构官方渠道」二维码页）
- 第三方岗位列表
- 岗位详情
- 来源机构
- 同步时间
...
- 文件管理
- AI服务管理
- 岗位信息源（托管 a：只留查看与紧急下架）
- 招聘会信息源（托管 a：只留查看与紧急下架）
- 合作机构管理（含入驻核验与终端绑定）
- 法务文档
- 用户管理
- 告警中心
...
- 机构资料
- 本机构官方渠道（托管 a，3.14）
- 岗位信息管理（托管 a：我们云上隐藏，只在 b 版本）
- 招聘会信息管理（同上）
- 政策公告管理（机构自己审核发布）
- 终端数据
- 数据统计
- 数据源管理（托管 a：我们云上隐藏，只在 b 版本）
- 同步日志（同上）
- 账号权限

## 10. 数据边界

> 托管 a 下我们云上不再入库外部岗位和招聘会（§1）。下面的字段与展示要求约束现有代码（切换前）与私有化部署（b）版本；「系统只记录」一段对所有版本都有效。
...
> 历史章节：第 0–8 阶段均已完成或被后续方案取代，本节只作项目起步时的顺序记录。当前排期只看 `docs/progress/next-tasks.md` 顶部。
...
目标：提供可复用的页面基础能力，不强制先完成固定视觉规范。

可选任务：

- 梳理当前任务需要的颜色、字体、间距、按钮、卡片、表格、状态标签、空态、加载和错误状态。
- 必要时沉淀公共组件；没有复用价值时可以直接在业务页面内实现。
- 如本轮是视觉探索或页面重做，可以先做页面方案，再按实际复用点回收组件。

验收：

- 页面可访问、可点击、状态真实。
- 没有为了统一视觉而牺牲合规、真实数据或硬件状态表达。
...
- 后续页面优先复用已有组件；当新设计需要不同视觉表达时，可以在任务范围内扩展或替换。
- 重复样式是否抽取按维护成本判断，不因“必须统一 UI”而阻塞页面交付。
...
- 微信小程序底部导航：当前 `main` 代码是首页、职业生活圈、求职、我的（四 Tab）；首发提审版按 2026-09-25 拍板改为 首页 / AI 工具 / 打印 / 我的，并停放岗位、招聘会、企业、政策等 20 页（next-tasks 2.6，feature-scope §1.1）。材料包与今日早报已接真实接口，职业圈动态页首发停放（2026-09-26 复核）；任何接口未到位的入口只能展示为明确标注「未开放」的可解释入口，不得伪造下单、动态或早报结果。Kiosk 导航仍按其独立信息架构维护，不能把小程序 Tab 机械套用到大屏。
- 当前首页与各业务板块里的功能入口已经定版；后续只做已有入口真实化、页面接真、按钮接线、状态补齐和「我的」数据闭环，不新增重复入口 / 同义卡片。详见 `docs/product/user-data-flow-matrix.md`。
- 企业招聘端删除；合作机构后台只做数据与运营后台（托管 a 下只维护本机构官方渠道、政策与本机构终端数据）；管理员后台管理整个终端运营体系，对招聘类内容只保留紧急下架。
- 打印机：奔图 CM2800/CM2820 系列彩色激光多功能一体机，Windows 驱动识别名 `Pantum CM2800ADN Series`，代码必须通过 `printerName` 配置项指定。
- 岗位/招聘会只做第三方/官方来源信息入口；2026-09-26 起我们云上不存岗位、招聘会、企业资料（托管 a，见 §1）。
...
> 口径修正（2026-08-01）：本节此前标注「2026-06-12 上线前收口」，已滞后约 7 周。实时进度只写进 `docs/progress/current-progress.md`，不再在本文件维护第二份阶段声明。下面列出的是已封板、长期不变的里程碑；其中岗位、招聘会、企业相关的里程碑是代码事实，托管 a 下这些能力在我们云上关闭（§1）：
...
- **阶段2 已完成（2026-06-10 ~ 2026-06-12）**：AI 简历生成 MVP、AI 简历优化真实化、真实模型联调 + 安全收口、招聘会场馆导览图、C-2D 会员资产中心真实管理、2C 模拟面试 + 2C+ 语音增强、2D 岗位匹配参考、2E 职业规划、P1 浏览/外部跳转记录接真。
...
## 16. 历史优先级基线（实时任务只看 next-tasks.md 顶部）

> 2026-09-26：当前排期是 `docs/progress/next-tasks.md` 顶部的商业收口方案（60 步）。本节只保留历史上已完成或已裁定的事项，不作为今天的任务来源。其中「场馆导览 Partner 配置入口」与「企业展示 / 找企业」在托管 a 下于我们云上暂停，只在私有化部署（b）里使用。
...
- ~~P1 浏览/外部跳转记录建模 +「我的」建设中入口接真~~ ✅ 已完成（2026-06-12）：`BrowseLog` / `ExternalJumpLog` 只记录本人浏览与打开来源平台或官方入口的行为；不记录投递/预约结果；浏览与跳转记录后端接口已接真（注：2026-06-14 信息架构整改后该明细归位到岗位/招聘会/政策业务页，「我的」页不再聚合「账号资产」分区，旧 Profile 聚合 Group 组件已删除，见 docs/product/user-data-flow-matrix.md 顶部整改）；`verify:activity-logs` 12 PASS 进双 CI。
...
**已完成补充（2026-06-12）：企业展示 / 找企业（CompanyProfile）**——岗位信息页内「找企业」入口 + 企业详情（指标受 Admin 开关控制）+ Admin/Partner 管理 + 浏览/跳转记录闭环。定位「来源企业与岗位导览」非招聘平台（不收简历、无平台内投递），长期红线见 docs/compliance/compliance-boundary.md §4.5。
...
Windows Terminal Agent（第 8 阶段已于 2026-05-29 封板，后续按 next-tasks 迭代）须遵守：
...
> **托管 a（2026-09-26）：** 本节描述的数据源接入（API、Excel、Webhook、定时同步）在我们云上按 next-tasks 3.13 停止写入，代码保留给私有化部署（b）。b 版本里审核发布主体是运营机构自己，管理员只保留紧急下架；下文第 3 条与末条「管理员审核后才展示」描述的是现有代码，切换前照旧，不得据此新增管理员审核发布功能。
...
> 先做干净架构 → 再做当前任务需要的 UI 基础能力 → 再做核心页面 → 再接后端 → 再接打印机 → 再上线测试。
