...
- 本项目手工创建或移动 Git worktree 时，物理路径统一使用仓库根目录下的 `.worktrees/<task-name>`；禁止继续在 `/Users/wanglei` 个人主目录创建 `AI求职打印服务终端-*`、`YITIJI-*` 等同级项目副本。Codex 自动托管 worktree 继续使用 Codex 自己的 Worktree root，不与项目手工候选混放。
- `stash`、分支提交或“工作区干净”都不构成删除活跃 worktree 的授权。执行 `git worktree remove` 前必须同时核对 Codex 活跃任务的 `cwd`、系统进程 `cwd`、开发工具打开路径和当日任务记录；任一命中即排除。批量清理必须先输出精确保留/删除清单，不得边枚举边删除。
...
- 微信小程序底部导航：首页、职业圈、求职、我的（四 Tab）。「职业生活圈」承载小青 AI 助手、真实 AI 求职工具、材料与打印入口；签到、福利中心、邀请有礼在后端权益合同到位前只能展示为不可点击预告，不得伪造 Token、会员或领取结果。Kiosk 导航仍按其独立信息架构维护，不能把小程序四 Tab 机械套用到大屏。
