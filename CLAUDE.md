...
- **托管 a**：我们云上不存岗位、招聘会、企业资料；一体机只在运营机构自己的终端上放该机构官方渠道的二维码；政策不属于招聘信息，由机构在我们云上自己审核发布，我们只留紧急下架。完整招聘功能只在客户私有化部署（b）里由客户运营。代码切换见 `docs/progress/next-tasks.md` 3.13–3.15，**切换前代码行为不变，但不得在我们云上新增岗位、招聘会、企业类功能**。
...
岗位和招聘会只能作为第三方/官方来源信息入口。托管 a 下我们云上连这个入口也不做（见 §1）；下面的按钮文案白名单约束现有代码（切换前）与客户私有化部署（b）版本。
...
- 微信小程序底部导航：当前 `main` 代码是首页、职业生活圈、求职、我的（四 Tab）；首发提审版按 2026-09-25 拍板改为 首页 / AI 工具 / 打印 / 我的，并停放岗位、招聘会、企业、政策等 20 页（next-tasks 2.6，feature-scope §1.1）。材料包、职业圈动态、今日早报在后端接口到位前只能展示为明确标注「未开放」的可解释入口，不得伪造下单、动态或早报结果。Kiosk 导航仍按其独立信息架构维护，不能把小程序 Tab 机械套用到大屏。
