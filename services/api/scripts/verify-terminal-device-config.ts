...
function prepareTempDatabase(): { previousUrl: string | undefined; dbPath: string } {
  ...
  else applySqliteMigrations(dbPath)
...
async function runServiceChecks(): Promise<void> {
  ...
  const tC = `term_vtd_${suffix}_c`
  const tD = `term_vtd_${suffix}_d`
  const codeA = `VTD-${suffix}-A`
  const codeB = `VTD-${suffix}-B`
  const codeC = `VTD-${suffix}-C`
  const codeD = `VTD-${suffix}-D`
  const tokenA = `vtd-token-a-${suffix}`
  const tokenC = `vtd-token-c-${suffix}`
  const tokenD = `vtd-token-d-${suffix}`
  ...
  async function cleanup(): Promise<void> {
    ...
    await prisma.terminalHeartbeat.deleteMany({ where: { terminalId: { in: [tA, tB, tC, tD] } } })
    await prisma.terminalSmartCampusConfig.deleteMany({ where: { terminalId: { in: [codeA, tA, codeB, tB, codeC, tC, codeD, tD, macKey] } } })
    await prisma.auditLog.deleteMany({ where: { actorId: adminId } })
    await prisma.terminal.deleteMany({ where: { id: { in: [tA, tB, tC, tD] } } })
  ...
  try {
    ...
    await prisma.terminal.create({
      data: { id: tC, terminalCode: codeC, agentToken: tokenC, deviceFingerprint: `fp-c-${suffix}` },
    })
    await prisma.terminal.create({
      data: { id: tD, terminalCode: codeD, agentToken: tokenD, deviceFingerprint: `fp-d-${suffix}` },
    })
    ...
    await terminals.heartbeat(
      ...
      {
        printerStatus: 'ok',
        displayName: 'Agent 覆盖名',
        locationLabel: 'Agent 覆盖位置',
        agentVersion: 'verify',
      } as Parameters<typeof terminals.heartbeat>[1],
      `Bearer ${tokenA}`,
    )
    const afterAgentProfileHeartbeat = await prisma.terminal.findUnique({ where: { id: tA } })
    if (
      afterAgentProfileHeartbeat?.displayName === '一号打印服务终端' &&
      afterAgentProfileHeartbeat.locationLabel === '就业中心一楼'
    ) {
      pass('2b. heartbeat 不覆盖 Admin 手工设备名/摆放位置')
    } else {
      fail(`2b. heartbeat 覆盖了 Admin 档案: ${afterAgentProfileHeartbeat?.displayName ?? 'null'} / ${afterAgentProfileHeartbeat?.locationLabel ?? 'null'}`)
    }
    ...
    const afterConflictMacHeartbeat = await prisma.terminal.findUnique({ where: { id: tD } })
    if (afterConflictMacHeartbeat?.macAddress === null) {
      pass('5c. 无 MAC 终端 heartbeat 冲突 MAC 不打挂心跳且不落库')
    } else {
      fail(`5c. heartbeat 冲突 MAC 被写入: ${afterConflictMacHeartbeat?.macAddress ?? 'null'}`)
    }
    await terminals.heartbeat(
      tA,
      { printerStatus: 'ok', macAddress: 'a8-5e-45-10-00-02', agentVersion: 'verify' },
      `Bearer ${tokenA}`,
    )
    const afterChangedMacHeartbeat = await prisma.terminal.findUnique({ where: { id: tA } })
    if (afterChangedMacHeartbeat?.macAddress === 'A8:5E:45:10:00:01') {
      pass('5d. heartbeat 合法新 MAC 不覆盖 Admin 已确认 MAC')
    } else {
      fail(`5d. heartbeat 合法新 MAC 覆盖了 Admin MAC: ${afterChangedMacHeartbeat?.macAddress ?? 'null'}`)
    }
    await terminals.heartbeat(
      tC,
      { printerStatus: 'ok', macAddress: 'a8-5e-45-10-00-03', agentVersion: 'verify' },
      `Bearer ${tokenC}`,
    )
    const afterInitialMacHeartbeat = await prisma.terminal.findUnique({ where: { id: tC } })
    if (afterInitialMacHeartbeat?.macAddress === 'A8:5E:45:10:00:03') {
      pass('5e. heartbeat 可为无 MAC 终端首次补录 MAC')
    } else {
      fail(`5e. heartbeat 未补录初始 MAC: ${afterInitialMacHeartbeat?.macAddress ?? 'null'}`)
    }
