...
function sqlite(db: string, sql: string): string {
  return execFileSync('sqlite3', ['-bail', db], { input: `PRAGMA foreign_keys=ON;\n${sql}`, encoding: 'utf8' }).trim()
}
...
function runPreflight(url: string, expectedStatus: 0 | 2): void {
  const result = spawnSync(process.execPath, ['-r', '@swc-node/register',
    'scripts/verify-recruitment-p1-preflight.ts'], {
    cwd: API_ROOT,
...
function assertNoSqliteDrift(db: string, label: string): void {
  ...
  const drift = spawnSync(process.execPath, [prismaCli, 'migrate', 'diff',
...
function verifyStaticContract(): void {
  ...
  assert.equal((sqliteSchema.match(/^model /gm) ?? []).length, 86, 'SQLite schema model count drifted')
  assert.equal((pgSchema.match(/^model /gm) ?? []).length, 86, 'PostgreSQL schema model count drifted')

  for (const schema of [sqliteSchema, pgSchema]) {
    requireTokens(modelBlock(schema, 'Organization'), 'Organization', [
      'contentTrustStatus String?', 'contentTrustReviewedBy String?', 'contentTrustReviewedAt DateTime?',
      'contentTrustReason String?', 'archivedAt DateTime?',
    ])
    requireTokens(modelBlock(schema, 'JobSource'), 'JobSource', [
      'approvalStatus String?', 'syncEnabled    Boolean?', 'trustStatus    String?',
      'allowedContentDomainsJson String?', 'redirectPolicy String?', 'archivedAt     DateTime?',
    ])
    requireTokens(modelBlock(schema, 'Job'), 'Job', [
      'sourceLastSeenAt', 'contentHash', 'contentVersion', 'approvedContentHash',
      'hashAlgorithmVersion', 'archivedAt', 'offlineBranchId', '@@index([sourceId, externalId])',
      'onDelete: Restrict',
    ])
    requireTokens(modelBlock(schema, 'OfflineJob'), 'OfflineJob', [
      'canonicalJobId String? @unique', 'migrationChecksum String?', 'onDelete: Restrict',
    ])
    requireTokens(modelBlock(schema, 'QualificationRecord'), 'QualificationRecord', [
      '@default("pending")', 'contentVersion', 'contentHash', 'approvedContentHash',
      'hashAlgorithmVersion', 'appliesToBranchId', 'evidenceFileId', 'onDelete: Restrict',
    ])
    const directory = modelBlock(schema, 'OnlinePlatformDirectory')
    requireTokens(directory, 'OnlinePlatformDirectory', [
      'officialDomainsJson', 'landingUrl', '@default("pending")', '@default("draft")',
      'contentVersion', 'approvedContentHash',
      '@relation("PlatformDirectoryOrg", fields: [organizationId], references: [id], onDelete: Restrict)',
    ])
    assert(!/\b(endpoint|authType|encryptedCredential|webhookSecret|responseConfig|syncEnabled)\b/.test(directory),
    ...
    requireTokens(modelBlock(schema, 'OfflineAgencyProfile'), 'OfflineAgencyProfile', [
      'organizationId       String   @unique', '@default("pending")', '@default("draft")',
    ])
    requireTokens(modelBlock(schema, 'OfflineAgencyBranch'), 'OfflineAgencyBranch', [
      'provinceCode', 'cityCode', 'districtCode', '@default("suspended")',
      'reviewStatus', 'publishStatus', 'contentVersion', 'contentHash', 'approvedContentHash',
      'hashAlgorithmVersion', 'onDelete: Restrict',
    ])
    const decision = modelBlock(schema, 'ReviewDecision')
    requireTokens(decision, 'ReviewDecision', [
      'contentVersion', 'contentHash', 'action', 'actorRole', 'occurredAt', 'correlationId', 'requestId',
    ])
    assert(!decision.includes('updatedAt'), 'ReviewDecision must be append-only (no updatedAt)')
    for (const model of ['QualificationRecord', 'OnlinePlatformDirectory', 'OfflineAgencyProfile',
      'OfflineAgencyBranch', 'ReviewDecision']) {
      const block = modelBlock(schema, model)
      assert(!block.includes('onDelete: Cascade'), `${model} must not cascade-delete governance history`)
      assert(!/\b(EndUser|resumeFileId|memberId|applicationStatus|appliedAt)\b/.test(block),
        `${model} must not link recruitment governance to job-seeker data`)
    }
    assert(!/model (Application|Applicant|Candidate|CandidatePool|ResumeSubmission|RecruiterInbox|InterviewInvite|Offer|Referral)\b/.test(schema),
      'recruitment-closure model is forbidden')
    assert(!/\b(applicationStatus|appliedAt|employerViewedAt|shortlist|interviewStatus|offerStatus|hireResult|recommendationResult)\b/.test(schema),
  ...
  for (const [label, sql] of [['SQLite', sqliteMigration], ['PostgreSQL', pgMigration]] as const) {
    assert(!/\b(DROP|TRUNCATE|RENAME\s+(?:TABLE|COLUMN)|INSERT INTO|DELETE FROM|UPDATE\s+"|SET NOT NULL)/i.test(sql),
      `${label} migration is not additive`)
    assert(!/\b(Application|Applicant|Candidate|ResumeSubmission|InterviewInvite|Offer)\b/.test(sql),
      `${label} migration contains recruitment-closure objects`)
  }
  assert(!/CREATE TABLE "new_|PRAGMA foreign_keys\s*=\s*OFF/i.test(sqliteMigration),
    'SQLite migration must not rebuild legacy tables or disable foreign keys')
  requireTokens(pgMigration, 'PostgreSQL migration', [
    'Job_offlineBranchId_fkey', 'OfflineJob_canonicalJobId_fkey', 'NOT VALID',
  ])
}
...
function verifySqliteFreshAndUpgrade(): void {
  ...
  `), '0', 'SQLite upgrade unexpectedly seeded governance tables')
  assert.equal(sqlite(upgradeDb, `
    SELECT
  ...
  `), '0', 'SQLite upgrade inferred governance state for legacy rows')
  assert.equal(sqlite(upgradeDb, 'PRAGMA foreign_key_check;'), '', 'upgraded SQLite has FK violations')
...
async function verifyPostgresUpgrade(): Promise<void> {
  ...
  assert(['localhost', '127.0.0.1', '::1'].includes(parsed.hostname), 'PostgreSQL upgrade gate must use localhost')
  assert(parsed.pathname.endsWith('_upgrade_ci'), 'PostgreSQL upgrade database name must end with _upgrade_ci')

  const { PrismaPg } = await import('@prisma/adapter-pg')
  const adapter = await new PrismaPg({ connectionString: url }).connect()
  const pool = adapter.underlyingDriver()
  try {
    const initial = await pool.query(`SELECT COUNT(*)::int AS count FROM information_schema.tables WHERE table_schema='public'`)
    ...
    runPreflight(url, 0)
    const legacyTables = ['Organization', 'JobSource', 'Job', 'OfflineAgency', 'OfflineJob']
    const before = await Promise.all(legacyTables.map(async (table) =>
    ...
    const after = await Promise.all(legacyTables.map(async (table) =>
      Number((await pool.query(`SELECT COUNT(*)::int AS count FROM "${table}"`)).rows[0]?.count)))
    ...
    `)
    assert.equal(Number(state.rows[0]?.new_rows), 0, 'PostgreSQL upgrade unexpectedly seeded governance tables')
    assert.equal(Number(state.rows[0]?.inferred_rows), 0, 'PostgreSQL upgrade inferred governance state')
    const invalidConstraints = await pool.query(`
      SELECT conname FROM pg_constraint
      WHERE connamespace = 'public'::regnamespace AND NOT convalidated
      ORDER BY conname
    `)
    assert.deepEqual(invalidConstraints.rows.map((row) => row.conname),
      ['Job_offlineBranchId_fkey', 'OfflineJob_canonicalJobId_fkey'])

    const drift = spawnSync(process.execPath, [join(API_ROOT, 'node_modules/prisma/build/index.js'),
      'migrate', 'diff', '--config', 'prisma.postgres.config.ts',
      '--from-config-datasource', '--to-schema', 'prisma/postgres/schema.prisma', '--exit-code'], {
      cwd: API_ROOT,
      env: { ...process.env, POSTGRES_URL: url },
      encoding: 'utf8',
    })
    assert.equal(drift.status, 0, `upgraded PostgreSQL/schema drift:\n${drift.stdout}\n${drift.stderr}`)
  } finally {
...
async function main(): Promise<void> {
  ...
  console.log(`PASS recruitment P1 schema: static + ${process.argv.includes('--postgres-upgrade') ? 'PostgreSQL upgrade' : 'SQLite fresh/upgrade'}`)
}
