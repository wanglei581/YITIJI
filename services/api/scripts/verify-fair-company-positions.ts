...
async function main() {
  ...
  const orgId = `org_vcp_${suffix}`

  const adminRow = await prisma.user.create({
    data: { username: `vcp_admin_${suffix}`, passwordHash: 'x', name: '验证管理员', role: 'admin' },
  })
  ...
  const cleanup = async () => {
    // fairCompany / jobFair 删除经 onDelete: Cascade 连带删 FairCompanyPosition。
