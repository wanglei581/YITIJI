...
async function main() {
  ...
  try {
    ...
    if (pList.total !== pList.items.length || pList.truncated) fail('9. Partner 企业列表 total/truncated 应反映完整结果')
    const pc = pList.items.find((c) => c.externalId === `pc1-${tag}`)
    ...
    const pAfter = (await companies.partnerList(orgB)).items.find((c) => c.id === pc.id)
    ...
    await companies.adminReview(pc.id, { action: 'approve' }, admin)
    await companies.adminPublish(pc.id, { publish: true }, admin)
    const beforeUnpub = (await companies.partnerList(orgB)).items.find((c) => c.id === pc.id)
