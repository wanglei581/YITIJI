...
async function main() {
  ...
  const orgId = `org_vaf_${suffix}`

  // AuditLog.actorId 有 FK → User,须建真实测试管理员行(脚本结束清理)。
  const adminRow = await prisma.user.create({
    data: { username: `vaf_admin_${suffix}`, passwordHash: 'x', name: '验证管理员', role: 'admin' },
  })
  ...
  const cleanup = async () => {
    // 资料对象在 deleteMaterial 已物理删;级联删 fair 子表
    await prisma.fairMaterial.deleteMany({ where: { jobFairId: { in: [fairPublished.id, fairDraft.id] } } })
  ...
  try {
    ...
    // ── 11. 统计 ───────────────────────────────────────────────────────────
    {
      ...
      if (stats.materialTotal !== 0) fail('11. 软删资料不应计入统计')
