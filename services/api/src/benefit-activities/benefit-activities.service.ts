...
export class BenefitActivitiesService {
  ...
  async adminList(query: AdminListBenefitActivitiesQueryDto): Promise<{ items: BenefitActivityListItem[]; total: number; truncated: boolean }> {
    ...
    return { items: rows.map((row) => this.toActivityItem(row, false)), total, truncated: total > rows.length }
  }
  ...
  async listClaims(id: string): Promise<{ items: BenefitActivityClaimItem[]; total: number; truncated: boolean }> {
  ...
  async listVisible(query: ListBenefitActivitiesQueryDto, endUserId?: string | null): Promise<{ items: BenefitActivityListItem[]; total: number; truncated: boolean }> {
    ...
    return { items: rows.map((row) => this.toActivityItem(row, claimedIds.has(row.id))), total, truncated: total > rows.length }
  }
