 ...
 *   彩色 / 自动双面在能力开关引入时**没有任何一台真机验过驱动映射**，所以没有可兼容的
 *   既有闭环，「未配置」的真实含义是「这台机器没验过」，必须拒绝。
 *
 *   2026-09-09 口径更正：上面那句此前写成「**从未**在任何一台真机上验证过」——
 *   到今天已经不成立。`docs/delivery/kiosk-redesign-r1/evidence-ledger.csv` 的
 *   **EV-013（2026-09-02，产品负责人真机实操，PASS）** 已验过彩色与自动双面出纸
 *   （证据 `.../evidence/EV-013-device-color-duplex.txt`）。
 *   产品负责人 2026-09-09 反馈「彩色一直用不了，可这功能早就验过了」，根因就是
 *   这句过期的话 + 那台终端从未登记，而不是功能被删。
 *
 *   **但本默认不变，而且不该变**：EV-013 验的是那一台机器上的那一次，
 *   `DEFAULT_DENY_CAPABILITY_KEYS` 要挡的是「这台机器没验过」，新机器仍须各自验。
 *   EV-013 自己也留了两条未验：长边/短边翻页方向未核对、故障恢复路径未验证。
 *
 * 后果不对称，所以默认必须偏向拒绝：
