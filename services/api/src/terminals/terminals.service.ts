...
export class TerminalsService implements OnModuleInit {
  ...
  private async buildDeviceProfilePatch(
    dto: Pick<HeartbeatDto, 'macAddress'>,
    ownerRef: string,
  ): Promise<{ macAddress?: string | null }> {
    const data: { macAddress?: string | null } = {}
    if (dto.macAddress !== undefined) {
      ...
      if (macAddress) {
        ...
        if (terminal?.macAddress) {
          if (terminal.macAddress !== macAddress) {
            this.logger.warn(`heartbeat ignored MAC address change from terminal ${ownerRef}`)
          }
          return data
        }
        try {
