...
export class ContractReviewProviderService {
  ...
  async reviewWithIdentity(input: ContractProviderReviewInput): Promise<ContractProviderReviewOutput> {
    ...
    try {
      ...
      // 基线 30 秒 + 每 2000 字符 20 秒，最终由 transport 按 MAX_TIMEOUT_MS 封顶。
      // 依据 2026-08-17 实测：数十字文本调 deepseek-v4-pro 约 13 秒，
      // 推理模型的耗时随输入长度显著增长。
      const inputChars = JSON.stringify(payload.messages ?? []).length
      const scaledTimeoutMs = DEFAULT_TIMEOUT_MS + Math.ceil(inputChars / 2000) * 20_000
