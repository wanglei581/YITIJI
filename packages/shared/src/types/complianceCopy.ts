...
export const COMPLIANCE_COPY = {
   ...
   */
  KIOSK_RESUME_UPLOAD_PRIVACY: '上传的简历仅用于你发起的 AI 分析,不进入简历库、不用于其他用途;识别与分析会使用第三方 OCR / AI 服务处理内容。未登录或高敏文件短期自动清理;登录后默认保存 90 天,可在「我的文档」中调整保存期限或随时删除。',
   ...
   */
  KIOSK_PRINT_SCAN_SENSITIVE: '证件照、身份证等敏感文件仅用于本次打印/扫描,完成后按隐私策略自动清理,不长期留存;打印前隐私检查时,扫描件/图片可能通过第三方 OCR 服务识别文字。',
