...
/**
 * 取件码长度：8（产品裁决 2026-08-18 方案 A）。
 *
 * 不是 6 位：立项时「100 万 ÷ 20 次每分钟 ≈ 35 天跑不完键空间」用错了指标 ——
 * 攻击者只需命中**任意一枚在用的码**，至少命中一次的概率 = 1 − exp(−N·K/S)
 * （N = 同时在用的码数 ≈ 日单量 × 有效期天数）。6 位在 24 小时口径下就已不成立。
 *
 * 8 位 + 7 天**单靠限流仍不够**（约 50.6%），承重的是按终端失败锁定。
 * 完整推导见 `services/api/src/common/pickup-code.ts`。
 */
...
// 旧码 10 位，新正则只收 8 位，请求在 DTO 就 400，根本到不了哈希查询。
// 所以下面这套 legacy 常量在过渡期内必须被 DTO 与 kiosk 输入框一起接受。
//
// 何时可以删：最后一枚 10 位码过期之后 = 本改动上线满 24 小时。
// 存量码按当时的 24h TTL 签发、过期时间早已落库，不会被新的 7 天 TTL 重算。
