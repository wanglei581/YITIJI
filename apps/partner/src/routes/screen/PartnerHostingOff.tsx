import type { ScreenSnapshotMetrics } from '@ai-job-print/shared'
import {
  SCREEN_HOSTING_OFF_NOTE,
  TwinMetricPanel,
  TwinTerminalWall,
  screenCount,
  screenReasonCopy,
  type ScreenGapEntry,
  type TwinCityTerminal,
} from '@ai-job-print/ui'

/**
 * 机构总览在招聘内容托管关闭（托管 a，我们云上的默认部署）时的几块。
 *
 * 托管 a：我们云上不再发布岗位、招聘会、企业资料（存量清理见 next-tasks 3.15），数据源也不再同步：原来的「数据同步」「招聘会」
 * 「本机构在架信息」三块整块是招聘内容，不再渲染成「未开启」或「没有同步批次」的说明块，
 * 块位让给真实数据 —— 左中「终端状态墙」长满两个块位（机构大、终端多也放得下），
 * 右上「本机构政策」一块写全：在架、待审核、岗位类存量一句、边界一句（全屏唯一一处）。
 * 「建设中的指标」在展示档不上屏，桌面档收成一行。
 *
 * 每屏一个数只出现一次：台数写在左上「本机构终端」里，状态墙的副标题不再写台数；
 * 待审核只在政策这一块里写。
 */

type Metrics = ScreenSnapshotMetrics

/** 状态墙最多画几格（含「另 N 台」那一格）：展示档两个块位高，三列七行、每格三行字放得下。 */
const ORG_WALL_LIMIT = 21

export function OrgFleetWallPanel({
  metric,
  terminals,
  focus,
  unassignedPlace,
  onOpen,
}: {
  metric: Metrics['fleetWall']
  terminals: readonly TwinCityTerminal[]
  focus: string | null
  unassignedPlace: string
  onOpen: (terminal: TwinCityTerminal) => void
}) {
  return (
    <TwinMetricPanel
      title={focus === null ? '终端状态墙' : `${focus}终端状态墙`}
      sub="点一格进入该终端的孪生"
      metric={metric}
      source="终端心跳投影，一格一台，只含登记在本机构名下的终端。最要紧的排前面：离线 → 告警 → 未上报 → 打印中 → 在线。每格写编号、状态与服务点位；点一格进入该终端的孪生。"
      render={() => (
        <TwinTerminalWall
          terminals={terminals}
          limit={ORG_WALL_LIMIT}
          onOpen={onOpen}
          emptyText={focus === null ? '本机构还没有登记的终端' : `${focus}没有登记的终端`}
          unassignedPlace={focus === null ? unassignedPlace : undefined}
        />
      )}
    />
  )
}

/**
 * 本机构政策：在架与待审核两个数、托管关闭前留下的岗位类存量一句、边界一句。
 * 待审核与存量都取自同一份在架统计（contentInventory 的 *Pending 即待审与审核中的合计），一块一个来源。
 */
export function OrgPolicyPanel({ metric, scope }: { metric: Metrics['contentInventory']; scope?: string }) {
  return (
    <TwinMetricPanel
      title="本机构政策"
      sub="本机构审核发布"
      scope={scope}
      tone="info"
      metric={metric}
      source="「在架」= 本机构审核通过、已发布且在有效期内的政策公告；「待审核」为待审与审核中的合计，审核通过并发布后才对外展示。政策由本机构在本平台自行审核发布。托管关闭前提交的岗位、招聘会、企业资料不再进入审核，只在这里提一句存量。"
      render={(value) => {
        const stock = value.jobsPending + value.fairsPending + value.companiesPending
        return (
          <>
            {/* 两个数与存量一句落在块的中间（桌面档这一行被告警撑高时不在中间留一截空白），边界句留在底部 */}
            <div className="twin-mid">
              <div className="twin-hero">
                <span className="twin-big">{screenCount(value.policiesPublished)}</span>
                <span className="twin-unit">条</span>
                <span className="twin-muted">在架政策</span>
              </div>
              <div className="twin-stat">
                <span>待本机构审核</span>
                <b className={value.policiesPending > 0 ? 'is-warn' : undefined}>
                  {screenCount(value.policiesPending)}
                  <span className="twin-unit">条</span>
                </b>
              </div>
              {stock > 0 ? <p className="twin-cap twin-muted">另有岗位类存量 {screenCount(stock)} 条（托管关闭后不再审核）</p> : null}
            </div>
            <p className="twin-cap">{SCREEN_HOSTING_OFF_NOTE}</p>
          </>
        )
      }}
    />
  )
}

/**
 * 桌面档底栏：「建设中的指标」收成一行，点开看是哪些、为什么没有。不是面板，不占一整块。
 * 托管关闭带来的那一类（打开来源平台入口等）不在这里重复列 —— 边界已在政策面板里说过一次。
 */
export function OrgGapLine({ entries, count }: { entries: ScreenGapEntry[]; count: number }) {
  if (count === 0) return null
  return (
    <details className="twin-gapline">
      <summary>另有 {screenCount(count)} 项指标待补齐机构归属等数据后显示</summary>
      <div className="twin-chips is-flow">
        {entries.flatMap((entry) =>
          entry.labels.map((label) => (
            <span key={`${entry.reason}-${label}`} className="twin-pend" title={screenReasonCopy(entry.reason).detail}>
              {label}
            </span>
          )),
        )}
      </div>
    </details>
  )
}
