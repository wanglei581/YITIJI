...
function verifySourceStructure(): void {
  ...
  assert.match(source, /setInterval\(\(\) => \{[\s\S]*if \(!paused\) void sweepFolder/, 'must periodically re-sweep the folder while scan intake is active')
  assert.match(source, /options: \{ startPaused\?: boolean \}/, 'updater maintenance must be able to start scan intake paused')
  assert.match(source, /pause: \(\) => \{ paused = true \}/, 'updater drain must be able to pause new scan intake')
  assert.match(source, /if \(wasPaused\) void sweepFolder/, 'resuming scan intake must immediately sweep files ignored during maintenance')
