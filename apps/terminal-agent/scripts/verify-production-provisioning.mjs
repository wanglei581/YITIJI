...
assert.match(provisioner, /打印机自检/)
assert.match(provisioner, /function Get-PrintablePortState/)
assert.match(provisioner, /function Invoke-PrintTestPage/)
assert.match(provisioner, /System\.Net\.Sockets\.TcpClient/)
assert.match(provisioner, /PRINT SUCCESS/)
assert.match(installer, /\[switch\]\$RepairProgramDataAcl/)
assert.match(installer, /Set-ProgramDataTreeAcl -Root \$programDataDir/)
assert.match(provisioner, /RepairProgramDataAcl = \$true/)
