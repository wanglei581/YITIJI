...
assert.equal(inputs.productVersion, '0.4.2')
assert.equal(
...
assert.match(
  bundle,
  /LaunchTarget="\[ProgramFiles64Folder\]AIJobPrintAgent\\provision\\provision-terminal\.cmd"/,
)
...
assert.match(wix, /ProvisioningWizardShortcut/)
assert.match(wix, /设备绑定向导/)
assert.match(wix, /ProvisioningWizardShortcut[\s\S]*Directory="AgentProgramMenuFolder"/)
assert.match(wix, /StandardDirectory Id="DesktopFolder"/)
assert.match(wix, /Id="AgentPanelDesktopInternetShortcut"/)
assert.match(wix, /Name="AI 求职打印服务终端\.url"/)
assert.match(wix, /ComponentRef Id="AgentPanelDesktopShortcutComponent"/)
...
assert.match(lifecycle, /Assert-ProvisioningShortcut/)
assert.match(lifecycle, /Assert-DesktopShortcut/)
assert.match(lifecycle, /Local status panel desktop shortcut remains after uninstall/)
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.4\.1"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.4\.2"/)
assert.match(upgradeLifecycle, /Assert-PanelShortcut/)
assert.match(upgradeLifecycle, /Assert-ProvisioningShortcut/)
assert.match(upgradeLifecycle, /Assert-DesktopShortcut/)
assert.match(upgradeLifecycle, /0\.4\.1 predecessor unexpectedly contains the 0\.4\.2 desktop panel shortcut/)
...
assert.match(workflow, /ref: 66579a4e62231de730206c2f12b4d83a527a2080/)
assert.match(workflow, /path: predecessor-0\.4\.1/)
assert.match(workflow, /predecessor-0\.4\.1\/apps\/terminal-agent\/installer\/build-staging\.ps1/)
assert.match(workflow, /predecessor-0\.4\.1\/apps\/terminal-agent\/installer\/artifacts\/exe/)
assert.ok(
