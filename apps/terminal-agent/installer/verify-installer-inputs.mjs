...
assert.equal(inputs.productVersion, '0.3.7')
assert.equal(agentPackage.version, inputs.productVersion)
assert.match(agentEntrypoint, /\.version\('0\.3\.7'\)/)
...
assert.match(exeUpgrade, /EXE_UPGRADE_PASS from=0\.3\.6 to=0\.3\.7/)
assert.match(exeUpgrade, /An unprovisioned upgrade must remain Stopped\/Manual until the GUI succeeds/)
assert.match(exeUpgrade, /ProgramData state was not retained across the 0\.3\.6 to 0\.3\.7 upgrade/)
...
assert.match(workflow, /134f352ad337dff527ca8c8f2467598d0f3f4ee3/)
