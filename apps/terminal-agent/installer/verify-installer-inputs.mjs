...
assert.equal(inputs.productVersion, '0.3.6')
assert.equal(agentPackage.version, inputs.productVersion)
assert.match(agentEntrypoint, /\.version\('0\.3\.6'\)/)
...
assert.match(exeUpgrade, /EXE_UPGRADE_PASS from=0\.3\.5 to=0\.3\.6/)
assert.match(exeUpgrade, /An unprovisioned upgrade must remain Stopped\/Manual until the GUI succeeds/)
assert.match(exeUpgrade, /ProgramData state was not retained across the 0\.3\.5 to 0\.3\.6 upgrade/)
...
assert.match(workflow, /cf24e226f6c3c48f933942740a42d536cbf40798/)
