...
const updateHelper = read('provision/terminal-update-helper.ps1')
const updateManifestBuilder = read('create-update-manifest.ps1')
const updatePolicyInjector = read('inject-update-policy.ps1')
const updateHelperVerify = read('verify-update-helper.ps1')
const releaseSigner = read('sign-windows-release.ps1')
...
assert.match(staging, /terminal-update-helper\.ps1/)
...
assert.match(fs.readFileSync(path.join(root, 'generate-wix-fragment.ps1'), 'utf8'), /provision\/terminal-update-helper\.ps1/)
...
assert.match(controlCenter, /检查在线更新/)
assert.match(controlCenter, /等待当前打印与扫描上传完成/)
assert.match(controlCenter, /terminal-update-helper\.ps1/)
assert.match(controlCenter, /在线升级发布通道尚未配置企业代码签名证书/)
assert.match(controlCenter, /Remove-Item -LiteralPath \$resultPath -Force -ErrorAction SilentlyContinue/)
assert.match(controlCenter, /-ExpectedManifestApprovalId/)
assert.match(controlCenter, /\$result\.manifestApprovalId/)
assert.match(controlCenter, /control-center-result-/)
assert.match(controlCenter, /-ResultPath/)
assert.match(controlCenter, /WaitForExit\(250\)/)
assert.match(controlCenter, /\$form\.ControlBox = -not \$Busy/)
assert.match(controlCenter, /rebootRequired/)
assert.doesNotMatch(controlCenter, /Start-Process[^\r\n]*-Wait[^\r\n]*terminal-update-helper/i)
...
assert.match(updateHelper, /Get-AuthenticodeSignature/)
assert.match(updateHelper, /Get-FileHash -LiteralPath \$Destination -Algorithm SHA256/)
assert.match(updateHelper, /UPDATE_MANIFEST_SIGNATURE_INVALID/)
assert.match(updateHelper, /UPDATE_MANIFEST_CHANGED/)
assert.match(updateHelper, /ExpectedManifestApprovalId/)
assert.match(updateHelper, /MaximumRedirection 0/)
assert.match(updateHelper, /SecurityProtocolType\]::Tls12/)
assert.match(updateHelper, /maxManifestBytes = 64 \* 1024/)
assert.match(updateHelper, /maxPackageBytes = 1024 \* 1024 \* 1024/)
assert.match(updateHelper, /UPDATE_MANIFEST_SIZE_INVALID/)
assert.match(updateHelper, /Assert-CanonicalText/)
assert.match(updateHelper, /Assert-ReleaseVersion/)
assert.match(updateHelper, /manifestApprovalId = \$manifestApprovalId/)
assert.match(updateHelper, /\$manifest\.rollback\.size/)
assert.match(updateHelper, /install\.lock/)
assert.match(updateHelper, /FileShare\]::None/)
assert.match(updateHelper, /UPDATE_ALREADY_IN_PROGRESS/)
assert.match(updateHelper, /local\/update\/drain\/begin/)
assert.match(updateHelper, /local\/update\/health/)
assert.match(updateHelper, /UPDATE_HEALTH_CHECK_FAILED/)
assert.match(updateHelper, /allowedManifestHost = "zyidai\.cn"/)
assert.match(updateHelper, /UPDATE_ROLLBACK_STARTED/)
assert.match(updateHelper, /UPDATE_ROLLED_BACK/)
assert.match(updateHelper, /update-maintenance\.json/)
assert.match(updateHelper, /UPDATE_TEST_HOOK_FORBIDDEN/)
assert.match(updateHelper, /UPDATE_CONTROL_TOKEN_INVALID/)
assert.match(updateHelper, /Invoke-LocalControl -Path "\/local\/update\/drain\/complete"/)
assert.match(updateHelper, /for \(\$attempt = 1; \$attempt -le 3; \$attempt\+\+\)/)
assert.match(updateHelper, /\$rsa\.KeySize -lt 2048/)
assert.doesNotMatch(updateHelper, /function Remove-MaintenanceMarker/)
assert.match(updateHelper, /WaitForExit\(30000\)/)
assert.match(updateHelper, /Write-MaintenanceMarker -CandidateVersion \$MaintenanceVersion/)
assert.doesNotMatch(updateHelper, /Invoke-Expression|iex\b|ScriptBlock::Create/i)
assert.match(updateManifestBuilder, /SignData/)
assert.match(updateManifestBuilder, /MinimumVersion must not be higher than Version/)
assert.match(updateManifestBuilder, /Package, rollback package, and output paths must be distinct/)
assert.match(updateManifestBuilder, /\$Manifest\.rollback\.size/)
assert.match(updateManifestBuilder, /\$uri\.IsDefaultPort/)
assert.match(updateManifestBuilder, /Manifest signing RSA key must be at least 2048 bits/)
assert.match(updateManifestBuilder, /Release packages must be non-empty and no larger than 1 GiB/)
assert.match(updateManifestBuilder, /three-part numeric Windows Installer versions/)
assert.match(updateManifestBuilder, /release notes exceed manifest limits/)
assert.match(updatePolicyInjector, /ManifestSignerPublicKeyPath/)
assert.match(updatePolicyInjector, /pinnedManifestSignerPublicKeyBase64/)
assert.match(updatePolicyInjector, /updatePublisherBase64/)
assert.match(updatePolicyInjector, /Manifest signer RSA public key must be at least 2048 bits/)
assert.match(updatePolicyInjector, /Updater policy marker must occur exactly once/)
assert.match(updatePolicyInjector, /Control center update policy marker must occur exactly once/)
assert.match(updateHelperVerify, /tamperRejected=true/)
assert.match(updateHelperVerify, /confirmationBound=true/)
assert.match(updateHelperVerify, /candidateChangeRejected=true/)
assert.match(updateHelperVerify, /staleApprovalRejected=true/)
assert.match(stagedPowerShellVerify, /create-update-manifest\.ps1/)
assert.match(stagedPowerShellVerify, /inject-update-policy\.ps1/)
assert.match(stagedPowerShellVerify, /sign-windows-release\.ps1/)
assert.match(stagedPowerShellVerify, /verify-update-helper\.ps1/)
assert.match(updateHelperVerify, /testHooksGuarded=true/)
assert.match(releaseSigner, /\[string\[\]\]\$Paths/)
assert.match(releaseSigner, /string\]::Equals\(\$signature\.SignerCertificate\.Subject, \$ExpectedPublisher, \[System\.StringComparison\]::OrdinalIgnoreCase\)/)
assert.match(upgradeLifecycle, /ExpectedPredecessorVersion = "0\.4\.7"/)
assert.match(upgradeLifecycle, /ExpectedCandidateVersion = "0\.4\.8"/)
assert.match(upgradeLifecycle, /RequireUpdateHelper/)
assert.match(workflow, /verify-update-helper\.ps1/)
assert.match(workflow, /ref: 7f7378ed734b0569dacc00d7bfefba717090b559/)
assert.match(workflow, /Exercise deployed 0\.4\.6-to-candidate upgrade/)
assert.match(workflow, /Exercise signed candidate over deployed 0\.4\.6/)
assert.match(workflow, /Verify final manifest with the client-pinned public key/)
assert.match(workflow, /UPDATE_CURRENT_VERSION_TOO_OLD/)
assert.match(workflow, /Sign inner MSIs, build bundles, and sign outer EXEs/)
assert.match(workflow, /terminal-update-key-pair-check/)
assert.match(workflow, /Update manifest private\/public keys do not form one key pair/)
const stableSigningStart = workflow.indexOf('Sign inner MSIs, build bundles, and sign outer EXEs')
const candidateMsiSigning = workflow.indexOf('-Paths $candidateMsi.FullName', stableSigningStart)
const candidateBundleBuild = workflow.indexOf('& apps/terminal-agent/installer/build-exe.ps1', candidateMsiSigning)
const candidateExeSigning = workflow.indexOf('-Paths $candidateExe.FullName', candidateBundleBuild)
assert.ok(stableSigningStart >= 0 && candidateMsiSigning > stableSigningStart)
assert.ok(candidateBundleBuild > candidateMsiSigning, 'stable MSI must be signed before it is embedded in the EXE')
assert.ok(candidateExeSigning > candidateBundleBuild, 'outer EXE must be signed after bundle construction')
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = \$ExpectedPredecessorVersion/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = \$ExpectedCandidateVersion/)
