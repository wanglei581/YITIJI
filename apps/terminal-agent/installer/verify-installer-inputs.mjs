...
assert.equal(inputs.productVersion, '0.4.4')
assert.equal(
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.4\.3"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.4\.4"/)
...
assert.match(upgradeLifecycle, /0\.4\.3 predecessor desktop panel shortcut is missing/)
...
assert.match(workflow, /ref: b843b63801c50119145e21deca28d9b1a172276e/)
assert.match(workflow, /verify-staged-powershell\.ps1/)
assert.match(workflow, /path: predecessor-0\.4\.3/)
assert.match(workflow, /predecessor-0\.4\.3\/apps\/terminal-agent\/installer\/build-staging\.ps1/)
assert.match(workflow, /predecessor-0\.4\.3\/apps\/terminal-agent\/installer\/artifacts\/exe/)
assert.ok(
