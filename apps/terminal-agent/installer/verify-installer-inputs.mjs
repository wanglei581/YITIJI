...
assert.equal(inputs.productVersion, '0.4.5')
assert.equal(
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.4\.4"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.4\.5"/)
...
assert.match(upgradeLifecycle, /0\.4\.4 predecessor desktop panel shortcut is missing/)
...
assert.match(workflow, /ref: 35cd2e815477a787fbea5aca6dc1d90f29e157b7/)
assert.match(workflow, /verify-staged-powershell\.ps1/)
assert.match(workflow, /path: predecessor-0\.4\.4/)
assert.match(workflow, /predecessor-0\.4\.4\/apps\/terminal-agent\/installer\/build-staging\.ps1/)
assert.match(workflow, /predecessor-0\.4\.4\/apps\/terminal-agent\/installer\/artifacts\/exe/)
assert.ok(
