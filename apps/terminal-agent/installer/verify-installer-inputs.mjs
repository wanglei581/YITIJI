...
assert.equal(inputs.productVersion, '0.3.5')
assert.equal(agentPackage.version, inputs.productVersion)
assert.match(agentEntrypoint, /\.version\('0\.3\.5'\)/)
...
assert.match(exeUpgrade, /EXE_UPGRADE_PASS from=0\.3\.4 to=0\.3\.5/)
assert.match(exeUpgrade, /An unprovisioned upgrade must remain Stopped\/Manual until the GUI succeeds/)
assert.match(exeUpgrade, /ProgramData state was not retained across the 0\.3\.4 to 0\.3\.5 upgrade/)
...
assert.match(workflow, /12b523b9afea2c8d1032de553a30d4264789d5da/)
