...
assert.equal(inputs.productVersion, '0.4.1')
assert.equal(
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.4\.0"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.4\.1"/)
assert.match(upgradeLifecycle, /Assert-PanelShortcut/)
assert.match(upgradeLifecycle, /Assert-ProvisioningShortcut/)
assert.match(upgradeLifecycle, /0\.4\.0 predecessor unexpectedly contains the 0\.4\.1 provisioning wizard/)
...
assert.match(workflow, /ref: 28c9202dcc57a6ad8f98f7de9e29ffcfb53738da/)
assert.match(workflow, /path: predecessor-0\.4\.0/)
assert.match(workflow, /predecessor-0\.4\.0\/apps\/terminal-agent\/installer\/build-staging\.ps1/)
assert.match(workflow, /predecessor-0\.4\.0\/apps\/terminal-agent\/installer\/artifacts\/exe/)
assert.ok(
