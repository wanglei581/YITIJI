...
assert.equal(inputs.productVersion, '0.3.4')
assert.equal(agentPackage.version, inputs.productVersion)
assert.match(agentEntrypoint, /\.version\('0\.3\.4'\)/)
...
assert.match(exeLifecycle, /INSTALLED_RUNTIME_ACL_PASS/)
assert.match(exeLifecycle, /-SelfTestRuntimeAcl/)
assert.match(exeLifecycle, /-ApiBaseUrl "https:\/\/probe\.invalid\/api\/v1"/)
assert.match(exeLifecycle, /Write-Host \$_/, 'EXE lifecycle must echo the runtime ACL probe evidence')
...
assert.match(exeUpgrade, /EXE_UPGRADE_PASS from=0\.3\.3 to=0\.3\.4/)
assert.match(exeUpgrade, /An unprovisioned upgrade must remain Stopped\/Manual until the GUI succeeds/)
assert.match(exeUpgrade, /ProgramData state was not retained across the 0\.3\.3 to 0\.3\.4 upgrade/)
...
assert.match(workflow, /be2d90445b6cd7d93e6657d4a1116c690da0c92f/)
