...
assert.equal(inputs.productVersion, '0.3.2')
assert.equal(
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.3\.1"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.3\.2"/)
...
assert.match(workflow, /Build predecessor 0\.3\.1 upgrade fixture/)
assert.equal(
  workflow.match(/-ProductVersion 0\.3\.1/g)?.length,
  2,
  'the MSI and Burn predecessor fixture must both remain at the installed 0.3.1 version',
)
