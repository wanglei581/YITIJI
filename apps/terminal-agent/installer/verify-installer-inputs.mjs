...
assert.equal(inputs.productVersion, '0.3.8')
assert.equal(agentPackage.version, inputs.productVersion)
assert.match(agentEntrypoint, /\.version\('0\.3\.8'\)/)
...
assert.match(exeUpgrade, /EXE_UPGRADE_PASS from=0\.3\.7 to=0\.3\.8/)
assert.match(exeUpgrade, /An unprovisioned upgrade must remain Stopped\/Manual until the GUI succeeds/)
assert.match(exeUpgrade, /ProgramData state was not retained across the 0\.3\.7 to 0\.3\.8 upgrade/)
...
assert.match(workflow, /82186e2320caaeed6b9515c3856b2df585d52bff/)
