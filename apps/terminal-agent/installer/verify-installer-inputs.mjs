...
assert.equal(inputs.productVersion, '0.4.3')
assert.equal(
...
assert.match(upgradeLifecycle, /PREDECESSOR_VERSION = "0\.4\.2"/)
assert.match(upgradeLifecycle, /CANDIDATE_VERSION = "0\.4\.3"/)
...
assert.match(upgradeLifecycle, /0\.4\.2 predecessor desktop panel shortcut is missing/)
...
assert.match(workflow, /ref: 98eb3674b684d02f9db4f34d734d6dc5a5352914/)
assert.match(workflow, /verify-staged-powershell\.ps1/)
assert.match(workflow, /path: predecessor-0\.4\.2/)
assert.match(workflow, /predecessor-0\.4\.2\/apps\/terminal-agent\/installer\/build-staging\.ps1/)
assert.match(workflow, /predecessor-0\.4\.2\/apps\/terminal-agent\/installer\/artifacts\/exe/)
assert.ok(
