...
try {
  ...
  Invoke-Bundle -BundlePath $previousExe -Action "/install" -LogName "install-0.3.5.log"
  $service = Get-AgentService
  if ($null -eq $service -or $service.State -ne "Stopped" -or $service.StartMode -ne "Manual") {
    throw "The 0.3.5 baseline did not install as Stopped/Manual"
  }
  if (-not (Test-Path -LiteralPath $provisionerGuiPath -PathType Leaf)) {
    throw "The 0.3.5 baseline Provisioner GUI is missing"
  }
  if (-not (Test-Path -LiteralPath $shortcutPath -PathType Leaf)) {
    throw "The 0.3.5 baseline Start menu shortcut is missing"
  }

  [System.IO.File]::WriteAllText($canaryPath, "retain-across-upgrade", [System.Text.UTF8Encoding]::new($false))

  Invoke-Bundle -BundlePath $currentExe -Action "/install" -LogName "upgrade-to-0.3.6.log"
  $service = Get-AgentService
  if ($null -eq $service) { throw "Service is missing after the 0.3.5 to 0.3.6 upgrade" }
  ...
  if ((Get-Content -Raw -LiteralPath $canaryPath) -ne "retain-across-upgrade") {
    throw "ProgramData state was not retained across the 0.3.5 to 0.3.6 upgrade"
  }
  if (-not (Test-Path -LiteralPath $provisionerGuiPath -PathType Leaf)) {
    throw "The 0.3.6 Provisioner GUI is missing after upgrade"
  }
  if (-not (Test-Path -LiteralPath $shortcutPath -PathType Leaf)) {
    throw "The 0.3.6 Start menu shortcut is missing after upgrade"
  }

  Invoke-Bundle -BundlePath $currentExe -Action "/repair" -LogName "repair-0.3.6.log"
  $service = Get-AgentService
  if ($null -eq $service -or $service.State -ne "Stopped" -or $service.StartMode -ne "Manual") {
  ...
  Write-Host "EXE_UPGRADE_PASS from=0.3.5 to=0.3.6 unprovisionedSafe=true stateRetained=true provisionerRetained=true"
} finally {
  if ($installAttempted) {
    try {
