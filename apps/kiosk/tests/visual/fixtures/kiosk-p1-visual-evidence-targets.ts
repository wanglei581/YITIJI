...
export const visualEvidenceTargets: readonly VisualEvidenceTarget[] = [
  ...
  primary({ targetId: '08', prototypePath: 'docs/design/kiosk-redesign-2026-08/26-browse-list.html', referenceKind: 'PRIMARY', routeOrState: ['/jobs'], captureUrls: ['/jobs'], precondition: 'Return a synthetic sourced-job list using the current jobs envelope.', readyMarker: '[data-qx-frame="true"]' }),
  primary({ targetId: '09', prototypePath: 'docs/design/kiosk-redesign-2026-08/27-browse-detail.html', referenceKind: 'PRIMARY', routeOrState: ['/jobs/:id'], captureUrls: ['/jobs/job-001'], precondition: 'Return one synthetic third-party job and source metadata.', readyMarker: '[data-qx-frame="true"]' }),
  primary({ targetId: '10', prototypePath: 'docs/design/kiosk-redesign-2026-08/28-jobfair-enhanced.html', referenceKind: 'PRIMARY', routeOrState: ['/job-fairs'], captureUrls: ['/job-fairs'], precondition: 'Return a synthetic official-source fair list.', readyMarker: '[data-qx-frame="true"]' }),
  primary({ targetId: '11', prototypePath: 'docs/design/kiosk-redesign-2026-08/28-jobfair-enhanced.html', referenceKind: 'PRIMARY', routeOrState: ['/job-fairs/:id'], captureUrls: ['/job-fairs/fair-001'], precondition: 'Return one synthetic fair with companies, zones, and source metadata.', readyMarker: '[data-qx-frame="true"]' }),  primary({ targetId: '12', prototypePath: 'docs/design/kiosk-redesign-2026-08/28-jobfair-enhanced.html', referenceKind: 'PRIMARY', routeOrState: ['/renshi'], captureUrls: ['/renshi'], precondition: 'Return synthetic published policy cards and preserve information-only wording.', readyMarker: '.w4-policy-page' }),
  ...
  primary({ targetId: '34', prototypePath: 'docs/design/kiosk-proto-2026-07/34-scan-start.html', referenceKind: 'PRIMARY', routeOrState: ['/scan#ready'], captureUrls: ['/scan'], precondition: 'Open the scan chooser before creating any scan session.', readyMarker: '[data-w2-page="scan-start"]' }),
  primary({ targetId: '35', prototypePath: 'docs/design/kiosk-proto-2026-07/35-scan-settings.html', referenceKind: 'PRIMARY', routeOrState: ['/scan'], captureUrls: ['/scan'], precondition: 'Enter through the visible start action and return synthetic server instructions.', readyMarker: '[data-w2-page="scan-settings"]' }),
  primary({ targetId: '36', prototypePath: 'docs/design/kiosk-proto-2026-07/36-scan-progress.html', referenceKind: 'PRIMARY', routeOrState: ['/scan'], captureUrls: ['/scan'], precondition: 'Enter from a created synthetic session and return an active status.', readyMarker: '[data-w2-page="scan-progress"]' }),
  primary({ targetId: '37', prototypePath: 'docs/design/kiosk-proto-2026-07/37-scan-result.html', referenceKind: 'PRIMARY', routeOrState: ['/scan'], captureUrls: ['/scan'], precondition: 'Enter from a completed synthetic session with non-personal result metadata.', readyMarker: '[data-w2-page="scan-result"]' }),
  primary({ targetId: '38', prototypePath: 'docs/design/kiosk-proto-2026-07/38-interview-setup.html', referenceKind: 'PRIMARY', routeOrState: ['/interview#setup'], captureUrls: ['/interview'], precondition: 'Open the blank setup form without uploading a resume.', readyMarker: '[data-kiosk-screen="interview-setup"]' }),
  primary({ targetId: '39', prototypePath: 'docs/design/kiosk-proto-2026-07/39-interview-session.html', referenceKind: 'PRIMARY', routeOrState: ['/interview'], captureUrls: ['/interview'], precondition: 'Provide a synthetic interview session and question.', readyMarker: '[data-kiosk-screen="interview-session"]' }),
  primary({ targetId: '40', prototypePath: 'docs/design/kiosk-proto-2026-07/40-interview-report.html', referenceKind: 'PRIMARY', routeOrState: ['/interview'], captureUrls: ['/interview'], precondition: 'Provide a synthetic interview report without user content.', readyMarker: '[data-kiosk-screen="interview-report"]' }),
  primary({ targetId: '41', prototypePath: 'docs/design/kiosk-proto-2026-07/41-interview-tips.html', referenceKind: 'PRIMARY', routeOrState: ['/interview'], captureUrls: ['/interview'], precondition: 'Open the static interview tips route.', readyMarker: '[data-kiosk-screen="interview-tips"]' }),
  primary({ targetId: '42', prototypePath: 'docs/design/kiosk-proto-2026-07/42-interview-reports.html', referenceKind: 'PRIMARY', routeOrState: ['/interview'], captureUrls: ['/interview'], precondition: 'Authenticate visibly and return synthetic report summaries.', readyMarker: '[data-kiosk-screen="interview-reports"]' }),
  ...
  primary({ targetId: '45', prototypePath: 'docs/design/kiosk-proto-2026-07/45-fair-company-detail.html', referenceKind: 'PRIMARY', routeOrState: ['/job-fairs/:id/companies/:companyId'], captureUrls: ['/job-fairs/fair-001/companies/fair-company-001'], precondition: 'Return one synthetic fair company with sourced jobs.', readyMarker: '[data-qx-frame="true"]' }),
  ...
  primary({ targetId: '53', prototypePath: 'docs/design/kiosk-proto-2026-07/53-companies.html', referenceKind: 'PRIMARY', routeOrState: ['/companies'], captureUrls: ['/companies'], precondition: 'Return a synthetic sourced-company list and aggregates.', readyMarker: '[data-qx-frame="true"]' }),
  primary({ targetId: '54', prototypePath: 'docs/design/kiosk-proto-2026-07/54-company-detail.html', referenceKind: 'PRIMARY', routeOrState: ['/companies/:id'], captureUrls: ['/companies/company-001'], precondition: 'Return one synthetic sourced company and job list.', readyMarker: '[data-qx-frame="true"]' }),
  ...
  primary({ targetId: '73', prototypePath: 'docs/design/kiosk-proto-2026-07/73-assistant-call.html', referenceKind: 'SUBVIEW_STATE', routeOrState: ['/assistant#voice-gate'], captureUrls: ['/assistant'], precondition: 'Open the production pre-call choice gate from the visible “语音咨询” CTA without starting TRTC.', readyMarker: '[data-kiosk-screen="assistant-call"][role="dialog"]:has-text("选择你习惯的沟通方式")', claimScope: 'Production pre-call gate and dialog reachability only.', knownLimits: 'Prototype 73 is a historical live-call reference. This pair does not prove /trtc/session, SDK room entry, microphone, audio, captions, live controls, or production TRTC; all still require real-service validation.' }),
  primary({ targetId: '74', prototypePath: 'docs/design/kiosk-proto-2026-07/74-job-detail-offline.html', referenceKind: 'PRIMARY', routeOrState: ['/jobs/:id/offline'], captureUrls: ['/jobs/offline-job-001/offline'], precondition: 'Return one synthetic offline-agency job and store metadata.', readyMarker: '[data-qx-frame="true"]' }),
  primary({ targetId: '75', prototypePath: 'docs/design/kiosk-proto-2026-07/75-offline-agencies.html', referenceKind: 'PRIMARY', routeOrState: ['/offline-agencies'], captureUrls: ['/offline-agencies'], precondition: 'Return synthetic agency locations using the current query envelope.', readyMarker: '[data-qx-frame="true"]' }),
  ...
  fusionState({ targetId: '34A', prototypePath: 'docs/design/kiosk-proto-2026-07-fusion/34A-scan-offline.html', routeOrState: ['/scan#pre-session', '/scan#session-create-failed'], captureUrls: ['/scan', '/scan'], captureKeys: ['scan-start', 'scan-settings'], captureReadyMarkers: ['[data-w2-page="scan-start"]:has-text("本页尚未创建任务")', '[data-w2-page="scan-settings"]:has-text("扫描任务未创建")'], fixture: 'contract-fixture: make no request on scan start; after the visible CTA reaches settings, reject only the current scan-session creation request with its supported unavailable response. Do not invent a device-status DTO.', precondition: 'Capture the honest pre-session boundary on start, then separately capture the failed session-creation branch on settings.', readyMarker: '[data-w2-page="scan-start"], [data-w2-page="scan-settings"]', knownLimits: 'The start page has no scanner-status knowledge; settings proves session-creation failure only. This does not prove a real scanner is offline.' }),
  fusionState({ targetId: '76A', prototypePath: 'docs/design/kiosk-proto-2026-07-fusion/76A-toolbox-empty.html', routeOrState: ['/toolbox#capability-unavailable'], captureUrls: ['/toolbox'], precondition: 'Return a valid disabled or empty toolbox configuration and wait for the route capability boundary.', readyMarker: '[data-capability-state="unavailable"]', claimScope: 'Historical empty-state reference paired with the current fail-closed capability boundary only.', knownLimits: 'The current runtime does not mount ToolboxZonePage without at least one safe launchable item; this pair does not prove that the legacy in-page empty branch remains reachable.' }),
]
...
export const routeEvidenceDispositions: readonly RouteEvidenceDisposition[] = [
  ...
  route('/interview', ['38', '39', '40', '41', '42']),
  redirect('/interview/setup', '/interview?stage=setup'),
  redirect('/interview/session', '/interview?stage=session'),
  redirect('/interview/report', '/interview?stage=report'),
  redirect('/interview/tips', '/interview?stage=tips'),
  redirect('/interview/reports', '/interview?stage=reports'),
  ...
  route('/scan', ['34', '34A', '35', '36', '37']),
  redirect('/scan/start', '/scan?stage=start'),
  redirect('/scan/settings', '/scan?stage=settings'),
  redirect('/scan/progress', '/scan?stage=progress'),
  redirect('/scan/result', '/scan?stage=result'),
  route('/jobs', ['08']),
