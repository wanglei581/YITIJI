...
assert(home.includes('if (!config.enabled) return null'), '百宝箱显式关闭时整块不渲染')
assert(home.includes('待配置') && home.includes('后续功能上线后将在这里展示'), '百宝箱空配置时保留首页占位')
assert(home.includes('getCachedKioskTerminalConfig(terminalId)') && home.includes('terminalConfig.toolbox'), '百宝箱从统一终端配置读取')
assert(terminalConfig.includes('toolbox: { enabled: true, items: [] }'), 'Kiosk OFF_CONFIG 默认显示百宝箱占位')
