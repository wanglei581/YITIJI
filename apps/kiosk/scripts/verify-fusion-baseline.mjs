...
await runGroup('router route inventory', async (fail) => {
  ...
  if (routes !== null && routes.length !== 99) {
    fail(`${displayPath(routerPath)}: expected exactly 99 normalized routes, received ${routes.length}`)
  }
  if (routes !== null) {
...
await runGroup('Playwright route manifest parity', async (fail) => {
  ...
  if (manifestRoutes.length !== 99) {
    fail(`${displayPath(manifestPath)}: expected exactly 99 route patterns, received ${manifestRoutes.length}`)
  }
