...
await runGroup('router route inventory', async (fail) => {
  ...
  if (routes !== null && routes.length !== 103) {
    fail(`${displayPath(routerPath)}: expected exactly 103 normalized routes, received ${routes.length}`)
  }
  if (routes !== null) {
...
await runGroup('Playwright route manifest parity', async (fail) => {
  ...
  if (manifestRoutes.length !== 103) {
    fail(`${displayPath(manifestPath)}: expected exactly 103 route patterns, received ${manifestRoutes.length}`)
  }
