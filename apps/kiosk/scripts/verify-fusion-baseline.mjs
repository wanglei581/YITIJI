...
await runGroup('router route inventory', async (fail) => {
  ...
  if (routes !== null && routes.length !== 87) {
    fail(`${displayPath(routerPath)}: expected exactly 87 normalized routes, received ${routes.length}`)
  }
  if (routes !== null) {
...
await runGroup('Playwright route manifest parity', async (fail) => {
  ...
  if (manifestRoutes.length !== 87) {
    fail(`${displayPath(manifestPath)}: expected exactly 87 route patterns, received ${manifestRoutes.length}`)
  }
