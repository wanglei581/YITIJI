...
await runGroup('router route inventory', async (fail) => {
  ...
  if (routes !== null && routes.length !== 93) {
    fail(`${displayPath(routerPath)}: expected exactly 93 normalized routes, received ${routes.length}`)
  }
  if (routes !== null) {
...
await runGroup('Playwright route manifest parity', async (fail) => {
  ...
  if (manifestRoutes.length !== 93) {
    fail(`${displayPath(manifestPath)}: expected exactly 93 route patterns, received ${manifestRoutes.length}`)
  }
