...
expectMatches(
  callHook,
  /const failCall = useCallback[\s\S]*?startedRef\.current = false[\s\S]*?sessionEpochRef\.current \+= 1[\s\S]*?await cleanup\(\)[\s\S]*?setPhase\('error'\)/,
  'TRTC failures stop the backend task, leave the room, and only then expose the error state',
)
expectMatches(
  callHook,
  /trtc\.on\(EVENT\.ERROR,[\s\S]{0,220}?void failCall\(/,
  'TRTC SDK error events use the idempotent failure cleanup path',
)
expectMatches(
  callHook,
