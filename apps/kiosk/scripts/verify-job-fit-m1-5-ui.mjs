...
expectIncludes(jobFitApi, 'export interface JobFitConsentStatus', '岗位匹配 API 声明匿名授权状态')
expectMatches(jobFitApi, /function anonymousAccess\(access: JobFitAccess\): JobFitAccess \{\s*return \{ accessToken: access\.accessToken \}\s*\}/, '匿名授权请求只保留 x-resume-access-token')
...
expectMatches(jobFitApi, /call<JobFitConsentStatus>\('\/resume\/job-fit\/consent', anonymousAccess\(access\), \{ method: 'POST', body: \{ taskId \} \}\)/, '匿名授权 grant 绑定 taskId 且剥离会员 Bearer')
expectMatches(jobFitApi, /call<JobFitConsentStatus>\(`\/resume\/job-fit\/consent\/\$\{encodeURIComponent\(taskId\)\}`, anonymousAccess\(access\)\)/, '匿名授权状态读取绑定 taskId 且剥离会员 Bearer')
expectMatches(jobFitApi, /call<JobFitConsentStatus>\(`\/resume\/job-fit\/consent\/\$\{encodeURIComponent\(taskId\)\}`, anonymousAccess\(access\), \{ method: 'DELETE' \}\)/, '匿名授权撤回绑定 taskId 且剥离会员 Bearer')
expectIncludes(jobFitPage, "err.code === 'JOB_FIT_ANONYMOUS_CONSENT_REQUIRED'", '分析请求显式分流匿名授权 403')
expectIncludes(jobFitPage, "err.code === 'USER_AI_CONSENT_REQUIRED'", '分析请求显式分流会员岗位 AI 授权 403')
expectIncludes(jobFitPage, 'setPendingConsentInput(input)', '匿名授权只保存当前一次待重试请求')
expectMatches(jobFitPage, /grantJobFitConsent\(taskId[\s\S]{0,650}analyzeJobFit\(/, '匿名授权成功后只重试当前请求')
expectIncludes(jobFitPage, 'getJobFitConsentStatus(taskId', '匿名页面独立读取授权状态')
expectIncludes(jobFitPage, 'revokeJobFitConsent(taskId', '匿名页面提供授权撤回')
expectIncludes(jobFitPage, 'anonymousConsentRevisionRef', '匿名授权状态读取不能覆盖用户刚做出的授权或撤回动作')
expectMatches(jobFitPage, /const consentStatusRevision = \+\+anonymousConsentRevisionRef\.current/, '匿名授权状态读取持有当前版本号')
expectMatches(jobFitPage, /consentStatusRevision === anonymousConsentRevisionRef\.current/, '过期匿名授权状态读取不得覆盖当前状态')
expectIncludes(jobFitPage, 'AnonymousJobFitConsentDialog', '岗位匹配页编排匿名授权弹窗')
expectIncludes(jobFitPage, 'AnonymousJobFitConsentCard', '岗位匹配页编排匿名授权撤回卡片')
expectIncludes(jobFitPage, 'MemberJobFitConsentCard', '岗位匹配页编排会员授权引导卡片')
expectIncludes(jobFitPage, "navigate('/jobs')", '会员缺授权时引导既有岗位 AI 授权入口')
expectMatches(anonymousConsentDialog, /role="dialog"[\s\S]*aria-modal="true"/, '匿名授权弹窗声明 modal 可访问语义')
expectMatches(anonymousConsentDialog, /className="[^"]*h-12[^"]*"/, '匿名授权弹窗操作按钮满足 48px 触控高度')
expectIncludes(anonymousConsentDialog, '本次简历诊断内容', '匿名授权弹窗诚实说明使用目的')
expectIncludes(anonymousConsentDialog, '诊断到期策略保存', '匿名授权弹窗诚实说明保存期限')
expectIncludes(anonymousConsentDialog, '可随时撤回', '匿名授权弹窗诚实说明撤回权利')
expectIncludes(anonymousConsentDialog, '不会向企业共享简历', '匿名授权弹窗说明不向企业共享')
expectMatches(anonymousConsentCard, /className="[^"]*h-12[^"]*"/, '匿名授权撤回入口满足 48px 触控高度')
...
expectMatches(ci, /verify:governed-job-fit\s*\n\s*pnpm --filter @ai-job-print\/api verify:job-fit-print/, 'CI 在岗位匹配治理门禁后执行打印守卫')
