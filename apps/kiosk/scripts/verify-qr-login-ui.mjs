...
const bridgeApi = read('src/services/localAgentBridge.ts')
...
assert.match(qrApi, /fetchProtectedLocalAgent/)
...
assert.match(bridgeApi, /\/local\/bridge\/session/)
assert.match(bridgeApi, /X-Local-Bridge-Token/)
assert.match(bridgeApi, /STATIC_BRIDGE_TOKEN/)
assert.match(bridgeApi, /response\.status !== 403/)
