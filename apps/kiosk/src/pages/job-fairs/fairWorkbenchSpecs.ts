...
export const FAIR_HEAD: Record<FairScreen, readonly [string, string]> = {
  ...
  materials: ['活动物料', '链接由服务端临时签发；打印价格以现场公示与服务端报价为准。'],
...
export const FAIR_PILL: Record<string, FairPill> = {
  ...
  'checkin:empty': { tone: 'unknown', label: '暂无配置 checkinUrl 的可用场次' },
  ...
  'visit-plan:generating': { tone: 'unknown', label: '已提交，等服务端返回' },
  'visit-plan:ready': { tone: 'ok', label: '清单已由服务端返回' },
