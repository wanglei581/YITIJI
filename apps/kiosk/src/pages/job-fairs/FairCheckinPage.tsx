...
import { CalendarIcon, MapPinIcon, QrCodeIcon, SmartphoneIcon } from 'lucide-react'
...
function CheckinEntryCard({ fair }: { fair: ExternalJobFairDTO }) {
  const sourceUrlAvailable = isValidSourceUrl(fair.sourceUrl)
    ...
    <Card className="grid gap-5 p-5 lg:grid-cols-[1fr_220px]">
      <div className="min-w-0">
        ...
        <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm leading-relaxed text-slate-600">
          请使用手机扫码前往来源平台办理入场相关事项。本系统仅展示第三方/官方来源入口，不保存现场办理结果。
        </p>
      </div>
      ...
      <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-100 bg-white p-4">
        {sourceUrlAvailable ? (
          <>
            <SourceUrlQr value={fair.sourceUrl} size={168} />
...
export function FairCheckinPage() {
  ...
  const availableFairs = useMemo(
    () => fairs.filter((fair) => fair.status === 'ongoing' || fair.status === 'upcoming'),
    [fairs],
  )
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="px-6 pt-6">
        <PageHeader
          title="来源平台入场入口"
          subtitle="展示招聘会官方/第三方来源二维码，不办理平台内入场结果"
          actions={
      ...
      <div className="mt-4 flex-1 overflow-y-auto px-6 pb-6">
        {error ? (
          ...
          <EmptyState
            icon={QrCodeIcon}
            title="暂无可展示的来源入口"
            description="当前没有进行中或即将开始的招聘会"
            className="py-20"
          />
        ) : (
          <div className="grid gap-4">
            {availableFairs.map((fair) => (
              <CheckinEntryCard key={fair.id} fair={fair} />
            ))}
          </div>
        )}
      </div>
