  ...
  if (!taskId) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <p className="text-base font-semibold text-gray-900">先上传简历，再生成参会准备单</p>
        ...
        <div className="mt-4 flex flex-1 flex-col gap-4 overflow-y-auto pb-32">
          ...
          <Section icon={BuildingIcon} title="现场优先了解企业">
            {(plan.priorityCompanies ?? []).length === 0 ? (
              <p className="text-sm text-gray-500">本场企业信息有限，建议先打印活动资料并按现场展位逐一了解。</p>
            ) : (
              <div className="flex flex-col gap-3">
                {(plan.priorityCompanies ?? []).map((company) => (
                  <div key={company.companyName} className="rounded-xl border border-primary-100 bg-primary-50/40 p-4">
                    ...
                    <p className="mt-1.5 text-xs leading-relaxed text-gray-600">{company.reason}</p>
                    {company.sourceUrl && <p className="mt-1 text-xs text-gray-400">来源平台：{company.sourceUrl}</p>}
                  </div>
                ))}
              </div>
            )}
