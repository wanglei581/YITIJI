  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="relative w-[22rem] max-w-full rounded-2xl bg-white p-7 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-4 text-xs leading-relaxed text-gray-500">{note}</p>
      </div>
    </div>
  )
}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-start justify-between gap-3 px-6 pb-3 pt-6">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-gray-400">信息来源：{company.sourceName}</p>
        </div>
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-6 pb-6">
        ...
        {company.promoVideoUrl ? (
          videoOpen ? (
            ...
            <button
              ...
              className="relative flex h-48 w-full items-center justify-center overflow-hidden rounded-xl bg-gray-900"
              aria-label="播放企业宣传片"
            >
              {company.coverImageUrl && (
                <img src={company.coverImageUrl} alt={`${company.name}封面`} className="absolute inset-0 h-full w-full object-cover opacity-70" />
              )}
              <span className="relative flex h-16 w-16 items-center justify-center rounded-full bg-white/90 text-gray-900 shadow-lg">
        ...
        <Card className="p-5">
          <div className="flex items-start gap-3">
            {company.logoUrl ? (
              <img src={company.logoUrl} alt={`${company.name} logo`} className="h-14 w-14 shrink-0 rounded-xl border border-gray-100 object-cover" />
            ) : (
            ...
            <div className="min-w-0 flex-1">
              ...
              {company.legalName && company.legalName !== company.name && (
                <p className="mt-0.5 text-xs text-gray-400">{company.legalName}</p>
              )}
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                ...
                {typeLabel && <span className="rounded bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-600">{typeLabel}</span>}
                {industryLabel && <span className="rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-500">{industryLabel}</span>}
                {company.fairParticipant && <span className="rounded bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-600">招聘会参展企业</span>}
                {company.tags.map((t) => (
          ...
          {company.honorTags.length > 0 && (
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              <span className="flex items-center gap-1 text-xs font-medium text-gray-500">
                <AwardIcon className="h-3.5 w-3.5 text-amber-500" aria-hidden="true" />
                企业荣誉
              </span>
              {company.honorTags.map((t) => (
                <span key={t} className="rounded border border-amber-100 bg-amber-50/60 px-2 py-0.5 text-xs text-amber-700">{t}</span>
              ))}
            </div>
          )}
        ...
        <div className="flex items-start gap-2 rounded-lg border border-primary-100 bg-primary-50/50 px-4 py-3">
          ...
          <p className="text-xs leading-relaxed text-gray-500">{company.dataSourceNote}</p>
        </div>
        ...
        <section id="company-jobs" aria-label="该公司在招岗位">
          <div className="mb-2 flex items-center justify-between">
            <p className="flex items-center gap-1.5 text-base font-semibold text-gray-900">
              ...
              该公司在招岗位
              {jobsTotal !== null && jobsTotal > 0 && <span className="text-sm font-normal text-gray-400">（{jobsTotal}）</span>}
            </p>
          </div>
            ...
            <div className="flex flex-col gap-3">
              {jobs.map((job) => (
                <div key={job.id} className="rounded-xl border border-gray-200 bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-base font-semibold text-gray-900">{job.title}</p>
                      ...
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        ...
                        <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[11px] text-gray-500">{job.city}</span>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      className="flex min-h-[48px] flex-1 items-center justify-center gap-1 rounded-lg border border-gray-200 text-sm font-semibold text-gray-700 active:bg-gray-50"
                    >
              ...
              {jobsCursor && (
                <button
                  ...
                  className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-lg border border-dashed border-gray-200 text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-60"
                >
        ...
        <Card className="p-5">
          ...
          <button
            ...
            className="mt-3 flex min-h-[48px] items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-4 text-sm font-semibold text-violet-700 active:bg-violet-100"
          >
            去做简历诊断与岗位匹配参考
        ...
        <Card className="p-5">
          <p className="mb-3 flex items-center gap-1.5 text-sm font-medium text-gray-700">
            <Building2Icon className="h-4 w-4 text-gray-400" aria-hidden="true" />
            数据来源
          </p>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between gap-4"><span className="text-gray-400">来源机构</span><span className="text-right text-gray-700">{company.sourceName}</span></div>
            <div className="flex justify-between gap-4"><span className="text-gray-400">外部编号</span><span className="text-right font-mono text-xs text-gray-600">{company.externalId}</span></div>
            <div className="flex justify-between gap-4"><span className="text-gray-400">同步时间</span><span className="text-right text-gray-700">{company.syncTime.slice(0, 10)}</span></div>
          </div>
          {sourceCanOpen && (
            <button
              type="button"
              onClick={openSourceQr}
              className="mt-3 flex min-h-[48px] items-center gap-1.5 rounded-lg border border-gray-200 px-4 text-sm font-medium text-primary-600 hover:bg-primary-50"
            >
