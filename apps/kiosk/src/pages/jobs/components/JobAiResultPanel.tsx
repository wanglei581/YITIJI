...
export function JobAiResultPanel({
  ...
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400">仅供参考，不代表录用结果。</p>
          </div>
        </div>
        {onClear && (
      ...
      {loading ? (
        <div className="mt-5 flex min-h-[8rem] items-center justify-center gap-2 text-sm text-gray-400">
...
function RecommendationList({ items, onOpen }: { items: JobAiRecommendationDTO[]; onOpen?: (jobId: string) => void }) {
  if (items.length === 0) {
    return <p className="rounded-xl bg-gray-50 px-4 py-3 text-sm text-gray-500">当前简历未匹配到更合适的岗位，请调整筛选条件或更新简历后再试。</p>
  }
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <div key={item.job.jobId} className="rounded-xl border border-gray-100 bg-white px-4 py-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-gray-900">{item.job.title}</p>
              <p className="mt-0.5 truncate text-xs text-gray-400">{item.job.company} · {item.job.sourceName}</p>
            </div>
  ...
  return (
    <div className="space-y-3">
      {value.dataQualityWarning && (
        <p className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">{value.dataQualityWarning}</p>
      )}
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-xl bg-primary-50 px-4 py-3">
        ...
        {value.jobFit.summary && <p className="mt-2 text-sm leading-relaxed text-gray-700">{value.jobFit.summary}</p>}
      </div>
