 ...
 *
 * 背景：一体机装的是**嵌入式影像扫码模组**，不是手持扫码枪。2026-09-07
 * Windows 真机实测（PR #913）：模组**不常亮**，靠接近感应触发；一旦触发，
 * 内容仍以 USB HID 键盘事件落入当前聚焦控件。误扫因此不再是「路过即扫」的
 * 默认态，但用户把手机凑近机身扫码区时，未授权页仍会中招，守卫必须保留。
 *
