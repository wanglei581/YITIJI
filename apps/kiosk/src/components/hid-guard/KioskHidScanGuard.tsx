...
/** 同一次连续误扫最多提示一次的节流窗口：接近感应被反复触发时不该刷屏。 */
 ...
 * 全局扫码误扫防护。
 *
 * 硬件事实：一体机装的是嵌入式影像扫码模组，不是手持枪。2026-09-07 Windows
 * 真机实测（PR #913）：模组不常亮，靠接近感应触发；触发后在操作系统眼里仍是
 * USB 键盘，内容会以按键事件落进当前聚焦的控件。用户把手机凑近扫码区时，
 * 若焦点在手机号 / 简历表单上，脏数据仍可能被一起提交落库；付款码/取件码
 * 这类一次性凭证尤其危险。判据与阈值不因「改为接近感应」而放宽。
 *
