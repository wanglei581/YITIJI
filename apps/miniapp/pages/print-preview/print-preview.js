...
Page({
  ...
  confirm() {
    ...
    const { color, duplex, copies, total, docName, fileId, pages } = this.data
    if (!fileId) {
      wx.showToast({ title: '缺少真实文件参数，请重新选择文档', icon: 'none' })
      return
    }
    wx.navigateTo({
      url: `/pages/print-store/print-store?fileId=${encodeURIComponent(fileId)}&color=${color}&duplex=${duplex}&copies=${copies}&total=${encodeURIComponent(total)}&pages=${pages}&name=${encodeURIComponent(docName)}`,
    })
  },
