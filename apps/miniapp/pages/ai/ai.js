...
Page({
  data: {
    ...
    // 既不能伪装成可用（点下去必然 501），也不该悄悄删掉入口假装从没规划过——
    // 用户看得见「有这件事、但还没开」，比看不见更诚实。reason 是点击后的实话。
    pending: [
      {
        id: 'package',
        icon: 'folder',
        title: '材料包',
        desc: '一次备齐多份材料再到机器打印',
        reason: '页面已完成，但服务端 POST /orders/package 尚未实现，现在下单必然失败。接口上线后本入口会直接开放。',
      },
      {
        id: 'community',
        icon: 'comment',
        title: '职业圈',
        desc: '同城求职者的经验与提醒',
        reason: '页面已完成，但服务端 GET /community/feeds 尚未实现，暂时没有任何真实内容可读，因此不放开入口而不是给你一屏假动态。',
      },
      {
        id: 'daily',
        icon: 'file-text',
        title: '今日早报',
        desc: '每日岗位与政策要点摘要',
        reason: '页面已完成，但服务端 POST /assistant/daily-report 尚未实现。早报必须来自真实数据，没有接口就不能生成，也不会用模板文字冒充。',
      },
    ],
  },
