...
mustContain(indexSrc, 'skip: (page - 1) * pageSize', 'FilesPage 按当前页向服务端传 skip')
mustContain(indexSrc, 'deleted:', 'FilesPage 把清理状态下推为 deleted 筛选')
mustContain(indexSrc, 'search: search.trim()', 'FilesPage 把搜索下推到服务端')
mustContain(indexSrc, 'sensitiveLevel', 'FilesPage 把敏感级别下推到服务端')
mustContain(indexSrc, 'retentionPolicy', 'FilesPage 把保存策略下推到服务端')
mustContain(indexSrc, 'expiry', 'FilesPage 把有效期内/待清理下推为 expiry')
mustNotMatch(indexSrc, /sensitiveLabel === sensitiveFilter/, 'FilesPage 不再用当前页敏感标签本地筛选')
mustNotMatch(indexSrc, /retentionPolicyLabel\(v\.raw\.retentionPolicy\)/, 'FilesPage 不再用当前页保存策略本地筛选')
mustNotMatch(indexSrc, /CLEAN_MAP\[v\.clean\]/, 'FilesPage 不再用当前页清理状态本地筛选')
mustNotMatch(indexSrc, /limit:\s*200/, 'FilesPage 不再一次拉 200 条做本地分页')
mustContain(filesApi, 'skip?: number', 'Admin files adapter 支持 skip')
mustContain(filesApi, "deleted?: boolean | 'all'", 'Admin files adapter 支持 deleted')
mustContain(filesApi, 'sensitiveLevel?:', 'Admin files adapter 支持 sensitiveLevel')
mustContain(filesApi, 'retentionPolicy?:', 'Admin files adapter 支持 retentionPolicy')
mustContain(filesApi, "expiry?: 'active' | 'expired'", 'Admin files adapter 支持 expiry')
mustMatch(filesApi, /items:\s*AdminFileRecord\[\]/, 'Admin files adapter 返回 {items,total}')
