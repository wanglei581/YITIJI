...
function assertProdApiMode(command: string, mode: string, env: Record<string, string>) {
  ...
  if (apiMode !== 'http') {
    throw new Error(
      `[admin] 生产构建被拒绝：VITE_API_MODE 必须为 "http"（当前 "${apiMode || '未设置'}"）。` +
        `默认 mock 会把内存假数据打进产物，造成上线即假数据。` +
        `请用 VITE_API_MODE=http VITE_API_BASE_URL=/api/v1 pnpm --filter @ai-job-print/admin build。`,
    )
  }
  if (!(env['VITE_API_BASE_URL'] ?? '').trim()) {
    console.warn('[admin] 生产构建未设置 VITE_API_BASE_URL，将回落默认 /api/v1。如生产 API 非同源请显式配置。')
  }
}
