...
export const benefitActivitiesAdminApi = {
  list(params?: { status?: AdminBenefitActivityStatus; source?: AdminBenefitActivitySourceType }): Promise<{ items: AdminBenefitActivityItem[]; total: number; truncated: boolean }> {
  ...
  claims(id: string): Promise<{ items: AdminBenefitActivityClaimItem[]; total: number; truncated: boolean }> {
    if (API_MODE !== 'http') return Promise.resolve(EMPTY)
