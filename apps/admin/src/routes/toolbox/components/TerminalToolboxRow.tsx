            ...
            return (
              <div key={item.key} className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                ...
                <div className="grid gap-3 lg:grid-cols-[1.1fr_1.6fr_0.8fr_auto]">
                  <input value={item.title} onChange={(e) => patchItem(index, { title: e.target.value, key: item.key || `tool-${index + 1}` })} placeholder="功能名称" disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100" />
                  <input value={item.description} onChange={(e) => patchItem(index, { description: e.target.value })} placeholder="功能说明" disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100" />
                  <select value={item.icon} onChange={(e) => patchItem(index, { icon: e.target.value })} disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100">
                    {ICON_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                  </select>
                  <button type="button" onClick={() => setItems((current) => current.filter((_, i) => i !== index))} disabled={isGovernedItem} className="flex h-10 w-10 items-center justify-center rounded-lg text-gray-400 hover:bg-white hover:text-red-500 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-gray-400" aria-label="删除功能">
                ...
                <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_1fr_1.8fr_auto]">
                  ...
                  })} disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100">
                  ...
                  }} placeholder={isQrLaunchMode ? '二维码图片地址，用于终端展示二维码图片' : LAUNCH_MODE_OPTIONS.find((option) => option.value === (item.launchMode ?? 'internal_route'))?.placeholder} disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100" />
                ...
                {isQrLaunchMode && (
                  <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_1.8fr_auto]">
                    <span className="flex h-10 items-center rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-600">扫码目标地址</span>
                    <input value={item.qrTargetUrl ?? ''} onChange={(e) => {
                      const value = e.target.value.trim() || null
                      patchItem(index, { qrTargetUrl: value })
                    }} placeholder={item.launchMode === 'qr_code' ? '扫码后打开的 HTTPS 地址，用于合规审计' : '小程序目标说明，用于合规审计'} disabled={isGovernedItem} className="h-10 rounded-lg border border-gray-200 px-3 text-sm disabled:bg-slate-100" />
                    <span className="flex h-10 items-center text-xs text-gray-400">图片地址和扫码目标分离保存</span>
                  </div>
                )}
              </div>
            )
